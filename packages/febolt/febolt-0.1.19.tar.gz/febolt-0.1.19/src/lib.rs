...
use numpy::{PyArray1, PyArray2, PyReadonlyArray2, PyReadonlyArrayDyn};
...
impl<'a> Probit<'a> {
    ...
    // Updated hessian method using BLAS rank‑1 update (dger).
    fn hessian(&self, xbeta: &ArrayView1<f64>, hess: &mut Array2<f64>) {
        ...
        // Allocate a flat vector to represent the Hessian in column‑major order.
        let mut hess_flat = vec![0.0_f64; k * k];

        // For each observation, update the Hessian via BLAS's dger.
        for i in 0..n {
            let w = weights[i];
            let sqrt_w = w.sqrt();
            // Compute the scaled row vector: v = sqrt(w) * x_i.
            let v: Vec<f64> = self.exog.row(i).iter().map(|&x| x * sqrt_w).collect();

            // dger performs: A := alpha * v * vᵀ + A.
            // Here, m = k, n = k, alpha = 1.0, and A is our hess_flat in column‑major order.
            let m = k as i32;
            let n_i32 = k as i32;
            let alpha = 1.0;
            let incx = 1;
            ...
            let lda = k as i32;
            unsafe {
                dger(m, n_i32, alpha, &v, incx, &v, incy, &mut hess_flat, lda);
            }
        }

        // Multiply by -1 to match the original formulation.
        for elem in &mut hess_flat {
            *elem = -*elem;
        }

        // Convert the flat, column‑major Hessian into an Array2.
        // Since Array2::from_shape_vec assumes row‑major order, we reverse the axes.
...
fn robust_covariance(
    ...
    if cluster.is_none() {
        // Allocate a flat vector for M in column‑major order.
        ...
        for i in 0..nobs {
            ...
            // Get the i-th row of exog as a vector.
            let row_vec: Vec<f64> = exog.row(i).to_vec();

            // Use dger to update M_flat with alpha = ratio.
            let m = kvars as i32;
            let alpha = ratio;
            let incx = 1;
            let incy = 1;
            ...
            unsafe {
                dger(m, m, alpha, &row_vec, incx, &row_vec, incy, &mut M_flat, lda);
            }
        }
        // Convert the flat column‑major vector into an Array2.
        let M_array = Array2::from_shape_vec((kvars, kvars), M_flat)
            .expect("M vector length must match matrix shape")
            .reversed_axes();
        // Now use views on all three matrices.
