...
fn fit_probit(
    py: Python<'_>,
    endog_py: &PyAny,  // Use &PyAny to handle both 1D and 2D arrays
    ...
    // Check for NaN or infinite values in endog and exog
    if endog.iter().any(|&x| x.is_nan() || x.is_infinite()) {
        return Err(PyValueError::new_err("endog contains NaN or infinite values"));
    }
    if exog.iter().any(|&x| x.is_nan() || x.is_infinite()) {
        return Err(PyValueError::new_err("exog contains NaN or infinite values"));
    }

    // Create Probit model
    let probit = Probit::new(endog.view(), exog.view());
    ...
    // Compute robust covariance if requested
    let cov_final = if do_robust {
        ...
        robust_covariance(&exog.view(), &xbeta.view(), &endog.view(), &h_inv.view(), cluster_view.as_ref())
    } else {
        cov_final
    };
