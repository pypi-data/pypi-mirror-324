...
from workflow_server.utils.utils import VembdaExecutionFulfilledBody, VembdaExecutionFulfilledEvent
...
def _create_workflow(executor_context, namespace) -> BaseWorkflow:
    workflow_context = _create_workflow_context(executor_context)
...
def _create_workflow_context(executor_context) -> WorkflowContext:
...
def _get_workflow_inputs(executor_context: WorkflowExecutorContext) -> Optional[BaseInputs]:
...
def gather_workflow(context: WorkflowExecutorContext) -> Tuple[BaseWorkflow, str]:
    try:
...
def gather_display_context(context: WorkflowExecutorContext) -> "WorkflowEventDisplayContext":
    workflow, namespace = gather_workflow(context)
