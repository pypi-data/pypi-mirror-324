...
def get_instance_guid(authenticator, is_cp4d: bool = False, service_url: str = None):
    ...
    if is_cp4d:
        ...
        resources_url = RESOURCES_URL_MAPPING[service_url]
...
def check_plan_usage(wos_client):
    """
    ...
    usage_service_url = "{0}/v2/usage".format(wos_client.service_url)

    token = wos_client.authenticator.token_manager.get_token()
    iam_headers = {
        "Content-Type": "application/json",
        "Authorization": "Bearer %s" % token
    }
...
def update_plan_usage(wos_client):
    """
