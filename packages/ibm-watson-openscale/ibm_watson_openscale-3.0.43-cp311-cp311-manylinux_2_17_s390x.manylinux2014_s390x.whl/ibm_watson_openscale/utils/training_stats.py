...
class TrainingStats():
    ...
    def __validate_training_data_info(self):
        ...
        self.class_probabilities = self.training_data_info.get(
            "class_probabilities")
        ...
        self.categorical_columns = self.training_data_info.get(
            "categorical_columns")
        self.__validate_columns("categorical_columns",
                                self.categorical_columns)

        # Text column checks
        self.text_columns = self.training_data_info.get("text_columns", [])
        self.__validate_columns("text_columns", self.text_columns)
    ...
    def __validate_columns(self, name, columns):
        if columns is not None and type(columns) is not list:
            raise Exception("'{0}' should be a list of values".format(name))

        # Verify existence of  categorical columns in feature columns
        if columns is not None and len(columns) > 0:
            check_col_existence = list(
                set(columns) - set(self.feature_columns))
            if len(check_col_existence) > 0:
                raise Exception("'{0}' should be subset of feature columns. Details: {1}".format(
                    columns, check_col_existence))
    ...
    def __get_common_configuration(self):
        ...
        if self.training_data_info.get("input_data_type") == "hybrid":
            common_configuration["text_fields"] = self.text_columns
            common_configuration["input_data_type"] = self.training_data_info.get(
                "input_data_type")
        ...
        common_configuration["enable_quality"] = self.training_data_info.get(
            "enable_quality", True)
        common_configuration["enable_fairness"] = self.training_data_info.get(
            "enable_fairness", True)
        common_configuration["enable_explainability"] = self.training_data_info.get(
            "enable_explainability", True)
        common_configuration["enable_drift_v2"] = self.training_data_info.get(
            "enable_drift_v2", True)
        common_configuration["enable_drift"] = self.training_data_info.get(
            "enable_drift", False)
    ...
    def get_training_statistics(self):
        ...
        if self.compute_explain:
            try:
                from ibm_metrics_plugin.metrics.explainability.entity.training_stats import TrainingStats as ExplainStats
            except Exception:
                msg = "Unable to find ibm-metrics-plugin library. Please install it and retry."
                raise Exception(msg)
            explain_training_stats = ExplainStats(self.training_data_frame, self.training_data_info)
            explain_config = explain_training_stats.get_explanability_statistics()
