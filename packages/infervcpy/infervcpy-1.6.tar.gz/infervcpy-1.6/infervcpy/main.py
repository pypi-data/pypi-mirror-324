from infervcpy.lib.log_config import logger
import torch
import gc
import numpy as np
import os
...
from infervcpy.lib.infer_pack.models import (
    SynthesizerTrnMs256NSFsid,
    SynthesizerTrnMs256NSFsid_nono,
    SynthesizerTrnMs768NSFsid,
    SynthesizerTrnMs768NSFsid_nono,
)
from infervcpy.lib.audio import load_audio
import soundfile as sf
from scipy import signal
from time import time as ttime
import faiss
from infervcpy.root_pipe import VC, change_rms, bh, ah
import librosa
...
def download_wav(url, output_dir=".", filename=None, extra_options=None):
    """
    Downloads a YouTube video as a WAV file using yt-dlp.

    Args:
        url: The URL of the YouTube video.
        output_dir: The directory to save the downloaded WAV file.
        filename: The desired filename for the WAV file (without extension). 
                   If None, yt-dlp's default naming will be used.
        extra_options: A dictionary of additional yt-dlp options.

    Returns:
        The path to the downloaded WAV file, or None if the download failed.
    """
    ydl_opts = {
        'format': 'bestaudio/best',  # Select the best audio quality available
        'outtmpl': os.path.join(output_dir, f'{filename if filename else "%(title)s"}.wav'), # Specify the output filename
        'noplaylist': True,  # Only download a single video, not playlists
        'extract_audio': True,  # Extract audio only
        'audio_format': 'wav',   # Specify WAV as the audio format
        'logger': log, # Use our configured logger
        'progress_hooks': [my_hook], # Add a progress hook
    }
    ...
    try:
        ...
        log.exception(f"An unexpected error occurred: {e}") # Log the full stack trace
        return None

def my_hook(d):
    if d['status'] == 'finished' and '_filename' in d:
        log.info(f"Done downloading, now converting {d['_filename']}")
    if d['status'] == 'downloading' and '_percent_str' in d and '_total_bytes_str' in d:
        log.info(f"{d['_percent_str']} of {d['_total_bytes_str']}")
