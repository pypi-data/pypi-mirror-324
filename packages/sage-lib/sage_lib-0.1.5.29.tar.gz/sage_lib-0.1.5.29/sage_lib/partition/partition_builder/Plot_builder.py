try:
    ...
    from sage_lib.miscellaneous.math_tools import normalize_matrix_to_doubly_stochastic
...
try:
    ...
    import sys
    sys.stderr.write(f"An error occurred while importing seaborn: {str(e)}\n")
    del sys
...
class Plot_builder(PartitionManager, ):
    """
    ...
    def plot_RBF(self, cutoff: float = 3.0, number_of_bins: int = 100, output_path: str = './',
        ...
        if individual:
            for container_i, container in enumerate(self.containers):
                ...
                container.AtomPositionManager.plot_RBF(output_path=container_path, save=True)
            ...
            for cluster_class_name, rbf_data in rbf.items():
