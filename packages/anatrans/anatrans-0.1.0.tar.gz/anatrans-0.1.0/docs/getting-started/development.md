# `anatrans` developer documentation
...
# install anatrans as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `anatrans`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d anatrans.XXXXXX)
git clone git@github.com:MiBiPreT/anatrans .
...
<!-- [https://test.pypi.org/project/anatrans](https://test.pypi.org/project/anatrans) -->
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d anatrans-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple anatrans
```
