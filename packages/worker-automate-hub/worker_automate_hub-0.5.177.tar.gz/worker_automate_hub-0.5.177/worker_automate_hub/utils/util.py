...
async def ocr_title(numero_nota: str, windows_title: str)-> RpaRetornoProcessoDTO:
    try:
        ...
        console.print(f"Tirando print da janela do Informação para realização do OCR...\n")
        ...
        username = getpass.getuser()
        path_to_png = f"C:\\Users\\{username}\\Downloads\\informacao_popup_{numero_nota}.png"
        screenshot.save(path_to_png)
        console.print(f"Print salvo em {path_to_png}...\n")
...
async def save_pdf_emsys(file_path):
    try:
        ...
        app = Application(backend='win32').connect(title="Print", class_name='TppPrintDialog')
        ...
        app = Application(backend='win32').connect(title="Salvar Saída de Impressão como")
        window_saida_impressao = app['Salvar Saída de Impressão como']
        #OK para salvar pdf
        file_box = window_saida_impressao.child_window(class_name="ComboBox", found_index=0)
...
async def delete_folder(folder_name):
    ...
    #         console.print(f"Pasta '{folder_path}' e todos os seus conteúdos foram deletados com sucesso.")
    #     except Exception as e:
    #         console.print(f"[red]Erro ao deletar a pasta '{folder_path}': {e}")
    # else:
    #     console.print(f"[yellow]A pasta '{folder_path}' não existe.")
    current_dir = os.getcwd()
    ...
    if os.path.exists(folder_path) and os.path.isdir(folder_path):
        ...
        console.print(f"[yellow]A pasta '{folder_name}' não foi encontrada no diretório '{current_dir}'.")
