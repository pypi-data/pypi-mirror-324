...
async def get_valor_remessa_cobranca(date: str):
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            async with session.post(f"{env_config['API_BASE_URL']}/emsys/consulta-valor-remessa-cobranca", data=body, 
                                    headers=headers_basic) as response:
                if response.status != 200:
                    raise Exception(f"Erro ao comunicar com endpoint do Simplifica: {response.text}")
                data = await response.json()
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            async with session.post(url, headers=headers_basic, json=body) as response:
                if response.status != 200:
                    raise Exception(f"Erro ao baixar arquivo do historico: {response.text}")
                data = await response.json()
        ...
        err_msg = f"Erro ao comunicar com endpoint para download do arquivo do historico: {e}"
        console.print(f"\n{err_msg}\n", style="bold green")
        logger.info(err_msg)