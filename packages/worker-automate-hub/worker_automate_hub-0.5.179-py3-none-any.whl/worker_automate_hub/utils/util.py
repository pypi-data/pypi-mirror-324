...
from worker_automate_hub.models.dto.system_info_dto import SystemInfoDTO
...
async def get_system_info() -> SystemInfoDTO:
    """
...
async def delete_folder(folder_name):
    ...
    if os.path.exists(folder_path) and os.path.isdir(folder_path):
        try:
            shutil.rmtree(folder_path)
            console.print(
                f"Pasta '{folder_name}' e todos os seus conteúdos foram deletados com sucesso."
            )
        except Exception as e:
            console.print(f"[red]Erro ao deletar a pasta '{folder_name}': {e}")
    else:
        console.print(
            f"[yellow]A pasta '{folder_name}' não foi encontrada no diretório '{current_dir}'."
        )
