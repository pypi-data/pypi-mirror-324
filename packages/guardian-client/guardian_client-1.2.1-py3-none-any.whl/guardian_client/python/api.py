...
from huggingface_hub import repo_info, hf_hub_download
...
class GuardianAPIClient:
    ...
    def scan_3p(self, repo_id: str, revision: str = "", allow_patterns: list[str] = [], ignore_patterns: list[str] = []) -> Dict[str, Any]:
        logging.info(f"Creating scan for {repo_id}/{revision if revision else 'latest'}")

        # if revision is not provided, it will default to the latest revision from Hugging Face
        if not revision:
            logging.debug(f"Revision not provided. Fetching latest revision for {repo_id} from Hugging Face")
            latest_revision = repo_info(repo_id)

            if not latest_revision:
                logging.error(f"Error: Could not fetch latest revision for {repo_id} from Hugging Face")
                return {
            ...
            logging.debug(f"Fetched latest revision from Hugging Face: {repo_id}/{revision}")

        if revision and not self._is_git_sha(revision):
            logging.debug(f"Given revision {revision} is not a SHA. Fetching a valid sha for revision")

            revision_check = repo_info(repo_id=repo_id, revision=revision)

            if not revision_check:
                logging.error(f"Error: Could not find the revision {revision} for {repo_id} from Hugging Face")
                return {
                    "http_status_code": HTTPStatus.BAD_REQUEST,
        ...
        if response.status_code != HTTPStatus.ACCEPTED and response.status_code != HTTPStatus.OK:
            return {
                "http_status_code": response.status_code,
                "error": self._decode_error(response),
            }
    ...
    def download_from_scan(self, scan_uuid: str, local_dir: str) -> Dict[str, Any]:
        ...
        if aggregate_eval_outcome == "ERROR":
            logging.error(f"Model is blocked because scan id {scan_uuid} encountered an unexpected error")
            return {
            ...
            logging.error(f"Model is blocked because scan id {scan_uuid} violated your organization's security policies")
            return {
                "http_status_code": HTTPStatus.FORBIDDEN,
                "scan_status_json": scan_status_json,
                "download_locations": [],
