...
def replace_variable(val, variables):
...
def replace_variables(config: dict | list, variables: dict):
    """
    ...
    if isinstance(config, dict):
        for key, val in config.items():
            if isinstance(val, dict) or isinstance(val, list):
                config[key] = replace_variables(val, variables)
            # elif isinstance(val, list):
            #     for element in val:
            #         element = replace_variables(element, variables)
            else:
                config[key] = replace_variable(val, variables)
    elif isinstance(config, list):
        for idx, element in enumerate(config):
            config[idx] = replace_variables(element, variables)
    else:
        config = replace_variable(config, variables)
