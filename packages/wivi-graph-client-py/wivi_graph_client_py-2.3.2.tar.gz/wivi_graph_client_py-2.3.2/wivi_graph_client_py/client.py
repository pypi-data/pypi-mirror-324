...
class GraphQL_Client:
    ...
    def execute(self, query, variables=None):
        max_retries = max(1, self.max_retries)# ensure atleast one attempt is made in case of 0 retry value
        for attempt in range(1, max_retries+1):
            try:
                ...
                if attempt == self.max_retries:
                    print(f"Failed to connect after {self.max_retries} attempts. Last error: {str(e)}")
                    # return {"data": None, "errors": f"Connection failed after {max_retries} retries"}
