        ...
        for attempt in range(1, max_retries+1):
            try:
                ...
                if attempt == max_retries:
                    print(f"Failed to connect after {max_retries} attempts. Last error: {str(e)}")
                    # return {"data": None, "errors": f"Connection failed after {max_retries} retries"}
                    raise RuntimeError(f"Connection failed after {max_retries} retries: {e}")  # 🚀 Raise error to stop looping
