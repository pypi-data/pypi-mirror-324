...
function showMenu(modes, annotations) {
  ...
  clearRulers.className = "col-auto btn btn-primary responsive-btn  mx-2";
...
let portsButton = document.getElementById("port-button");
let portsDropdown = document.getElementById("show-ports");
...
Array.from(portsDropdown.children).forEach(function(el) {
  let link = el.children[0]
  let data = link.getAttribute("data-ports");
  let hierarchy = [-1,-1];
  if (data === "1") {
    hierarchy = [0,0];
    showPorts = 1;
  ...
  link.addEventListener("click", function(event) {socket.send(JSON.stringify({"msg": "show-ports", "hierarchy": hierarchy}))});
});
