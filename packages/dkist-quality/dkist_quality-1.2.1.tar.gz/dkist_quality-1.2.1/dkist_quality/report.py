...
class ReportMetric:
    ...
    def make_simple_plot(self) -> list[Flowable]:
        ...
        for plot_data in plot_list:
            ...
            for k, v in natsorted(plot_data.series_data.items()):
                # `plot` used because `scatter` does not interact with the cycler in a nice way
                l = ax.plot(v[0], v[1], label=k, linestyle="none", marker=".", ms=6)[0]
                ...
                if plot_data.is_multi_series:
                    ...
                    ax.plot(v[0], v[1], linestyle="-", color=l.get_color(), alpha=line_alpha)

            if plot_data.is_multi_series:
                fig.set_size_inches(fig_width_in + legend_width_in, fig_height_in)
                box = ax.get_position()
                legend_frac = fig_width_in / (fig_width_in + legend_width_in)
    ...
    def make_histogram(self) -> list[Flowable]:
        ...
        for histogram_data in histogram_list:
            ...
            self.format_plot(ax, histogram=True)
    ...
    def make_modulation_matrices_histograms(self) -> list[Flowable]:
        ...
        fig.set_size_inches(
            fig_width_in, fig_height_in * 2.8
    ...
    def make_raincloud_plot(self) -> list[Flowable]:
        ...
        if self.raincloud_data.hue_column_name:
            fig.set_size_inches(fig_width_in + legend_width_in, fig_height_in * 1.5)
            box = ax.get_position()
            legend_frac = fig_width_in / (fig_width_in + legend_width_in)
            ax.set_position([box.x0, box.y0, box.width * legend_frac, box.height])
            legend = ax.legend(
    ...
    def make_efficiency_plot(self) -> list[Flowable]:
        ...
        fig.set_size_inches(fig_width_in, fig_height_in * 2)
    ...
    def format_plot(self, ax, histogram=False):
    ...
    def setup_plot_style(self):
        ...
        fig_height_in = fig_width_in / 2  # A nice ratio
        legend_width_in = 2  # If we need a legend, this how much space we give to it
        line_alpha = 0.2  # The opacity of connecting lines in multi-line plots
        dkist_blue = "#1E317A"
        dkist_orange = "#FAA61C"
        plt.style.use("classic")
        rc("figure", figsize=(fig_width_in, fig_height_in))
        ...
        # Not used now, but the default legend style is trash
        ...
        return fig_width_in, fig_height_in, legend_width_in, line_alpha
