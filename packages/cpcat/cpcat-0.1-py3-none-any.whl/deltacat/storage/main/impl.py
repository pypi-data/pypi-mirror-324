    ...
    **kwargs,
    ...
    if partition_values:
        if not partition_scheme_id:
            ...
            # TODO(pdames): A read shouldn't initiate two transactions that
            #  read against different catalog snapshots. To resolve this, ddd
...
def get_delta(
    ...
    **kwargs,
    ...
    locator = DeltaLocator.of(
        locator=partition.locator_alias,
...
def get_partition(
    stream_locator: Union[StreamLocator, StreamLocatorAlias],
    partition_values: Optional[PartitionValues] = None,
    *args,
    **kwargs,
) -> Optional[Partition]:
    ...
    should not be specified for unpartitioned tables. If a stream locator alias
    is provided, then the partition scheme ID for the given partition is
    resolved to the partition scheme ID of the latest stream revision (i.e.,
    the current partition scheme for the stream's parent table version).
    """
