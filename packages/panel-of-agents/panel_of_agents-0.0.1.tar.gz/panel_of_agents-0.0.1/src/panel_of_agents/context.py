...
from src.panel_of_agents.types.context import Artifact, Action, ConversationTurnIndex, DecisionRunIndex, ActionPlan, CustomProp
...
class Context:
    ...
    def conversation_history(self, value: List[Union[HumanMessage, AIMessage]]):
        ...
        len_items = len(value)
        assert all(isinstance(x, (HumanMessage, AIMessage))
                   for x in value), "conversation_history must be a list of HumanMessage or AIMessage"
        if len_items == 1:
            raise ValueError(
        ...
        assert all(isinstance(value[i], HumanMessage)
                   for i in even_indices), "conversation_history must have HumanMessage at even indices"
        assert all(isinstance(value[i], AIMessage)
                   for i in odd_indices), "conversation_history must have AIMessage at odd indices"
    ...
    def artifacts(self, value: Dict[ConversationTurnIndex, Artifact]):
        ...
        assert artifact_length <= len(self.conversation_history) / \
            2, "There must only be as many artifacts as there are conversation turns"

        ordered_artifacts = OrderedDict()
        for key, content in value.items():
            if not isinstance(key, ConversationTurnIndex):
                raise TypeError(
                    "Keys of artifacts must be ConversationTurnIndex")
            if not isinstance(content, Artifact):
                raise TypeError(
                    "contents of artifacts must be of type Artifact")
    ...
    def get_json_action_plan(self) -> ActionPlan:
        try:
            ...
            raise AttributeError(
                "current_action_plan is not JSON serializable")
        except (KeyError, TypeError, ValidationError) as e:
            raise KeyError(f"Invalid action plan structure: {str(e)}")
        except Exception as e:
            raise AttributeError(
                f"Unexpected error creating action plan: {str(e)}")
