...
from src.panel_of_agents.types.agents import AgentVote, FunctionParams, FunctionDesc, Capability, CapabilityResult
from src.panel_of_agents.types.context import Artifact, Action, ConversationTurnIndex, DecisionRunIndex, ArtifactType
from src.panel_of_agents.utils import format_conversation_history
...
"""
    ...
    def __init__(
        ...
        for func in dir(self):
            if hasattr(getattr(self, func), "is_agent_capability") and func != "_invoke" and func != "_ainvoke" and func != "_stream_chunks" and func != "_astream_chunks":
    ...
    def function_to_schema(self, func) -> dict:
        ...
        try:
            ...
            raise ValueError(
                f"Failed to get signature for function {
                    func.__name__}: {str(e)}"
            )
    ...
    def _format_voting_prompt(self, context: Context) -> Runnable:
        ...
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content=self.voting_prompt.format(
    ...
    def set_peer_information(self, peer_information: Dict[str, str]):
        ...
        if not all(isinstance(k, str) and isinstance(v, str) for k, v in peer_information.items()):
    ...
    def prepare_dynamic_prompt(self, context: Context, tool_name: str, tool_input: dict, output: dict, tool_artifact: Union[str, Artifact]) -> bool:
...
"""
    ...
    def call_action(self, context: Context):
        ...
        try:
            ...
            if not function_to_call:
                raise NameError(
                    f"Tool {tool_name} not found in agent capabilities")
    ...
    def decide(self, context: Context, in_panel: bool = True) -> Generator[str, None, None]:
        """
    ...
    async def adecide(self, context: Context, in_panel: bool = True) -> AsyncGenerator[str, None]:
        """
    ...
    def _stream_chunks(self, question: str, chain: Runnable) -> Generator[str, None, None]:
        for chunk in chain.stream({"question": question}):
            yield chunk

    async def _astream_chunks(self, question: str, chain: Runnable) -> AsyncGenerator[str, None]:
