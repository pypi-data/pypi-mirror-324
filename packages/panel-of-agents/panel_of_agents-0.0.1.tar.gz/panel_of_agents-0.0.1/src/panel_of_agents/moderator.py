...
from src.panel_of_agents.types.agents import AgentVote, ModeratorVote
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.messages import SystemMessage, HumanMessage
from pydantic import ValidationError
import json
from src.panel_of_agents.utils import format_conversation_history, get_json_from_string
...
class Moderator:
    ...
    def __init__(self, panel_of_agents: List[Agent], leader: str, moderator_cast_vote: bool = False, moderator_model: BaseChatModel = None):
    ...
    def _format_voting_prompt(self, context: Context) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages([
            SystemMessage(content=self.moderator_voting_prompt.format(
    ...
    def cast_vote(self, context: Context) -> Dict[str, int]:
        ...
        leader_agent = self.panel_of_agents[self.leader] if self.moderator_model is None else self.moderator_model
        structured_model = leader_agent.model.with_structured_output(
            ModeratorVote)
    ...
    def execute(self, context: Context) -> Generator[str, None, None]:
        ...
        if context.target_agent is None:
            votes = self.cast_vote(
                context) if self.moderator_cast_vote else self.conduct_sync_vote(context)
    ...
    async def aexecute(self, context: Context) -> AsyncGenerator[str, None]:
        ...
        # Handle undirected queries
        if context.target_agent is None:
            votes = await (self.acast_vote(context) if self.moderator_cast_vote
