...
class Client:
    ...
    def __init__(
        ...
        self._detector = Detector(ctl_dev, *rcv_devs)
    ...
    def prepare_acq(
        ...
        ctl, acq, proc = topology.distribute_acq(
            ...
            num_receivers=len(self.recv_devs),
        )
    ...
    def pipeline(self, uuid: Optional[UUID] = None) -> Pipeline:
        ...
        # Note: sort list of pipeline instances by name such that pipelines are ordered
        # by receiver index. This is necessary e.g. in a strict round-robin topology to
        # be able to find a frame by index.
        proc_devs = [tg.DeviceProxy(url) for url in sorted(list(instances))]

        # Instantiate pipeline
        self.__pipelines[uuid] = pipeline_class(uuid, proc_devs, self.topology_kind)
