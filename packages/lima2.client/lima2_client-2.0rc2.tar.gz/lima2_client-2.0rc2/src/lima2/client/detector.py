...
class Detector:
    ...
    def __init__(
        ...
        *recv_devs: tango.DeviceProxy,
        ...
        try:
            ...
            raise CommError(
                "Failed to ping Tango servers (are the devices running?)"
            ) from e

        self.__params_schema = None
    ...
    def __try_params_schema(self):
        """
        Try to init the parameters from a lazy constructed device Proxy
        (to be used when the DS is not running when Detector is constructed)
        """
        try:
            if not self.__params_schema:
                self.__params_schema = self.__get_params_schema()
            return True
        except (tango.DevFailed, RuntimeError) as e:
            print(f"Failed to get params schema: {e}")
            return False
    ...
    def sync_hard(self):
        ...
        Group:
            State
        """
        states = [d.acq_state for d in self.devs]
        if not all(s == states[0] for s in states):
            _logger.warning(f"Inconsistant server states {states}")
        server_state = states[0]
        ServerState = type(server_state)
        map = {
        ...
        self.__fsm.state = map[server_state]
        return self.__fsm.state
    ...
    def prepare_acq(
        ...
        # TODO: Validate proc_params against the schema
