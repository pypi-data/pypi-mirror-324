...
from cht_meteo.cht.meteo.meteo import MeteoMatrixScalar, MeteoMatrixVector

date_format = "%Y%m%d %H%M%S"

"""
Meteo Database
meteo = Meteo(path=r'c:/data/meteo')
meteo.download("gfs_forecast_0p25_conus", [t0, t1])

meteo_dataset   = meteo.get_data("gfs_forecast_0p25_conus", tlim=None, xlim=None, ylim=None) # Collects data from netcdf files and stores in xarray dataset
meteo_dataset_2 = meteo_dataset.get_subset(tlim=None, xlim=None, ylim=None) # Collects data from netcdf files and stores in xarray dataset

# subset.ds = XArray dataset
meteo_grid.write_to_delft3d(file_name, version="1.03", path=None, header_comments=False, refdate=None, parameters=None, time_range=None) # Writes data to delft3d file
"""


class Meteo:
    def __init__(self, path=None):
        self.database = MeteoDatabase(path)
    ...
    def download(self, dataset_name, tlim):
        """Download meteo data from source."""
        self.database.dataset[dataset_name].download(tlim)

    def get_data(self, dataset_name, tlim=None, xlim=None, ylim=None):
        """Get meteo data from database."""
        if dataset_name in self.database.dataset:
            return self.database.dataset[dataset_name].get_data(tlim, xlim, ylim)
        else:
            print("Error! " + dataset_name + " not found in database!")
            return None

    def create_grid(self, tlim, xlim, ylim, dt, dx, dy):
        """Create empty meteo grid."""
        meteo_grid = MeteoGrid(None)
        meteo_grid.set_coords(tlim, xlim, ylim, dt, dx, dy)
        return meteo_grid

    def merge(self, meteo_grid1, meteo_grid2):
        """Merge two meteo grids."""
        pass


class MeteoDatabase:
    def __init__(self, path):
        # The sources are stored in a yaml file in the source code folder
        source_path = os.path.dirname(os.path.abspath(__file__))
        source_file = os.path.join(source_path, "meteo_sources.yml")
        self.read_meteo_sources(source_file)
        if path:
            dataset_file = os.path.join(path, "meteo_datasets.yml")
            self.read_meteo_datasets(dataset_file)
        else:
            self.dataset = dict()

    def read_meteo_sources(self, file_name):
        """Read meteo sources from ../meteo/meteo_sources.yml."""
        pass

    def read_meteo_datasets(self, file_name):
        """Read meteo datasets from PATH/meteo_datasets.yml."""
        pass
...
class MeteoDataset:
    ...
    def download(self, time_range, parameters=None, path=None):
        """Download data from source and store in netcdf files."""
        pass

    def get_data(self, tlim=None, xlim=None, ylim=None):
        """Creates meteo grid with xarray dataset."""
        meteo_grid = MeteoGrid(self)
        meteo_grid.collect(tlim, xlim, ylim)
        return meteo_grid
...
#     def collect(tlim, xlim, ylim):
#         """Collect data from netcdf files and store in xarray dataset."""
#         pass

#     def get_subset(self, tlim=None, xlim=None, ylim=None, xystride=1, tstride=1):
#         subset = MeteoGrid(self.dataset)
#         subset.ds = self.ds
#         return subset

#     def set_coords(self, tlim, xlim, ylim, dt, dx, dy):
#         """Create empty meteo grid."""
#         pass
...
class MeteoGrid:
    ...
    def find_cyclone_tracks(
        ...
        # Default pcyc changed to 99500 which matches a TC of CAT 3 based on Holland, 2008 pressure-wind speed

        # Settings of the algorithm
        tstride = 1  # np.round(dt/self.source.time_interval).astype(int)   => take all into account
        tracks = []
...
def find_priorityTC(tracks, priority_storm, dist=2.0, dt=6):
    # Settings
    time_range = datetime.timedelta(hours=dt)
    ...
    if np.any(remove_eye == 0):
        ...
        print("Priority storm was NOT found - we are going to use the longest track")
        imax = 0
...
def find_cyclone_eyes(x, y, u, v, p, mindist, xlim, ylim, method, pcyc, vcyc, vmin):
    ...
    #     cs = ax[0].pcolor(xx, yy, rot_z, vmin=0.0, vmax=30.0, cmap='jet')
    # #    q = ax.quiver(xx, yy, u, v)
    #     ax[0].axis("equal")
    #     fig.colorbar(cs)
    #     pp = ax[1].pcolor(xx, yy, p, cmap='jet')
    # #    q = ax.quiver(xx, yy, u, v)
    #     ax[1].axis("equal")
    #     fig.colorbar(pp)
    #     vel = ax[2].pcolor(xx, yy, vmag, vmin=0.0, vmax=20.0, cmap='jet')
    #     q = ax[2].quiver(xx, yy, u, v)
    #     ax[2].axis("equal")
    #     fig.colorbar(vel)
    #     fig.show()

    #     fig2, ax2 = plt.subplots()
    #     cs = ax2.scatter(xb, yb, s=30)
    #     ax2.axis("equal")
    #     fig2.show()
    ...
    if np.size(xb) > 0:
        ...
        for k, cluster in enumerate(clusters):
            ...
            # Check for minimum wind speed
            vmx = np.max(vmag[i0:i1, j0:j1])
            if vmx > vmin:
                xeye.append(xxxf[i])
                yeye.append(yyyf[j])
                vmax.append(vmx)
