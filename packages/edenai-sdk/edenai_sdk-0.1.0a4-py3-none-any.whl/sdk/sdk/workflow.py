    ...
    def _make_request(
        ...
        try:
            ...
            try:
                ...
                raise Exception("Response was not JSON as excpected", exc_info=json_exc) from json_exc
        except requests.exceptions.RequestException as http_exc:
            raise Exception(f"Request failed with status: {response.status_code}") from http_exc



class WorkflowClient(BaseClient):
    def create_or_update(self, name: str, data: Any) -> Any:
        return self._make_request("workflow/", method="POST", data={"code": data, "name": name})
    ...
    def execute(self, name: str, data: Any) -> Any:
        ...
        while True:
            ...
            # if execution["content"]["status"] == "success":
            #     break
            # if execution["content"]["status"] == "failed":
            #     raise Exception("Execution failed")]
            if execution["content"]["status"] == "error":
                raise Exception(f"Execution failed. Response: {execution}")
            if execution["content"]["status"] != "running":
                break
            

         
            time.sleep(1)  # Wait for 1 second before polling again
        if execution["content"]["status"] == "success" and "output" in execution["content"]["results"]:
...
def workflow_client(api_key: str) -> WorkflowClient:

    return Client(client=WorkflowClient(api_key=api_key))
