...
class PayInProduct(StrEnum):
    ...
    # generated from
    # grep -roE 'payInProduct":[^,]+' wise_banking_api_client/ | grep -oE '"[^"]+"$' | sort | uniq
    ADVANCED = "ADVANCED"
    BALANCE = "BALANCE"
    CHEAP = "CHEAP"
    EASY = "EASY"
...
class QuoteResponse(BaseModel):
    ...
    def enabled_payments(self) -> list[PaymentOption]:
        ...
        return [
            payment_option for payment_option in self.paymentOptions if not payment_option.disabled
        ]
...
class PaymentMethod(StrEnum):
    ...
    # grep -roE '"payIn":"[^"]+' wise_banking_api_client/ | grep -oE '[^"]+$' |sort | uniq
...
class QuoteRequest(ExampleQuoteRequest):
    ...
    >>> from wise_banking_api_client import QuoteRequest
