...
class ReactionModel(OdeModel):
    ...
    def update_parameters(self, parameters: dict[str, StaticExpression]) -> "ReactionModel":
        new_parameters = self.parameters.copy()
        for key, value in parameters.items():
            if key in new_parameters:
                new_parameters[key] = replace(self.parameters[key], value=value)
            else:
                raise NameError(f"Cannot update nonexistent parameter: {key}")

        return replace(self, parameters=new_parameters)

    def update_schedules(self, schedules: dict[str, Schedule]) -> "ReactionModel":
        new_routes = self.routes.copy()
        for name, schedule in schedules.items():
            if name in self.routes:
                new_routes[name] = replace(self.routes[name], schedule=schedule)
            else:
                raise NameError(f"Cannot update schedule of nonexistent route: {name}")

        return replace(self, routes=new_routes)
