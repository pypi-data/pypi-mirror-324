...
from ._helper.get_model import get_model, get_model_types_i
...
def optimize(
    *,
    ...
    remote_model_map = {
        model_path: get_model(model_path).parse().store().refresh(include_types=True) for model_path in model_paths
    }
...
class PosteriorSampleResult:
    ...
    _jobs: list[Job[OdeParameterPosteriorSample, DataFrame, None]]
...
class OptimizationResult:
    ...
    _job: Job[Optimization, OdeOptimizationResult, None]
        Optimization job.
    _optimization: Optimization
        Optimization object.
    """
    ...
    scenario: Scenario
    _contract: Contract
    _job: Job[OdeOptimization, OdeOptimizationResult, None]
    ...
    def _fim(self) -> Job[OdeFisherInformationMatrix, dict[str, dict[str, UnittedValue]], None]:
    ...
    def profile_likelihood(
        ...
        *,
        ...
        def G_and_G_dT(name: str, T: float) -> Result[tuple[float, float], None]:
            ...
            optimization_job: Job[OdeOptimization, OdeOptimizationResult, None] = jobs[0].refresh(include_output=True)
            gradient_job: Job[OdeGradient, dict[str, UnittedValue], None] = jobs[1].refresh(include_output=True)
