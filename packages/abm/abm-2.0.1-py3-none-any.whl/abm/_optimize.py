...
from ._helper.profile_likelihood import check_empty_profiles, threaded_profile_likelihood
...
def optimize(
    *,
    ...
    method : `'scipy-L-BFGS-B'`,
        ...
        `'scipy-Newton-CG'`, and `'scipy-Powell'` ignore bounds.
    ...
    if isinstance(processed_models, Job):
        one_model_for_all_simulations = True
        model = processed_models
    ...
    # 1) get the model for the set of labels if these is not one model for all simulations.
    ...
    for i_lbs, lbs in enumerate(simulation_labels):
        # Step 1) get the model if there is not one model for all simulations
        if not one_model_for_all_simulations:
            model_list_i = models_map.get_values([lbs.labels.get(k, "*") for k in models_labels])
            model = get_model_i(model_list_i, remote_model_map, lbs.labels)
        ...
        for p in parameter_list_i:
            for v in p.values():
                v["is_model_parameter"] = v["parameter"] in all_parameters.keys()
        ...
        for p in parameter_list_i_is_fit:
            for v in p.values():
                ...
                global_params_i[v["global_parameter_name"]] = ScenarioParameter(
                    value=f"{value}:{maybe_add_parentheses(unit)}", unit=unit, prior=prior
                )
        ...
        model_reference = OdeModelReference(model.id, parameter_dict_i_values, route_schedules_i)
...
class PosteriorSampleResult:
    ...
    _jobs: list[Job[OdeParameterPosteriorSample, DataFrame]]
...
class OptimizationResult:
    ...
    _job: Job[Optimization, OdeOptimizationResult]
        Optimization job.
    _optimization: Optimization
        Optimization object.
    """
    ...
    scenario: Scenario
    _contract: Contract
    _job: Job[OdeOptimization, OdeOptimizationResult]
    ...
    def _fit_scenario(self):
        ...
        return Scenario(global_parameters=self._fit_global_parameters, likelihood_functions=self._likelihood_functions)

    def _fim(self) -> Job[OdeFisherInformationMatrix, dict[str, dict[str, UnittedValue]]]:
    ...
    def parameter_posterior_sample(
        ...
        PosteriorSampleResult
        """
        configuration = OdePosteriorSampleConfiguration(method=method, tune=tune, thin=thin)
    ...
    def profile_likelihood(
        ...
        *,
        ...
        configuration = self._job.refresh(include_definition=True).definition.configuration
        ...
        def G_and_G_dT(name: str, T: float) -> Result[tuple[float, float], None]:
            ...
            updated_parameters[name] = replace(
                updated_parameters[name], value=f"{U}:{parameter_units_with_parentheses[name]}"
            )
            ...
            optimization_job: Job[OdeOptimization, OdeOptimizationResult] = jobs[0].refresh(include_output=True)
            gradient_job: Job[OdeGradient, dict[str, UnittedValue]] = jobs[1].refresh(include_output=True)
