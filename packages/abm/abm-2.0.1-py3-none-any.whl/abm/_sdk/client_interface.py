...
from .job import Job, JobDefinition, JobDescriptor, JobOutput
...
from .ode_model import OdeModel, OdeModelFromText
...
def load_configuration() -> ClientConfiguration:
    if global_config_path.exists():
        ...
        raise RuntimeError(
            "Expected ./abm/client.toml configuration file at /etc/ or $XDG_CONFIG_HOME "
            "or ~/.config/, but no configuration file was found."
        )
...
class Client:
    def __init__(self, host: str | None = None, root: str = "/api/v1"):
        ...
        self.httpx_client = httpx.Client(
            timeout=300,
            event_hooks={"request": [self.add_jwt_to_request], "response": [raise_for_http_status]},
        ...
        self.job_descriptors: dict[str, JobDescriptor[JobDefinition, JobOutput]] = {
            "assess-parameter-scan-1d": JobDescriptor[AssessParameterScan1D, AssessParameterScan1DResult](
                definition=AssessParameterScan1D,
                output=AssessParameterScan1DResult,
            ),
            "assess-parameter-scan-2d": JobDescriptor[AssessParameterScan2D, AssessParameterScan2DResult](
                definition=AssessParameterScan2D,
                output=AssessParameterScan2DResult,
            ),
            "assess-simulation": JobDescriptor[AssessSimulation, AssessSimulationResult](
                definition=AssessSimulation,
                output=AssessSimulationResult,
            ),
            "data-frame": JobDescriptor[DataFrame, None](
                definition=DataFrame,
                output=serializer(NoneType),
            ),
            "data-pipe": JobDescriptor[DataPipe, DataFrame](
                definition=DataPipe,
                output=DataFrame,
            ),
            "distribution-sample": JobDescriptor[DistributionSample, DataFrame](
                definition=DistributionSample,
                output=DataFrame,
            ),
            "ode-fisher-information-matrix": JobDescriptor[
                OdeFisherInformationMatrix, dict[str, dict[str, UnittedValue]]
            ](
                definition=OdeFisherInformationMatrix,
                output=serializer(dict[str, dict[str, UnittedValue]]),
            ),
            "ode-gradient": JobDescriptor[OdeGradient, dict[str, UnittedValue]](
                definition=OdeGradient,
                output=serializer(dict[str, UnittedValue]),
            ),
            "ode-measurement-likelihood-sample": JobDescriptor[OdeMeasurementLikelihoodSample, Scenario](
                definition=OdeMeasurementLikelihoodSample,
                output=Scenario,
            ),
            "ode-model": JobDescriptor[OdeModel, None](
                definition=OdeModel,
                output=serializer(NoneType),
            ),
            "ode-model-from-text": JobDescriptor[OdeModelFromText, OdeModel](
                definition=OdeModelFromText,
                output=OdeModel,
            ),
            "ode-optimization": JobDescriptor[OdeOptimization, OdeOptimizationResult](
                definition=OdeOptimization,
                output=OdeOptimizationResult,
                display_job_progress=DisplayJobProgressOptimization,
            ),
            "ode-optimization-batch": JobDescriptor[OdeOptimizationBatch, list[str]](
                definition=OdeOptimizationBatch,
                output=serializer(list[str]),
            ),
            "ode-parameter-posterior-sample": JobDescriptor[OdeParameterPosteriorSample, DataFrame](
                definition=OdeParameterPosteriorSample,
                output=DataFrame,
            ),
            "ode-prediction": JobDescriptor[OdePrediction, ScenarioPredictions](
                definition=OdePrediction,
                output=ScenarioPredictions,
            ),
            "ode-residual": JobDescriptor[OdeResidual, ScenarioResiduals](
                definition=OdeResidual,
                output=ScenarioResiduals,
            ),
            "ode-simulation": JobDescriptor[OdeSimulation, TimeCourse](
                definition=OdeSimulation,
                output=TimeCourse,
            ),
            "ode-simulation-batch": JobDescriptor[OdeSimulationBatch, list[str]](
                definition=OdeSimulationBatch,
                output=serializer(list[str]),
            ),
            "to-latex-ode-model": JobDescriptor[ToLatexOdeModel, str](
                definition=ToLatexOdeModel,
                output=serializer(str),
            ),
            "to-matlab-ode-simulation": JobDescriptor[ToMatlabOdeSimulation, str](
                definition=ToMatlabOdeSimulation,
                output=serializer(str),
            ),
            "to-simbiology-ode-simulation": JobDescriptor[ToSimbiologyOdeSimulation, str](
    ...
    def add_jwt_to_request(self, request: httpx.Request) -> None:
        # h11, used by httpx, doesn't allow trailing spaces in HTTP headers, such as "Bearer ". So, instead of returning
        # an empty string if there isn't a token available, we return a string that will fail auth on the server when
        # auth is enabled.
        if self.configuration.auth_token_path is None:
            jwt = "not_a_real_token"
        elif self.configuration.auth_token_path.exists():
            jwt = self.configuration.auth_token_path.read_text().strip()
        else:
            # Better hope this is a dev machine with auth turned off
            jwt = "not_a_real_token"
        request.headers.update({"Authorization": f"Bearer {jwt}"})
    ...
    def create_jobs(
        self, definitions: list[JobDefinition], *, deduplicate: bool = True, include_definition: bool = False
    ) -> list[Job]:
