                ...
                for k in range(i+1,i+len(file)):
                    if len(file[k]) >=1: 
                        if file[k][0] == 'k' or file[k][0] == 'Writing' or file[k][0] == 'highest':
                            ...
                            if version == '7.1' or version == '7.0' or version == '7.2' or version == '7.3':
                                if file[k][0] != ' ':
                                    for item in file[k]:
                                        bands_line.append(float(item))
                            elif version == '6.7MaX' or version == '6.7':
                                if file[k][0] != ' ' and file[k][0] != 'occupation':
                                    if file[k][0] != '1.0000' and file[k][0] != '0.0000':
                                        for item in file[k]:
                                            bands_line.append(float(item))
                                    else:
                                        if file[k+1][0] != file[k][0] and file[k-1][0] != file[k][0]:
                                            for item in file[k]:
                                                bands_line.append(float(item))
                
                #if len(bands_line) != n_band:
                #    print(bands_line)
                #    print(len(bands_line))
...
#dos
#QE
def Dos_files(load_dict, calculation, fermi_energy_corr=True):  # dos via projwfc.x, nao usar dos.x
        nscfout_file_path = os.path.join(load_dict['files_folder'], load_dict['nscf_out_name'])
        nscfout_file = open_file(nscfout_file_path)
        version_QE = version_quantum(nscfout_file)
        if version_QE == '6.7MaX' or version_QE == '7.1' or version_QE == '7.2' or  version_QE == '7.3':
            fermi_energy = E_fermi(nscfout_file,load_dict['nscf_out_name'],version_QE)

        if calculation == 'projwfc.x':
            dos_file_path = os.path.join(
                load_dict['files_folder'], load_dict['pdos_dat_name'])
            change_coluns_pdos(dos_file_path)
            dos_file = open_file(dos_file_path)
            dos = DataFrame(dos_file)
            dos = dos.drop(0)
            dos = dos.rename(columns={0: 'E', 1: 'dos', 2: 'pdos'})
            dos = dos.astype(float)
            if fermi_energy_corr == True:
                dos[dos.columns[0]] -= fermi_energy
            ...
            dos_file_path = os.path.join(
                load_dict['files_folder'], load_dict['dos_dat_name'])
            change_coluns_pdos(dos_file_path)
            dos_file = open_file(dos_file_path)
            dos_file.remove(dos_file[0])
            dos = DataFrame(dos_file)
            dos = dos.rename(
...
def Pdos_files(scfin_file,load_dict, fermi_energy_corr=True): 

        pdos_per_atoms_files, pdos_per_orb_files = pdos_files_get(load_dict['files_folder'],load_dict['pdos_prefix'],atoms(scfin_file))  
        ...
        pdos_files = {}
        for atom in pdos_per_atoms_files.keys():
            i = 0
            for file in pdos_per_atoms_files[atom]:
        ...
        pdos_per_atoms = pdos_files
        
        #per orb
        orb_files = {}
        for atom in pdos_per_orb_files.keys():
            orb_files[atom] = {}
            for orb in pdos_per_orb_files[atom]:
                i =0
                for file in pdos_per_orb_files[atom][orb]:
                    change_coluns_pdos(file)
                    if i ==0:
                        df = read_csv(file, delim_whitespace= True,dtype=float)
                        df.drop(df.columns[2:], axis='columns', inplace=True)
                ...
                if fermi_energy_corr == True:
                    ...
                    print(
                            'Wrong value for fermi_energy_corr parameter, only True or False values are accepted. Default = True')   
        pdos_per_orb = orb_files 
        return [pdos_per_atoms,pdos_per_orb,atom_number , atom_orb_number]
