...
def stable_rank(
    ...
    if normalize:
        metric = 1 - 1 / (1 + metric)
    return Ok(float(metric))


def self_cluster(embeddings: NDArray[Any]) -> InvalidArgumentError | Ok[float]:
    ...
    return Ok(float(numerator / denominator))
...
def rank_me(
    ...
    return Ok(float(np.exp(entropy)))


def mu_coherence(embeddings: NDArray[Any]) -> InvalidArgumentError | Ok[float]:
    """Coherence metric.
    ...
    r = u.shape[1]
    assert r < n
    return Ok(float(np.sqrt(n) * np.max(np.abs(u))))


def mu0_coherence(embeddings: NDArray[Any]) -> InvalidArgumentError | Ok[float]:
    """μ0-Coherence metric.
    ...
    r = u.shape[1]
    assert r < n
    return Ok(float((n / r) * np.max(linalg.norm(u, axis=1, ord=2) ** 2)))


def alpha_req(embeddings: NDArray[Any]) -> InvalidArgumentError | Ok[float]:
    r"""alpha-ReQ metric.
...
def classification(
    ...
    return Ok(float(accuracy_score(y_true=y_test, y_pred=y_pred)))
...
def regression(
    ...
    model.fit(X=x_train, y=y_train)
    y_pred = model.predict(X=x_test)
    return Ok(float(mean_absolute_error(y_true=y_test, y_pred=y_pred)))
