...
class Relationship:
    ...
    def to_column_name(self) -> str:
        if self.end_source_id == self.fkey_relationship.source_with_fkey:
            return self.fkey_relationship.fkey_column_name
        return self.fkey_relationship.pkey_column_name
...
class FeatureViewSource(
    ...
    @classmethod
    def orm_to_proto(
        cls, orm_obj: orm.FeatureViewSource
    ) -> models_pb2.FeatureViewSource:
        return feature_view_source_orm_to_proto(orm_obj)

    @classmethod
    def proto_to_orm(
        cls, proto_obj: models_pb2.FeatureViewSource, session: orm.Session
    ) -> Ok[orm.FeatureViewSource] | InvalidArgumentError:
        return feature_view_source_proto_to_orm(proto_obj, session)

    @classmethod
    def delete_by_ids(
        cls, ids: Sequence[FeatureViewSourceID], session: orm.Session
    ) -> Ok[None] | InvalidArgumentError:
        return feature_view_source_delete_orms(ids, session)
...
class FeatureView(BaseModel[FeatureViewID, models_pb2.FeatureView, orm.FeatureView]):
    ...
    @classmethod
    def orm_to_proto(cls, orm_obj: orm.FeatureView) -> models_pb2.FeatureView:
        return feature_view_orm_to_proto(orm_obj)

    @classmethod
    def proto_to_orm(
        cls, proto_obj: models_pb2.FeatureView, session: orm.Session
    ) -> Ok[orm.FeatureView] | InvalidArgumentError:
        return feature_view_proto_to_orm(proto_obj, session)

    @classmethod
    def delete_by_ids(
        cls, ids: Sequence[FeatureViewID], session: orm.Session
    ) -> Ok[None] | InvalidArgumentError:
    ...
    def orm_load_options(cls) -> list[LoaderOption]:
        return [
            ...
            .selectinload(orm.SourceResourceAssociation.resource),
            sa_orm.selectinload(orm.FeatureView.feature_view_sources)
            .selectinload(orm.FeatureViewSource.source)
    ...
    @functools.cached_property
    def output_sources(self) -> set[SourceID]:
        return {
    ...
    def _get_feature_view_source(
        ...
        if isinstance(source, str):
            ...
            val = next(
                (
                    val
                    for val in self.feature_view_sources
                    if val.source.name == source_name
                ),
                None,
            )
            if not val:
    ...
    def with_source(
        ...
        *,
        ...
        new_proto_self = copy.deepcopy(self.proto_self)
        new_proto_self.feature_view_sources.append(proto_feature_view_source)
    ...
    def with_row_numbers(
        ...
        for fvs in self.feature_view_sources:
            if fvs.source.id in self.output_sources:
