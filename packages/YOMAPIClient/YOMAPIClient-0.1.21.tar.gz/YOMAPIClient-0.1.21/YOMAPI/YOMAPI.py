...
class YOMAPI(metaclass=Singleton):
    ...
    def __get_visit_record_task_surveys_page(self, page, limit, token, input_params=None):
        path = f'{self.url_v3}/api/v3/admin/accounts/task-manager/survey/visit-records'
        session = self.__build_session(token)
        try:
            params = {
                'page': page,
    ...
    def get_visit_record_task_surveys(self, token=None, input_params=None):
        current_page = 0
        total_pages = float('inf')
        taskSurveys = []
        while (current_page < total_pages):
            current_page += 1
            status, response = self.__get_visit_record_task_surveys_page(
                page=current_page,
                limit=100,
                token=token,
                input_params=input_params,
            )
            taskSurveys += response['docs']
            total_pages = response['pages']
            self.logger.info('Sending task surveys query for page {}/{}'.format(current_page, total_pages))
        return taskSurveys
