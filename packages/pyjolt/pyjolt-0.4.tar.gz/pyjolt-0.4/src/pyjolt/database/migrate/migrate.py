...
def register_db_commands(app: PyJolt, migrate: 'Migrate'):
    ...
    # db-init
    sp_init = app.subparsers.add_parser("db-init", help="Initialize the Alembic migration environment.")
    sp_init.set_defaults(func=lambda args: migrate.init())

    # db-migrate
    sp_migrate = app.subparsers.add_parser("db-migrate", help="Generate a new revision file (autogenerate).")
    ...
    # db-upgrade
    sp_upgrade = app.subparsers.add_parser("db-upgrade", help="Upgrade the database to a specified (or head) revision.")
    ...
    # db-downgrade
    sp_downgrade = app.subparsers.add_parser("db-downgrade", help="Downgrade the database to a specified (or one step) revision.")
    ...
    # db-history
    sp_history = app.subparsers.add_parser("db-history", help="Show revision history.")
    ...
    # db-current
    sp_current = app.subparsers.add_parser("db-current", help="Show the current revision of the database.")
    ...
    # db-heads
    sp_heads = app.subparsers.add_parser("db-heads", help="Show all current 'head' revisions.")
    ...
    # db-show
    sp_show = app.subparsers.add_parser("db-show", help="Show details of a given revision.")
    ...
    # db-stamp
    sp_stamp = app.subparsers.add_parser("db-stamp", help="Stamp the database with a given revision (no actual migration).")
...
class Migrate:
    ...
    """
    def __init__(self, app: PyJolt = None, db: SqlDatabase = None):
        self.app = None
        self.db = None
