...
def run(bm, x0=None, maxIter=1000, debug=False):
    """
    Runs iRprop (Improved Resilient Backpropagation) from Pints using a benchmarker.
    ...
    model, opt = classes_pints.pints_setup(bm, x0, pints.IRPropMin, maxIter, debug)
    x, f = opt.run()
...
def get_modification(modNum=1):
    ...
    """
    mod = ionbench.modification.Empty(name='rprop_pints')
    return mod
