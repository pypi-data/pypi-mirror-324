...
from applitools.common import DynamicTextType, RectangleSize, Region
...
def parse_dynamic_text(input_string):
    # type: (Text) -> list[Text]
    if not input_string:
        return []

    dynamic_text_types = "|".join(re.escape(t.value) for t in DynamicTextType)
    pattern = rf"({dynamic_text_types})|((?:[^,\\]|\\.)+)"

    def unescape(s: str) -> str:
        if s in {t.value for t in DynamicTextType}:
            return s
        return s.replace("\\,", ",")  # Only unescape commas

    parts = re.findall(pattern, input_string)
    result = [
        unescape(part[0] or part[1]).strip() for part in parts if part[0] or part[1]
    ]
