...
class RunnerOptionsTrafaret(trf.Trafaret):
    scheme = trf.Dict(
        {
            ...
            trf.Key("mask_log", optional=True): trf.Bool,
            trf.Key("remove_duplicate_tests", optional=True): trf.Bool,
        }
    )
    ...
    def check_and_return(self, value, context=None):
        ...
        return (
            RunnerOptions()
            .test_concurrency(sanitized.get("test_concurrency"))
            .remove_duplicate_tests(sanitized.get("remove_duplicate_tests"))
            .mask_log(sanitized.get("mask_log"))
        )
...
class AccessibilitySettingsTrafaret(trf.Trafaret):
    scheme = trf.Dict(
        {
            trf.Key("level"): UpperTextToEnumTrafaret(AccessibilityLevel),
            trf.Key("guidelines_version"): UpperTextToEnumTrafaret(
                AccessibilityGuidelinesVersion
            ),
        }
    )
    ...
    def check_and_return(self, value, context=None):
        ...
        return AccessibilitySettings(
            level=sanitized["level"], guidelines_version=sanitized["guidelines_version"]
        )
    ...
    shared_scheme = trf.Dict(
        {
            ...
            trf.Key(
                "accessibility_validation", optional=True
            ): AccessibilitySettingsTrafaret,
