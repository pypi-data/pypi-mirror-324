...
def checks(self, func_name: str, *args):
    ...
    if func_name == "move_dimension":
        ...
        if any(char in CUBE_NAME_FORBIDDEN_CHARS for char in cube_name):
            raise ValueError("В названии мультисферы не должно быть запрещенных символов: "
                             "% ^ & = ; : ± § ` ' ~ ] [ } { < > \\")
        ...
        if len(source_name) < 5:
            raise ValueError("Название источника должно состоять из 5 и более символов!")
        if any(char in SOURCE_NAME_FORBIDDEN_CHARS for char in source_name):
            raise ValueError("В названии мультисферы не должно быть пробелов или запрещенных символов: "
                             "% ^ & = ; : ± § ` ' ~ ] [ } { < > \\")
