...
class BusinessLogic:
    ...
    def set_measure_precision(self, measure_names: List[str], precision: List[int]) -> Dict:
        ...
        loaded_settings = self.execute_manager_command(command_name="user_iface",
                                                       state="load_settings",
        ...
        save_settings_rp = self.execute_manager_command(command_name="user_iface",
                                                        state="save_settings",
                                                        module_id=self.multisphere_module_id,
                                                        settings=settings)
    ...
    def set_measure_format(self, measure_names: List[str], measure_formats: List[Dict],
        ...
        try:
            param_types = [
                ["measure_names", measure_names, list],
                ["measure_formats", measure_formats, list],
                ["set_to_default", set_to_default, bool, False]
            ]
            ...
            if not set_to_default:
                ...
                error_handler.checks(self, self.func_name, measure_names, measure_formats, extracted_settings)
        except Exception as e:
            return self._raise_exception(ValueError, str(e))
    ...
    def sort_measure(self, measure_name: str, sort_type: str) -> Dict:
        ...
        top_total_transfer = module_configuration['queries'][0]['command']['top_total_transfer']
        ...
        try:
            line = measures.index(measure_id) if top_total_transfer \
    ...
    def create_sphere(self, cube_name: str, source_name: str, file_type: str, update_params: Dict = None,
        ...
        """
        Создать новый куб.
        :param cube_name: (str) название создаваемого куба, оно должно состоять из 5 и более символов, допустимы буквы,
            цифры, специальные символы (кроме: % ^ & = ; : ± § ` ' ~ ] [ } { < > \\).
        :param source_name: (str) название источника данных, оно должно состоять из 5 и более символов, допустимы буквы,
            цифры, специальные символы (кроме: % ^ & = ; : ± § ` ' ~ ] [ } { < > \\ <ПРОБЕЛ>).
        ...
        Имеет структуру:
            {
                ...
                ["ручное", "по расписанию", "интервальное", "инкрементальное", "обновление измененных записей"];

                schedule - планировщик, позволяющий задать время и день/число, необязательный параметр
                для интервального и инкрементального обновления.
                Имеет структуру:
                    {
                        'type': <value>,
                        'time': <value>,
        ...
        try:
            param_types = [
                ...
                ["update_params", current_update_params, dict, None],
                ["sql_params", sql_params, dict, None],
                ["user_interval", user_interval, str, "с текущего дня"],
                ["filepath", filepath, str, str()],
                ["separator", separator, str, str()],
                ["increment_dim", increment_dim, str, str()],
                ["interval_dim", interval_dim, str, str()],
                ["interval_borders", interval_borders, list, None],
                ["encoding", encoding, str, str()],
                ["delayed", delayed, bool, False],
                ["modified_records_params", modified_records_params, dict, None]
            ]
            check_f = error_handler.check_types()
            for param_info in param_types:
                check_f(param_info)
        ...
        if update_type == "ручное":
            ...
            for dim in dims:
                if dim.get("name") == increment_dim:
                    ...
                    if current_type not in (
                        ...
                        error_msg = f'Dimension "{interval_dim}" has type "{current_type}", ' \
            ...
            modified_records_key_id = str()
            modified_records_date_id = str()
            for dim in dims:
                ...
                if dim.get("name") == modified_records_date:
                    ...
                    if current_type not in ("date", "datetime"):
                        error_msg = 'Dimension "{}" has type "{}", expected "date" or "datetime"!'.format(
                            modified_records_date, current_type)
                        return self._raise_exception(PolymaticaException, error_msg, with_traceback=False)
    ...
    @timing
    def update_cube(self, cube_name: str, update_params: Dict = None, user_interval: str = "с текущего дня",
                    delayed: bool = False, increment_dim: str = str(),
                    interval_dim: str = str(), interval_borders: list = None,
                    modified_records_params: Dict = None) -> dict:
        """
        ...
        try:
            param_types = [
                ...
                ["interval_borders", interval_borders, list, None],
                ["modified_records_params", modified_records_params, dict]
            ]
            check_f = error_handler.check_types()
            for param_info in param_types:
                check_f(param_info)
    ...
    def get_total_mode(self) -> bool:
        ...
        result = self.execute_olap_command(command_name="view", state="get")
    ...
    def set_all_inter_horizontal_total(self, show: bool) -> Dict:
        ...
        if left_dims < 2:
            return self._raise_exception(PolymaticaException, 'There must be at least 2 left dimensions!',
                                         with_traceback=False)
    ...
    def set_all_inter_vertical_total(self, show: bool) -> Dict:
        ...
        if top_dims < 2:
            return self._raise_exception(PolymaticaException, 'There must be at least 2 top dimensions!',
                                         with_traceback=False)
    ...
    def set_inter_horizontal_total(self, dimension_name: str, show: bool) -> Dict:
        ...
        if left_dims[-1] == dim_id:
            return self._raise_exception(PolymaticaException, 'The dimension should not be at the rightmost level!',
                                         with_traceback=False)
    ...
    def set_inter_vertical_total(self, dimension_name: str, show: bool) -> Dict:
        ...
        if top_dims[-1] == dim_id:
            return self._raise_exception(PolymaticaException, 'The dimension should not be at the lowest level!',
                                         with_traceback=False)
