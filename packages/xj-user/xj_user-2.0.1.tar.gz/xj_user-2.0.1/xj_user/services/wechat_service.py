...
class WechatService:
    ...
    def wechat_login(self, phone_code, login_code, sso_serve_id=None, platform_id=None, other_params=None):
        ...
        if not current_user:
            ...
            # 生成登录token
            token = LoginService.make_token(user_info.detail('id', None), phone, platform_id, platform_code)
    ...
    def login_integration_interface(params):
        ...
        try:
            ...
            # ----------------------------------------------------------------------
                ...
                token = LoginService.make_token(user_info.detail('id', ""), user_info.detail("username", ""), platform_id,
                                                platform_code)
                # 修改用户登录信息，绑定token
