...
# 获取配置
module_config = JDict(settings.CONFIG.get('xj_user'))

app_id = module_config.app_id or ""
app_secret = module_config.secret or ""
jwt_secret_key = module_config.jwt_secret_key or "@zxmxy2021!"
expire_day = module_config.expire_day or ""
expire_second = module_config.expire_second or ""
...
base_fields = [
    'user_id', 'user_uuid', 'user_no', 'username', 'nickname', 'fullname',
    'phone', 'email', 'avatar', 'user_type', 'user_vip', 'user_level', 'user_info',
    'privacies', 'is_auth', 'is_using'
]
...
class UserService:
    ...
    def check_account(account):
        ...
        """
        # print("> check_account account:", account)
        # 账号类型判断
        if re.match(r'(^1[356789]\d{9}$)|(^\+?[78]\d{10}$)', account):
            ...
            return None, "账号必须是用户名、手机或者邮箱，用户名不能少于4个字符且不能以数字开头"
        # 用户ID user_list = BaseInfo.objects.filter(Q(username=account) | Q(phone=account) | Q(email=account)) \
        ...
        #     'user_id', 'username', 'fullname', 'nickname', 'phone', 'email', 'user_info', 'privacies'
        # )
        # 用户ID user_list = BaseInfo.objects.filter(Q(username=account) | Q(phone=account) | Q(email=account)) \
        user_list = BaseInfo.objects
        if account_type == 'phone':
            user_list = user_list.filter(username=account)
        elif account_type == 'email':
            user_list = user_list.filter(email=account)
        elif account_type == 'username':
            user_list = user_list.filter(username=account)
        else:
            return None, "账号无法匹配，这应该是一个不可能出现的提示！"
        user_list = user_list.annotate(user_id=F("id")).extra(
            select={
                "user_uuid": 'REPLACE(uuid, "-", "")',
            }
        ).values(*base_fields)
        # print("> check_account user_list query:", user_list.query)
        # print("> check_account user_list:", account_type, user_list)
    ...
    def check_token(token):
        ...
        try:
            ...
            jwt_payload = jwt.decode(token, key=jwt_secret_key, verify=True, algorithms=["RS256", "HS256"])
        ...
        user_uuid = jwt_payload.get('user_uuid', None)
        ...
        return {'account': account, 'user_uuid': user_uuid, 'platform_id': platform_id}, None
    ...
    def user_list(params=None, allow_user_list=None, ban_user_list=None, need_Pagination=True, is_strict_mode=True):
        ...
        search_params = format_params_handle(
            param_dict=params,
            filter_filed_list=["username", "email", "phone", "nickname", "fullname", "is_using", "user_identity"],
            alias_dict={
        ...
        try:
            ...
            user_base_set = user_base_set.order_by(sort).annotate(user_id=F("id")).values(
                "user_id", "email", "fullname", "username", "nickname", "phone", "register_time",
    ...
    def user_add(params):
        ...
        try:
            ...
            Auth.objects.create(**auth)
            return current_user.to_json(), None
        except SyntaxError:
            return None, "语法错误"
        except LookupError:
            return None, "无效数据查询"
    ...
    def user_edit(params=None, user_id=None):
        ...
        base_info_params = format_params_handle(
            ...
            filter_filed_list=["fullname", "nickname", "email", "user_info", "privacies", "phone", "is_using"],
            is_remove_null=True
        )
        # 更新基础信息
        base_query_set = BaseInfo.objects.filter(id=user_id)
    ...
    def user_delete(user_id):
        user_set = BaseInfo.objects.filter(id=user_id).first()
        if not user_set:
            return None, f'用户不存在 (uid: {str(user_id)})'
        user_set.is_delete = False
        user_set.save()
        return user_set.to_json(), None
    ...
    def user_search(user_id_list: list = None, search_params: dict = None, need_map: bool = False,
        ...
        if filter_fields is None:
            ...
            filter_fields = ["id", "username", "fullname", "nickname", "phone", "email",
