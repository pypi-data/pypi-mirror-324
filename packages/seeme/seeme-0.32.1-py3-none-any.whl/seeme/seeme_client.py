...
class Client():
  ...
  def download_dataset(self, dataset_id:str, dataset_version_id:str, split_id:str="", extract_to_folder:str="data", download_file:str="dataset.zip", remove_download_file:bool=True, export_format:DatasetFormat=None):
    self.requires_login()
  ...
  def create_structured_dataset_version(self, dataset_version:DatasetVersion, column_names:List, csv_separator:str=",") -> DatasetVersion:
    if column_names:
        config = {
    ...
    return self.create_dataset_version(dataset_version.dataset_id, dataset_version)
  ...
  ## Tabular / Structured
  def read_df(self, folder:str, filename:str, extension=".csv"):
    import pandas as pd
  ...
  def download_df(self,
    ...
    df = self.read_df(download_folder, dataset_version_id)
  ...
  def upload_df(self,
    ...
    fn = f"{name}.csv"
    df.to_csv(fn, index=keep_index, index_label=index_label)
