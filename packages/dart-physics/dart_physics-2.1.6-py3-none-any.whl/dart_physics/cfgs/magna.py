...
def random_xyz(i):
    return [random.uniform(-0.2, 0.2), random.uniform(0.1, 0.25), 0.06 + 0.03 * i ]
...
task_cfg = { 
    ...
    "objects": ["manga_bumper", "magna_housing"], 

    "task_description": "Pair two parts together by inserting the peg into the hole.",
    ...
    "default_poses": {
        "manga_bumper": [0.0, 0.3, 0.1] + R.from_euler('xyz', [0, 0, 0], degrees = True).as_quat(scalar_first = True).tolist(), 
        "magna_housing": [0.0, 0.1, 0.1] + R.from_euler('xyz', [0, 0, 0], degrees = True).as_quat(scalar_first = True).tolist(),
    }, 
