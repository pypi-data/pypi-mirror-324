...
def outline_mask_color(path_img,path_mask,path_file,algo,threshold,dico_algo_min,dico_algo_max):
    ...
    for file_mask in os.listdir(path_mask+"/"+algo):
        ...
        for i in df_mask.index:
            ...
            if df_mask.loc[i,'iou']>=threshold:
                color=(0,255,0)
            if df_mask.loc[i,'iou']==0:
                color=(0,0,1)
            else:
                color=(0,1*iou_cell,1*iou_cell)
...
def violin_iou(path_mask,path_file,title,dico_algo_min={},dico_algo_max={}):
    ...
    plt.xticks([r + width for r in range(len(list_iou_roi))],os.listdir(path_mask+"/"+algo),rotation=70)
    plt.title("IOU per images")
    plt.xlabel("Images")
    plt.xlabel("IOU")
    ...
    plt.title("Mean IOU per image",fontsize=10)
