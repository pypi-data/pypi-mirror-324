...
class RedisClient(metaclass=Singleton):
    def __init__(self,  host: str = None, port: int = None):
        ...
        if (self.host is None or self.port is None):
            logger.critical('RedisClient: failure to init redis - no host or port defined')
            return None

    def setConnectionDetails(self, host: str = None, port: int = None):
        if (self.host is None or self.port is None):
            logger.critical('RedisClient: failure to init redis - no host or port defined')
            return None
    ...
    def getDatabase(self):
        if (self.redisInstance is None):
            ...
            self.redisInstance = redis.Redis(host=self.host, port=self.port, db=0, decode_responses=True)
    ...
    def saveTempToDB (self, objId: str, dataObj : BaseObj, ttlType: TTL_Type = TTL_Type.LONG):
        ...
        objTTL = self.envUtils.get_ttl_by_type(ttlType=ttlType)
