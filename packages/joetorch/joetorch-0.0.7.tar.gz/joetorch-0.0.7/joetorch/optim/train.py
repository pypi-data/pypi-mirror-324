...
def train(
    ...
    for epoch in loop:
        ...
        if save_dir is not None:
            ...
            if save_metric[0] == 'minimise':
                if metrics[save_metric[2]] < best_metric:
                    best_metric = metrics[save_metric[2]]
                    # remove old model files
                    for f in os.listdir(save_dir):
                        os.remove(os.path.join(save_dir, f))
                    torch.save(model.state_dict(), os.path.join(save_dir, f'best_{save_metric[2]}_epoch_{epoch}.pth'))
            else:
                if metrics[save_metric[2]] > best_metric:
                    best_metric = metrics[save_metric[2]]
                    for f in os.listdir(save_dir):
