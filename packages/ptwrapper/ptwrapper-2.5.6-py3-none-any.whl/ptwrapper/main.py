...
def execute(root_scenario_path, session_file_path, mk, verbose=False):
    """
    ...
    theOsvePtrLogger = OsvePtrLogger(mk)
    ...
    ptr_log = theOsvePtrLogger.log(verbose=verbose)

    return execution, ptr_log


def simulation(mk, ptr_content, time_step=30, no_power=False, sa_ck=False, mga_ck=False,
               quaternions=False):
    """
    ...
    # Step 3: Execute OSVE
    execution, ptr_log = execute(temp_parent_path.name, session_file_path, mk)
