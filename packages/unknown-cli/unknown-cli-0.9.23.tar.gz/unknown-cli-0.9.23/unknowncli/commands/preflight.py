    ...
    CustomTool(
        ...
        custom_tool_folder=CustomToolFolder(
            ...
            custom_tool_def_list = [
                ...
                CustomToolDef(
                    name="Preflight and Commit on Server - Quick", command = pwsh_tool_exe, add_separator_before = True,
                    arguments = f"{pwsh_cmd_base} {base_cmd_quick} --use-server-submit"
                )
            ]
        )
    )
...
def get_shelve_preflight_build_id(shelve_changelist, type=PreflightType.FULL, stream="", find=False, print_output=True, abort_on_fail=True, submit_on_server=False):
    cmd = [
        sys.executable,
...
def move_open_files_to_default(shelve_changelist: str):
    ...
    if opened_file_list:
        if any('+l' in x['type'] for x in opened_file_list):
            secho("\nYour CL contains exclusive edit files:")
            for x in opened_file_list:
                secho(f"\n  {x['depotFile']}")
            secho(f"\nMake sure not to revert these files until your preflight completes! (Press any key to continue)", fg="green", bold=True)
            input('')
...
def main(
    ...
    use_server_submit: Optional[bool] = Option(False, help="If committing on successful preflight, should server side commit be used.")
):
    """
    ...
    if not build_id:
        ...
        if get_opened_file_list(change_obj.change):
            ...
            if use_server_submit:
                if not confirm(f"\nFiles are opened in CL {change_obj.change} and must be re-opened in default to submit on server. Re-open files?"):
                    abort("Aborting preflight due to open files in CL..")
                move_open_files_to_default(change_obj.change)
        else:
        ...
        build_id = get_shelve_preflight_build_id(
            shelve_changelist, type=type, stream=get_client_stream(change_obj.client), submit_on_server=use_server_submit
        )
