...
if TYPE_CHECKING:
    Data = Annotated[np.ndarray, "Blocksize x Channels"]
    RefData = Annotated[np.ndarray, "Blocksize"]
    Peaks = Annotated[np.ndarray, "Channels"]

def get_corrs_sampleshifts(indata: "Data", refdata: "RefData", burstLength: int) -> tuple["Data", "Peaks"]:
    ...
    Args:
        ...
        burstLength (int): The length of the burst in samples, to better fit the peak finding algorithm

    Returns:
        tuple[Data, Peaks]: The full cross correlations in the format (Blocksize x Channel) and a vector containing the peak positions for each channel
    """
    corrs = []
    sampleshifts = []
