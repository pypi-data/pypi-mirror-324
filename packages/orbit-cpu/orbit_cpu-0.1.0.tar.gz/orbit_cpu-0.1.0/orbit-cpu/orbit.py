...
DEVICES= {'cuda': True, 'cpu': False, 'auto': None}
...
def orbit_algorithm(
    ...
    knn_dist= faiss_knn(entities, m, d, k_core+1)
    ...
    distance_penalty= torch.exp(- core_dist_normalized)/(1 + l2_dist_normalized)
    ...
    return Clusters(
        entities= entities[sim_mask][label_mask].cpu().numpy(),
        classes= corrected_labels.cpu().numpy(),
