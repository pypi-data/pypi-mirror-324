...
class DSSATAnyLMPipeline:
    ...
    def answer_query(self, farmer_input_query):
        ...
        try:
            ...
            if self.interpreter_model_id.startswith("llama"):
                import time
    ...
    def interpret_simulation_results_for_farmer_llama(
        ...
        try:
            ...
            import json
            try:
                response_processed = response.replace("```", "").replace("json", "")
                json_response = json.loads(response_processed)
    ...
    def create_formulaic_ground_truth_answer_(self, sim_outputs_json, question_statement):
        """
