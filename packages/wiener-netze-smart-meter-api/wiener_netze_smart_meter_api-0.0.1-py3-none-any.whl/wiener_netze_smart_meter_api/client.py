...
class WNAPIClient:
    ...
    def get_quarter_hour_values(self, zaehlpunkt: str = None):
        """ 
        Fetches quarter-hourly measured values.
        If a zaehlpunkt is provided, fetches measured values for that specific meter.        
        """
        return self.get_messwerte("QUARTER_HOUR", zaehlpunkt)

    def get_daily_values(self, zaehlpunkt: str = None):
        """ 
        Fetches daily measured values. 
        If a zaehlpunkt is provided, fetches measured values for that specific meter.
        """
        return self.get_messwerte("DAY", zaehlpunkt)

    def get_meter_readings(self, zaehlpunkt: str = None):
        """ Fetches meter readings. 
        If a zaehlpunkt is provided, fetches meter readings for that specific meter."""
        return self.get_messwerte("METER_READ", zaehlpunkt)