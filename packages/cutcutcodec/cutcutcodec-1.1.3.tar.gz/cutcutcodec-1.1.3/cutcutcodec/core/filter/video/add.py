...
from cutcutcodec.core.filter.mix.video_cast import to_bgr, to_bgr_alpha
...
class _StreamVideoAdd(StreamVideo):
    ...
    def _snapshot(self, timestamp: Fraction, mask: torch.Tensor) -> torch.Tensor:
        ...
        frame_ref = self._frame_to_float(
            streams.pop(0)._snapshot(timestamp, mask)  # pylint: disable=W0212
        )
        for stream in streams:
            ...
            # combination
            other = self._frame_to_float(stream._snapshot(timestamp, mask))  # pylint: disable=W0212
