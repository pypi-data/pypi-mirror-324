...
class _StreamVideoFFMPEG(StreamVideo, _StreamFFMPEGBase):
    ...
    def _snapshot(self, timestamp: Fraction, mask: torch.Tensor) -> torch.Tensor:
        ...
        if frame_np.dtype != np.float32:
            # print(frame_np.min(), frame_np.max(), len(set(frame_np[:, :, 0].ravel().tolist())))
            frame_np = frame_np.astype(np.float32, copy=False)
            # frame_np /= 2**frame_av.format.components[0].bits - 1  # in range [0, 1]
            frame_np /= (1 << av.video.format.VideoFormat(pix_fmt).components[0].bits) - 1
        ...
        # rgb(a) to bgr(a)
            ...
            frame_np = np.swapaxes(frame_np, 0, 2)
