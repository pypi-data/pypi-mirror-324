...
async def process_file_w_llm(
    ...
    carrier = "other"
    if (
        "preprocessing" in prompt_library.library.keys() and
        "carrier" in prompt_library.library["preprocessing"].keys() and
        input_doc_type in prompt_library.library["preprocessing"]["carrier"]["placeholders"].keys()
    ):
        carrier_schema = prompt_library.library["preprocessing"]["carrier"]["placeholders"][input_doc_type]

        carrier_prompt = prompt_library.library["preprocessing"]["carrier"]["prompt"]
        carrier_prompt = carrier_prompt.replace("DOCUMENT_TYPE_PLACEHOLDER", input_doc_type)

        # identify carrier for customized prompting
        carrier = await identify_carrier(document, llm_client, carrier_prompt, carrier_schema)
    ...
    if (
        ...
        # TODO: This is a temporary solution to add description to the prompt.
        #  It will be removed once all the prompt files are updated to Doc Ai.
