  ...
  for Z in settings['Zlist']:
    for z1 in range(1, Z+1):
      ...
      for ftype in ['ir','lv','la','ec','pc','dr','ci','ai','em','pi']:
        if fmap[ftype][i]==b'': continue
        if not os.path.exists(fmap[ftype][i].decode()):
          a=atomdb.get_data(Z,z1,ftype)
        datsum=pyfits.getval(fmap[ftype][i].decode(), 'DATASUM', 1)
...
#
  ...
  if not cie:
    ...
    if isinstance(init_pop, str):
      ...
      # this is an initial temperature
      kT_init = util.convert_temp(init_pop, teunit, 'keV')
      init_pop_calc = _solve_ionbal_eigen(Z, kT_init, \
                       teunit='keV', \
                       filename=filename, datacache=datacache, debug=debug)
  ...
  # now get the index for the data in the eigenfile: could be different
  elsymb = atomic.Ztoelsymb(Z)
  if d.__contains__(elsymb.lower()):
    eigenhdu = elsymb.lower()
  else:
    eigenhdu = 'EIGEN'
  ...
  if cie:
    ...
    if Tdiff > 0.0:
      ...
      equilib = factorlow * d[eigenhdu].data['FEQB'][ite[0]]+\
                factorhigh * d[eigenhdu].data['FEQB'][ite[1]]
    else:
      equilib = d[eigenhdu].data['FEQB'][ite[0]]
  ...
  for ikT, kT in enumerate(kT_vec):
    ...
    if Z==1:
      for i in range(Z):
        for j in range(Z):
          lefteigenvec[i,j] = d[eigenhdu].data['VL'][kTindex]
          righteigenvec[i,j] = d[eigenhdu].data['VR'][kTindex]
    else:
      for i in range(Z):
        for j in range(Z):
          lefteigenvec[i,j] = d[eigenhdu].data['VL'][kTindex][i*Z+j]
          righteigenvec[i,j] = d[eigenhdu].data['VR'][kTindex][i*Z+j]


    work = numpy.array(init_pop_calc[1:] - d[eigenhdu].data['FEQB'][kTindex][1:], dtype=float)
    ...
    for itau, ttau in enumerate(tau_vec):
      if Z >1:
        for i in range(Z):
          worktmp[i] = fspectmp[i]*numpy.exp(d[eigenhdu].data['EIG'][kTindex,i]*delt*ttau)

      else:
        worktmp[0] = fspectmp[0]*numpy.exp(d[eigenhdu].data['EIG'][kTindex]*delt*ttau)

      frac = numpy.zeros(Z+1)
      for i in range(Z):
        for j in range(Z):
          frac[i+1] += worktmp[j]*righteigenvec[j][i]
        frac[i+1] += d[eigenhdu].data['FEQB'][kTindex][i+1]
