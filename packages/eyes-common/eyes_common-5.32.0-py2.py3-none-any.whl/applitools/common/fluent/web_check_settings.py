...
class WebCheckSettings(CheckSettings):
    ...
    def _dynamic_provider_from(self, region, dynamic_settings):
        if isinstance(region, string_types) or is_webelement(region):
            return DynamicRegionBySelector(TargetPath.region(region), dynamic_settings)
        elif is_list_or_tuple(region):
            return DynamicRegionBySelector(TargetPath.region(*region), dynamic_settings)
        elif isinstance(region, RegionLocator):
            return DynamicRegionBySelector(region, dynamic_settings)
