...
class CheckSettings(object):
    ...
    def dynamic(self, *args):
        ...
        try:
            self.values.dynamic_regions.append(
                self._dynamic_provider_from(region, dynamic_settings)
            )
        except TypeError as e:
            raise_from(TypeError("Wrong argument in .dynamic()"), e)
        return self
    ...
    def _dynamic_provider_from(self, region, dynamic_settings):
        if isinstance(region, (Region, Rectangle)):
            return DynamicRegionByRectangle(region, dynamic_settings)
        raise TypeError(
