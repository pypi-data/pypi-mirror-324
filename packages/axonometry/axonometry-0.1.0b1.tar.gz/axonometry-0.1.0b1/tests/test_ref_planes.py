...
class TestAxonometryReferencePlanes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.axo = Axonometry(self.alpha, self.beta, ref_planes_distance=100)
        p_xy = Point(x=100, y=50)
        p_yz = Point(y=100, z=50)
        p_zx = Point(z=100, x=50)
        p_xyz = Point(x=20, y=70, z=10)
        _ = self.axo["xy"].add_point(p_xy)
        _ = self.axo["yz"].add_point(p_yz)
        _ = self.axo["zx"].add_point(p_zx)
        _ = self.axo.add_point(p_xyz)
