    ...
    for i in range(gen):
        ...
        for j in range(activation_potentiation_len):
            ...
            if fit_start is True and i == 0:
                ...
                W = fit(cp.array(x_train_batch, dtype=dtype, copy=False), cp.array(y_train_batch, dtype=dtype, copy=False), activation_potentiation=act_pop[-1], dtype=dtype)
                weight_pop.append(W)
            
            model = evaluate(cp.array(x_train_batch, dtype=dtype, copy=False), cp.array(y_train_batch, dtype=dtype, copy=False), W=weight_pop[j], activation_potentiation=act_pop[j])
            acc = model[get_acc()]
