...
# Check if the package is installed or running from source
...
# TODO: I don't know why If don't get monitors here, the screen resolution will be wrong for secondary screen. Seems there are some conflict with computer_use_ootb_internal.computer_use_demo.tools
screens = get_monitors()
print(screens)
from computer_use_ootb_internal.computer_use_demo.loop import (
    PROVIDER_TO_DEFAULT_MODEL_NAME,
    APIProvider,
    sampling_loop_sync,
)

from computer_use_ootb_internal.computer_use_demo.tools import ToolResult
...
def process_input(user_input, state):
    ...
    if state["model"] == "gpt-4o + ShowUI" or state["model"] == "qwen2-vl-max + ShowUI" or state["model"] == "qwen-vl-7b-instruct + ShowUI":
...
def launch_app():
    ...
    with resources.files('computer_use_ootb_internal').joinpath('assets/examples/ootb_examples.json').open('r') as f:
    ...
    def update_textbox(selected_category, selected_option, selected_task):
        ...
        if preview_image_path:
            # Convert Windows path to Unix-style and get just the filename
            filename = os.path.basename(preview_image_path.replace('\\', '/'))
            preview_image = str(resources.files('computer_use_ootb_internal').joinpath('assets/examples/init_states').joinpath(filename))
            print(f"preview_image: {preview_image}")
        else:
            preview_image = None
    ...
    with gr.Accordion("Quick Start Prompt", open=False):  # open=False 表示默认收
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                initial_image_value = str(resources.files('computer_use_ootb_internal').joinpath('assets/examples/init_states/honkai_star_rail_showui.png'))
    ...
    package_assets_dir = str(resources.files('computer_use_ootb_internal').joinpath('assets'))
    demo.launch(share=True, allowed_paths=[package_assets_dir], server_port=7888)  # TODO: allowed_paths
