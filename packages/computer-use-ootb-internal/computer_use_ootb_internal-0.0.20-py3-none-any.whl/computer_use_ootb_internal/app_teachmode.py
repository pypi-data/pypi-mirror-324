...
# Create a shared state that can be accessed by both FastAPI and Gradio
class SharedState:
...
async def update_parameters(request: Request):
    ...
    return {"status": "success", "message": "Parameters updated", "new_args": vars(shared_state.args)}
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=8):
                ...
                # Store reference to chat_input in shared_state
        ...
        def check_for_updates():
            if shared_state.args is not None:
                return shared_state.args.task
            return None

        demo.load(fn=check_for_updates, outputs=[chat_input], every=1)

    # Mount FastAPI endpoint
