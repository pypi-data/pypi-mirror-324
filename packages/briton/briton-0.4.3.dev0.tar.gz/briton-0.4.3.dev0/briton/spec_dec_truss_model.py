...
class Model:
    def __init__(self, **kwargs):
        ...
        self._tool_call_id_fn = TOOL_CALL_ID_FNS.get(self._base_model)
    ...
    async def predict(self, model_input: Dict[str, Any], request: fastapi.Request):
        ...
        if schema is not None:
            spec_dec_request.output_schema_hash = await add_schema_to_cache(
                fsm_cache=self._fsm_cache, schema=schema, timeout=FSM_CREATION_TIMEOUT
            )
        ...
        if validated_input.stream:
            return create_completion_chunks(
                ...
                tool_call_id_fn=self._tool_call_id_fn,
            )
        else:
