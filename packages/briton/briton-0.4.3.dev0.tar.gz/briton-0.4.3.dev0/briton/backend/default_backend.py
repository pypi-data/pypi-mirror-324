...
class DefaultBackend(InferBackend):
    ...
    async def infer(
        ...
        response_streams = [self._briton_stub.Infer(briton_request)]
        if model_input.n is not None and model_input.n > 1:
            for i in range(1, model_input.n):
                ...
                response_streams.append(self._briton_stub.Infer(new_request))
        ...
        return await openai_spec_response(
            ...
            tool_call_id_fn=self._config.tool_call_id_fn,
