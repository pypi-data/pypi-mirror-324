...
from ai_kit.utils.prompts import load_prompt
...
# llama-3.1-8B-instruct is much faster than gpt-4o
# gpt-4o: 2.79, 1.87
# llama-3.1-8B-instruct: 0.92, 0.87


class Router:
    def __init__(
        self, route_registry: RouteRegistry, model: Optional[str] = None
    ):
        """
        ...
        # Try to load model from config if not provided
        if model is None:
            try:
                config = UserConfig.load_config()
                model = config.get("router", {}).get("default_model", "llama-3.3-70b-versatile")
            except Exception as e:
                logger.warning(f"Failed to load router default model from config.json: {e}")
                model = "llama-3.3-70b-versatile"  # Fallback to default
    ...
    def get_system_prompt(self) -> str:
        ...
        system_prompt = load_prompt(ROUTER_PROMPT_PATH)
        assert system_prompt is not None and system_prompt.strip() != ""
        with_route_list = system_prompt.format(route_list=route_list)
        return with_route_list
    ...
    def route(self, query: str) -> RoutingDecision:
        ...
        decision = self.client.structured_output_completion(
            messages=messages,
        ...
        if decision.route not in self.registry.route_names:
            ...
            # You can decide a default fallback
