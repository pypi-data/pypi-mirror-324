...
from rhino_health.lib.endpoints.data_schema.data_schema_dataclass import DataSchema, LTSDataSchema
from rhino_health.lib.endpoints.dataset.dataset_dataclass import Dataset, LTSDataset
...
class LTSProjectEndpoints(Endpoint):
    """
    @autoapi False
    ...
    @rhino_error_wrapper
    def get_projects(self, project_uids: Optional[List[str]] = None) -> List[LTSProject]:
        """
        @autoapi True
...
class ProjectEndpoints(LTSProjectEndpoints):
    """
    @autoapi True
    @hide_parent_class
    ...
    def get_project_stats(self, project_uid: str) -> Dict[str, Any]:
        return self.session.get(f"/projects/{project_uid}/stats").raw_response.json()

    @rhino_error_wrapper
    def add_project(self, project: ProjectCreateInput) -> LTSProject:
        """
    ...
    @rhino_error_wrapper
    def get_datasets(self, project_uid: str) -> List[LTSDataset]:
        """
    ...
    @rhino_error_wrapper
    def get_collaborating_workgroups(self, project_uid: Union[str, Project]):
        collaborating_workgroup_uids = (
            project_uid.collaborating_workgroup_uids
            if not isinstance(project_uid, str)
            else self.session.project.get_projects([project_uid])[0].collaborating_workgroup_uids
        )
        return self.session.workgroup.get_workgroups(collaborating_workgroup_uids)
    ...
    def aggregate_dataset_metric(
        ...
        return get_cloud_aggregated_metric_data(
            self.session, dataset_uids, metric_configuration, aggregation_method_override
        )
