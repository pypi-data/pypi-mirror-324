...
class LTSDataset(WithinProjectModel, WithinWorkgroupModel, BaseDataset, UserCreatedModel):
    """
    @autoapi False
    """
...
class Dataset(LTSDataset):
    ...
    @property
    def data_schema_info(self):
        """
        @autoapi False
        """
        return alias(
            self.data_schema,
            old_function_name="data_schema_info",
    ...
    def dataset_info(self):
        ...
        if not self._dataset_info:
            ...
            if "data" in result:
                ...
                # In progress so we don't want to cache this
                result = None
