...
class LTSCodeObjectEndpoints(Endpoint):
    """
    @autoapi False
    ...
    def search_for_code_objects_by_name(
        ...
        results = self.session.get("/code_objects", params=query_params)
    ...
    def get_build_logs(self, code_object_uid: str):
        ...
        "Starting to build..."
        """
        return self.session.get(f"/code_objects/{code_object_uid}/build_logs").raw_response.json()


class CodeObjectEndpoints(LTSCodeObjectEndpoints):
    """
    @autoapi True
    ...
    def create_code_object(
        ...
        if (
            ...
            cloud_provider = self.session.get("/system/cloud_provider").raw_response.json()["data"]
