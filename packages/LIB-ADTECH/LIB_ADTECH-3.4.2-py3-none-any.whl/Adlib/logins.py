...
def login_decorator(func):
    ...
    def loginCaptchaFunctionPrototype(driver: Chrome, usuario: str, senha: str, enumBanco: EnumBanco, enumProcesso: EnumProcesso) -> tuple[bool, str, str]:
        pass
...
@login_decorator
@captcha_decorator
def loginIBConsig(driver: Chrome, usuario: str, senha: str, enumBanco: EnumBanco, enumProcesso: EnumProcesso) -> tuple[bool, str, str]:
    
    def successfulLogin() -> bool:
    ...
    imgPath = saveCaptchaImage(driver, captchaElement, enumBanco, enumProcesso)
...
def loginBMG(driver: Chrome, usuario: str, senha: str, enumBanco: EnumBanco, enumProcesso: EnumProcesso) -> tuple[bool, str, str]:
    ...
    def successfulLogin():
        return driver.current_url == "https://www.bmgconsig.com.br/principal/fsconsignataria.jsp" # URL após login bem sucedido
...
@login_decorator
@captcha_decorator
def loginDaycoval(driver: Chrome, usuario: str, senha: str, enumBanco: EnumBanco, enumProcesso: EnumProcesso) -> tuple[bool, str, str]:
    ...
    if logou:=successfulLogin():
        mensagemTelegram(token, chatId, f"Entrou! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ✅")

    return logou, imgPath, captcha
...
if __name__=="__main__":
    ...
    user, senha = getCredenciais(227)
    
    loginDaycoval(driver, user, senha, EnumBanco.DAYCOVAL_CARTAO, EnumProcesso.IMPORTACAO)
