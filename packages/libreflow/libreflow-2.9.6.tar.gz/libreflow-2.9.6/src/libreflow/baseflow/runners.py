...
FILE_EXTENSIONS = ["blend", "kra", "png", "jpg", "txt", "nk", "abc", "mov", "psd", "psb", "aep", "prproj", "zip", "mp4", "mxf", "fbx", "ai", "json", "jsx", "obj", "wav", "xpix", "usd", "usda", "usdz","tvpp","tvp","tvpx"]
...
class RemoveHandler(flow.Action):
    ...
    def allow_context(self, context):
        return context and (
            'edit' in self._handler.status.get() and 'default' not in self._handler.status.get()
        )
...
class EditRunnerHandler(flow.Action):
    ...
    runner = flow.Param('', HandlerRunnerName).ui(editable=False, choice_icons=RUNNER_ICONS)
    ...
    def mapped_names(self, page_num=0, page_size=None):
        if self._cache is None:
            ...
            for runner_name, handlers in base_handlers.items():
                ...
                for handler in handlers:
                    ...
                    data.update(
                        ...
                        status='default',
                        enabled=True,
                        icon=runner_icon if runner_icon else ('icons.gui', 'cog-wheel-silhouette')
                    )
            ...
            for name in edit_names:
                ...
                runner_icon = factory.get_runner(handler_edit.runner.get()).runner_icon()
                data = {}
