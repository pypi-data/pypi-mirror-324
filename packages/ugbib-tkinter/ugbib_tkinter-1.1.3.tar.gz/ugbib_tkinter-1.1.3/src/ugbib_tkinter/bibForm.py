...
class Tooltip():
    ...
    def showTooltip(self, event=None):
        """showTooltip - Zeigt den Tooltip an der aktuellen Mausposition
        """
        x, y, _, _ = self.wdg.bbox("insert")      # Hole Position des Widgets
        x += self.wdg.winfo_rootx() + 25          # Verschiebe x etwas nach rechts
        y += self.wdg.winfo_rooty() + 25          # Verschiebe y etwas nach unten
        
        # Erstelle ein Top-Level-Window
        self.tooltip = tk.Toplevel(self.wdg)
        self.tooltip.wm_overrideredirect(True)    # Entferne Fensterrahmen
        
        # Erstelle Label im Tooltip
        label = tk.Label(self.tooltip, text=self.text, 
                      justify=tk.LEFT,
                      background="#ffffe0",      # Hellgelber Hintergrund
                      relief=tk.SOLID,           # Solider Rahmen
                      borderwidth=1,
...
class NaviForm(NaviWidget):
    ...
    def connectToModell(model):
        """connectToModell - Setzt die Factories Getter, Saver, Deleter
        
            Setzt für das Navi GetterValues, SaverValues und DeleterValues
        """
        m = model()
