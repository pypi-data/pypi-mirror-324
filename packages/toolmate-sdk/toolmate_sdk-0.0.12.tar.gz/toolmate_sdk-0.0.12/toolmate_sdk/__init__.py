...
def generate(
        ...
        tool:
            ...
            each item must be either one of the following options:
                ...
                4. a python script containing at least three variables:
                    ...
                    ii. TOOL_SCHEMA
                    iii. TOOL_METHOD
            remarks: parameters of both `schema` and `func` are ignored when `tool` parameter is given
    ...
    if system:
        ...
        if os.path.isfile(possible_system_file_path_1):
    ...
    if context:
        ...
        if os.path.isfile(possible_context_file_path_1):
            ...
            if context_file_content:
                context_content = context_file_content
        messagesCopy[-1]["content"] = context_content + messagesCopy[-1]["content"]
    ...
    if tool:
        ...
        possible_tool_file_path_1 = os.path.join(PACKAGE_PATH, "tools", f"{tool_entity}.py")
        possible_tool_file_path_2 = os.path.join(TOOLMATE_PATH, "tools", f"{tool_entity}.py")
        if os.path.isfile(possible_tool_file_path_1):
    ...
    is_last_request = True if not follow_up_prompt and not system and not context and not tool and not prefill else False
        ...
        if func:
            ...
            if function_response is None or function_response: # fall back to regular completion if function_response is None; chat extension if function_response
                if function_response:
                    ...
                    addContextToMessages(messagesCopy, function_response)
                return generate(
                    messagesCopy,
                    backend,
                    model=model,
