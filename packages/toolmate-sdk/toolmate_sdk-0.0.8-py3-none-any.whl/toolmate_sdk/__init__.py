...
def generate(
    ...
    if system:
        ...
        if os.path.isfile(possible_system_file_path):
            ...
            if system_file_content:
                system_instruction = system_file_content
        messagesCopy = updateSystemMessage(messagesCopy, system_instruction)
    ...
    if schemaCopy is not None: # structured output or function calling; allow schema to be an empty dict
        ...
        if func:
            ...
            output = json.dumps(dictionary_output, indent=4)
...
def updateSystemMessage(messages: List[Dict[str, str]], system: str) -> dict:
    new_messages = {i for i in messages if not i.get("role", "") == "system"}
    if system:
        new_messages.insert(0, {"role": "system", "content": system})
    return new_messages

def addContextToMessages(messages: List[Dict[str, str]], context: str) -> dict:
