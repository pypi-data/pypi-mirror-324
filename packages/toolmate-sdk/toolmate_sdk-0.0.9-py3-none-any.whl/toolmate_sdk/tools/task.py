...
from toolmate_sdk.utils.device_info import get_linux_distro
import platform
...
TOOL_SCHEMA = {
    ...
    "parameters": {
        ...
        "required": ["generated_python_code", "title", "risk"],
    },
}

def task(generated_python_code: str, title: str, risk: str):
    print(f"Task: {title}")
    #print(f"Risk: {risk}") # implement risk management later
    print()

    refined_python_code = fineTunePythonCode(generated_python_code)
