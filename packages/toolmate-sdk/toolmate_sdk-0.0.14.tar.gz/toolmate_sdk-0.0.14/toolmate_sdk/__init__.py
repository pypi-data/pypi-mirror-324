...
def generate(
    ...
    if messagesCopy.get("role", "") == "user":
        user_input = messagesCopy.get("content", "")
    else:
        user_input = follow_up_prompt.pop(0) if follow_up_prompt else DEFAULT_FOLLOW_UP_PROMPT
        messagesCopy.append({"role": "user", "content": user_input})
    ...
    if input_content_plugin:
        ...
        if input_content_plugin_func and user_input:
            messagesCopy[-1]["content"] = input_content_plugin_func(user_input)
        try:
            del CONTENT_PLUGIN
        except:
            pass
    ...
    is_last_request = True if not follow_up_prompt and not system and not context and not tool and not input_content_plugin and not output_content_plugin and not prefill else False
