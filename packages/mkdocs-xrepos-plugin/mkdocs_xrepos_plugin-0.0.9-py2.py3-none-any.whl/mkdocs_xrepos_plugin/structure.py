...
class Repo:
    ...
    async def sparse_clone(self, paths: List[str] = None) -> Tuple[str, str]:
        ...
        args = [self.url, self.name, self.branch, os.getenv("AzureDevOpsPAT")] + paths
...
class DocsRepo(Repo):
    ...
    def _fix_edit_uri(self, edit_uri: str) -> str:
        ...
        # Vérification si edit_uri est None ou vide
        if not edit_uri:
        ...
        # Si la branche est 'master' et que ce n'est pas la branche actuelle, on la remplace
        if parts > 1 and edit_uri_parts[1] == "master" and self.branch != "master":
            edit_uri_parts[1] = self.branch
        # Si le troisième élément est 'docs', on le supprime
        if parts > 2 and edit_uri_parts[2] == "docs":
            del edit_uri_parts[2]
        # Rejoint les parties de l'URI et ajoute un séparateur de fin si nécessaire
    ...
    def load_config(self) -> Dict:
        """Loads the repo's xrepos config file"""
