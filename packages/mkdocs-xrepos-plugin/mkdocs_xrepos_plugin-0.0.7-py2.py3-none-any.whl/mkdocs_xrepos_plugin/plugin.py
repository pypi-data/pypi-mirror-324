...
class XreposPlugin(BasePlugin):
    ...
    def setup_imported_repo(self, config: Config) -> Path:
        temp_dir = tempfile.mkdtemp(prefix="xrepos_")
    ...
    def handle_imported_repo(self, config: Config) -> Config:
        ...
        if "plugins" in new_config:
            ...
            for p in plugins_copy:
                if "search" in p:
                    log.info("Xrepos removing search")
        ...
        for repo in repos:
            ...
            docs_repo_objs.append(
                DocsRepo(
                    ...
                    branch=import_stmt.get("branch", repo.branch),
    ...
    def handle_nav_repos_import(self, config: Config, nav_repos: List[NavRepoConfig]) -> Config:
        need_to_derive_edit_uris = config.get("edit_uri") is None
        docs_repo_objs: List[DocsRepo] = []
        for nr in nav_repos:
            ...
            # Initialiser `derived_edit_uri` avec une valeur par défaut (vide ou autre)
            derived_edit_uri = ""
            
            # Si l'edit_uri n'est pas défini, on essaie de le dériver
            if need_to_derive_edit_uris:
                derived_edit_uri = self.derive_config_edit_uri(
            ...
            repo: DocsRepo = DocsRepo(
                ...
                edit_uri=import_stmt.get("edit_uri")
                or config.get("edit_uri")
                or derived_edit_uri,  # Utilisation sûre de `derived_edit_uri`
            )
            if repo.cloned:
                repo.delete_repo()
    ...
    def on_config(self, config: Config) -> Config:
        try:
            multi_config: XreposConfig = dc.from_dict(
                data_class=XreposConfig,
                data=self.config,
            ...
            raise ReposConfigException(
                f"unknown config key(s), {formatted_keys}, for XreposConfig"
            )
        if multi_config.imported_repo:
            ...
            if nav and repos:
                log.warning(
                    "Xrepos plugin is ignoring plugins.xrepos.repos. Nav takes precedence."
                )
            if not nav and nav_repos:
                log.warning(
                    "Xrepos plugin has nav_repos configuration without a nav section."
                )
            log.info("Xrepos plugin importing docs...")
            ...
            for repo in self.repos.values():
                ...
                for f in repo_files:
                    if f.src_path == repo_config_path:
                        log.info(
                            f"Xrepos plugin is not copying config file: {f.src_path}"
                        )
                    else:
    ...
    def on_post_build(self, config: Config) -> None:
        if self.config.get("imported_repo"):
            ...
            log.info(f"Xrepos plugin is cleaning up {temp_dir}/")
            shutil.rmtree(str(self.temp_dir))
