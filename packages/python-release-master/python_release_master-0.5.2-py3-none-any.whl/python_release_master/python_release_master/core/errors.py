...
ERROR_DESCRIPTIONS = {
    ...
    ErrorCode.PYPI_METADATA_ERROR: "Package metadata version is incompatible with the build system.",
...
ERROR_FIXES = {
    ...
    ErrorCode.PYPI_METADATA_ERROR: [
        "Update your build system configuration in pyproject.toml",
        "Switch to a newer build backend like hatchling or setuptools>=61.0.0",
        "Or remove any fields that require newer metadata versions",
    ...
    if isinstance(error, ReleaseError):
        ...
        if error.cause:
            ...
            if error.cause.__traceback__:
                logger.debug("Cause traceback:\n%s", "".join(traceback.format_tb(error.cause.__traceback__)))
