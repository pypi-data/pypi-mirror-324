...
class VersionManager:
    ...
    def get_current_versions(self) -> Dict[str, str]:
        ...
        if len(set(versions.values())) > 1:
            # Create a formatted version table
            ...
            raise ReleaseError(
                ...
                context={
                    "versions": versions,
                    "files_checked": list(versions.keys()),
                    "unique_versions": list(set(versions.values()))
                },
                fix_instructions=[
                    "The following files have different version numbers:",
                    version_table,
                    "",
                    "To fix this:",
                    "1. Choose which version is correct",
                    "2. Update all files to use the same version",
                    "3. Consider using a version management tool to keep versions in sync"
                ]
            )
