...
async def carla_game(carla_client):
    ...
    try:
        while not kill_carla_client:                    
            ...
            # buffer.close()
            jpeg_as_text = base64.b64encode(jpeg_bytes).decode()
            # print("Sending jpeg bytes through carla_game_frame", len(jpeg_bytes))

            await sio.emit('carla_game_frame', jpeg_as_text)
