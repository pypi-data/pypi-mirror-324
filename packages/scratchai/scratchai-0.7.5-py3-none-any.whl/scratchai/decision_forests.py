...
class RandomForest:
    ...
    def _build_tree(self, X: np.ndarray, y: np.ndarray) -> Tuple[DecisionTreeClassifier, Optional[np.ndarray]]:
        ...
        tree.fit(X[bootsraped], y[bootsraped], self.num_split_features)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, n_trees: Optional[int] = 50, oob_eval: Optional[bool] = False):
        ...
        self.num_split_features = int(np.sqrt(n_features))
