...
class HabrParser:
    ...
    def get_user_page(self, url):
        ...
        avatar =soup.find('link', rel='image_src')
        if avatar and avatar.get('href'):
        ...
        if not avatar:
            avatar = soup["src"] if (soup:=new_soup.select_one("div.user-info_profile > div > a > img.avatario")) else ""
