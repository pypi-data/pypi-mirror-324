...
class XParticle(iGeo, Particle):
    ...
    def _pre_lambda(self, old_value: Walk, new_value: Walk, op):
    ...
    def _pre_max(self, old_value, new_value):
        return self._pre_lambda(old_value, new_value, self._lambda_max)

    def _pre_min(self, old_value, new_value):
        return self._pre_lambda(old_value, new_value, self._lambda_min)

    def _pre_average(self, old_value, new_value):
        return self._pre_lambda(old_value, new_value, self._lambda_average)

    def _pre_copy(self, old_value, new_value):
        return self._pre_average(old_value, new_value)

    def _pre_freq(self, old_value, new_value):
        return self._pre_lambda(old_value, new_value, self._lambda_freq)

    def _pre_sum(self, old_value, new_value):
        return self._pre_lambda(old_value, new_value, operator.add)
    ...
    async def _wip_build_walk(self, data) -> List:
        ...
        if geo_key_values:
            ...
            log.warning(
                "Ingoring measure because is outside of geolocator: [%s] -> [%s]",
                _geo_hash,
                geo_json,
            )
            foo = 1
...
class XParticleLayout(XParticle):
    async def _compute(self, edge, ekeys):
        ...
        """
        # TODO: review and adapt the result to the desired data layout ...
