...
class PulumiContextManager:
    ...
    def _package_requirements(self, requirements: str, job_name: str) -> str:
        ...
        # find max whl version in dist/
        whl_files = list(Path("dist").glob("*.whl"))
        max_whl_version = max(whl_files, key=lambda x: x.stem.split("-")[1])
...
COPY {max_whl_version} /{max_whl_version}

RUN python3 -m pip install /{max_whl_version}
