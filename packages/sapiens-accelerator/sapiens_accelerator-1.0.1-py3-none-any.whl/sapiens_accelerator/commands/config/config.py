...
from sapiens_accelerator.utils import ComputeEnvironment
...
def get_user_input():
    ...
    if compute_environment == ComputeEnvironment.AMAZON_SAGEMAKER: config = get_sagemaker_input()
    else: config = get_cluster_input()
    return config
def config_command_parser(subparsers=None):
    if subparsers is not None: parser = subparsers.add_parser("config", description=description)
    else: parser = argparse.ArgumentParser("SapiensAccelerator config command", description=description)
    ...
    if subparsers is not None: parser.set_defaults(func=config_command)
    return parser
def config_command(args):
    config = get_user_input()
    if args.config_file is not None: config_file = args.config_file
    else:
    ...
    else: config.to_yaml_file(config_file)
def main():
