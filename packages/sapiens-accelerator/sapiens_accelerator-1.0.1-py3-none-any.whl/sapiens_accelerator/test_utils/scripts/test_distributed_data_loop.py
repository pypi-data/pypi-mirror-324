"""
    ...
    ########################################################################################################################################################
"""
from torch.utils.data import (BatchSampler, DataLoader, Dataset, IterableDataset, RandomSampler, TensorDataset, default_collate)
from sapiens_accelerator.accelerator import Accelerator, DataLoaderConfiguration
...
class DummyDataset(Dataset):
    def __len__(self): return NUM_ELEMENTS
    def __getitem__(self, index):
        squeeze = False
        if isinstance(index, int):
            index = [index]
            squeeze = True
        elif isinstance(index, slice): index = list(range(*index.indices(self.size)))
        else: index = list(index)
        batch = [{"index": i, "label": i % 2, "random_augmentation": torch.rand(1).item()} for i in index]
        if squeeze: batch = batch[0]
        return batch
...
def create_dataloader(accelerator: Accelerator, dataset_size: int, batch_size: int, iterable: bool = False, shuffle: bool = False):
    ...
    if shuffle: values = values[torch.randperm(values.size(0))]
    if iterable: dataset = DummyIterableDataset(values)
    else: dataset = TensorDataset(torch.as_tensor(range(dataset_size)))
...
def verify_dataloader_batch_sizes(accelerator: Accelerator, dataset_size: int, batch_size: int, process_0_expected_batch_sizes: List[int], process_1_expected_batch_sizes: List[int]):
    ...
    if accelerator.process_index == 0: assert batch_sizes == process_0_expected_batch_sizes
    elif accelerator.process_index == 1: assert batch_sizes == process_1_expected_batch_sizes
def test_default_ensures_even_batch_sizes():
    accelerator = create_accelerator()
    verify_dataloader_batch_sizes(accelerator, dataset_size=3, batch_size=1, process_0_expected_batch_sizes=[1, 1], process_1_expected_batch_sizes=[1, 1])
    verify_dataloader_batch_sizes(accelerator, dataset_size=7, batch_size=2, process_0_expected_batch_sizes=[2, 2], process_1_expected_batch_sizes=[2, 2])
...
def test_can_join_uneven_inputs():
    ...
    if accelerator.process_index == 0: assert batch_idxs == [0, 1]
    elif accelerator.process_index == 1: assert batch_idxs == [0]
def test_join_raises_warning_for_non_ddp_distributed(accelerator):
    with warnings.catch_warnings(record=True) as w:
        with accelerator.join_uneven_inputs([Mock()]): pass
...
def test_join_can_override_for_mixed_type_dataloaders():
    ...
    with warnings.catch_warnings():
        ...
        try:
            with accelerator.join_uneven_inputs([ddp_model], even_batches=overridden_even_batches): batch_dl_overridden_value = batch_dl.batch_sampler.even_batches
        except AttributeError: raise AssertionError
...
def test_join_raises_warning_for_iterable_when_overriding_even_batches():
    ...
    create_dataloader(accelerator, dataset_size=3, batch_size=1, iterable=True)
    with warnings.catch_warnings(record=True) as w:
        with accelerator.join_uneven_inputs([ddp_model], even_batches=False): pass
        assert issubclass(w[-1].category, UserWarning)
...
def test_data_loader(data_loader, accelerator):
    ...
    assert (len(set(sorted_all_examples)) == NUM_ELEMENTS), "Not all the dataset elements have been iterated in an epoch due to duplication of samples across processes."
def test_stateful_dataloader(accelerator):
    ...
    try:
        ...
        for step, batch in enumerate(prepared_dl):
            if step == last_batch_num - 1: state_dict = prepared_dl.state_dict()
            if step >= last_batch_num: untrained_batches.append(batch)
        not_skipped_batches = accelerator.gather(untrained_batches)
        prepared_dl.load_state_dict(state_dict)
        resumed_batches = []
        for batch in prepared_dl: resumed_batches.append(batch)
        resumed_batches = accelerator.gather(resumed_batches)
        for b1, b2 in zip(not_skipped_batches, resumed_batches):
            for v1, v2 in zip(b1, b2): assert torch.equal(v1, v2), f"Batch {b1} and {b2} are not equal"
    finally: accelerator.dataloader_config = old_dataloader_config
def test_stateful_dataloader_save_state(accelerator):
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            last_batch_num = total_batches - 1
            for step, batch in enumerate(prepared_dl):
                if step == last_batch_num - 1: accelerator.save_state(tmpdir)
                if step >= last_batch_num: untrained_batches.append(batch)
            not_skipped_batches = accelerator.gather(untrained_batches)
