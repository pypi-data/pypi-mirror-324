...
if is_megatron_lm_available():
    from megatron.training import (build_train_valid_test_data_iterators, get_optimizer_param_scheduler, num_floating_point_operations, setup_model_and_optimizer, train_step, training_log)
    from megatron.initialize import (_compile_dependencies, _init_autoresume, _initialize_distributed, _set_random_seed, set_jit_fusion_options, write_args_to_tensorboard)
    from megatron.utils import (average_losses_across_data_parallel_group, calc_params_l2_norm, get_ltor_masks_and_position_ids, unwrap_model)
    from megatron.text_generation.generation import (beam_search_and_return_on_first_stage, generate_tokens_probs_and_return_on_first_stage)
    from megatron.arguments import (_add_data_args, _add_validation_args, core_transformer_config_from_args, parse_args, validate_args)
    from megatron import (get_args, get_num_microbatches, get_tensorboard_writer, get_tokenizer, print_rank_last)
...
def model_provider_func(pre_process=True, post_process=True, add_encoder=True, add_decoder=True):
    ...
    if args.model_type_name == "bert":
        if args.pretraining_flag:
            ...
            model = BertModel(config=config, num_tokentypes=num_tokentypes, add_binary_head=args.bert_binary_head, parallel_output=True, pre_process=pre_process, post_process=post_process)
        else: model = Classification(config=config, num_classes=args.num_labels, num_tokentypes=2, pre_process=pre_process, post_process=post_process)
    elif args.model_type_name == "gpt": model = GPTModel(config=config, num_tokentypes=0, parallel_output=True, pre_process=pre_process, post_process=post_process)
    elif args.model_type_name == "t5": model = T5Model(config=config, num_tokentypes=0, parallel_output=True, pre_process=pre_process, post_process=post_process, add_encoder=add_encoder, add_decoder=add_decoder)
    else: raise ValueError(f"Unsupported model type: {args.model_type_name}")
    return model
def prepare_model_optimizer_scheduler(accelerator):
    ...
    if accelerator.state.megatron_lm_plugin.custom_prepare_model_function is not None:
        if accelerator.state.megatron_lm_plugin.custom_model_provider_function is None: raise ValueError("You must provide a `custom_model_provider_function` when using a `custom_prepare_model_function`.")
        ...
        if args.model_type_name == "t5": model_type = ModelType.encoder_and_decoder
        model_provider_func_ = model_provider_func
        if accelerator.state.megatron_lm_plugin.custom_model_provider_function is not None: model_provider_func_ = accelerator.state.megatron_lm_plugin.custom_model_provider_function
        (model, optimizer, scheduler) = setup_model_and_optimizer(model_provider_func_, model_type, no_wd_decay_cond=args.no_wd_decay_cond, scale_lr_cond=args.scale_lr_cond, lr_mult=args.lr_mult)
...
class MegatronLMDummyDataLoader:
    ...
    def get_train_valid_test_datasets_provider(self, accelerator):
        def train_valid_test_datasets_provider(train_val_test_num_samples):
            args = get_args()
            dataset_args = {"data_prefix": args.data_path if isinstance(args.data_path, (list, tuple)) else [args.data_path], "splits_string": args.split, "train_valid_test_num_samples": train_val_test_num_samples, "seed": args.seed}
            if args.model_type_name == "bert": dataset_args.update({"max_seq_length": args.seq_length, "binary_head": args.bert_binary_head})
            elif args.model_type_name == "gpt": dataset_args.update({"max_seq_length": args.seq_length})
            elif args.model_type_name == "t5": dataset_args.update({"max_seq_length": args.encoder_seq_length, "max_seq_length_dec": args.decoder_seq_length, "dataset_type": "t5"})
            else: raise ValueError(f"Unsupported model type: {args.model_type_name}")
            train_ds, valid_ds, test_ds = build_train_valid_test_datasets(**dataset_args)
            return train_ds, valid_ds, test_ds
        if accelerator.state.megatron_lm_plugin.custom_megatron_datasets_provider_function is not None: return accelerator.state.megatron_lm_plugin.custom_megatron_datasets_provider_function
        try:
            args = get_args()
            if args.model_type_name == "bert":
    ...
    def build_train_valid_test_data_iterators(self, accelerator):
        ...
        else: train_data_iterator, valid_data_iterator, test_data_iterator = build_train_valid_test_data_iterators(train_valid_test_dataset_provider)
...
def _handle_megatron_data_iterator(accelerator, data_iterator):
    ...
    torch.distributed.broadcast(is_src_data_iterator_empty, get_tensor_model_parallel_src_rank(), group=get_tensor_model_parallel_group())
...
def prepare_data_loader(accelerator, dataloader):
    ...
    if not args.megatron_dataset_flag:
        ...
        if kwargs["batch_size"] is None:
            if isinstance(kwargs["sampler"], torch.utils.data.BatchSampler): kwargs["sampler"].batch_size = micro_batch_size
            else:
                del kwargs["sampler"]
                del kwargs["shuffle"]
        ...
        return prepare_data_loader(dataloader, accelerator.device, num_processes=mpu.get_data_parallel_world_size(), process_index=mpu.get_data_parallel_rank(),
        split_batches=False, put_on_device=True, rng_types=accelerator.rng_types.copy(), dispatch_batches=accelerator.dispatch_batches)
    else:
        if args.consumed_samples is not None: (args.consumed_train_samples, args.consumed_valid_samples, args.consumed_test_samples) = args.consumed_samples
        else: args.consumed_train_samples, args.consumed_valid_samples, args.consumed_test_samples = 0, 0, 0
        args.micro_batch_size = args.micro_batch_size * args.num_micro_batches
        (train_data_iterator, valid_data_iterator, test_data_iterator) = dataloader.build_train_valid_test_data_iterators(accelerator)
        args.micro_batch_size = args.micro_batch_size // args.num_micro_batches
        ...
        return train_data_iterator, valid_data_iterator, test_data_iterator
class MegatronLMOptimizerWrapper(SapiensAcceleratordOptimizer):
    def __init__(self, optimizer): super().__init__(optimizer, device_placement=False, scaler=None)
    def zero_grad(self, set_to_none=None): pass
    def step(self): pass
    @property
    def step_was_skipped(self): return self.optimizer.skipped_iter
...
class MegatronLMDummyScheduler:
    def __init__(self, optimizer, total_num_steps=None, warmup_num_steps=0, **kwargs):
        ...
        self.kwargs = kwargs
class MegatronLMSchedulerWrapper(SapiensAcceleratordScheduler):
...
class AbstractTrainStep(ABC):
    def __init__(self, name):
        super().__init__()
        self.name = name
    def get_batch_func(self, accelerator, megatron_dataset_flag): pass
    def get_forward_step_func(self): pass
    def get_loss_func(self, accelerator): pass
class BertTrainStep(AbstractTrainStep):
    def __init__(self, accelerator, args):
        ...
        if not args.model_return_dict: self.model_output_class = None
        else:
    ...
    def get_batch_func(self, accelerator, megatron_dataset_flag):
        def get_batch_megatron(data_iterator):
            ...
            if data_iterator is not None: data = next(data_iterator)
            else: data = None
        ...
        def get_batch_transformer(data_iterator):
            ...
            if "token_type_ids" in data: types = data["token_type_ids"].long()
            else: types = None
            if "labels" in data:
                ...
                lm_labels = None
                loss_mask = None
            if "next_sentence_label" in data: sentence_order = data["next_sentence_label"].long()
            else: sentence_order = None
            return tokens, types, sentence_order, loss_mask, lm_labels, padding_mask
        if accelerator.state.megatron_lm_plugin.custom_get_batch_function is not None: return accelerator.state.megatron_lm_plugin.custom_get_batch_function
        ...
        else: return get_batch_transformer
    def get_loss_func(self, accelerator, pretraining_flag, num_labels):
        ...
        if accelerator.state.megatron_lm_plugin.custom_loss_function is not None: return accelerator.state.megatron_lm_plugin.custom_loss_function
        if pretraining_flag: return loss_func_pretrain
        else: return loss_func_finetune
    def get_forward_step_func(self, pretraining_flag, bert_binary_head):
        def forward_step(data_iterator, model):
            ...
            if not bert_binary_head: types = None
            if pretraining_flag:
...
class GPTTrainStep(AbstractTrainStep):
    ...
    def get_loss_func(self, accelerator):
        ...
        def loss_func(loss_mask, output_tensor):
            if args.return_logits: losses, logits = output_tensor
            else: losses = output_tensor
            losses = losses.float()
            ...
            if args.context_parallel_size > 1:
                ...
                loss = loss[0] / loss[1]
            else: loss = torch.sum(losses.view(-1) * loss_mask) / loss_mask.sum()
            ...
            if args.return_logits: output_dict.update({"logits": logits})
...
class T5TrainStep(AbstractTrainStep):
    ...
    def get_batch_func(self, accelerator, megatron_dataset_flag):
        ...
        def get_batch_transformer(data_iterator):
            ...
            if "decoder_input_ids" in data: tokens_dec = data["decoder_input_ids"].long()
            else:
...
def initialize(accelerator, extra_args_provider=None, args_defaults={}):
    ...
    args = get_args()
    if getattr(args, "padded_vocab_size", None) is None: args.padded_vocab_size = _vocab_size_with_padding(args.orig_vocab_size, args)
    if args.model_type_name == "bert" and args.pretraining_flag and args.num_labels == 2: args.bert_binary_head = True
    else: args.bert_binary_head = False
    args.iteration = 0
class MegatronEngine(torch.nn.Module):
    def __init__(self, accelerator, model, optimizer, scheduler):
        ...
        args = get_args()
        if accelerator.state.megatron_lm_plugin.custom_train_step_class is not None: self.train_step_handler = accelerator.state.megatron_lm_plugin.custom_train_step_class(args, **accelerator.state.megatron_lm_plugin.custom_train_step_kwargs)
        elif args.model_type_name == "bert": self.train_step_handler = BertTrainStep(accelerator, args)
        elif args.model_type_name == "gpt": self.train_step_handler = GPTTrainStep(accelerator, args)
        elif args.model_type_name == "t5": self.train_step_handler = T5TrainStep(accelerator, args)
        else: raise ValueError(f"Unsupported model type: {args.model_type_name}")
        ...
        if args.tensorboard_dir is not None: write_args_to_tensorboard()
    def get_module_config(self):
        ...
        if isinstance(self.module[0], LocalDDP) and args.overlap_grad_reduce:
            ...
            if len(self.module) == 1: config.no_sync_func = config.no_sync_func[0]
            if args.delay_grad_reduce:
                config.grad_sync_func = [model_chunk.start_grad_sync for model_chunk in self.module]
                if len(self.module) == 1: config.grad_sync_func = config.grad_sync_func[0]
        if args.overlap_param_gather and args.delay_param_gather:
            ...
            if len(self.module) == 1: config.param_sync_func = config.param_sync_func[0]
        config.finalize_model_grads_func = finalize_model_grads
        return config
    def train(self):
        for model_module in self.module: model_module.train()
        if self.module_config is None: self.module_config = self.get_module_config()
        self.log_eval_results()
    def eval(self):
        for model_module in self.module: model_module.eval()
        if self.module_config is None: self.module_config = self.get_module_config()
    def get_batch_data_iterator(self, batch_data):
        ...
        if len(batch_data) > 0:
            if args.num_micro_batches > 1:
                for i in range(0, args.num_micro_batches): data_chunks.append({k: v[i * args.micro_batch_size : (i + 1) * args.micro_batch_size] for k, v in batch_data.items()})
            else: data_chunks = [batch_data]
        if len(self.module) > 1: batch_data_iterator = ([iter(data_chunks) for _ in range(len(self.module))] if len(batch_data) > 0 else [None] * len(self.module))
        else: batch_data_iterator = iter(data_chunks) if len(batch_data) > 0 else None
        return batch_data_iterator
    def train_step(self, **batch_data):
        batch_data_iterator = self.get_batch_data_iterator(batch_data)
        loss_reduced, skipped_iter, grad_norm, num_zeros_in_grad = train_step(forward_step_func=self.train_step_handler.forward_step, data_iterator=batch_data_iterator,
        model=self.module, optimizer=self.optimizer, opt_param_scheduler=self.scheduler, config=self.module_config)
    ...
    def eval_step(self, **batch_data):
        ...
        loss_dicts = forward_backward_func(forward_step_func=self.train_step_handler.forward_step, data_iterator=batch_data_iterator, model=self.module,
        num_microbatches=get_num_microbatches(), seq_length=args.seq_length, micro_batch_size=args.micro_batch_size, forward_only=True)
        if args.empty_unused_memory_level >= 1: torch.cuda.empty_cache()
        ...
        if mpu.is_pipeline_last_stage(ignore_virtual=True):
            ...
            for key in loss_dicts[0]:
                ...
                if len(losses_reduced_for_key[0].shape) == 0: loss_reduced[key] = sum(losses_reduced_for_key) / len(losses_reduced_for_key)
                else: loss_reduced[key] = torch.concat(losses_reduced_for_key)
            return loss_reduced
        return {}
    def forward(self, **batch_data):
        ...
        if self.module[0].training:
            ...
            if args.tensorboard_dir is not None:
                ...
                params_norm = None
                if args.log_params_norm: params_norm = calc_params_l2_norm(self.model)
                self.report_memory_flag = training_log(loss_dict, self.total_loss_dict, self.optimizer.param_groups[0]["lr"], self.iteration, loss_scale,
                self.report_memory_flag, skipped_iter, grad_norm, params_norm, num_zeros_in_grad)
        else:
        ...
        for key in loss_dict:
            if len(loss_dict[key].shape) == 0: loss += loss_dict[key]
        logits = None
        if "logits" in loss_dict: logits = loss_dict["logits"]
        if self.train_step_handler.model_output_class is not None: return self.train_step_handler.model_output_class(loss=loss, logits=logits)
        return loss
    def log_eval_results(self):
        args = get_args()
        if args.tensorboard_dir is None or self.iteration == 0: return
        args = get_args()
        writer = get_tensorboard_writer()
        string = f"validation loss at iteration {self.iteration} | "
        for key in self.eval_total_loss_dict:
            if key.endswith("_num_iters"): continue
            ...
            if args.pretraining_flag: string += f"{key} PPL: {ppl} | "
            if writer:
                writer.add_scalar(f"{key} validation", value.item(), self.iteration)
                if args.pretraining_flag: writer.add_scalar(f"{key} validation ppl", ppl, self.iteration)
        length = len(string) + 1
        self.eval_total_loss_dict = {}
    ...
    def load_checkpoint(self, input_dir):
        ...
        if args.fp16 and self.iteration == 0: self.optimizer.reload_model_params()
    def megatron_generate(self, inputs, attention_mask=None, max_length=None, max_new_tokens=None, num_beams=None, temperature=None, top_k=None, top_p=None, length_penalty=None, **kwargs):
        args = get_args()
        if args.model_type_name != "gpt": raise NotImplementedError("Generate method is not implemented for this model")
        if args.data_parallel_size > 1: raise ValueError("Generate method requires data parallelism to be 1")
        if args.sequence_parallel: raise ValueError("Generate method requires sequence parallelism to be False")
        if args.recompute_granularity is not None: raise ValueError("Checkpoint activations cannot be set for inference")
        if args.vocab_file is None: raise ValueError("Vocab file is required for inference")
        if max_length is None and max_new_tokens is None: raise ValueError("`max_length` or `max_new_tokens` are required for inference")
        if temperature is None: temperature = 1.0
        elif not (0.0 < temperature <= 100.0): raise ValueError("temperature must be a positive number less than or equal to 100.0")
        if top_k is None: top_k = 0
        elif not (0 <= top_k <= 1000): raise ValueError("top_k must be a positive number less than or equal to 1000")
        if top_p is None: top_p = 0.0
        elif top_p > 0.0 and top_k > 0.0: raise ValueError("top_p and top_k sampling cannot be set together")
        else:
            if not (0.0 <= top_p <= 1.0): raise ValueError("top_p must be less than or equal to 1.0")
        top_p_decay = kwargs.get("top_p_decay", 0.0)
        if not (0.0 <= top_p_decay <= 1.0): raise ValueError("top_p_decay must be less than or equal to 1.0")
        top_p_bound = kwargs.get("top_p_bound", 0.0)
        if not (0.0 <= top_p_bound <= 1.0): raise ValueError("top_p_bound must be less than or equal to 1.0")
        add_BOS = kwargs.get("add_BOS", False)
        if not (isinstance(add_BOS, bool)): raise ValueError("add_BOS must be a boolean")
        beam_width = num_beams
        if beam_width is not None:
            if not isinstance(beam_width, int): raise ValueError("beam_width must be an integer")
            if beam_width < 1: raise ValueError("beam_width must be greater than 0")
            if inputs.shape[0] > 1: return "When doing beam_search, batch size must be 1"
        tokenizer = get_tokenizer()
        stop_token = kwargs.get("stop_token", tokenizer.eod)
        if stop_token is not None:
            if not isinstance(stop_token, int): raise ValueError("stop_token must be an integer")
        if length_penalty is None: length_penalty = 1.0
        sizes_list = None
        prompts_tokens_tensor = None
        prompts_length_tensor = None
        if torch.distributed.get_rank() == 0:
            if attention_mask is None: prompts_length_tensor = torch.cuda.LongTensor([inputs.shape[1]] * inputs.shape[0])
            else: prompts_length_tensor = attention_mask.sum(axis=-1).cuda()
            if max_new_tokens is None: max_new_tokens = max_length - inputs.shape[1]
            if max_new_tokens <= 0: raise ValueError("max_new_tokens must be greater than 0")
            if add_BOS:
        ...
        if beam_width is not None: tokens, _ = beam_search_and_return_on_first_stage(unwrapped_model, context_tokens_tensor, context_length_tensor, beam_width, stop_token=stop_token, num_return_gen=1, length_penalty=length_penalty)
        else: tokens, _, _ = generate_tokens_probs_and_return_on_first_stage(unwrapped_model, context_tokens_tensor, context_length_tensor, return_output_log_probs=False,
        top_k=top_k, top_p=top_p, top_p_decay=top_p_decay, top_p_bound=top_p_bound, temperature=temperature, use_eod_token_for_early_termination=True)
        return tokens
def avg_losses_across_data_parallel_group(losses): return average_losses_across_data_parallel_group(losses)
