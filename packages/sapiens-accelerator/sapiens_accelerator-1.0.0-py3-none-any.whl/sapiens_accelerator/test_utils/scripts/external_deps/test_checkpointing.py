...
def training_function(config, args):
    ...
    if args.partial_train_epoch is not None: ending_epoch = args.partial_train_epoch
    if args.resume_from_checkpoint:
        accelerator.load_state(args.resume_from_checkpoint)
        epoch_string = args.resume_from_checkpoint.split("epoch_")[1]
        state_epoch_num = ""
        for char in epoch_string:
            if char.isdigit(): state_epoch_num += char
            else: break
        ...
        with open(os.path.join(args.output_dir, f"state_{starting_epoch - 1}.json")) as f:
            ...
            assert (resumed_state["lr"] == lr_scheduler.get_lr()[0]), "Scheduler learning rate mismatch, loading from checkpoint failed"
            assert (resumed_state["optimizer_lr"] == optimizer.param_groups[0]["lr"]), "Optimizer learning rate mismatch, loading from checkpoint failed"
    ...
    for epoch in range(starting_epoch, ending_epoch):
        ...
        if accelerator.is_main_process:
            with open(os.path.join(args.output_dir, f"state_{epoch}.json"), "w") as f: json.dump(state, f)
...
def main():
    ...
    parser.add_argument("--partial_train_epoch", type=int, default=None, help="If passed, the training will stop after this number of epochs.")
