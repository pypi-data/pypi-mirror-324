...
from ...utils import (ComputeEnvironment, DistributedType, is_deepspeed_available, is_fp8_available, is_mlu_available, is_mps_available, is_msamp_available,
is_musa_available, is_npu_available, is_transformer_engine_available, is_transformers_available, is_xpu_available)
...
def get_cluster_input():
    ...
    if distributed_type in [DistributedType.MULTI_GPU, DistributedType.MULTI_MLU, DistributedType.MULTI_MUSA, DistributedType.MULTI_NPU, DistributedType.MULTI_XPU, DistributedType.MULTI_CPU]:
        ...
        if num_machines > 1:
            ...
            if not same_network: rdzv_backend = _ask_field("What rendezvous backend will you use? ('static', 'c10d', ...): ", default="static")
    ...
    if distributed_type == DistributedType.NO: use_cpu = _ask_field("Do you want to run your training on CPU only (even if a GPU / Apple Silicon / Ascend NPU device is available)? [yes/NO]:", _convert_yes_no_to_bool, default=False, error_message="Please enter yes or no.")
    elif distributed_type == DistributedType.MULTI_CPU: use_cpu = True
    else: use_cpu = False
    ipex_config = {}
    mpirun_config = {}
    if use_cpu:
        ...
        if distributed_type == DistributedType.MULTI_CPU:
            use_mpirun = _ask_field("Do you want sapiens_accelerator to launch mpirun? [yes/NO]: ", _convert_yes_no_to_bool, default=False, error_message="Please enter yes or no.")
            if use_mpirun:
    ...
    if (not use_cpu and is_xpu_available() and distributed_type not in [DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.MULTI_MLU,
    DistributedType.XLA, DistributedType.MULTI_MUSA]):
    ...
    deepspeed_config = {}
    if (distributed_type in [DistributedType.MULTI_GPU, DistributedType.MULTI_XPU, DistributedType.MULTI_NPU, DistributedType.MULTI_MLU,
    DistributedType.MULTI_MUSA, DistributedType.NO] and not use_mps):
        ...
        if distributed_type == DistributedType.DEEPSPEED:
            ...
            if use_deepspeed_config: deepspeed_config["deepspeed_config_file"] = _ask_field("Please enter the path to the json DeepSpeed config file: ", str, default="none")
            else:
                ...
                if deepspeed_config["zero_stage"] >= 2:
                    ...
                    if deepspeed_config["offload_param_device"] == "nvme": deepspeed_config["offload_param_nvme_path"] = _ask_field("Nvme Path to offload parameters?", str, default="/nvme")
                    if deepspeed_config["offload_optimizer_device"] == "nvme": deepspeed_config["offload_optimizer_nvme_path"] = _ask_field("Nvme Path to offload optimizer states?", str, default="/nvme")
                ...
                if use_gradient_clipping: deepspeed_config["gradient_clipping"] = _ask_field("What is the gradient clipping value? [1.0]: ", float, default=1.0)
                if deepspeed_config["zero_stage"] == 3: deepspeed_config["zero3_save_16bit_model"] = _ask_field("Do you want to save 16-bit model weights when using ZeRO Stage-3? [yes/NO]: ", _convert_yes_no_to_bool, default=False, error_message="Please enter yes or no.")
            ...
            if deepspeed_config["zero3_init_flag"]:
                if not is_transformers_available(): raise Exception("When `zero3_init_flag` is set, it requires Transformers to be installed. Please run `pip3 install transformers`.")
            ...
            if use_moe: deepspeed_config["deepspeed_moe_layer_cls_names"] = _ask_field("Specify the comma-separated list of transformers MoE layer class names (case-sensitive), e.g : `MixtralSparseMoeBlock`, `Qwen2MoeSparseMoeBlock`, `JetMoEAttention,JetMoEBlock` ... : ", str)
            if num_machines > 1:
                ...
                if deepspeed_config["deepspeed_multinode_launcher"] != DEEPSPEED_MULTINODE_LAUNCHERS[1]:
                    ...
                    if is_exclusion_filter: deepspeed_config["deepspeed_exclusion_filter"] = _ask_field("DeepSpeed exclusion filter string: ", str)
                    ...
                    if is_inclusion_filter: deepspeed_config["deepspeed_inclusion_filter"] = _ask_field("DeepSpeed inclusion filter string: ", str)
    fsdp_config = {}
    if distributed_type in [DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.MULTI_MLU, DistributedType.MULTI_MUSA, DistributedType.MULTI_XPU]:
        ...
        if use_fsdp: distributed_type = DistributedType.FSDP
        if distributed_type == DistributedType.FSDP:
            ...
            if fsdp_config["fsdp_auto_wrap_policy"] == FSDP_AUTO_WRAP_POLICY[0]:
                use_no_split_modules = _ask_field("Do you want to use the model's `_no_split_modules` to wrap. Only applicable for Sapiens Transformers [yes/NO]: ", _convert_yes_no_to_bool, default=False, error_message="Please enter yes or no.")
                if not use_no_split_modules: fsdp_config["fsdp_transformer_layer_cls_to_wrap"] = _ask_field("Specify the comma-separated list of transformer layer class names (case-sensitive) to wrap ,e.g, : `BertLayer`, `GPTJBlock`, `T5Block`, `BertLayer,BertEmbeddings,BertSelfOutput` ...? : ", str)
            elif fsdp_config["fsdp_auto_wrap_policy"] == FSDP_AUTO_WRAP_POLICY[1]: fsdp_config["fsdp_min_num_params"] = _ask_field("What should be your FSDP's minimum number of parameters for Default Auto Wrapping Policy? [1e8]: ", int, default=100000000)
            ...
            fsdp_config["fsdp_cpu_ram_efficient_loading"] = _ask_field("Do you want to enable CPU RAM efficient model loading? Only applicable for Sapiens Transformers models. [YES/no]: ", _convert_yes_no_to_bool, default=True, error_message="Please enter yes or no.")
            if fsdp_config["fsdp_cpu_ram_efficient_loading"]: fsdp_config["fsdp_sync_module_states"] = True
            else: fsdp_config["fsdp_sync_module_states"] = _ask_field("Do you want each individually wrapped FSDP unit to broadcast module parameters from rank 0 at the start? [YES/no]: ", _convert_yes_no_to_bool, default=True, error_message="Please enter yes or no.")
    ...
    if distributed_type in [DistributedType.MULTI_GPU]:
        ...
        if use_megatron_lm: distributed_type = DistributedType.MEGATRON_LM
        if distributed_type == DistributedType.MEGATRON_LM:
            ...
            if megatron_lm_config[prefix + "tp_degree"] > 1: megatron_lm_config[prefix + "sequence_parallelism"] = _ask_field("Do you want to enable Sequence Parallelism? [YES/no]: ", _convert_yes_no_to_bool, default=True, error_message="Please enter yes or no.")
            ...
            if megatron_lm_config[prefix + "pp_degree"] > 1: megatron_lm_config[prefix + "num_micro_batches"] = _ask_field("What is the number of micro-batches? [1]:", int, default=1, error_message="Please enter an integer.")
    ...
    if distributed_type in [DistributedType.MULTI_CPU, DistributedType.MULTI_XPU, DistributedType.MULTI_GPU, DistributedType.MULTI_MLU, DistributedType.MULTI_MUSA, DistributedType.MULTI_NPU, DistributedType.XLA]:
        ...
        if machine_type == "TPU": machine_type += " cores"
        elif machine_type == "CPU": machine_type = "processes"
        else: machine_type += "(s)"
    ...
    elif distributed_type in [DistributedType.FSDP, DistributedType.DEEPSPEED, DistributedType.MEGATRON_LM]: num_processes = _ask_field("How many GPU(s) should be used for distributed training? [1]:", int, default=1, error_message="Please enter an integer.")
    else: num_processes = 1
    if (distributed_type == DistributedType.MULTI_GPU) and (num_machines == 1) and (num_processes == 1): raise ValueError(f"Specified distributed type {distributed_type} but only using 1 GPU on a single machine. Please select `No distributed training` for the type of machine you are using.")
    if (distributed_type in [DistributedType.MULTI_GPU, DistributedType.MULTI_MLU, DistributedType.MULTI_MUSA, DistributedType.MULTI_NPU, DistributedType.MULTI_XPU, DistributedType.NO] and not use_cpu and not use_mps):
        if is_npu_available(): machine_type = "NPU(s)"
        elif is_mlu_available(): machine_type = "MLU(s)"
        elif is_musa_available(): machine_type = "MUSA(s)"
        else: machine_type = "GPU(s)"
    ...
    if distributed_type in (DistributedType.NO, DistributedType.MULTI_GPU) and not use_cpu and not use_mps: enable_cpu_affinity = _ask_field("Would you like to enable numa efficiency? (Currently only supported on NVIDIA hardware). [yes/NO]: ", _convert_yes_no_to_bool, default=False, error_message="Please enter yes or no.")
    fp8_config = None
    if distributed_type == DistributedType.XLA:
        ...
        if distributed_type == DistributedType.DEEPSPEED and use_deepspeed_config: mixed_precision = None
        else:
            mixed_precision = _ask_options("Do you wish to use mixed precision?", ["no", "fp16", "bf16", "fp8"], _convert_mixed_precision)
            if mixed_precision == "fp8":
                if not is_fp8_available(): raise ValueError("FP8 (either Transformer Engine or MSAMP) is not installed on this machine.")
                fp8_config = {}
                ...
                if fp8_config["backend"] == "TE":
                    if not is_transformer_engine_available(): raise ValueError("TransformersEngine was selected, but it is not installed on this machine.")
                    ...
                    if not is_msamp_available(): raise ValueError("MSAMP was selected, but it is not installed on this machine.")
    ...
    if use_dynamo and mixed_precision == "no" and not use_cpu: print("Torch dynamo used without mixed precision requires TF32 to be efficient. SapiensAccelerator will enable it by default when launching your scripts.")
    if distributed_type == DistributedType.XLA and mixed_precision == "bf16": tpu_downcast_bf16 = _ask_field("Should `torch.float` be cast as `bfloat16` and `torch.double` remain `float32` on TPUs?", default="no")
    return ClusterConfig(compute_environment=ComputeEnvironment.LOCAL_MACHINE, distributed_type=distributed_type, num_processes=num_processes, gpu_ids=gpu_ids,
    mixed_precision=mixed_precision, downcast_bf16=tpu_downcast_bf16, machine_rank=machine_rank, num_machines=num_machines, main_process_ip=main_process_ip,
    main_process_port=main_process_port, main_training_function=main_training_function, fp8_config=fp8_config, deepspeed_config=deepspeed_config, fsdp_config=fsdp_config,
    megatron_lm_config=megatron_lm_config, ipex_config=ipex_config, mpirun_config=mpirun_config, use_cpu=use_cpu, rdzv_backend=rdzv_backend, same_network=same_network,
    commands=tpu_commands, command_file=tpu_command_file, tpu_env=tpu_env, tpu_name=tpu_name, tpu_vm=tpu_vm, tpu_zone=tpu_zone, tpu_use_sudo=tpu_use_sudo,
    tpu_use_cluster=tpu_use_cluster, dynamo_config=dynamo_config, debug=debug, enable_cpu_affinity=enable_cpu_affinity)
"""
    ########################################################################################################################################################
    # This algorithm is part of a code library for optimizing the Artificial Intelligence models of Sapiens Technology®, and its disclosure, distribution, #
    # or reverse engineering without the company's prior consent will be subject to legal proceedings and actions pursued by our legal department.         #
