"""
    ...
    ########################################################################################################################################################
"""
from sapiens_accelerator.accelerator import Accelerator, DataLoaderConfiguration, GradientAccumulationPlugin
from sapiens_accelerator.test_utils import RegressionDataset, RegressionModel
from sapiens_accelerator.utils import DistributedType, set_seed
from sapiens_accelerator.state import GradientState
...
def check_model_parameters(model_a, model_b, did_step, iteration, **kwargs):
    for param, grad_param in zip(model_a.parameters(), model_b.parameters()):
        ...
        if not did_step: assert (torch.allclose(param.grad, grad_param.grad, **kwargs) is False), f"Gradients in sync when they should not be at iteration {iteration}:\nmodel_a grad ({param.grad}) == model_b grad ({grad_param.grad})"
        else: assert (torch.allclose(param.grad, grad_param.grad, **kwargs) is True), f"Gradients not in sync when they should be at iteration {iteration}:\nmodel_a grad ({param.grad}) != model_b grad ({grad_param.grad})"
def step_model(model, input, target, accelerator, do_backward=True):
    ...
    if not do_backward:
        ...
        loss.backward()
    else: accelerator.backward(loss)
def get_training_setup(accelerator, sched=False):
    ...
    if sched: ddp_model, ddp_opt, ddp_sched, dataloader = accelerator.prepare(ddp_model, ddp_opt, ddp_sched, dataloader)
    else: ddp_model, dataloader = accelerator.prepare(ddp_model, dataloader)
    if sched: return (model, opt, sched, dataloader, ddp_model, ddp_opt, ddp_sched)
...
def test_noop_sync(accelerator):
    ...
    for iteration in range(3):
        ...
        if iteration % 2 == 0:
            with accelerator.no_sync(ddp_model): step_model(ddp_model, ddp_input, ddp_target, accelerator)
        else: step_model(ddp_model, ddp_input, ddp_target, accelerator)
...
def test_distributed_sync(accelerator):
    ...
    for iteration in range(3):
        ...
        for param, ddp_param in zip(model.parameters(), ddp_model.parameters()):
            if not param.requires_grad: continue
            if iteration % 2 == 0: assert (torch.allclose(param.grad, ddp_param.grad) is False), f"Gradients in sync when they should not be:\nModel grad ({param.grad}) == DDP grad ({ddp_param.grad})"
            else: assert (torch.allclose(param.grad, ddp_param.grad) is True), f"Gradients not in sync when they should be:\nModel grad ({param.grad}) != DDP grad ({ddp_param.grad})"
        torch.manual_seed(1337 + iteration)
        ddp_input = ddp_input[torch.randperm(len(ddp_input))]
def test_distributed_sync_multiple_fwd(accelerator):
    ...
    for iteration in range(num_iterations):
        ...
        if iteration < num_iterations - 1:
            ...
            with accelerator.trigger_sync_in_backward(ddp_model): accelerator.backward(loss)
            for param, ddp_param in zip(model.parameters(), ddp_model.parameters()):
                if not param.requires_grad: continue
...
def test_gradient_accumulation(split_batches=False, dispatch_batches=False, sync_each_batch=False):
    ...
    accelerator = Accelerator(dataloader_config=dataloader_config, gradient_accumulation_plugin=gradient_accumulation_plugin)
    model, ddp_model, dataloader = get_training_setup(accelerator)
    for iteration, batch in enumerate(dataloader):
        ...
        with accelerator.accumulate(ddp_model): step_model(ddp_model, ddp_input, ddp_target, accelerator)
        for param, ddp_param in zip(model.parameters(), ddp_model.parameters()):
            if not param.requires_grad: continue
            ...
            else: assert (torch.allclose(param.grad, ddp_param.grad) is False), f"Gradients in sync when they should not be at iteration {iteration}:\nModel grad ({param.grad}) == DDP grad ({ddp_param.grad})"
        torch.manual_seed(1337 + iteration)
        ddp_input = ddp_input[torch.randperm(len(ddp_input))]
...
def test_gradient_accumulation_with_opt_and_scheduler(split_batches=False, dispatch_batches=False, sync_each_batch=False):
    ...
    for iteration, batch in enumerate(dataloader):
        ...
        if ((iteration + 1) % 2 == 0) or ((iteration + 1) == len(dataloader)):
            if split_batches: sched.step()
            else:
                for _ in range(accelerator.num_processes): sched.step()
        ...
        if accelerator.num_processes > 1: check_model_parameters(model, ddp_model, did_step or sync_each_batch, iteration, rtol=1e-3)
        if did_step: opt.zero_grad()
        ddp_opt.zero_grad()
        torch.manual_seed(1337 + iteration)
    GradientState._reset_state()
        ...
        if iteration < len(first_dataloader) - 1:
            ...
            if iteration == 1:
                for batch_num, _ in enumerate(second_dataloader):
                    ...
                    if batch_num < len(second_dataloader) - 1: assert not accelerator.gradient_state.end_of_dataloader
                    else: assert accelerator.gradient_state.end_of_dataloader
        else: assert accelerator.gradient_state.end_of_dataloader
    assert accelerator.gradient_state.active_dataloader is None
def main():
    accelerator = Accelerator()
    state = accelerator.state
    if state.distributed_type != DistributedType.XLA: test_dataloader_break()
    if state.distributed_type == DistributedType.NO: test_noop_sync(accelerator)
    if state.distributed_type in (DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.MULTI_MLU, DistributedType.MULTI_MUSA, DistributedType.MULTI_CPU):
        test_distributed_sync(accelerator)
        test_distributed_sync_multiple_fwd(accelerator)
    if state.distributed_type in (DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.MULTI_MLU, DistributedType.MULTI_MUSA):
        for split_batch in [True, False]:
            for dispatch_batches in [True, False]:
                for sync_each_batch in [True, False]: test_gradient_accumulation(split_batch, dispatch_batches, sync_each_batch)
    test_gradient_accumulation_with_opt_and_scheduler()
    if state.distributed_type in (DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.MULTI_MLU, DistributedType.MULTI_MUSA):
        for split_batch in [True, False]:
            for dispatch_batches in [True, False]:
                for sync_each_batch in [True, False]:
                    if not split_batch and not dispatch_batches and not sync_each_batch: continue
