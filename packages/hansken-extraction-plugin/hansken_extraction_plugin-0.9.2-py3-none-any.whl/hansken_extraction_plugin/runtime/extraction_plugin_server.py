...
class ProcessHandler:
    ...
    def handle_message(self, message: Any, force_sync=False):
        """
    ...
    def finish(self):
        ...
        try:
            ...
            self.handle_message(request, force_sync=True)
        finally:
            self._finish()
...
class ExtractionPluginServicer(ExtractionPluginServiceServicer):
    ...
    def transform(self, request: RpcTransformerRequest, grpc_context: grpc.ServicerContext) -> RpcTransformerResponse:
        ...
        if missing_parameters:
            self.raise_grpc_exception(ValueError(f'Missing parameters while calling transformer {request.methodName}. '
        ...
        if unexpected_parameters:
            self.raise_grpc_exception(ValueError(f'Unexpected parameters while calling transformer '
                                                 f'{request.methodName}. Unexpected: {unexpected_parameters}.'),
                                      grpc_context,
                                      grpc.StatusCode.INVALID_ARGUMENT)
