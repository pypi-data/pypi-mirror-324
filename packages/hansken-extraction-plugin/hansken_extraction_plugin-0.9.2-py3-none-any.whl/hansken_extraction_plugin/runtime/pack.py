...
from hansken_extraction_plugin.framework.PrimitiveMessages_pb2 import (RpcBoolean, RpcBytes, RpcDouble, RpcDoubleList,
                                                                       RpcEmptyList, RpcEmptyMap, RpcIsoDateString,
                                                                       RpcLatLong, RpcList, RpcLong, RpcLongList,
                                                                       RpcMap, RpcString,
...
from hansken_extraction_plugin.framework.RpcCallMessages_pb2 import RpcBeginChild, RpcBeginDataStream, RpcEnrichTrace, \
    ...
    RpcSearchScope, RpcWriteDataStream
...
def transformer_response(value: Any) -> RpcTransformerResponse:
    ...
    if isinstance(value, bool):
        ...
        return RpcTransformerResponse(response=RpcTransformerArgument(boolean=packed))
    elif isinstance(value, bytes):
        packed = _primitive_matchers[bytes](value)
        return RpcTransformerResponse(response=RpcTransformerArgument(bytes=packed))
    elif isinstance(value, int):
        packed = _primitive_matchers[int](value)
        return RpcTransformerResponse(response=RpcTransformerArgument(long=packed))
    elif isinstance(value, float):
        packed = _primitive_matchers[float](value)
        return RpcTransformerResponse(response=RpcTransformerArgument(double=packed))
    elif isinstance(value, str):
        packed = _primitive_matchers[str](value)
        return RpcTransformerResponse(response=RpcTransformerArgument(string=packed))
    elif isinstance(value, Vector):
        packed = _primitive_matchers[Vector](value)
        return RpcTransformerResponse(response=RpcTransformerArgument(vector=packed))
    elif isinstance(value, GeographicLocation):
        packed = _primitive_matchers[GeographicLocation](value)
        return RpcTransformerResponse(response=RpcTransformerArgument(latLong=packed))
    elif isinstance(value, datetime):
        packed = _primitive_matchers[datetime](value)
        return RpcTransformerResponse(response=RpcTransformerArgument(datetime=packed))
    elif isinstance(value, Sequence):
        packed = generic_list(value)
        return RpcTransformerResponse(response=RpcTransformerArgument(list=packed))
    elif isinstance(value, Mapping):
        packed = generic_map(value)
        return RpcTransformerResponse(response=RpcTransformerArgument(map=packed))
    else:
        raise Exception("Cannot pack RpcTransformerArgument because it's type is not supported.")


def transformer_request(method_name: str, arguments: Dict[str, RpcTransformerArgument]):
    """
    ...
    """
    return RpcTransformerRequest(methodName=method_name, namedArguments=arguments)
