...
#             rebalAmt = evalAmt - holdAmt - holdStockCnt * ePrice
    ...
    columns = ['일자','종가', '변동률', '매매주식수', '보유주식수','투자현금','비투자현금','현금','총평가금액', '분할회차']        
    return  pd.DataFrame(resList, columns=columns)     
...
def loc_infinite_purchase(dfStock, splitCnt = 40, invAmt = 100000000, settlePct=10, initPct=100, market='KOR'):    
    ...
    columns = ['일자','종가','매수건수','매수가','매수금액','매도건수','매도가','매도금액','보유주식수',
               '누적매수금액','평균단가','주식평가금액','현금','총평가금액','잔여회차'] 
    return  pd.DataFrame(resList, columns=columns)     
...
#             for i, [buyP, buyDt, buyCnt, sellP] in enumerate(holdList):
    ...
    columns = ['일자','분할회차','매매구분','매매주식수','매매가','보유주식수','정산금액','현금','주식평가금액','총평가금액',
...
def get_daily_evaluation(dfTrade, dfStock):
    ## 컬럼내 '일자' 위치 구하기
    i_dt = [i for i in range(len(dfTrade.columns)) if dfTrade.columns.tolist()[i] == '일자'][0]
    i_stocks = [i for i in range(len(dfTrade.columns)) if dfTrade.columns.tolist()[i] == '보유주식수'][0]
    i_cash = [i for i in range(len(dfTrade.columns)) if dfTrade.columns.tolist()[i] == '현금'][0]
    
    liTmp = dfTrade.values.tolist()
    ## 일자별로 최종 데이터 외 제거
    liTrade = [liTmp[i] for i in range(len(liTmp)) if (i == (len(liTmp)-1) or liTmp[i][i_dt] < liTmp[i+1][i_dt])  ]
    ...
    cash = dfTrade.iloc[0]['총평가금액']

    for idx1, [dt, ePrice] in enumerate(dfStock[['일자','종가']].values.tolist()):
        if idx2 == 0 and dt < liTrade[idx2][i_dt]:
            liMerge.append([])
        else:
            liMerge.append(liTrade[idx2])
            
        if idx2 < rLen2 and dt == liTrade[idx2][i_dt]:
            stocks = liTrade[idx2][i_stocks]
            cash = liTrade[idx2][i_cash]
            idx2 += 1 
        liCash.append(cash)
        liStocks.append(stocks)
        liEPrice.append(ePrice)
        
#         liMerge.append([dt, ePrice, stocks, cash, round(ePrice * stocks + cash,2)])
    dfRes = pd.DataFrame(liMerge, columns=dfTrade.columns.tolist())
    dfRes['종가'] = liEPrice
    dfRes['현금'] = liCash
    dfRes['총평가금액'] = round(dfRes['종가'] * dfRes['보유주식수'] + dfRes['현금'],2)    
