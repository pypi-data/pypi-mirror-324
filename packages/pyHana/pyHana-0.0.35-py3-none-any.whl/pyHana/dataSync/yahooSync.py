...
def SyncStockTradeInfoUSA(shCode='', sDate='20140101', eDate='99991231', truncInd='N'):
    SyncEtfTradeInfoUSA(shCode=shCode, sDate=sDate, eDate=eDate, truncInd=truncInd, gb='STOCK')

def SyncEtfTradeInfoUSA(shCode='', sDate='20140101', eDate='99991231', truncInd='N', gb='ETF'):
    ''' 
    **kwargs
    ...
    if gb == 'ETF':
        dfCmpList = code.EtfListUSA()
    else:
        dfCmpList = tradeview.StockInfoUSA()[['종목코드','종목명']]
    ...
    for idx, [shCode, shName] in enumerate(dfCmpList[['종목코드','종목명']].values.tolist()):
        ...
        newData = yahoo.GetEtfTradeInfoUSA(shCode=shCode, frDt = frDt, toDt = toDt).values.tolist()
    ...
    print('\n' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), ': 일별 미국 ETN 거래정보 저장 완료')   
