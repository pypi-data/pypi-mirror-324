...
from .networks.pHLAII_atom import DeepGCN
...
def Inference(file_path, model_path=''):
    ...
    abpath = os.path.abspath(__file__)
    folder = os.path.dirname(abpath)
    hla_pseudo = pd.read_csv(os.path.join(folder, 'load_dataset', 'hlaII_pseudo_seq.csv'))
    ...
    for _,row in df.iterrows():
        ...
        alpha_n=row['alpha_n']
        beta_n=row['beta_n']
        alpha_pseudo_seq = hla_pseudo.loc[hla_pseudo['hla_name_1'] == alpha_n, "pseudo_seq"].iloc[0]
        beta_pseudo_seq = hla_pseudo.loc[hla_pseudo['hla_name_1'] == beta_n, "pseudo_seq"].iloc[0]
        pseudo = alpha_pseudo_seq+beta_pseudo_seq
        ...
        for i in range(len(p_perm)):
            ...
            for j in range(len(m_perm)):
                ...
                if m_residue_num<=15:
                    m_residue_nums.append(str(alpha_pseudo_site[m_residue_num-1]))
                    m_chains.append('A')
                else:
                    m_residue_nums.append(str(beta_pseudo_site[m_residue_num-16]))
                    m_chains.append('B')
        ...
        xlabels = []
        for num, res_name,atom_name,chain in zip(m_residue_nums,m_residue_names,mas,m_chains):
            label = atom_name+'\n'+res_name+'\n'+num+'\n'+chain
...
def absolute_residue(data_dir, pdbid, relative_atom_num):
    actual_res_num = -1
    actual_res_chain = ''
    with open(data_dir+'pdb_Extracted_absolute/'+pdbid+'_mhc.pdb') as f:
        lines = f.readlines()
        for (i,line) in enumerate(lines):
            if str(i+1) == relative_atom_num:
                actual_res_num = line[23:26].strip(' ')
                actual_res_chain = line[21:22].strip(' ')
                break
    return actual_res_chain, actual_res_num
...
def Train(file_path, config_path=''):
    ...
    for train_index, val_index in kf.split(dataset):
        ...
        if os.path.exists('./antigenHLAII_training_log/seq-level_parameters.pt'):
            state = torch.load('./antigenHLAII_training_log/seq-level_parameters.pt', map_location=device)
            pretrain_state_dict = state['model']
        else:
        ...
        for epoch in range(args['epochs']):
            ...
            # if loss<min_loss:
                # min_loss = loss
                # if not os.path.exists(args.save_dir+dataset.pdbids[val_index[0]].lower()):
                    # os.makedirs(args.save_dir+dataset.pdbids[val_index[0]].lower())
                # save_file = args.save_dir+dataset.pdbids[val_index[0]].lower()+'/best_model.pt'
        ...
        pep_file = data_dir+'pdb_Extracted_relative/'+pdb+'_peptide.pdb'
        mhc_file = data_dir+'pdb_Extracted_relative/'+pdb+'_mhc.pdb'
        ...
        for i in range(len(p_perms)):
            ...
            for j in range(len(m_perms)):
                ...
                m_residue_num = m_atom.GetPDBResidueInfo().GetResidueNumber()
                m_residue_name = m_atom.GetPDBResidueInfo().GetResidueName()
                actual_m_res_chain, actual_m_res_num = absolute_residue(data_dir,pdb,str(m_idx+1))
                m_chains.append(actual_m_res_chain)
                mas.append(ma)
                m_residue_names.append(m_residue_name)
                m_residue_nums.append(str(actual_m_res_num))
