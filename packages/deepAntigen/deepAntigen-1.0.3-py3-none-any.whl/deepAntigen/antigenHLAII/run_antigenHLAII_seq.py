import os
from .load_dataset.load_seq import pMHC_DataSet, collate
from .utils.model_utils import *
from .networks.pHLAII_seq import DeepGCN
...
def train_one_epoch(args, train_loader, model, criterion, optimizer, epoch, device):
    ...
    train_trues = []
    train_scores = []
    for idx, (_, _, _, _, labels, peptide_graphs,pseudo_graphs) in enumerate(train_loader):
...
def valid(valid_loader, model, criterion, epoch, device):
    ...
    with torch.no_grad():
        model.eval()
        for idx, (_, _, _, _, labels, peptide_graphs,pseudo_graphs) in enumerate(valid_loader):
            peptide_graphs = Batch.from_data_list(peptide_graphs)
            peptide_graphs = peptide_graphs.to(device)
            pseudo_graphs = Batch.from_data_list(pseudo_graphs)
            pseudo_graphs = pseudo_graphs.to(device)
...
def test(test_loader, model, device):
    ...
    with torch.no_grad():
        model.eval()
        for idx, (_, peptides, alpha_ns, beta_ns, labels, peptide_graphs,crystal_graphs) in enumerate(test_loader):
            peptide_graphs = Batch.from_data_list(peptide_graphs)
            peptide_graphs = peptide_graphs.to(device)
            crystal_graphs = Batch.from_data_list(crystal_graphs)
            crystal_graphs = crystal_graphs.to(device)
            labels = labels.to(device)
            logits = model(peptide_graphs, crystal_graphs)
            preds = logits.argmax(dim=1)
            scores = logits[:,1]
            ...
            all_alphas.extend(alpha_ns)
            all_betas.extend(beta_ns)
    ...
    return all_peptides, all_alphas, all_betas, all_scores, all_trues
...
def Inference(test_file_path, model_path='', multi_process=8):
    ...
    if len(model_path)==0:
        ...
        state = torch.load(os.path.join(folder,'Weights','seq-level_parameters.pt'),map_location='cpu')
    else:
        state = torch.load(model_path,map_location='cpu')
    pretrain_state_dict = state['model']
    args = state['opt']
    args['output'] = './antigenHLAII_Output/seq-level/'
    dataset = pMHC_DataSet(test_file_path)
    dataloader = DataLoader(dataset, batch_size=args['batchsize'], collate_fn=collate, shuffle=False, pin_memory=True, num_workers=multi_process)
    model= set_model(args, pretrain_state_dict,device)
    all_peptides, all_alphas, all_betas, all_scores, all_labels = test(dataloader, model, device)

    if 'label' in headline.columns:
        df = pd.DataFrame({"peptide":all_peptides,"alpha_n":all_alphas,"beta_n":all_betas,"score":all_scores,"label":all_labels},index=list(range(len(all_peptides))))
    else:
        df = pd.DataFrame({"peptide":all_peptides,"alpha_n":all_alphas,"beta_n":all_betas,"score":all_scores},index=list(range(len(all_peptides))))
    if not os.path.exists(args['output']):
        os.makedirs(args['output'])
    df.to_csv(args['output']+'pHLAII_predictions.csv',index=False)
    print('Prediction results have been saved to'+args['output']+'pHLAII_predictions.csv')
    return df
...
def Train(k_fold_dir, config_path=''):
    ...
    train_dataset = pMHC_DataSet(train_path,aug=True,test=False)
    val_dataset = pMHC_DataSet(val_path,aug=False,test=False)
    start_epoch = 0
