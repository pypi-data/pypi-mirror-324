...
class pMHC_DataSet(DATA.InMemoryDataset):
    def __init__(self, path, aug=False, test=True):
        ...
        self.rawdata['MHC']=self.rawdata['alpha_n']+self.rawdata['beta_n']
        pep_counts = self.rawdata['peptide'].value_counts()
        MHC_counts = self.rawdata['MHC'].value_counts()
    ...
    def __getitem__(self, idx):
        ...
        if self.test:
            ...
            if alpha_n+beta_n in self.high_freq_mhc:
...
def collate(batch):
    ...
    peptide_graphs = [pickle.loads(item[5]) for item in batch]
    pseudo_graphs = [pickle.loads(item[6]) for item in batch]
