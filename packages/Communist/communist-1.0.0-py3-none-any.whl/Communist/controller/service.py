...
from Communist.agent.views import ActionModel, ActionResult,FireflinkNLP
from Communist.browser.context import BrowserContext
from Communist.controller.registry.service import Registry
from Communist.controller.views import (
	ClickElementAction,
	DoneAction,
...
from Communist.utils import time_execution_async, time_execution_sync
...
class Controller:
	...
	def _register_default_actions(self):
		...
		async def search_google(params: SearchGoogleAction, browser: BrowserContext):
			...
			logger.info(msg)
			self.result.add_step({"nlpName":"NavigateToURL","input":f'https://www.google.com/search?q={params.query}&udm=14',"element":"","aiReferenceElement":""})
		...
		async def go_to_url(params: GoToUrlAction, browser: BrowserContext):
			...
			logger.info(msg)
			self.result.add_step({"nlpName":"NavigateToURL","input":params.url,"element":"","aiReferenceElement":""})
			return ActionResult(extracted_content=msg, include_in_memory=True)
		...
		async def go_back(_: NoParamsAction, browser: BrowserContext):
			...
			msg = '🔙  Navigated back'
			logger.info(msg)
			self.result.add_step({"nlpName":"NavigateBack","input":"","element":"","aiReferenceElement":""})
			return ActionResult(extracted_content=msg, include_in_memory=True)
		...
		async def click_element(params: ClickElementAction, browser: BrowserContext):
			...
			if await browser.is_file_uploader(element_node,fireflinkElementObject=self.result):
			...
			try:
				await browser._click_element_node(element_node,fireflinkElementObject=self.result)
				...
				if len(session.context.pages) > initial_pages:
					...
					await browser.switch_to_tab(-1,self.result)
				return ActionResult(extracted_content=msg, include_in_memory=True)
		...
		async def input_text(params: InputTextAction, browser: BrowserContext):
			...
			element_node = state.selector_map[params.index]
			await browser._input_text_element_node(element_node, params.text,self.result)
		...
		async def switch_tab(params: SwitchTabAction, browser: BrowserContext):
			await browser.switch_to_tab(params.page_id,self.result)
