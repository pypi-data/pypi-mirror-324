...
from Communist.browser.views import BrowserError, BrowserState, TabInfo, URLNotAllowedError
from Communist.dom.service import DomService
from Communist.dom.views import DOMElementNode, SelectorMap
from Communist.utils import time_execution_sync

if TYPE_CHECKING:
	from Communist.browser.browser import Browser
...
class BrowserContext:
	...
	async def close_current_tab(self,fireflinkElementObject=None):
		...
		if session.context.pages:
			await self.switch_to_tab(0,fireflinkElementObject)
	...
	async def _input_text_element_node(self, element_node: DOMElementNode, text: str,fireflinkObject):
		try:
			...
			fireflinkObject.add_step({"nlpName":"SendKeys","input":text,"element":"","aiReferenceElement":element_node.xpath})
			await page.wait_for_load_state()

		except Exception as e:
			raise Exception(f'Failed to input text into element: {repr(element_node)}. Error: {str(e)}')

	async def _click_element_node(self, element_node: DOMElementNode,fireflinkElementObject):
		"""
		...
		try:
			...
			try:
				...
				fireflinkElementObject.add_step({"nlpName":"Click","input":"","element":"","aiReferenceElement":element_node.xpath})
				print(element_node)
				await page.wait_for_load_state()
	...
	async def switch_to_tab(self, page_id: int,fireflinkElementObject) -> None:
		...
		page = pages[page_id]
		fireflinkElementObject.add_step({"nlpName":"SwitchToWindowByHandle","input":page,"element":"","aiReferenceElement":""})
	...
	async def is_file_uploader(self,element_node: DOMElementNode,fireflinkElementObject,max_depth: int = 3,current_depth: int = 0) -> bool:
		...
		if element_node.children and current_depth < max_depth:
			for child in element_node.children:
				if isinstance(child, DOMElementNode):
					if await self.is_file_uploader(child, max_depth, current_depth + 1,fireflinkElementObject):
						return True
