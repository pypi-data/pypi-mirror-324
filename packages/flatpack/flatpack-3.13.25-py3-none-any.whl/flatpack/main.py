...
def initialize_database_manager(flatpack_directory):
    ...
    db_manager = DatabaseManager(flatpack_directory, db_path)
...
def add_package_to_toml(
    ...
    """Add or update a package in flatpack.toml, only modifying that section."""
...
def ensure_database_initialized():
    ...
    if db_manager is None:
        ...
        db_manager = initialize_database_manager(str(flatpack_directory))
...
async def fpk_build(directory: Union[str, None], use_euxo: bool = False):
    ...
    build_dir = (flatpack_dir / "build").resolve()
    web_dir = (flatpack_dir / "web").resolve()
...
def fpk_display_disclaimer(directory_name: str, local: bool):
    ...
    """
    disclaimer_template = """
[bold]STOP AND READ BEFORE YOU PROCEED[/bold]
...
https://codeberg.org/flatpack/Flatpack/src/branch/main/warehouse/{directory_name}
        """
    else:
        please_note_content = ""
...
def fpk_fetch_git_dirs(session: httpx.Client) -> List[str]:
    """
    ...
    try:
        response = session.get(f"{CODEBERG_REPO_URL}/contents/warehouse")
...
def fpk_list_directories(session: httpx.Client) -> str:
    ...
    """
    try:
        dirs = fpk_fetch_git_dirs(session)
        directories_str = "\n".join(dirs)
...
def fpk_unbox(
    ...
    if fpk_path is not None:
        ...
        package_manager = PackageManager()
        metadata = package_manager.get_metadata(fpk_file)

        if metadata and metadata.get('prebuilt', False):
            logger.info("Detected pre-built package, using direct unpack")

            try:
                package_manager.unpack(str(fpk_file), str(target_dir))
                return True
            except Exception as e:
                logger.error("Failed to unpack pre-built package: %s", e)
                cleanup_flatpack_dir(target_dir)
                return False
...
def setup_arg_parser():
    ...
    parser_pack.add_argument(
        "--prebuild",
        action="store_true",
        help="Pre-build flatpack before packing"
    )

    parser_pack.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing flatpack"
    )
    ...
    parser_add = subparsers.add_parser(
        "add",
        help="Add a package to flatpack.toml"
    )
    ...
    parser_add.add_argument(
        "--flatpack",
        help="Flatpack directory to add package to (uses last unboxed flatpack if not specified)"
    )

    parser_add.set_defaults(func=fpk_cli_handle_add)

    return parser


def fpk_cli_handle_add(args, session):
    """Handle the 'add' command to add packages to flatpack.toml."""
    ...
    if args.flatpack:
        flatpack_dir = Path(args.flatpack)
    else:
        last_flatpack = fpk_get_last_flatpack()

        if not last_flatpack:
            console.print(
                "[bold red]Error: No flatpack specified and no cached flatpack found. Please specify a flatpack with --flatpack[/bold red]"
            )
            return
        flatpack_dir = Path(last_flatpack)

    toml_path = flatpack_dir / "flatpack.toml"

    if not toml_path.exists():
        console.print(
            f"[bold red]Error: flatpack.toml not found in {flatpack_dir}[/bold red]"
        )
        return
    ...
    try:
        ...
        console.print(
            f"[bold green]Successfully added {args.package_type} package: {args.package_name} to {flatpack_dir}[/bold green]"
    ...
    async def abort_build(
        ...
        async with abort_lock:
            try:
                ...
                abort_file = Path(flatpack_directory) / "build" / "ABORT_BUILD"
                abort_file.touch()
...
def fpk_cli_handle_unbox(args, session):
    ...
    if input_path.suffix == ".fpk":
        ...
        existing_dirs = fpk_fetch_git_dirs(session)
...
def fpk_pack_with_prebuild(input_path: str, overwrite: bool = False) -> None:
    input_path = os.path.abspath(input_path)
    output_fpk = f"{input_path}.fpk"

    if os.path.exists(output_fpk) and not overwrite:
        raise FileExistsError

    print("\nUnboxing dependencies...")
    ...
    print("\nBuilding project...")
    asyncio.run(fpk_build(str(input_path)))

    print("\nPacking pre-built project...")
    package_manager = PackageManager()
...
def fpk_cli_handle_pack(args, session):
    """Handle the pack command with optional pre-building."""
    try:
        if hasattr(args, 'prebuild') and args.prebuild:
            console.print("[bold blue]Pre-building package...[/bold blue]")

            fpk_pack_with_prebuild(args.input_path, overwrite=args.overwrite)

            console.print("")
            console.print(
                f"[bold green]Successfully created pre-built package: {args.input_path}.fpk[/bold green]"
            )
        else:
            package_manager = PackageManager()
            package_manager.pack(args.input_path, overwrite=args.overwrite)
            console.print(
                f"[bold green]Successfully compressed {args.input_path} to {args.input_path}.fpk[/bold green]"
            )
    except FileExistsError:
        console.print(
            f"[bold red]The package '{args.input_path}.fpk' already exists. Use --overwrite to overwrite.[/bold red]"
        )
    except Exception as e:
