...
def fpk_cli_handle_verify(args, session):
    ...
    logger.info(
        "Verifying flatpack in directory: %s",
        directory_name
    )
...
def fpk_cli_handle_pack(args, session):
    ...
    try:
        package_name = f"{args.input_path}-prebuilt.fpk" if hasattr(
            args,
            'prebuild'
        ) and args.prebuild else f"{args.input_path}.fpk"
        ...
        if hasattr(args, 'prebuild') and args.prebuild:
            ...
            console.print(
                f"\n[bold green]Successfully created pre-built package: {package_name}[/bold green]"
            )
        else:
            package_manager = PackageManager()
