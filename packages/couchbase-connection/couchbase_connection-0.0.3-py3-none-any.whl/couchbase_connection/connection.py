...
class CouchbaseConnection(BaseConnection[Cluster]):    
    """
    The connector class for performing common Couchbase operations such as insert, get, replace, and remove documents.
        
    Example:
        If, the environment variables are set and contain the needed connstr, username and password 
        ```
        ...
        ```
        import streamlit as st
        connection = st.connection("couchbase", type=CouchbaseConnection, connstr=<CONNSTR>, username=<USERNAME>, password=<PASSWORD>)
        connection.set_bucket_scope_coll("new_bucket", "new_scope", "new_collection")
        st.help(connection)
        ```
    """
    ...
    def _connect(self, **kwargs):
        ...
        Args:
            **kwargs: Keyword arguments including connstr, username, and password.

        Raises:
            AuthenticationException: Raised if the authentication fails.
            TimeoutException: Raised if the connection times out.
            CouchbaseException: Any general Couchbase related exceptions are caught.
        ...
        connstr = kwargs.pop("connstr", None) or self._secrets.get("connstr")
        username = kwargs.pop("username", None) or self._secrets.get("username")
        password = kwargs.pop("password", None) or self._secrets.get("password")
    ...
    # Create    
    def insert_document(self, doc_id: str, doc: JSONType, opts: InsertOptions = InsertOptions(timeout=timedelta(seconds=5), durability=Durability.MAJORITY), **kwargs):
        """
    ...
    # Query
    def query(self, q, opts=QueryOptions(metrics=True)):
        try:
