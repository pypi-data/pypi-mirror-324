...
def test_inhom():
    ...
    states[1] = pig.run(models, inhom, weights, states[0], Nt=2, dt=0.1)
...
def test_cell():
    ...
    for i in range(states.shape[0] - 1):
        states[i + 1] = pig.run(models, inhom, weights, states[i], Nt=200, dt=0.1)
...
def test_cable():
    ...
    states = models.resting_states(inhom, Nframes=3)
    states[0, x < 5, 0] = 0.4
    weights = pig.weights(dz, dy, dx, inhom, diffusivity=1)
    Nt, dt = 400, 0.05
    for i in range(states.shape[0] - 1):
        states[i + 1] = pig.run(models, inhom, weights, states[i], Nt=Nt, dt=dt)
...
def test_aniso():
    ...
    weights = pig.weights(dz, dy, dx, inhom, diffusivity=diffusivity)
    Nt, dt = 400, 0.05
    for i in range(states.shape[0] - 1):
        states[i + 1] = pig.run(models, inhom, weights, states[i], Nt=Nt, dt=dt)
...
def test_double():
    ...
    inhom = np.ones_like(x)
    models = pig.Models("aliev1996simple")
    states: np.ndarray[Any, Any] = models.resting_states(inhom).astype(np.float64)
    states[0, ..., 0] = 0.4
    weights = pig.weights(dz, dy, dx, inhom, diffusivity=0)
    assert states[0].ndim == 4
    states_single = pig.run(models, inhom, weights, states[0], Nt=200, dt=0.1)
    states_double = pig.run(
        models, inhom, weights, states[0], Nt=200, dt=0.1, double_precision=True
    )
...
def test_dt():
    ...
    weights = pig.weights(dz, dy, dx, inhom, diffusivity=0)
    dt = 0.0001
    states[1] = pig.run(models, inhom, weights, states[0], Nt=101, dt=dt)
    states[2] = pig.run(models, inhom, weights, states[0], Nt=11, dt=dt * 10)
