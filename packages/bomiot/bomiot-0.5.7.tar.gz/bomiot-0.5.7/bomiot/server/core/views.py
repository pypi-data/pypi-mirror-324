            ...
            if user_data.is_superuser:
                ...
                if "Delete One User" not in self.request.auth.permission:
                    ...
                    if self.request.auth.id == int(data.get('id')):
                        ...
                        copy_files(join(settings.MEDIA_ROOT, user_data.username), join(settings.MEDIA_ROOT, 'recycle'))
                        user_data.save()
                        shutil.rmtree(join(settings.MEDIA_ROOT, user_data.username))
                        return Response(msg_message_return(self.request.META.get('HTTP_LANGUAGE', ''),
...
class UserUpload(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        for file_name in data:
            ...
            f.close()
            context = {}
            msg = other_message_return(self.request.META.get('HTTP_LANGUAGE', ''),  "Success upload files")
