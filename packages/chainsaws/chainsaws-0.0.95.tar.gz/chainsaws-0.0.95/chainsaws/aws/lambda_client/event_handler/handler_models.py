...
from typing import Any, Optional, Dict, Union, TypeVar, Generic, TypedDict, BinaryIO, Iterator, Tuple, Protocol, runtime_checkable
from email.parser import Parser
from io import BytesIO

from chainsaws.utils.dict_utils import convert_decimal_to_number, CustomJSONEncoder
...
class LambdaResponse:
    ...
    def create(
        ...
        # Add status description for ALB Response
        if status_description:
            response["statusDescription"] = status_description

        # Format body
        if isinstance(body, (dict, list)):
            # If body is already in our format (has data, rslt_cd, etc), use it as is
            if isinstance(body, dict) and all(key in body for key in ["data", "rslt_cd", "rslt_msg", "duration"]):
                formatted_body = body
            else:
                # For list type or non-standard dict, wrap it in our format
                formatted_body = {
                    "data": body,
                    "rslt_cd": "S00000",
                    "rslt_msg": "Success",
            ...
            # For non-dict/list bodies, wrap in a standard response format
            formatted_body = {
                "data": str(body),
                "rslt_cd": "S00000",
                "rslt_msg": "Success",
