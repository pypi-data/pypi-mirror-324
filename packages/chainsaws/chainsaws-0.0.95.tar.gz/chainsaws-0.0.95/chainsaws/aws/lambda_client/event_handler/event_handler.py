    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., dict]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> dict:
            ...
            try:
                ...
                # Return if it is already in LambdaResponse format
                ...
                duration = time.time() - start_time
                # Handle HTTP exceptions (including AppError)
                error_info = {
                ...
                if config.use_traceback:
                    error_body["data"]["traceback"] = str(
                        traceback.format_exc())

                # Notify for all HTTP exceptions, with special handling for AppError
                if config.error_receiver:
                    if not isinstance(ex, AppError) or ex.code not in {e.code for e in config.ignore_app_errors}:
                        try:
                            message = make_error_description(event)
                            config.error_receiver(message)
                        except Exception as err:
                            error_body["data"]["error_receiver_failed"] = str(
                                err)
                ...
                error_body = {
                    ...
                    "rslt_cd": "E00002",
                    "rslt_msg": "Internal Server Error",
                    "duration": duration
                }
                ...
                if config.error_receiver:
                    try:
                        ...
                        error_body["data"]["error_receiver_failed"] = str(err)

                # AWS 서비스 에러의 경우 해당 상태 코드 사용
                status_code = HTTPStatus.INTERNAL_SERVER_ERROR
                if hasattr(ex, "response"):
                    aws_status = error_body["data"]["service_error"]["http_status"]
                    if aws_status:
