...
from chainsaws.aws.lambda_client.types.context import Context as LambdaContext
...
class APIGatewayHttpResolver(BaseResolver[APIGatewayProxyV2Event]):
    ...
    def __init__(
        self,
        base_path: str = "",
        openapi_config: Optional[OpenAPIConfigDict] = None
    ):
        """Initialize resolver with OpenAPI configuration and base path.

        Args:
            base_path: Base path for all routes (e.g. "/api/v1")
            openapi_config: OpenAPI configuration dictionary
        """
        super().__init__(base_path=base_path)
        self.openapi_config = openapi_config or {
        ...
        # Share OpenAPI configuration with ResponseSchemaRegistry
        if openapi_config:
            ResponseSchemaRegistry.configure(openapi_config)

        self.openapi_generator = OpenAPIGenerator(config=self.openapi_config)
    ...
    def resolve(self, event: APIGatewayProxyV2Event, context: Any = None) -> dict[str, Any]:
        ...
        try:
            ...
            path = event.get('requestContext', {}).get(
                'http', {}).get('path', '')

            # Handle documentation endpoints
            if path in ["/docs", "/openapi.json"]:
                docs_url = self.openapi_generator.get_docs_url()
                if not docs_url:
                    raise ValueError(
                        "S3 bucket not configured for documentation")

                # Redirect to appropriate S3 website URL
                redirect_url = f"{docs_url}/{'docs' if path ==
                                             '/docs' else 'openapi.json'}"
...
def example_handler(
    ...
    """Example handler function for API Gateway.
