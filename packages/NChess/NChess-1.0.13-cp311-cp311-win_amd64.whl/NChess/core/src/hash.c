...
NCH_STATIC_INLINE int
is_same_board(const uint64 bitboards[NCH_SIDES_NB][NCH_PIECE_NB], BoardNode* node){
...
get_node(const uint64 bitboards[NCH_SIDES_NB][NCH_PIECE_NB], BoardNode* node){
    if (node->empty){
        return NULL;
    }
...
int
BoardDict_GetCount(BoardDict* dict, const uint64 bitboards[NCH_SIDES_NB][NCH_PIECE_NB]){
