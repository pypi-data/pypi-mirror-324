...
class SlotFilter(Filter):
    ...
    def __repr__(self) -> str:
        return f'{self.__class__.__name__}(domain={self.domain!r}, bus={self.bus!r}, device={self.device!r}, function={self.function!r})'

    def __str__(self) -> str:
        return '{}:{}:{}.{}'.format(*(
            f'{x:x}' if x is not None else ''
            for x in (self.domain, self.bus, self.device, self.function)
        ))
...
class DeviceFilter(Filter):
    ...
    def __repr__(self) -> str:
        return f'{self.__class__.__name__}(cls={self.cls!r}, vendor={self.vendor!r}, device={self.device!r})'

    def __str__(self) -> str:
        return ':'.join(
            f'{x:x}' if x is not None else ''
            for x in (self.vendor, self.device, self.cls)
        )
