...
class Slot:
    ...
    def __str__(self) -> str:
        output: str = f'{self.domain:04x}:{self.bus:02x}:{self.device:02x}.{self.function:01x}'
        if self.parent:
            return f'{self.parent!s}/{output}'
        return output
...
class NameWithID:
    ...
    def __str__(self) -> str:
        if self.id and self.name:
            return f'{self.name} [{self.id:04x}]'
        elif self.name:
            return self.name
        elif self.id:
...
class PCIAccessParameter:
    ...
    def __init__(self, value: str) -> None:
        ...
        if not match:
            raise ValueError(
                f'Could not parse {value!r} into a parameter')
        gd = match.groupdict()
    ...
    def __str__(self) -> str:
        return f'{self.name}\t{self.description} ({self.default})'
