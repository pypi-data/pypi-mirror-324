...
from ..models.ag_db import Gallery, GalleryConfig, Response, Run, Session
...
async def create_gallery_entry(
    ...
    for run in gallery_data.runs:
        ...
        if (
            not session_result.status
            or not session_result.data
            or session_result.data[0].user_id != user_id
        ):
            raise HTTPException(
                status_code=403, detail=f"Not authorized to add run {run.id} to gallery"
            )
...
async def get_gallery_entry(
    gallery_id: int, user_id: str, db: DatabaseManager = Depends(get_db)
) -> Response:
    ...
    if gallery.config["visibility"] != "public" and gallery.user_id != user_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to view this gallery entry"
        )
...
async def list_gallery_entries(
    user_id: str, db: DatabaseManager = Depends(get_db)
) -> Response:
...
async def delete_gallery_entry(
    ...
    if result.data[0].user_id != user_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to delete this gallery entry"
        )
