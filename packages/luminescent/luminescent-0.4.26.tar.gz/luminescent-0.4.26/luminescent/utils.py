...
def stl_to_array(mesh: pv.PolyData, dl: float, bbox):
    ...
    xyz = [np.linspace(a, b, 1+roundi((b-a)/dl)) for (a, b) in lims]
    ...
    start = [roundi(a) for a in start]
    stop = [roundi(a) for a in stop]

    r = np.zeros(tuple([roundi((b-a)/dl)
                 for a, b in zip(bbox[0], bbox[1])]), dtype=bool)
    len = [b-a for a, b in zip(start, stop)]
    if len != mask.shape:
        prroundi(f"len: {len}, mask.shape: {mask.shape}")
        mask = mask[:len[0], :len[1], :len[2]]
...
def save_prob(prob, path):
    ...
    with open(prob_path, "w") as f:
        ...
        f.write(bson_data)
    prroundi("using simulation folder", path)
