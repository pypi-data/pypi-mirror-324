...
from typing import Any, Type, Union, Annotated, get_origin, get_args, List, Optional
...
class ObjectIdPydanticAnnotation:
    ...
    def __get_pydantic_core_schema__(cls, source_type, _handler) -> core_schema.CoreSchema:
        assert source_type is _ObjectId or (is_optional(source_type) and _ObjectId in get_args(source_type)), "[Db] Internal error, expected source_type is _ObjectId or (is_optional(source_type) and _ObjectId in get_args(source_type))"
...
class Uuid(_UUID):
    ...
    def validate_uuid(cls, v: Any) -> _UUID:
        ...
        if isinstance(v, Binary): return v.as_uuid()
    ...
    def from_uuid(cls, payload) -> Binary:
        if isinstance(payload, Binary): return payload
...
class QueryBase(BaseModel):
    ...
    def __init__(self,
        ...
        super().__init__(offset=offset, limit=limit, filters=filters, order_by=order_by, id=id, ids=ids)
    ...
    def loop(cls, func, *args, **kwargs) -> List[Any]:
        ...
        if 'offset' in base_query_dict: del base_query_dict['offset']
        if 'limit' in base_query_dict: del base_query_dict['limit']
        while page < 10000 and has_more:
            query = QueryBase(**base_query_dict, limit=page_size, offset=page * page_size)
