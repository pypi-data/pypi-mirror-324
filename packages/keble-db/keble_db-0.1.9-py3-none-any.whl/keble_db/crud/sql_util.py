...
def parse_query_for_list(model, query: Optional[QueryBase]) -> SqlQueryParsed:
    if query is None: return SqlQueryParsed(where=None)
    where_list = __parse_ids(model, query) + __parse_filters(model, query=query)
    assert query.offset is None or isinstance(query.offset, int), "[Db] Offset must be int"
    return SqlQueryParsed(where=_build_where(where_list))
...
def __parse_filters(
        model,  # noqa
        query: Optional[QueryBase]) -> List[BinaryExpression]:
    if query is None or query.filters is None or len(query.filters) == 0: return []
    assert not isinstance(query.filters, dict), "[Db] Query.filters only accept list of expression for SQL query"
