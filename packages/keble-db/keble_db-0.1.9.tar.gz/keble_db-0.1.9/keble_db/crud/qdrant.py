...
class QdrantCRUDBase(Generic[ModelType, VectorModelType]):
    ...
    def __convert_record(self, record: qdrant_types.Record) -> Record[VectorModelType]:
        assert record.vector is None or isinstance(
            record.vector, dict
    ...
    def search(
        ...
        **kwargs,
        ...
        scored_points: List[qdrant_types.ScoredPoint] = client.search(
            ...
            **self._build_search_kwargs(query),
            **kwargs,
        )
        return [
            Record(
    ...
    async def asearch(
        ...
        **kwargs,
        ...
        scored_points: List[qdrant_types.ScoredPoint] = await client.search(
            self.collection,
            query_vector=(vector_key, vector),
            score_threshold=score_threshold,
            **self._build_search_kwargs(query),
    ...
    def _build_scroll_kwargs(cls, query: Optional[QueryBase | dict] = None):
        ...
        assert (
            query is None or query.limit is not None
