...
class MaximLangchainTracer(BaseCallbackHandler):
    ...
    def __validate_maxim_metadata(self, metadata: Dict[str, Any]):
        """
    ...
    def __get_container_from_metadata(self, run_id: UUID) -> Container:
        ...
        if container is None:
            container = TraceContainer(
                trace_id=str(uuid.uuid4()), logger=self.logger, trace_name="Trace"
            )
            container.create()

        if maxim_metadata is not None and container.type() == "trace":
            session_id: Optional[str] = maxim_metadata.session_id
            container.parent = session_id
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        generation_config = GenerationConfig(
            id=generation_id,
            name=maxim_metadata.generation_name,
            provider=provider,
            model=model,
            messages=messages,
            model_parameters=model_parameters,
            tags=maxim_metadata.generation_tags,
        )
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        if kwargs.get("parent_run_id",None) is None:
            generation_name = maxim_metadata.generation_name or None
        ...
        try:
            ...
            if isinstance(output, ToolMessage):
                if output.status == "success":
                    ...
                    self.logger.tool_call_error(str(run_id), output.content)
            if container.parent() is None:
                container.end()
                self.containers.pop(str(run_id))
        except Exception as e:
    ...
    def on_tool_error(
        ...
        try:
            ...
            if container is None:
                ...
                return
            self.logger.tool_call_error(str(run_id), str(error))
            if container.parent() is None:
                container.end()
                self.containers.pop(str(run_id))
        except Exception as e:
