...
class Maxim:
    ...
    def __format_prompt_chain(
        ...
        json_data = {
            "promptChainId": prompt_chain_version.promptChainId,
            "versionId": prompt_chain_version.id,
            "version": prompt_chain_version.version,
            "nodes": (
                [
                    n
                    for n in prompt_chain_version.config.nodes
                    if hasattr(n, "prompt") or (isinstance(n, dict) and "prompt" in n)
                ]
                if prompt_chain_version.config
                else []
            ),
        }
    ...
    def __get_prompt_version_for_rule(
        self,
        prompt_version_and_rules: VersionAndRulesWithPromptId,
        ...
        if rule:
            ...
            for (
                ...
                for version_rule in version_rules:
                    ...
                    if not version:
                        continue
                    query = RuleGroupType(**version_rule.rules.query.__dict__)
                    objects.append(QueryObject(query=query, id=version_id))
            ...
            if prompt_chain_version_and_rules.rules:
                for (
                    ...
                    logging.debug(f"type: {type(version_rules[0].rules)}")
                    does_query_exist = any(
                        ruleElm.rules.query != None for ruleElm in version_rules
                    )
                    if does_query_exist:
                        deployed_version = next(
                            (
    ...
    def get_prompt_chain(self, id: str, rule: QueryRule) -> Optional[PromptChain]:
        ...
        if not version_and_rules:
            return None
        prompt_chains = self.__get_prompt_chain_version_for_rule(
            version_and_rules, rule
        )
