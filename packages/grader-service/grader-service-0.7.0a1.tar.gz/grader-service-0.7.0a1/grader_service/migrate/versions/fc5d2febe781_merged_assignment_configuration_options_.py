...
def upgrade():
    ...
    conn = op.get_bind()
    assignments = conn.execute(sa.text("SELECT id, duedate, type, automatic_grading, max_submissions, allow_files, settings FROM assignment"))
...
def downgrade():
    ...
    if 'type' not in columns:
        op.add_column('assignment', sa.Column('type', sa.Enum("user", "group", name='assignment_type'), nullable=False, server_default="user"))
    if 'automatic_grading' not in columns:
        op.add_column('assignment', sa.Column('automatic_grading', sa.Enum('unassisted', 'auto', 'full_auto', name='automatic_grading'), server_default='auto', nullable=False))
    if 'max_submissions' not in columns:
        op.add_column('assignment', sa.Column('max_submissions', sa.Integer(), nullable=True, server_default=None, unique=False))
    if 'allow_files' not in columns:
        op.add_column('assignment', sa.Column('allow_files', sa.Boolean(), nullable=False, server_default='f', default=False))
    ...
    for assignment in assignments:
        ...
        conn.execute(
            ...
            {
                "duedate": datetime.datetime.fromisoformat(settings['deadline']) if settings.get('deadline') else None,
