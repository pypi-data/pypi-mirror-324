...
class EOStoreCantLoadContainerError(EOStoreException):
    """When enable to open a product as container"""
...
class MappingMissingError(EOStoreException):
    """Raised when no mapping has been found for a product type"""
...
class EOQCConfigFactoryNoDefaultConfiguration(EOQCError):
    """When there is no default configuration for the given product type"""


class EOQCConfigFactoryAlreadyDefaultConfiguration(EOQCError):
    """When there is already a default configuration for the given product type"""
