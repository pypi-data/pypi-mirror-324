...
from eopf.computing.abstract import ADF
...
class EORunner(ABC):
    ...
    def run(
        ...
        with dask_context:
            ...
            # Open input products
            io_opened_products: dict[str, EOProduct | EOContainer | DataTree[Any]] = self.open_input_products(
                inputs_io_products,
                parsers_results.processing_workflow,
            )
            # List ADFs
            ...
            # Write results
            self.write_outputs(output_io_products, outputs)
    ...
    def write_outputs(
        ...
        outputs: dict[str, EOProduct | EOContainer | DataTree[Any]],
    ) -> None:
        """
        ...
        for output_unit_id, eoproduct in outputs.items():
            if EOConfiguration().triggering__use_datatree:
            ...
            output_id: str = output_unit_id.split(".")[1]
            try:
                output_io_param = output_io_products[output_id]
            ...
            if EOConfiguration().has_value("dask__export_graphs"):
                ...
                self.logger.info("EOVariables Dask graphs export requested in {folder}")
                eoproduct.export_dask_graph(folder)
            output_store[product_name] = eoproduct

    @staticmethod
    def run_workflow(
        io_interpreted_adf: dict[str, ADF],
        io_opened_products: dict[str, EOProduct | EOContainer | DataTree[Any]],
        processing_workflow: EOProcessorWorkFlow,
    ) -> dict[str, EOProduct | EOContainer | DataTree[Any]]:
        ...
        if EOConfiguration().triggering__validate_run:
            ...
            outputs: dict[str, EOProduct | EOContainer | DataTree[Any]] = processing_workflow.run_validating(
    ...
    def list_input_adfs(
        ...
        for adf in inputs_io_adfs.values():
            adf_id = adf["id"]
            new_adf = ADF(name=adf_id, path=adf["path"], store_params=adf["store_params"])
    ...
    def open_input_products(
        self,
        inputs_io_products: dict[str, Any],
        processing_workflow: EOProcessorWorkFlow,
    ) -> dict[str, EOProduct | EOContainer | DataTree[Any]]:
        """
        ...
        for input_product in inputs_io_products.values():
            ...
            try:
                ...
                if EOConfiguration().triggering__use_datatree and input_product["store_class"] == EOZarrStore:
                ...
                if EOConfiguration().triggering__use_datatree:
                    ...
                    try:
                        # First try to open it as container
                        product = input_product["store_class"](url=input_produt_anypath).load_container(
                            input_product["id"],
                        )
                    except EOStoreCantLoadContainerError:
                        # Then try to open it as product
                        product = input_product["store_class"](url=input_produt_anypath).load(input_product["id"])
                    io_opened_products[product_id] = product
            except StoreLoadFailure as err:
