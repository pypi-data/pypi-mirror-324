...
class EOProcessorWorkFlow(EOProcessingUnit):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        prod: dict[str, EOProduct | EOContainer | DataTree[Any]] = {}
        available_products: dict[str, EOProduct | EOContainer | DataTree[Any]] = {
            product.name: product for product in inputs.values()
        }
        if adfs is not None:
            available_adfs: dict[str, ADF] = {adf.name: adf for adf in adfs.values()}
        else:
            available_adfs = {}
        for unit_description in self.workflow:
            if not unit_description.active:
                ...
                break
            self.logger.info(f"RUN {unit_description.processing_unit.identifier}")
            unit_inputs = {}
            ...
            if not isinstance(unit_outputs, dict):
                raise TriggerInvalidWorkflow(f"ProcessingUnit {unit_description.identifier} is not outputing a dict")
            for output_payload_regex, output_payload_id in unit_description.outputs.items():
                ...
                for output_name in unit_outputs.keys():
                    if re.match(output_payload_regex, output_name):
                        prod[f"{unit_description.identifier}.{output_payload_id}"] = unit_outputs[output_name]
                        self.logger.info(
