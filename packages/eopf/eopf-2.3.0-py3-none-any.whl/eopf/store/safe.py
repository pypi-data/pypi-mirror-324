...
from eopf.exceptions.warnings import EOSafeStoreWarning
...
class EOSafeStore(EOProductStore):
    DEFAULT_TRANSFORMERS_LIST: list[tuple[str, Callable[[EOObject, Any], EOObject]]] = [
        ...
        ("dimensions", transformation_dimensions),  # dimensions should be after dimension dependant tranfo
        (
            "rechunk",
            transformation_rechunk,
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._attrs_map: dict[str, Any] = dict()
        self._data_map: dict[str, Any] = dict()
        self._coords_list: list[Any] = list()
        self._eop: Optional[EOProduct] = None
        self._eov_kwargs: dict[str, Any] = dict()
        self._accessors_cache: dict[int, EOAccessor] = dict()
    ...
    def _eov_assign_coords(self, eov: EOVariable) -> EOVariable:
        """
        Assign coordinate to the given EOVariable by seaching in the coords_list
        ...
        EOVariable
        """
        from eopf.exceptions.errors import EOVariableAssignCoordsError
        from eopf.exceptions.warnings import MappingMissingDimensionsWarning

        coords_dict = dict()

        try:
            for coord_description in self._coords_list:
                if "attributes" in coord_description and "dimensions" in coord_description["attributes"]:
                    # TBD, harmonisation, attributes should be under transform not directly under the eo_obj_description
                ...
                if self._is_coord(eov.dims, coord_dims):
                    _, coord_name = coord_description["target_path"].split(":")
                    coord = self._get_eo_obj(coord_description)
                    if coord is not None and isinstance(coord, EOVariable):
        ...
        return eov._init_similar(data_with_coord)
        ...
        for transformer_name, transformer_parameters in self.DEFAULT_TRANSFORMERS_LIST:
            try:
                if transformer_name in eo_obj_description["transform"]:
                    ...
                    # Safe store needs to be able to overide EOConfiguration mask and scale
                    # hence we pass the parameter specifically to this transformer as it is
                    # also responsible of masking and scalling
                    if transformer_name == "attributes":
                ...
                raise EOStoreInvalidRequestError(
                    f"Applying parameter {transformer_name} on {eo_obj_description} failed.",
                ) from err
    ...
    def _get_eo_obj(self, eo_obj_description: Dict[str, Any]) -> "EOObject":
        ...
        # retrieve accesor configurations as kwargs
        ...
        # the accessor implementation should extract the propper arguments
        accessor_config: dict[str, Any] = dict()
        ...
        # apply transformers in neccessary or return the object as read by the accessor
    ...
    def __getitem__(self, key: str) -> "EOObject":
        ...
        if key.startswith("attrs:"):
            # this is an attribute of the eop
            eo_obj_description = self._attrs_map[key]
            eo_obj = self._get_eo_obj(eo_obj_description)
            if not is_None_EOObj(eo_obj):
                return eo_obj

        if not key.endswith("/"):
            if key in self._data_map or key in self._short_names_map:
                ...
                if eo_obj is not None:
                    if isinstance(eo_obj, EOVariable):
                        return self._eov_assign_coords(eo_obj)
                    else:
                        return eo_obj
            else:
                self._logger.warning(f"{key} does not exist", EOSafeStoreWarning)
        else:
            self._logger.warning("EOSafeStore does not support EOGroup get", EOSafeStoreWarning)
    ...
    def open(
        ...
        **kwargs: Any,
        ...
        if mode == OpeningMode.OPEN:
            ...
            self._mapping, short_names_map = self._mapping_manager.parse_mapping(self.url)
            ...
            try:
                ...
                if not self.url.islocal():
                    self._original_url = (
                        self.url
                    )  # The tmp_dir is kept by this instance, is deleted then garbage collect ...
                ...
                for eo_obj_description in self._mapping["data_mapping"]:
                    ...
                    if target_path.startswith("attrs:"):
                        self._attrs_map[target_path] = eo_obj_description
                    elif target_path.startswith("coords:"):
                        self._coords_list.append(eo_obj_description)
                    else:
                        self._data_map[target_path] = eo_obj_description

            except Exception as err:
                raise StoreOpenFailure(f"{err}")
    ...
    def load_container(self, name: str = "", **kwargs: Any) -> "EOContainer":
        raise EOStoreCantLoadContainerError("EOContainer not supported in EOSafeStore")
    ...
    def load(self, name: Optional[str] = None, **kwargs: Any) -> EOProduct:
        ...
        if self._is_loaded is True and self._eop is not None:
            return self._eop
        ...
        # compute the EOP attrs
        attrs: Dict[str, Any] = dict()
        for key in self._attrs_map.keys():
            cur_attr = self.__getitem__(key).attrs
            _ = self._merge_dict_attrs(attrs, cur_attr)

        # determine EOProduct type
        if (
            self._mapping is not None
            and "recognition" in self._mapping
            and "product_type" in self._mapping["recognition"]
        ):
            product_type = self._mapping["recognition"]["product_type"]
        else:
            product_type = ""
        self._logger.info(f"Found product type : {product_type}")
            ...
            for var_path in self.iter(""):
                if var_path not in self._eop:
                    ...
                    if not is_None_EOObj(eov) and isinstance(eov, EOVariable):
                        self._eop[var_path] = eov
        except Exception as err:
            raise StoreLoadFailure(f"{err}")
        finally:
            # mark product as beeing loaded
    ...
    def close(self) -> None:
        ...
        self._mapping = {}
        self._eov_kwargs = {}
        self._mapping_manager = EOPFMappingManager()
        self._short_names_map = {}
        self._attrs_map = {}
        self._data_map = {}
        self._coords_list = []
        self._eop = None
