...
from eopf.exceptions.warnings import AlreadyOpen, StoreNotOpen
...
class EOProductStore(MutableMapping[str, "EOObject"]):
    ...
    @abstractmethod
    def load(self, name: str = "", **kwargs: Dict[str, Any]) -> "EOProduct":
        """
        Load the entire EOProduct hierarchy.
    ...
    def close(self) -> None:
        """Close the store"""
        if self._status == StorageStatus.CLOSE:
            self._logger.warning(StoreNotOpen())
        self._status = StorageStatus.CLOSE
