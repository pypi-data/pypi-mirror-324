...
class EOExtractRoiUnit(EOProcessingUnit):
    ...
    def run(
        self,
        inputs: dict[str, EOProduct | EOContainer | DataTree[Any]],
        ...
        **kwargs: Any,
        ...
        region: Str
            region of interest to be extracted as string "(x,y,w,h)", w,h positive
            x and y can be indices or the geodetic coordinates lon and lat depending on the region type
        type: Str
            the region type, can be one of the following :
            "slice" or "geo_region"

        ref_variable: Str, optional
            processing context, not used by this processing unit
        ...
        # return the ROI
        ret: dict[str, EOProduct | EOContainer | DataTree[Any]] = {}
        for key, eoproduct in inputs.items():
            if isinstance(eoproduct, (EOContainer, DataTree)):
                raise TypeError("EOContainer not allowed on this processing Unit")
...
class EORechunkingUnit(EOProcessingUnit):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        for key, eoproduct in inputs.items():
            if not isinstance(eoproduct, DataTree):
                self.rechunk_product(eoproduct, chunks)
        return inputs

    def rechunk_product(self, prod: EOProduct | EOContainer, chunks: Dict[str, int]) -> None:
...
class EOConstantUnit(EOProcessingUnit):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        for key, eoproduct in inputs.items():
            if isinstance(eoproduct, (EOContainer, DataTree)):
                raise TypeError("EOContainer not allowed on this ProcessingUnit")
...
class EODummyUnit(EOProcessingUnit):
    ...
    def run(
        self,
        inputs: dict[str, EOProduct | DataTree[Any]],
        adfs: Optional[dict[str, ADF]] = None,
        **kwargs: Any,
    ) -> dict[str, EOProduct | DataTree[Any]]:
        """
        Return the input
        ----------
