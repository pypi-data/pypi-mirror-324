...
class EOMultiSourceRasterIOAccessor(EOAccessor):
    ...
    def _get_ndarray_from_stores(self) -> np.typing.NDArray[Any]:
        ...
        if len(ndarray_list) == 1:
            ...
            return np.pad(ndarray_list[0], pad_width, constant_values=np.nan)
...
class EOFoldedMultiSourceRasterIOAccessor(EOMultiSourceRasterIOAccessor):
    ...
    def __init__(self, url: str, **kwargs: Any) -> None:
        ...
        self._folded_dimension_size_xpath: Optional[str] = None
        self._dimension_to_fold = None
    ...
    def __getitem__(self, key: str) -> "EOObject":
        ...
        if self._folded_dimension_size_xpath is not None:
            folded_dimension_size = int(self._parse_xml_xpath(self._folded_dimension_size_xpath))
            ndarray = super()._get_ndarray_from_stores()
            if self._dimension_to_fold != 1:
                raise NotImplementedError
            else:
                folded_shape = [ndarray.shape[0], -1, folded_dimension_size, ndarray.shape[2]]
