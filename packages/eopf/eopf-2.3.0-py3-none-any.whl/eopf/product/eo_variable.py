...
class EOVariable(EOObjectWithDims, EOVariableOperatorsMixin["EOVariable"]):
    ...
    **kwargs: Any
    ...
    def mask(
        ...
        """
        Mask the data of the current EOVar
        ...
        if not mask_apply:
            self._logger.warning("Masking not applied: mask_apply is False")
            return self

        # non numeric data types will not be masked
        if not np.issubdtype(self.dtype, np.number):
            self._logger.warning("Masking not applied: the EOVar dtype is non numeric")
            return self
        ...
        if fill_value is None:
            if XARRAY_FILL_VALUE in self._data.attrs:
                ...
                self._logger.warning("fill_value is automatically set to numpy default fill value")
        ...
        if valid_min is not None and np.isnan(valid_min):
            self._logger.warning("Ignoring valid_min: should not be NaN")
            valid_min = None
        if valid_max is not None and np.isnan(valid_max):
            self._logger.warning("Ignoring valid_max: should not be NaN")
            valid_max = None
    ...
    def scale(
        ...
        """
        Scale the data of the current EOVar
        ...
        if not scale_apply:
            # no scaling applied
            self._logger.warning("Scaling not applied: scale_apply is False")
            return self

        # non numeric data types will not be scaled
        if not np.issubdtype(self.dtype, np.number):
            self._logger.warning("Scaling not applied: the EOVar dtype is non numeric")
            return self

        # let the user know it is performing scaling on already scaled data
        if self.is_scaled is True:
            self._logger.warning("Current EOVar is already scaled")
        ...
        if (
            ...
            self._logger.warning("Scaling not applied: scale_factor and add_offset were not specified")
            return self
        ...
        # remove scale_factor and add_offset from data.attrs
        # to mark it as beeing masked
    ...
    def unscale(
        ...
        """
        Un-scale the data of the current EOVar
        ...
        if not scale_apply:
            ...
            self._logger.warning("Unscaling not applied: the EOVar apply_scale is False")
            return self

        # non numeric data types are not scaled
        if not np.issubdtype(self.dtype, np.number):
            self._logger.warning("Unscaling not applied: the EOVar dtype is non numeric")
            return self

        # let the user know it is performing an unscalling over a variable which was not scaled previously
        if not self.is_scaled:
            self._logger.warning("The current EOVariable is not scaled")
        ...
        if add_offset is None and scale_factor is None:
            self._logger.warning("Unscaling not applied: neither scale_factor nor add_offset were specified")
            return self
        ...
        if target_dtype is None:
            if DTYPE in self._attrs:
                ...
                self._logger.warning("Target dtype was not specified using dtype of the data")
    ...
    def _print_tree_structure(self, obj: Union["EOObject", tuple[str, "EOObject"]], level: int, detailed: bool) -> None:
        ...
        if detailed:
            ...
            print(f"{indent}     Data:")
            print(f"{indent}        {variable.data}")
            print(f"{indent}     Coordinates:")
            print(f"{indent}        {variable.coords}")
            print(f"{indent}     Dimensions:")
            print(f"{indent}        {variable.dims}")
