...
def transformation_attributes(eo_obj: "EOObject", parameter: Dict[str, Any]) -> "EOObject":
    ...
    if not isinstance(eo_obj, EOVariable):
        ...
        if "mask_and_scale" in parameter:
            # we need to remove the mask and scale as it may affect the attributes update
            mask_and_scale: Optional[bool] = parameter.pop("mask_and_scale", False)
        else:
            mask_and_scale = None

        # dimensions are treated by a separate transformer
        # we keep them in the mapping for the sake of backwards copatibility
        ...
        parameter["mask_and_scale"] = mask_and_scale
        transformation_mask_and_scale(eo_obj, parameter)
...
def transformation_mask_and_scale(eo_obj: "EOObject", parameter: Any) -> "EOObject":
    ...
    # masking
    if (
        mask_and_scale_apply
        and parameter
        and (VALID_MIN in parameter or VALID_MAX in parameter or FILL_VALUE in parameter)
    ):
    ...
    # scaling
    if mask_and_scale_apply and (parameter is not None) and (SCALE_FACTOR in parameter or ADD_OFFSET in parameter):
