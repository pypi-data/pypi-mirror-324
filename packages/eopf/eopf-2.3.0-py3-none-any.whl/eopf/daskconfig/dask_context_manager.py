...
class DaskContext:
    ...
    def __init__(
        ...
        try:
            ...
            self._logger.debug("No Dask client found")
    ...
    def _setup_gateway_cluster(self) -> None:
        try:
            ...
            workers = self._cluster_config.pop("workers", 1)
            auth_type = auth_kwargs.pop("auth")
            auth_class = get_auth(auth_type)
            auth = auth_class(**auth_kwargs)
            # setup gateway
            ...
            if previous_cluster_name is not None and any(
                previous_cluster_name == cluster.name for cluster in available_clusters
            ):
