...
def exists(url: Any, **storage_options: dict[str, Any]) -> bool:
...
def is_dir(url: Any, **storage_options: dict[str, Any]) -> bool:
    # If the url is not an str or a Path it is most likely a memory mapper like zapp.memory
    if not isinstance(url, str) or isinstance(url, Path):
        return True
    if url.startswith("s3:") or url.startswith("zip::s3:"):
...
def is_file(url: Any, **storage_options: dict[str, Any]) -> bool:
    # If the url is not an str or a Path it is most likely a memory mapper like zapp.memory
    if not isinstance(url, str) or isinstance(url, Path):
        return True
    if url.startswith("s3:") or url.startswith("zip::s3:"):
...
class AnyPath:
        ...
        **kwargs: dict[str, Any],
        ...
        if len(protocol_list) > 1:
            self._parent = type(self)(
                path[prefix_size:],
                parent=None,
                trim_archive_component=trim_archive_component,
        ...
        if protocol != "zip" and auto_open_archives and path.endswith(".zip") and self._fs.isfile(path):
            source_fs = self._fs
            source_url = path

            next_parent = type(self)("")
            next_parent._path = path
        ...
        self._path: str = path.rstrip(self._fs.sep) or self._fs.root_marker
    ...
    def basename(self) -> str:
        ...
        """
        if self._path == self._fs.root_marker:
            return self._path
        return self._path.split(self.sep)[-1]
    ...
    def dirname(self) -> Self:
        ...
        out = self.copy()
        if out._path == self._fs.root_marker:
            return out
        basename_length = len(self.basename)
        dir_path = self._path[:-basename_length]

        out._path = dir_path.rstrip(self._fs.sep) or self._fs.root_marker
    ...
    def ls(self) -> list[Self]:
        ...
        if self.isdir():
            for path in self._fs.ls(self._path, detail=False):
                ...
                item._path = path
                output.append(item)
            # ~ return [AnyPath(path, parent=self) for path in self._fs.ls(self._path, detail=False)]
