...
from eopf.product.conveniences import get_product_type, set_product_type
...
class EOContainer(EOObject):
        ...
        for product in products:
            ...
            if product_type is None:
                if container_type is None:
                    ...
                    set_product_type(product, container_type)
            else:
                # product_type is not None
                if container_type is None:
                    container_type = product_type
                else:
                    if container_type != product_type:
                        # diffferent product:type
                        raise EOContainerSetitemError(
                            "Different product types are not allowed,"
                            f" container: {container_type} product: {product_type}",
                        )
                    else:
                        # same product:type
                        pass
        ...
        try:
            ...
            if isinstance(value, (EOContainer, EOProduct)):
                if len(eopath_parts) > 1:
                    ...
                    # add the EOProduct/EOContainer under current EOProduct

                    value_type = get_product_type(value)
                    if self.type is None and value_type is None:
                        # allows users to add non defined products as they please
                        pass
                    elif self.type is None and value_type is not None:
                        # current container gets the type from children
                        set_product_type(self, value_type)
                    elif self.type is not None and value_type is None:
                        # the child container/product gets the type from current cotainer
                        set_product_type(value, self.type)
                    else:
                        # both current container and child container/product have product:type defined
                        if self.type != value_type:
                            raise TypeError(
                                f"Can't have different product types in container, "
                                f"contained {self.type} while putting {value_type}",
                            )
                        else:
                            # same product:type
                            pass
    ...
    def _declare_as_container(self) -> None:
        ...
        self._attrs.setdefault("other_metadata", dict()).setdefault("eopf_category", "eocontainer")
    ...
    def is_container(objwithattr: Any) -> "bool":
        ...
        try:
            return (
                ...
                and objwithattr.attrs["other_metadata"]["eopf_category"] == "eocontainer"
            )
        except KeyError:
            return False
