...
class EOSafeInit(ABC):
    ...
    Init class abstract to fill the init_function section in the mappings
        ...
        "init_function": {
            ...
            "class" : "default_init_function" >>> Class implementing this interface
        },
        "finalize_function": {
            "module" : "tests.store.fake_init_function",
...
class EOSafeFinalize(ABC):
    ...
    Finalize class abstract to fill the finalize_function section in the mappings
        ...
        "finalize_function": {
            "module" : "tests.store.fake_init_function",
            "function" : "default_finalize_function" >>> Class implementing this interface
        },
        "data_mapping": [
            {
            ...
...
class EOSafeStore(EOProductStore):
    ...
    def _eov_assign_coords(self, eov: EOVariable, namespace: str) -> EOVariable:
        """
        Assign coordinate to the given EOVariable by seaching in the coords_dict
        ...
        try:
            ...
            for coord_name, _ in data_with_coord.coords.items():
                if not data_with_coord[coord_name].attrs:
                    data_with_coord[coord_name].attrs = coords_attrs[coord_name]
    ...
    def _detect_init_finalize(self) -> None:
        ...
        if self._mapping is not None and "finalize_function" in self._mapping:
            ...
            if not issubclass(finalize_class, EOSafeFinalize):
                raise StoreOpenFailure(f"{finalize_class} is not implementing EOSafeInit")
    ...
    def load_container(self, name: str = "", **kwargs: Any) -> "EOContainer":
        warn("Deprecated method, use load method; ", EOPFDeprecated)
        return EOContainer("empty")

    @staticmethod
    def __get_node_type(attrs: dict[str, Any]) -> str:
        if "other_metadata" in attrs and "eopf_category" in attrs["other_metadata"]:
            if attrs["other_metadata"]["eopf_category"] in ("eocontainer", "eoproduct"):
                return attrs["other_metadata"]["eopf_category"]
        return "unknown"
    ...
    def load(self, name: Optional[str] = None, **kwargs: Any) -> EOProduct | EOContainer:
        ...
        try:
            ...
            # Finalize the eop
            self._finalize_eop(eop_kwargs, eop_mapping_manager)
    ...
    def _finalize_eop(self, eop_kwargs: Any, eop_mapping_manager: EOPFAbstractMappingManager) -> None:
