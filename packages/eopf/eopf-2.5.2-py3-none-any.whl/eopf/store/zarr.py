...
class EOZarrStore(EOProductStore):
    ...
    def _decode_xarray_dataset(self, dataset: Dataset) -> None:
        ...
        for var_name in dataset.variables:
            # manage datatime64
            # convert to datetime64 when _datetime64 attr is found as decode_cf is set to False
            if (
                ZARR_EOV_ATTRS in dataset[var_name].attrs
                and ZARR_DECODE_DATETIME_ATTR in dataset[var_name].attrs[ZARR_EOV_ATTRS]
            ):
                datetime64_dtype = dataset[var_name].attrs[ZARR_EOV_ATTRS].pop(ZARR_DECODE_DATETIME_ATTR)
                if var_name not in dataset.coords:
                    # decode eovariables' data
                    dataset[var_name].data = dataset[var_name].data.astype(datetime64_dtype)
                else:
                    # decode coordinates' data
                    # coordinates can not be directly converted to a specific dtype
                    # we need to drop the coordinate convert to specific dtype
                    # and ultimately reassign the coordinates
                    # only ZARR_DECODE_DATETIME_ATTR uses coordinates ZARR_EOV_ATTRS, hence we drop it
                    dataset[var_name].attrs.pop(ZARR_EOV_ATTRS)
                    tmp_var = dataset[var_name].astype(datetime64_dtype)
                    tmp_var.attrs["dtype"] = np.dtype(datetime64_dtype)
                    dataset = dataset.drop(var_name)
                    dataset.assign_coords({var_name: tmp_var})
                    dataset[var_name] = tmp_var

            # the cpm does not allow nan in fill_value, hence only xarray can add it
            # when mask_and_scale parameter of open_zarr is set to false
            # this will break the logic of is_masked, hence we need to pop it
            if XARRAY_FILL_VALUE in dataset[var_name].attrs:
                try:
                    if isnan(dataset[var_name].attrs[XARRAY_FILL_VALUE]):
                        dataset[var_name].attrs.pop(XARRAY_FILL_VALUE)
                except TypeError:
                    # isnan does not work on non-numeric data
                    pass
    ...
    def _write_eog(
        ...
        # manage datetime64 writing
        # as decode_cf is False on reading a zarr with xarray datetime variables will be decoded as integers
        # hence, we introduce a private attrs _datetime64 to force datetime decoding
        for var_name in ds.variables:  # type: ignore
            if np.issubdtype(ds[var_name].dtype, np.datetime64):
                if ZARR_EOV_ATTRS not in ds[var_name].attrs:
                    ds[var_name].attrs[ZARR_EOV_ATTRS] = dict()
                ds[var_name].attrs[ZARR_EOV_ATTRS][ZARR_DECODE_DATETIME_ATTR] = str(ds[var_name].dtype)
    ...
    def __setitem__(self, __key: str, __value: EOObject) -> None:
        ...
        if isinstance(__value, (EOProduct, EOContainer)):
            ...
            if isinstance(__value, EOProduct):
                __value.clean_short_names_in_variables()
                __value.dump_short_names_to_variables()
            ...
            self._write_eog(group_fspath, group_fspath.to_zarr_store(), __value)
        elif isinstance(__value, EOVariable):
            if __key == "":
