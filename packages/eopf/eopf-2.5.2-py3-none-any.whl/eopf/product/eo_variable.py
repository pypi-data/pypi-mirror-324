    ...
    **kwargs: Any
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if self.name != other.name:
            return False  # EOVariable names are different

        if self.is_scaled != other.is_scaled:
            return False  # not both scaled or unscaled

        if self.is_masked != other.is_masked:
    ...
    def mask(
        ...
        if self.is_masked is True:
            self._logger.debug(f"{self.path}. Masking not applied EOVariable is already masked")
            return self
    ...
    def scale(
        ...
        if target_dtype is None:
            if TARGET_DTYPE in self._attrs:
                target_dtype = self._attrs[TARGET_DTYPE]
        ...
        for coord_name in self._data.coords:
            if coord_name not in self._data.coords.indexes.dims:
                ...
                target_dtype = coord.attrs.get(target_dtype, None)
