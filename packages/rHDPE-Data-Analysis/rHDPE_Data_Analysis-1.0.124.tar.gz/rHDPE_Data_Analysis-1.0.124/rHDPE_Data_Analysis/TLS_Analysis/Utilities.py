...
def compute_derivatives( data, width = 1 ):
    ...
    for i in range( len( data[1] ) ):
        ...
        derivative_data[2].append( np.array( gu.derivative( data[1][i], data[2][i], width ) ) )
...
def read_and_analyse_features( ip, file_data ):
    ...
    df.to_csv( ip.output_directory + "TLS/Features/Mean_Features_Unnormalised.csv" )
    ...
    df.to_csv( ip.output_directory + "TLS/Features/Std_of_Features_Unnormalised.csv" )
