...
def pca( directory, output_directory, shiny, features_df, std_of_features_df, name_appendage = "" ):
    ...
    if pca_of_whole_dataset:
        ...
        gu.perform_pca( directory, features_df_copy, [int( i ) for i in features_df_copy.index], std_error = True, std_of_features_df = std_of_features_df_copy, num_components = 2, filename = output_directory + "Global/PCA/Overall.pdf", name_appendage = name_appendage )
    ...
    if pca_of_pca_of_individual_datasets:
        ...
        if shiny:
            ...
            pca_ft_df, pca_std, components, pca_feature_names = gu.perform_pca( directory, overall_pca_df, [int( i ) for i in overall_pca_df.index], std_error = True, std_of_features_df = overall_stds_df, num_components = 2, filename = output_directory + "Global/PCA/Overall.png", name_appendage = name_appendage )

        else:

            pca_ft_df, pca_std, components, pca_feature_names = gu.perform_pca( directory, overall_pca_df, [int( i ) for i in overall_pca_df.index], std_error = True, std_of_features_df = overall_stds_df, num_components = 2, filename = output_directory + "Global/PCA/Overall.pdf", name_appendage = name_appendage )
...
def best_model_for_escr_prediction( ip, datasets_to_read, name_appendage = "" ):
    ...
    features_df, std_of_features_df, _ = Preprocessing.read_files_and_preprocess( ip, normalised = False )
    ...
    # resin_data = gu.get_list_of_resins_data( ip.directory, name_appendage )
    #
    # gu.plot_scatterplot_of_two_features( ip.directory, prediction_unnormalised, target.to_numpy(), features_df.index, [resin_data.loc[i]["Label"] for i in features_df.index], errorbars = False, std = "", line_of_best_fit = False, exponential_fit = False, xlabel = "Prediction", ylabel = "Target", savefig = False, filename = "" )
...
def escr_predictor( ip, datasets_to_read, name_appendage = "" ):
    ...
    dataset_labels = ["FTIR", "DSC", "TGA", "Rhe", "Colour", "TT", "SHM", "TLS", "ESCR"]
...
def sandbox( ip, features_df, std_of_features_df ):
    ...
    if perform_best_model_for_escr_prediction:
        ...
        best_model_for_escr_prediction( ip, [2, 7] )

    if perform_escr_predictor:

        escr_predictor( ip, [2, 7] )
