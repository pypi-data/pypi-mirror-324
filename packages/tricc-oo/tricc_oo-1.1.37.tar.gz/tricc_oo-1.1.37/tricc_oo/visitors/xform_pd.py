    ...
    for s in hidden_names:
        ...
        for i in above_s_grouprows:
            if df_above_s.iloc[i]['type']=='begin group':
                ...
                fullpath = fullpath[:-1]
        if len(fullpath)>0:
            fullpath = 'content.' + chf_clean_name(s, True) +' = getField(report, \'' + '.'.join(fullpath) + '.' + chf_clean_name(s) + '\');'
        else:
            fullpath = 'content.' + chf_clean_name(s, True) +' = getField(report, \'' + chf_clean_name(s) + '\');'
        d[s]=fullpath
