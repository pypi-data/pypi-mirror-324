from .find_elements import find_element_with_wait_backcode, By
...
def backcode__dont_use__launch_browser(download_dir: str, extension_path, captcha_name, captcha_api_key, extension_id) -> WebElement:
    global driver
    ...
    if captcha_name:
        ...
        for name in cap_monster_names:
            if captcha_name.lower() == name:
                backcode__dont_use__capmonster(captcha_api_key, extension_id)
...
def backcode__dont_use__capmonster(api_key, extension_id) -> None:
    global driver

    driver.get("chrome://extensions/")
    time.sleep(10)
    print("iniciando extração ID")
    ...
    find_element_with_wait_backcode(driver, By.ID, "client-key-input").send_keys(api_key)
    time.sleep(2.5)
    find_element_with_wait_backcode(driver, By.XPATH, '//label[span[input[@id="captcha-radio-token-ReCaptcha2"]]]').click() # icone salvar
    find_element_with_wait_backcode(driver, By.ID, "client-key-save-btn").click() # icone salvar
