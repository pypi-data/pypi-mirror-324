...
from databricks.ml_features.constants import OVERWRITE
...
class OnlineStorePublishNoSqlClient(OnlineStorePublishClient):
    ...
    def get_or_create_online_table(self, df, primary_keys, timestamp_keys):
        ...
        if not cloud_provider_unique_id:
            ...
            new_table = False
        return self._get_online_table(cloud_provider_unique_id, new_table)

    # Gets Online Table, Throws if the table doesn't exist.
    def get_online_table(self):
        cloud_provider_unique_id = self.nosql_engine.get_cloud_provider_unique_id()
        return self._get_online_table(cloud_provider_unique_id)

    def _get_online_table(self, cloud_provider_unique_id, new_table=False):
        if not cloud_provider_unique_id:
            raise ValueError(f"Online Table {self.nosql_engine.table_name} Not Found")
        return OnlineTable(
    ...
    # online_table_name not used but needed to override method in OnlineStorePublishClient
    def _drop_online_table(
        self,
        online_table_name: str,
    ):
        # for NoSQL stores, table_name is baked into engine, no need to provide as input to drop_table
        self.nosql_engine.drop_table()
