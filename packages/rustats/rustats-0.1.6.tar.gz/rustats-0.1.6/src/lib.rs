...
//------------------------------------------------------------------------------
// A partial pivot matrix inversion, returns None if near singular
//------------------------------------------------------------------------------
...
impl Probit {
    ...
    /// sum ln( Phi(q_i x_i'beta) ), q_i=2y_i-1
...
//------------------------------------------------------------------------------
// RustProbitResults => has .params, .model, .cov_params(), etc.
//------------------------------------------------------------------------------
#[pyclass]
...
fn fit_probit(
    ...
    // defaults
    let max_iter = max_iter.unwrap_or(35);   // default 35
    let tol = tol.unwrap_or(1e-6);          // default 1e-6
    ...
    let mut exog_names = Vec::with_capacity(kvars);
    for j in 0..kvars {
