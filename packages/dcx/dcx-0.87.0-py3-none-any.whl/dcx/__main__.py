...
parser.add_argument("--hot-port", nargs=1, type=int, default=[0], help="TCP Port for *hot* step submission via tcp:127.0.0.1:PORT (default is 0=off)")
...
import socketserver
global_hot_port = int(args.hot_port[0])
...
class HotHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self) -> None:
        ...
        global_hot_queue.put(""+self.data.decode())
...
def process_report(logbasedir_):
    ...
    if args.report:
        ...
        pdf.page(the_page).text(1,15,"*** THIS DOCUMENT MAY CONTAIN CONFIDENTIAL DATA *** DO NOT TRANSMIT AS UNENCRYPTED ***")
        ...
        pdf.page(the_page).text(52,20,testdata)
        ...
        for report_i in range(0, len(play)):
            ...
            pdf.page(the_page).text( 3,10, "COMMAND                    :  " + str(report_play_part[1]))
            ...
            pdf.page(the_page).text(12,10, "URL BEFORE :  %s" % "https://")
            pdf.page(the_page).text(13,10, "URL AFTER  :  %s" % "https://")
