...
from femtodriver.plugins.model_export import FemtoFile
...
class SPURunner(FemtoRunner):
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            zynq_host : str :
                the host or ip address of the zynq board
        """
    ...
    def _hw_write_from_compiled_data(self, mem: Tuple[str]) -> None:
        cidx, memname, _ = mem
        datas = self.compiled_data.mems_per_core[cidx][memname][0]
    ...
    def check_interrupt_reg(self):
        ...
        if cfg.ISA < 1.3:
            logger.info("trying to check interrupt reg for ISA < 1.3")
            logger.info("sleeping for a second instead")
            logger.info("stuff might break")
            self.wait(1000, 1)
            return np.array([1], dtype=np.uint64)
    ...
    def program_network_from_meta(self):
        ...
        for cidx in range(self.used_cores):
            ...
            for mem, hex_fname in hex_fnames.items():
                logger.debug(f"PROGRAMMING {mem}")
                logger.debug(f"  fname is {hex_fname}")
