...
| --- | --- | --- |
| `t2s.get_` | Fetch some data | [`t2s.get_related_ddl(...)`][text2sql.base.base.Text2SqlBase.get_related_ddl] |
| `t2s.add_` | Adds something to the retrieval layer | [`t2s.add_question_sql(...)`][text2sql.base.base.Text2SqlBase.add_question_sql] <br> [`t2s.add_ddl(...)`][text2sql.base.base.Text2SqlBase.add_ddl] |
| `t2s.generate_` | Generates something using AI based on the information in the model | [`t2s.generate_sql(...)`][text2sql.base.base.Text2SqlBase.generate_sql] <br> [`t2s.generate_explanation()`][text2sql.base.base.Text2SqlBase.generate_explanation] |
| `t2s.run_` | Runs code (SQL) | [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql] |
| `t2s.remove_` | Removes something from the retrieval layer | [`t2s.remove_training_data`][text2sql.base.base.Text2SqlBase.remove_training_data] |
| `t2s.connect_` | Connects to a database | [`t2s.connect_to_snowflake(...)`][text2sql.base.base.Text2SqlBase.connect_to_snowflake] |
| `t2s.update_` | Updates something | N/A -- unused |
| `t2s.set_` | Sets something | N/A -- unused  |

# Open-Source and Extending

Text2Sql is open-source and extensible.

The following is an example of where various functions are implemented in the codebase when using the default "local" version of Text2Sql. `text2sql.base.Text2SqlBase` is the base class which provides a `text2sql.base.Text2SqlBase.ask` and `text2sql.base.Text2SqlBase.train` function. Those rely on abstract methods which are implemented in the subclasses `text2sql.openai_chat.OpenAI_Chat` and `text2sql.chromadb_vector.ChromaDB_VectorStore`. `text2sql.openai_chat.OpenAI_Chat` uses the OpenAI API to generate SQL and Plotly code. `text2sql.chromadb_vector.ChromaDB_VectorStore` uses ChromaDB to store training data and generate embeddings.

If you want to use Text2Sql with other LLMs or databases, you can create your own subclass of `text2sql.base.Text2SqlBase` and implement the abstract methods.

```mermaid
flowchart
    subgraph Text2SqlBase
        ask
        train
    end
...
class Text2SqlBase(ABC):
    ...
    def generate_sql(self, question: str, allow_llm_to_see_data=False, **kwargs) -> str:
        ...
        Example:
        ```python
        t2s.generate_sql("What are the top 10 customers by sales?")
        ```

        Uses the LLM to generate a SQL query that answers a question. It runs the following methods:

        - [`get_similar_question_sql`][text2sql.base.base.Text2SqlBase.get_similar_question_sql]

        - [`get_related_ddl`][text2sql.base.base.Text2SqlBase.get_related_ddl]

        - [`get_related_documentation`][text2sql.base.base.Text2SqlBase.get_related_documentation]

        - [`get_sql_prompt`][text2sql.base.base.Text2SqlBase.get_sql_prompt]

        - [`submit_prompt`][text2sql.base.base.Text2SqlBase.submit_prompt]
    ...
    def extract_sql(self, llm_response: str) -> str:
        ...
        Example:
        ```python
        t2s.extract_sql("Here's the SQL query in a code block: ```sql\nSELECT * FROM customers\n```")
        ```
    ...
    def extract_table_metadata(ddl: str) -> TableMetadata:
      """
        Example:
        ```python
        t2s.extract_table_metadata("CREATE TABLE hive.bi_ads.customers (id INT, name TEXT, sales DECIMAL)")
        ```
    ...
    def is_sql_valid(self, sql: str) -> bool:
        ...
        Example:
        ```python
        t2s.is_sql_valid("SELECT * FROM customers")
        ```
    ...
    def generate_rewritten_question(self, last_question: str, new_question: str, **kwargs) -> str:
        ...
        **Example:**
        ```python
        rewritten_question = t2s.generate_rewritten_question("Who are the top 5 customers by sales?", "Show me their email addresses")
        ```
    ...
    def generate_followup_questions(
        ...
        **Example:**
        ```python
        t2s.generate_followup_questions("What are the top 10 customers by sales?", sql, df)
        ```

        Generate a list of followup questions that you can ask Text2Sql.
        ...
        Returns:
            list: A list of followup questions that you can ask Text2Sql.
        """
    ...
    def generate_questions(self, **kwargs) -> List[str]:
        ...
        **Example:**
        ```python
        t2s.generate_questions()
        ```

        Generate a list of questions that you can ask Text2Sql.
        """
    ...
    def generate_summary(self, question: str, df: pd.DataFrame, **kwargs) -> str:
        ...
        **Example:**
        ```python
        t2s.generate_summary("What are the top 10 customers by sales?", df)
        ```
    ...
    def remove_training_data(self, id: str, **kwargs) -> bool:
        ...
        Example:
        ```python
        t2s.remove_training_data(id="123-ddl")
        ```
    ...
    def submit_prompt(self, prompt, **kwargs) -> str:
        ...
        t2s.submit_prompt(
            [
                t2s.system_message("The user will give you SQL and you will try to guess what the business question this query is answering. Return just the question without any additional explanation. Do not reference the table name in the question."),
                t2s.user_message("What are the top 10 customers by sales?"),
            ]
        )
        ```
        ...
        **kwargs
        ...
        try:
            ...
            raise DependencyError(
                ...
                " \npip install text2sql[snowflake]"
            )
    ...
    def connect_to_sqlite(self, url: str, check_same_thread: bool = False,  **kwargs):
        """
        Connect to a SQLite database. This is just a helper function to set [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql]
        ...
        path = "/tmp/" + os.path.basename(urlparse(url).path)
    ...
    def connect_to_postgres(
        ...
        **kwargs
        ...
        """
        Connect to postgres using the psycopg2 connector. This is just a helper function to set [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql]
        **Example:**
        ```python
        t2s.connect_to_postgres(
            host="myhost",
            dbname="mydatabase",
            user="myuser",
            password="mypassword",
    ...
    def connect_to_oracle(
        ...
        **kwargs
        ...
        """
        Connect to an Oracle db using oracledb package. This is just a helper function to set [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql]
        **Example:**
        ```python
        t2s.connect_to_oracle(
        user="username",
        password="password",
        dsn="host:port/sid",
        )
    ...
    def connect_to_bigquery(
        ...
        **kwargs
        ...
        """
        Connect to gcs using the bigquery connector. This is just a helper function to set [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql]
        **Example:**
        ```python
        t2s.connect_to_bigquery(
    ...
    def connect_to_duckdb(self, url: str, init_sql: str = None, **kwargs):
        """
        Connect to a DuckDB database. This is just a helper function to set [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql]
    ...
    def connect_to_mssql(self, odbc_conn_str: str, **kwargs):
        """
        Connect to a Microsoft SQL Server database. This is just a helper function to set [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql]
    ...
    def connect_to_hive(
      ...
      """
        Connect to a Hive database. This is just a helper function to set [`t2s.run_sql`][text2sql.base.base.Text2SqlBase.run_sql]
    ...
    def run_sql(self, sql: str, **kwargs) -> pd.DataFrame:
        ...
        Example:
        ```python
        t2s.run_sql("SELECT * FROM my_table")
        ```
        ...
        """
        raise Exception(
            "You need to connect to a database first by running t2s.connect_to_snowflake(), t2s.connect_to_postgres(), similar function, or manually set t2s.run_sql"
        )
    ...
    def ask(
        ...
        **Example:**
        ```python
        t2s.ask("What are the top 10 customers by sales?")
        ```

        Ask Text2Sql a question and get the SQL query that answers it.

        Args:
            question (str): The question to ask.
            print_results (bool): Whether to print the results of the SQL query.
            auto_train (bool): Whether to automatically train Text2Sql on the question and SQL query.
    ...
    def train(
        ...
        **Example:**
        ...
        Train Text2Sql on a question and its corresponding SQL query.
        ...
        If you call it with the sql argument, it's equivalent to [`t2s.add_question_sql()`][text2sql.base.base.Text2SqlBase.add_question_sql].
        If you call it with the ddl argument, it's equivalent to [`t2s.add_ddl()`][text2sql.base.base.Text2SqlBase.add_ddl].
        If you call it with the documentation argument, it's equivalent to [`t2s.add_documentation()`][text2sql.base.base.Text2SqlBase.add_documentation].
        Additionally, you can pass a [`TrainingPlan`][text2sql.types.TrainingPlan] object. Get a training plan with [`t2s.get_training_plan_generic()`][text2sql.base.base.Text2SqlBase.get_training_plan_generic].
