...
class Milvus_VectorStore(Text2SqlBase):
    """
    ...
    def _create_collections(self):
        self._create_sql_collection("text2sql-sql")
        self._create_ddl_collection("text2sql-ddl")
        self._create_doc_collection("text2sql-doc")
    ...
    def _create_sql_collection(self, name: str):
        if not self.milvus_client.has_collection(collection_name=name):
            text2sql_schema = MilvusClient.create_schema(
                auto_id=False,
                enable_dynamic_field=False,
            )
            text2sql_schema.add_field(field_name="id", datatype=DataType.VARCHAR, max_length=65535, is_primary=True)
            text2sql_schema.add_field(field_name="text", datatype=DataType.VARCHAR, max_length=65535)
            text2sql_schema.add_field(field_name="sql", datatype=DataType.VARCHAR, max_length=65535)
            text2sql_schema.add_field(field_name="vector", datatype=DataType.FLOAT_VECTOR, dim=self._embedding_dim)

            text2sql_index_params = self.milvus_client.prepare_index_params()
            text2sql_index_params.add_index(
                field_name="vector",
                index_name="vector",
            ...
            self.milvus_client.create_collection(
                ...
                index_params=text2sql_index_params,
                consistency_level="Strong"
            )

    def _create_ddl_collection(self, name: str):
        if not self.milvus_client.has_collection(collection_name=name):
            text2sql_ddl_schema = MilvusClient.create_schema(
                auto_id=False,
                enable_dynamic_field=False,
            )
            text2sql_ddl_schema.add_field(field_name="id", datatype=DataType.VARCHAR, max_length=65535, is_primary=True)
            text2sql_ddl_schema.add_field(field_name="ddl", datatype=DataType.VARCHAR, max_length=65535)
            text2sql_ddl_schema.add_field(field_name="vector", datatype=DataType.FLOAT_VECTOR, dim=self._embedding_dim)

            text2sql_ddl_index_params = self.milvus_client.prepare_index_params()
            text2sql_ddl_index_params.add_index(
                field_name="vector",
                index_name="vector",
                index_type="AUTOINDEX",
                metric_type="L2",
            ...
            self.milvus_client.create_collection(
                collection_name=name,
                schema=text2sql_ddl_schema,
                index_params=text2sql_ddl_index_params,
                consistency_level="Strong"
            )

    def _create_doc_collection(self, name: str):
        if not self.milvus_client.has_collection(collection_name=name):
            text2sql_doc_schema = MilvusClient.create_schema(
                auto_id=False,
                enable_dynamic_field=False,
            )
            text2sql_doc_schema.add_field(field_name="id", datatype=DataType.VARCHAR, max_length=65535, is_primary=True)
            text2sql_doc_schema.add_field(field_name="doc", datatype=DataType.VARCHAR, max_length=65535)
            text2sql_doc_schema.add_field(field_name="vector", datatype=DataType.FLOAT_VECTOR, dim=self._embedding_dim)

            text2sql_doc_index_params = self.milvus_client.prepare_index_params()
            text2sql_doc_index_params.add_index(
                field_name="vector",
                index_name="vector",
                index_type="AUTOINDEX",
                metric_type="L2",
            ...
            self.milvus_client.create_collection(
                collection_name=name,
                schema=text2sql_doc_schema,
                index_params=text2sql_doc_index_params,
                consistency_level="Strong"
            )
    ...
    def remove_training_data(self, id: str, **kwargs) -> bool:
        if id.endswith("-sql"):
            self.milvus_client.delete(collection_name="text2sql_sql", ids=[id])
            return True
        elif id.endswith("-ddl"):
            self.milvus_client.delete(collection_name="text2sql_ddl", ids=[id])
            return True
        elif id.endswith("-doc"):
            self.milvus_client.delete(collection_name="text2sql_doc", ids=[id])
            return True
        else:
            return False
