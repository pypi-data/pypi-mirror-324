...
def extract_numeric_fields_from_raw(ar_front_data, third_part_text, name_extracted):
    ...
    try:
        ...
        So you will get occupation in arabic and translate it into english as well and send it as part of your response. Th results should always be a dictionary with only 3 keys as mentioned above and nothing else.  Here's the text for your task: {ar_front_data}"""
        ...
        if front_data.get('occupation_en', ''):
            if front_data['occupation_en'].lower() in ['not available', 'unspecified', 'not specified', 'not provided'] or front_data['occupation_en'].lower().startswith('director of nationality'):
                front_data['occupation'], front_data['occupation_en'] = '', ''

        if response.get('nationality', ''):
            front_data['nationality'] = response['nationality']
