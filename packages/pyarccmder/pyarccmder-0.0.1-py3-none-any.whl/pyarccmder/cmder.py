...
def helpHandler(
    ...
    if config:
        ...
        for confData in config:
            print(f"- la commande \"{confData['name']}\" (-{confData['shortname']} ou --{confData['name']}): {confData['description']}")
            if (
                type(confData) == dict and
                'demo' in tuple(confData.keys()) and
                confData['demo']
            ):
                for ex in confData['demo']:
                    print(f"\t ex: {ex}")
    return value
...
def getConfigSchema(lang='fr'):
    ...
    return JON.Object(lang).struct({
        ...
        'description': JON.String(lang).required().min(1),
...
def arcCmder(
    ...
    '''
    try:
        helpConfig = getConfigSchema(lang).sanitize(helpConfig) or {
            'name': "help",
            'shortname': "h",
            'description': "Permet d'afficher toutes les commandes disponibles",
            'demo': [
                f"{name} -h",
                f"{name} --help",
            ],
        ...
        unknownConfig = {
            'name': "unknown",
            'shortname': "unk",
            'description': "tous les arguments non répertoriés",
            'hasValue': False,
