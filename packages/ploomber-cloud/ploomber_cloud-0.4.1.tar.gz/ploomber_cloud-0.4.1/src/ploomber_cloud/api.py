...
class PloomberCloudClient(client.PloomberBaseClient):
    ...
    def _is_eks_deployment(self, id):
        response = self.get_job_by_id(id)
        return "deploy-to-eks" in response.get("labels", [])

    def get_job_logs_by_id(self, id):
        """Return docker logs and webservice logs by job id"""

        logs_endpoint = f"{API_ROOT}/jobs/{id}/logs"
        if self._is_eks_deployment(id):
    ...
    def start_job(self, id):
        """Start a job"""
        start_endpoint = f"{API_ROOT}/jobs/{id}/service_start"
        if self._is_eks_deployment(id):
    ...
    def stop_job(self, id):
        """Stop a job"""
        stop_endpoint = f"{API_ROOT}/jobs/{id}/service_stop"
        if self._is_eks_deployment(id):
