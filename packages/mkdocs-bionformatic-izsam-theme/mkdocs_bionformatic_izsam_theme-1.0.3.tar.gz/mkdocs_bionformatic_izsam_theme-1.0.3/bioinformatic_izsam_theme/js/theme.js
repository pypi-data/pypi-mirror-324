...
wiki.theme.onScroll = function() {
  ...
  if (wiki.theme.tocInViewport) {
    ...
    if (activeLink) {
      let ancestor = activeLink.closest('.has-children');
      let parent = activeLink.parentElement;
      if (parent.classList.contains('has-children') && !parent.classList.contains('show-nav-child')) {
        wiki.theme.toggleTocNavChild({ target: activeLink });
      }
