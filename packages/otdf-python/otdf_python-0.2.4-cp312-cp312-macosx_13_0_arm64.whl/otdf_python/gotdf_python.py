...
# ---- Functions ---
def DecryptFilesGlobNPE(pattern, config):
	"""DecryptFilesGlobNPE(str pattern, object config) []str, str
	...
	"""
	return go.Slice_string(handle=_gotdf_python.gotdf_python_DecryptFilesGlobNPE(pattern, config.handle))
def EncryptFilePE(inputFilePath, outputFilePath, config, token, dataAttributes):
	"""EncryptFilePE(str inputFilePath, str outputFilePath, object config, object token, []str dataAttributes) str, str
	
	Encrypts a file as a PE (Person Entity), returning a TDF manifest and the cipher text.
	"""
	return _gotdf_python.gotdf_python_EncryptFilePE(inputFilePath, outputFilePath, config.handle, token.handle, dataAttributes.handle)
def EncryptString(inputText, config, dataAttributes):
	"""EncryptString(str inputText, object config, []str dataAttributes) str, str"""
	return _gotdf_python.gotdf_python_EncryptString(inputText, config.handle, dataAttributes.handle)
...
def EncryptFile(inputFilePath, outputFilePath, config, dataAttributes):
	"""EncryptFile(str inputFilePath, str outputFilePath, object config, []str dataAttributes) str, str"""
	return _gotdf_python.gotdf_python_EncryptFile(inputFilePath, outputFilePath, config.handle, dataAttributes.handle)
def EncryptFilesGlobNPE(pattern, config, dataAttributes):
	"""EncryptFilesGlobNPE(str pattern, object config, []str dataAttributes) []str, str
	...
	"""
	return go.Slice_string(handle=_gotdf_python.gotdf_python_EncryptFilesGlobNPE(pattern, config.handle, dataAttributes.handle))
def EncryptFilesInDirNPE(dirPath, config, dataAttributes):
	"""EncryptFilesInDirNPE(str dirPath, object config, []str dataAttributes) []str, str
	...
	in the same directory as the input files, with a .tdf extension added to the file name.
	"""
	return go.Slice_string(handle=_gotdf_python.gotdf_python_EncryptFilesInDirNPE(dirPath, config.handle, dataAttributes.handle))
def Hello():
	"""Hello() str
...
def DecryptFilesInDirNPE(dirPath, config):
	"""DecryptFilesInDirNPE(str dirPath, object config) []str, str
	
	DecryptFilesInDirNPE decrypts all files in the specified directory
	Work is performed as an NPE (Non-Person Entity). Decrypted files are placed
	in the same directory as the input files, with the .tdf extension removed from the file name.
	"""
	return go.Slice_string(handle=_gotdf_python.gotdf_python_DecryptFilesInDirNPE(dirPath, config.handle))
