...
class AppWrapper:
    ...
    def download_from_s3(cls, bucket: str, key: str, download_path: str):
        if AppWrapper.LOCAL_MODE:
            ...
            logger.info(f"Starting download for '{encoded_key}'")
            ...
            if res.status_code != 200:
                error_log = f"Failed: File download. \
                                    ...
                                    key: {encoded_key} presigned url: {presigned_get_url}"
            ...
            logger.info(f"Finished download for '{encoded_key}'")
    ...
    def download_file_from_s3(self):
        ...
        for _, s3_path in enumerate(self.app_input_files):
            ...
            if res.status_code != 200:
                error_log = f"Failed: File download. \
                                    status code: {res.status_code} detail: {res.reason} \
                                    file_key: {encoded_file_key} presigned url: {presigned_get_url}"
                self.update_status(Status.Failed.value, error_log)

            logger.info(f"Starting download for '{encoded_file_key}'")

            file_path = os.path.join(self.file_folder, encoded_file_key)
            ...
            with open(file_path, "wb") as f:
                f.write(res.content)
            logger.info(f"Finished download for '{encoded_file_key}'")
    ...
    def upload_file_to_s3(self, result):
        ...
        if res.status_code != 200:
            error_log = f"Failed: File upload. \
                            status code: {res.status_code} detail: {res.reason} \
                            file_key: {encoded_file_name} presigned url: {presigned_put_url}"
            self.update_status(Status.Failed.value, error_log)
