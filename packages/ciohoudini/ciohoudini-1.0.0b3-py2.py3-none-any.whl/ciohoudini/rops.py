...
"""
task_templates_dict = {
  "hython": "{hserver}hython {render_script} -f {first} {last} {step} -d {render_rop} {render_scene}", #Hython Command
...
def save_usd(node, driver_path):
    ...
    try:
        usd_node = hou.node(driver_path)
        usd_export_path = get_usd_path(node)
        if usd_node:
...
def get_solaris_image_output(driver_path):
    """
    Retrieves the image output for the Solaris render ROP node, checking both the USD stage and Houdini nodes.
    ...
    try:
        ...
        if driver_node:
            ...
            if not image_output:
                ...
                # If not found in USD, fallback to RenderProduct Houdini node
                if not image_output:
                    image_output = get_parameter_value(driver_node, "outputimage", string_value=True)
                    if not image_output:
                        render_product_node = find_ancestor_render_product(driver_node)
