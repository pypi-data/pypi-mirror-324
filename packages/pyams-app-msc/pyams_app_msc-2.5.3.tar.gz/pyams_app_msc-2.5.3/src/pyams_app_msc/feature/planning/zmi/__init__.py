...
from pyams_app_msc.feature.planning.zmi.interfaces import IActivitySessionsTable, IPlanningMenu
...
from pyams_zmi.table import I18nColumnMixin, IconColumn, Table, TableAdminView, TableElementEditor, get_table_id
...
@pagelet_config(name='activity-sessions.html',
                context=IWfPlanningTarget, layer=IPyAMSLayer,
                permission=VIEW_PLANNING_PERMISSION)
class ActivitySessionsView(TableAdminView):
    """Activity sessions view"""

    table_label = _("Activity planned sessions")
    table_class = IActivitySessionsTable


@factory_config(IActivitySessionsTable)
@implementer(IObjectData, IView)
class ActivitySessionsTable(Table):
...
@adapter_config(required=(IPlanningTarget, IAdminLayer, IActivitySessionsTable),
                provides=IValues)
@adapter_config(required=(IWfPlanningTarget, IAdminLayer, IActivitySessionsTable),
                provides=IValues)
class ActivitySessionsTableValues(ContextRequestViewAdapter):
    """Activity sessions table values adapter"""

    @property
    def values(self):
        yield from IPlanning(self.context).values()
