...
def nnedi3_resample_kernel(
    ...
    if hResample:
        ...
        last = nnedi3_resample_kernel_vertical(
            last,
            target_width,
            src_left,
            src_width,
...
def nnedi3_resample_kernel_vertical(
    ...
    # Scaling with nnedi3
    last = nnedi3_rpow2_vertical(
        input, eTimes, 1, nsize, nns, qual, etype, pscrn, opt, int16_prescreener, int16_predictor, exp, mode, device
    )
    ...
    if h != last.height or sy != 0 or sh != last.height:
        if h < last.height and invks is True:
            last = core.fmtc.resample(
                last, w, h, sx, sy, sw, sh, kernel=kernel, taps=taps, a1=a1, a2=a2, invks=True, invkstaps=invkstaps
            )
        else:
...
def nnedi3_rpow2_vertical(
    ...
    if eTimes >= 1:
        last = nnedi3_dh(
            input, field, nsize, nns, qual, etype, pscrn, opt, int16_prescreener, int16_predictor, exp, mode, device
        )
        eTimes = eTimes - 1
        field = 0
    else:
...
def nnedi3_dh(
    ...
    if not (has_znedi3 or has_nnedi3 or has_nnedi3cl):
        raise ValueError("nnedi3_dh: znedi3, nnedi3 or nnedi3cl installation not found.")
...
def GammaToLinear(
    src, fulls=True, fulld=True, curve="709", planes=[0, 1, 2], gcor=1.0, sigmoid=False, thr=0.5, cont=6.5
):
...
def SigmoidInverse(src, thr=0.5, cont=6.5, planes=[0, 1, 2]):
    ...
    def get_lut(x):
        ...
        return min(
            max(
                round(
                    (thr - math.log(max(1 / max(x / 65536 * (x1 - x0) + x0, 0.000001) - 1, 0.000001)) / cont) * 65536
                ),
                0,
            ),
            65535,
