...
def mvfrc(
    ...
    bofp = {"thscd1": it, "thscd2": int(scp * 255 / 100), "blend": blend, "num": num, "den": den}
    ############
...
def textsub(input, file, charset=None, fps=None, vfr=None, mod=False, Matrix=None):
    """
    ---------------------------
    textsub
    ---------------------------
...
def NonlinUSM(src, z=6.0, pow=1.6, str=1.0, rad=9, ldmp=0.001):
    ...
    g = core.resize.Bicubic(
        clip, m4(width / rad), m4(height / rad), filter_param_a=1 / 3, filter_param_b=1 / 3
    ).resize.Bicubic(width, height, filter_param_a=1, filter_param_b=0)
    expr = "x x y - abs {bs} / {z} / 1 {pow} / pow {z} * {str} * x y - {bs} / dup * * x y - {bs} / dup * {ldmp} + / x y - * {bs} / x y - abs {bs} / 0.001 + / {bs} * +".format(
        bs=2 ** (bit - 8), z=z, pow=pow, str=str, ldmp=ldmp
    )
...
def SAdeband(src, thr=128, f3kdb_arg={}, smoothmask=0, Sdbclip=None, Smask=None, tvrange=True):
    ...
    expr = "x {thr} > 0 65535 x {low} - 65535 * {thr} {low} - / - ?".format(
        thr=scale(thr), low=scale(16) if tvrange else 0
    )
...
def FIFP(
    ...
    if mode == 0:
        deinterlace = (
            core.nnedi3cl.NNEDI3CL(src, field=order, device=device) if opencl else core.znedi3.nnedi3(src, field=order)
        )
        ...
        if _pass == 1:
            ...
            # deinterlace
            deinterlace = (
                core.nnedi3cl.NNEDI3CL(src, field=order + 2, device=device)
                if opencl
                else core.znedi3.nnedi3(src, field=order + 2)
            )
...
def Overlaymod(clipa, clipb, x=0, y=0, alpha=None, aa=False):
    ...
    if clipa.format.subsampling_w > 0 or clipa.format.subsampling_h > 0:
        ...
        clipa = core.resize.Point(
            clipa,
            format=core.register_format(
                clipa.format.color_family, clipa.format.sample_type, clipa.format.bits_per_sample, 0, 0
            ).id,
        )
    else:
        clipa_src = None
...
def InterFrame(
    ...
    if InputType == "SBS":
        FirstEye = InterFrameProcess(
            core.std.CropRel(Input, right=Input.width // 2), core.std.CropRel(oInput, right=Input.width // 2)
        )
        SecondEye = InterFrameProcess(
            core.std.CropRel(Input, left=Input.width // 2), core.std.CropRel(oInput, left=Input.width // 2)
        )
        return core.std.StackHorizontal([FirstEye, SecondEye])
    elif InputType == "OU":
        FirstEye = InterFrameProcess(
            core.std.CropRel(Input, bottom=Input.height // 2), core.std.CropRel(oInput, bottom=Input.height // 2)
        )
        SecondEye = InterFrameProcess(
            core.std.CropRel(Input, top=Input.height // 2), core.std.CropRel(oInput, top=Input.height // 2)
        )
        return core.std.StackVertical([FirstEye, SecondEye])
    elif InputType == "HSBS":
        FirstEye = InterFrameProcess(
            core.std.CropRel(Input, right=Input.width // 2).resize.Spline36(Input.width, Input.height),
            core.std.CropRel(oInput, right=oInput.width // 2).resize.Spline36(oInput.width, oInput.height),
        )
        SecondEye = InterFrameProcess(
            core.std.CropRel(Input, left=Input.width // 2).resize.Spline36(Input.width, Input.height),
            core.std.CropRel(oInput, left=oInput.width // 2).resize.Spline36(oInput.width, oInput.height),
        )
        return core.std.StackHorizontal(
            [
                core.resize.Spline36(FirstEye, Input.width // 2, Input.height),
                core.resize.Spline36(SecondEye, Input.width // 2, Input.height),
            ]
        )
    elif InputType == "HOU":
        FirstEye = InterFrameProcess(
            core.std.CropRel(Input, bottom=Input.height // 2).resize.Spline36(Input.width, Input.height),
            core.std.CropRel(oInput, bottom=oInput.height // 2).resize.Spline36(oInput.width, oInput.height),
        )
        SecondEye = InterFrameProcess(
            core.std.CropRel(Input, top=Input.height // 2).resize.Spline36(Input.width, Input.height),
            core.std.CropRel(oInput, top=oInput.height // 2).resize.Spline36(oInput.width, oInput.height),
        )
        return core.std.StackVertical(
            [
                core.resize.Spline36(FirstEye, Input.width, Input.height // 2),
...
def statsinfo2csv(clip, plane=None, Max=True, Min=True, Avg=False, bits=8, namebase=None):
    ...
    def info(clip, t, p):
        ...
        last = core.std.FrameEval(
            clip,
            functools.partial(write, clip=clip, core=core, Max=Max, Min=Min, Avg=Avg, bits=bits),
            prop_src=statsclip,
        )
        return last
...
def props2csv(
    clip: vs.VideoNode, props: list, titles: list, output="info.csv", sep="\t", charset="utf-8", tostring=None
):
    """
...
def csv2props(
    ...
    if props is None:
        ...
        tmp = []
        for prop in props:
            tp = [None, prop + [None, None, None], prop + [None, None], prop + [None], prop][len(prop)]
...
def lbdeband(clip: vs.VideoNode, dbit=6):
    ...
    deband = (
        mvf.LimitFilter(down, luma, thr=0.2, elast=8.0)
        .f3kdb.Deband(31, 64, 0, 0, 0, 0, output_depth=16)
        .f3kdb.Deband(15, 64, 0, 0, 0, 0, output_depth=16)
    )
    if isGary:
        return deband
    else:
...
def ssharp(clip, chroma=True, mask=False, compare=False):
    ...
    if chroma and not isGRAY:
        ...
        if mask:
            mask1 = (
                src.tcanny.TCanny(sigma=0.5, t_h=20.0, t_l=8.0, mode=1).std.Expr("x 30000 < 0 x ?").rgvs.RemoveGrain(4)
            )
        ...
        if mask:
            mask1 = (
                srcy.tcanny.TCanny(sigma=0.5, t_h=20.0, t_l=8.0, mode=1).std.Expr("x 30000 < 0 x ?").rgvs.RemoveGrain(4)
            )
            mask1 = inpand(expand(mask1, cycle=1), cycle=1)
    ...
    if sfilter in ["ffms2", "ffms", "ff", "f", "ffvideosource"]:
        ...
        if cache:
            ...
            for i in range(mpls["count"]):
                clips.append(
                    core.ffms2.Source(
                        source=mpls["clip"][i], cachefile=os.path.join(cache, mpls["filename"][i].decode() + ".ffindex")
                    )
                )
        else:
...
def mwaa(
    ...
    def aa_kernel_vertical(clip):
        ...
        return core.std.ShufflePlanes(
            [clip_y if aa_y else clip_blk, clip_u if aa_c else clip_blk, clip_v if aa_c else clip_blk],
            [0, 0, 0] if aa_c else [0, 1, 2],
            vs.YUV,
        )

    def aa_resample_vercial(clip, height, chroma_shift=0):
        return clip.fmtc.resample(
            h=height,
            sx=0,
            sy=[-0.5, -0.5 * (1 << clip.format.subsampling_h) - chroma_shift * 2],
            kernel=["spline36", "bicubic"],
            a1=0,
            a2=0.5,
            planes=[3 if aa_y else 1, 3 if aa_c else 1, 3 if aa_c else 1],
        )
...
def mwcfix(
    ...
    if restore > 0:
        rst_u = pp_u.knlm.KNLMeansCL(
            d=0, a=a, s=1, h=restore, rclip=clip_y, device_type=device_type, device_id=device_id
        ).rgvs.Repair(pp_u, 13)
        rst_v = pp_v.knlm.KNLMeansCL(
            d=0, a=a, s=1, h=restore, rclip=clip_y, device_type=device_type, device_id=device_id
        ).rgvs.Repair(pp_v, 13)
        low_u = rst_u
        low_v = rst_v
...
def mwlmask(clip, l1=80, h1=96, h2=None, l2=None):
    ...
    if h2 >= white:
        ...
        expr = "x {h2} <= {white} x {l2} < x {l2} - {slope2} * {black} ? ?".format(
            black=black, white=white, h2=h2, l2=l2, slope2=white / (h2 - l2)
        )
...
def mwdbmask(
    ...
    if chroma:
        ...
        if yuv420:
            ...
            emaskC = emaskC.fmtc.resample(
                sw, sh, 0.25 - cs_h / 2, 0 - cs_v / 2, kernel="bilinear", fulls=True
            ).fmtc.bitdepth(bits=bits)
        else:
...
def mwenhance(diffClip, chroma=False, Strength=2.0, Szrp8=8, Spwr=4, SdmpLo=4, SdmpHi=48, Soft=0, useExpr=False):
    ...
    if useExpr:
        ...
        res1 = " {absDiff} {Szrp} / {miSpwr} pow {SzrpMulStrength} * {signMul} * ".format(
            absDiff=absDiff, Szrp=Szrp, miSpwr=miSpwr, SzrpMulStrength=SzrpMulStrength, signMul=signMul
        )
        res2 = " {diff8Sqr} {Szrp8SqrPlusSdmpLo} * {diff8Sqr} {SdmpLo} + {Szrp8Sqr} * / ".format(
            diff8Sqr=diff8Sqr, Szrp8SqrPlusSdmpLo=Szrp8SqrPlusSdmpLo, SdmpLo=SdmpLo, Szrp8Sqr=Szrp8Sqr
        )
        res3 = " 0 " if SdmpHiEqual0 else " {absDiff8} {SdmpHi} / 4 pow ".format(absDiff8=absDiff8, SdmpHi=SdmpHi)

        enhanced = " {res1} {res2} * {Szrp8DivSdmpHiPower4Plus1} * 1 {res3} + /".format(
            res1=res1, res2=res2, res3=res3, Szrp8DivSdmpHiPower4Plus1=Szrp8DivSdmpHiPower4Plus1
        )
        enhanced = " {ceil} {floor} {neutral} {enhanced} + max min ".format(
            ceil=ceil, floor=floor, neutral=neutral, enhanced=enhanced
        )
...
def rescale(
    ...
    w: int = None,
    h: int = None,
    mask: Union[bool, vs.VideoNode] = True,
    ...
    **args,
    ...
    luma_rescale, mask, luma_de = MRcore(
        luma,
        kernel[2:],
        w,
        h,
        mask=mask,
        mask_dif_pix=mask_dif_pix,
        postfilter_descaled=postfilter_descaled,
        mthr=mthr,
        taps=taps,
        b=b,
        c=c,
...
def rescalef(
    ...
    **args,
    ...
    luma_rescale, mask, luma_de = MRcoref(
        luma,
        kernel[2:],
        w,
        h,
...
def ivtc_t(
    ...
    match = core.tivtc.TFM(
        src,
        order=order,
        field=field,
        mode=mode,
        PP=pp if isinstance(pp, int) and pp > 0 else 0,
        slow=slow,
        mChroma=mchroma,
        cthresh=cthresh,
        MI=mi,
...
def getCSS(w, h):
    css = {(1, 1): "420", (1, 0): "422", (0, 0): "444", (2, 2): "410", (2, 0): "411", (0, 1): "440"}
    sub = (w, h)
...
def nnedi3(
    ...
    mode = mode.lower()
    if mode == "nnedi3":
        return clip.nnedi3.nnedi3(
            field=field, dh=dh, planes=planes, nsize=nsize, nns=nns, qual=qual, etype=etype, pscrn=pscrn, exp=exp
        )
    elif mode == "znedi3":
        return clip.znedi3.nnedi3(
            field=field, dh=dh, planes=planes, nsize=nsize, nns=nns, qual=qual, etype=etype, pscrn=pscrn, exp=exp
        )
    elif mode == "nnedi3cl":
        return clip.nnedi3cl.NNEDI3CL(
            field=field,
            dh=dh,
            dw=dw,
            planes=planes,
...
def MRcore(
    ...
    **args,
    ...
    rescale = nnedi3_resample(
        descaled, src_w, src_h, nsize=nsize, nns=nns, qual=qual, etype=etype, pscrn=pscrn, exp=exp, sigmoid=sigmoid
    ).fmtc.bitdepth(bits=16)

    if mask is True:
        mask = core.std.Expr([clip, upscaled.fmtc.bitdepth(bits=16, dmode=1)], "x y - abs").std.Binarize(
            mask_dif_pix * 256
        )
        if callable(maskpp):
            mask = maskpp(mask)
    ...
    if show.lower() == "result":
        ...
        return (
            core.std.ModifyFrame(rescale, [diff, rescale], calc),
            core.std.ModifyFrame(mask, [diff, mask], calc),
            descaled,
        )
...
class cropping_args:
    ...
    def descale_gen(self):
        ...
        argsw = {
            ...
            "src_left": (self.cropped_width - self.src_width) / 2,
        }
        argsh = {
            "height": self.cropped_height,
            "src_height": self.src_height,
            "src_top": (self.cropped_height - self.src_height) / 2,
        }
