...
def ToRGB(
    ...
    **kwargs: int | float | None,
    ...
    if sIsRGB:
        ...
        if matrix == "OPP":
            clip = core.fmtc.matrix(
                clip, fulls=fulls, fulld=fulld, coef=[1, 1, 2 / 3, 0, 1, 0, -4 / 3, 0, 1, -1, 2 / 3, 0], col_fam=vs.RGB
            )
            clip = SetColorSpace(clip, Matrix=0)
        elif matrix == "2020cl":
...
def ToYUV(
    ...
    **kwargs: int | float,
    ...
    if sIsYUV:
        ...
        UV = core.std.BlankClip(
            clip, width=widthc, height=heightc, color=1 << (dbitPS - 1) if dSType == vs.INTEGER else 0
        )
...
def BM3D(
    ...
    radius1: int | None = None,
    radius2: int | None = None,
    ...
    cu_a2: float | None = None,
    cu_cplace: str | None = None,
    cd_kernel: str | None = None,
    cd_taps: int | None = None,
    cd_a1: float | None = None,
    cd_a2: float | None = None,
    cd_cplace: str | None = None,
    block_size1: int | None = None,
    block_step1: int | None = None,
    group_size1: int | None = None,
    bm_range1: int | None = None,
    bm_step1: int | None = None,
    ps_num1: int | None = None,
    ps_range1: int | None = None,
    ps_step1: int | None = None,
    th_mse1: float | None = None,
    hard_thr: float | None = None,
    block_size2: int | None = None,
    block_step2: int | None = None,
    group_size2: int | None = None,
    bm_range2: int | None = None,
    bm_step2: int | None = None,
    ps_num2: int | None = None,
    ps_range2: int | None = None,
    ps_step2: int | None = None,
    th_mse2: float | None = None,
    **kwargs: int | float,
) -> vs.VideoNode:
    # input clip
    clip = input
    ...
    if sIsGRAY:
        ...
        clip = ToRGB(clip, matrix, pbitPS, pSType, fulls, cu_kernel, cu_taps, cu_a1, cu_a2, cu_cplace, **kwargs)
        if pre is not None:
            pre = ToRGB(pre, matrix, pbitPS, pSType, fulls, cu_kernel, cu_taps, cu_a1, cu_a2, cu_cplace, **kwargs)
        if ref is not None:
            ref = ToRGB(ref, matrix, pbitPS, pSType, fulls, cu_kernel, cu_taps, cu_a1, cu_a2, cu_cplace, **kwargs)
        # Convert full range RGB to full range OPP
        clip = ToYUV(clip, "OPP", "444", pbitPS, pSType, True, cu_kernel, cu_taps, cu_a1, cu_a2, cu_cplace, **kwargs)
        if pre is not None:
            pre = ToYUV(pre, "OPP", "444", pbitPS, pSType, True, cu_kernel, cu_taps, cu_a1, cu_a2, cu_cplace, **kwargs)
        if ref is not None:
            ref = ToYUV(ref, "OPP", "444", pbitPS, pSType, True, cu_kernel, cu_taps, cu_a1, cu_a2, cu_cplace, **kwargs)
    ...
    if sIsGRAY:
        ...
        if onlyY:
            ...
            clip = core.std.ShufflePlanes(
                [flt, clip if sigma[1] <= 0 else flt, clip if sigma[2] <= 0 else flt], [0, 1, 2], vs.YUV
            )
        else:
            clip = flt
        ...
        if output <= 1:
            ...
            clip = ToRGB(clip, "OPP", pbitPS, pSType, True, cu_kernel, cu_taps, cu_a1, cu_a2, cu_cplace, **kwargs)
        if output <= 0 and not sIsRGB:
            # Convert full range RGB to YUV
            clip = ToYUV(
                clip, matrix, css, dbitPS, dSType, fulld, cd_kernel, cd_taps, cd_a1, cd_a2, cd_cplace, **kwargs
            )
        else:
...
def PlaneCompare(
    ...
    if rmse or psnr:
        ...
        def _PlaneRMSEnPSNRTransfer(n, f):
            ...
            if psnr:
                fout.props.PlanePSNR = (
                    min(10 * math.log(valueRange * valueRange / f[1].props.PlaneMSE, 10), 99.99999999999999)
                    if f[1].props.PlaneMSE > 0
                    else 100.0
                )
            return fout
    ...
    if cov or corr:
        ...
        if hasattr(core, "akarin"):
            CoDclip = core.akarin.Expr(
                [clip1Plane, clip1Mean, clip2Plane, clip2Mean],
                f"x y.PlaneMean {valueRange} * - z a.PlaneMean {valueRange} * - *",
                format=floatFormat.id,
            )
        else:
            ...
            CoDclip = core.std.FrameEval(
                floatBlk, functools.partial(_PlaneCoDFrame, clip1=clip1Plane, clip2=clip2Plane), [clip1Mean, clip2Mean]
            )
        ...
        if corr:
            if hasattr(core, "akarin"):
                SDclip1 = core.akarin.Expr(
                    [clip1Plane, clip1Mean], f"x y.PlaneMean {valueRange} * - dup *", format=floatFormat.id
                )
                SDclip2 = core.akarin.Expr(
                    [clip2Plane, clip2Mean], f"x y.PlaneMean {valueRange} * - dup *", format=floatFormat.id
                )
            else:
...
##         default: 7
################################################################################################################################
def ShowAverage(clip: vs.VideoNode, alignment: int | None = None) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
################################################################################################################################
def FilterIf(src: vs.VideoNode, flt: vs.VideoNode, prop_name: str, props: vs.VideoNode | None = None) -> vs.VideoNode:
    # input clip
...
##         default: None (use "src")
################################################################################################################################
def FilterCombed(src: vs.VideoNode, flt: vs.VideoNode, props: vs.VideoNode | None = None) -> vs.VideoNode:
...
################################################################################################################################
def Min(
    clip1: vs.VideoNode, clip2: vs.VideoNode, mode: list[int] | None = None, neutral: int | float | None = None
) -> vs.VideoNode:
...
##         default: [1,1,1] for YUV/RGB input, [1] for Gray input
################################################################################################################################
def Avg(clip1: vs.VideoNode, clip2: vs.VideoNode, mode: list[int] | None = None) -> vs.VideoNode:
...
################################################################################################################################
def MinFilter(
    src: vs.VideoNode, flt1: vs.VideoNode, flt2: vs.VideoNode, planes: Sequence[int] | None = None
) -> vs.VideoNode:
...
##         default: 0
################################################################################################################################
def PointPower(clip: vs.VideoNode, vpow: float | None = None, hpow: float | None = None) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
def CheckMatrix(
    ...
    def _FrameProps(n, f):
        fout = f.copy()
        fout.props.__setattr__(
            props[3],
            (f.props.__getattr__(props[0]) + f.props.__getattr__(props[1]) + f.props.__getattr__(props[2])) / 3,
        )
        return fout
...
################################################################################################################################
def postfix2infix(expr: str) -> str:
...
def SetColorSpace(
    clip: vs.VideoNode,
    ChromaLocation: int | None = None,
    ColorRange: bool | int | None = None,
...
################################################################################################################################
def AssumeFrame(clip: vs.VideoNode) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
################################################################################################################################
def AssumeTFF(clip: vs.VideoNode) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
## This frame property will override the field order set in those de-interlace filters.
################################################################################################################################
def AssumeBFF(clip: vs.VideoNode) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
################################################################################################################################
def AssumeField(clip: vs.VideoNode, top: bool) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
##         default: True
################################################################################################################################
def AssumeCombed(clip: vs.VideoNode, combed: bool = True) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
##         default: True
################################################################################################################################
def CheckVersion(version: int, less: bool = False, equal: bool = True, greater: bool = True) -> bool:
...
################################################################################################################################
def GetMatrix(
    clip: vs.VideoNode, matrix: int | str | None = None, dIsRGB: bool | None = None, id: bool = False
) -> int | str:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
def zDepth(
    ...
    if zimgResize:
        clip = core.resize.Bicubic(
            clip, format=format.id, range=range, range_in=range_in, dither_type=dither_type, cpu_type=cpu_type
        )
        ...
        clip = core.z.Depth(
            clip, dither=dither_type, sample=sample, depth=depth, fullrange_in=range_in, fullrange_out=range
        )
    else:
...
################################################################################################################################
def PlaneAverage(clip: vs.VideoNode, plane: int | None = None, prop: str | None = None) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
##         default: 0
################################################################################################################################
def GetPlane(clip: vs.VideoNode, plane: int | None = None) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
################################################################################################################################
def GrayScale(clip: vs.VideoNode, matrix: int | str | None = None) -> vs.VideoNode:
    # input clip
    if not isinstance(clip, vs.VideoNode):
        raise type_error('"clip" must be a clip!')
...
def Preview(
    ...
    def _Conv(clip):
        ...
        return core.resize.Bicubic(
            ...
            ref.height,
            format=dFormat,
            matrix_in=GetMatrix(clip, matrix, True, True),
            range_in=full,
            filter_param_a=0,
            filter_param_b=0.5,
            resample_filter_uv=kernel,
            filter_param_a_uv=a1,
            filter_param_b_uv=a2,
            dither_type=dither,
        )
...
def CheckColorFamily(
    color_family: vs.ColorFamily, valid_list: Iterable[str] | None = None, invalid_list: Iterable[str] | None = None
) -> None:
...
################################################################################################################################
def RemoveFrameProp(clip: vs.VideoNode, prop: str) -> vs.VideoNode:
...
################################################################################################################################
def get_func_name(num_of_call_stacks: int = 1) -> str:
    import inspect
...
def _quantization_parameters(
    sample: vs.SampleType | None = None, depth: int | None = None, full: bool | None = None, chroma: bool | None = None
) -> dict[str, int | float]:
    ...
    def gen_expr(chroma, mode):
        ...
        if gain != 1 or offset != 0 or clamp:
            ...
            if offset != 0:
                expr = expr + f" {offset} + "
            if clamp:
                if dQP["floor"] * scale > exprLower:
                    expr = expr + f" {dQP['floor'] * scale} max "
                if dQP["ceil"] * scale < exprUpper:
                    expr = expr + f" {dQP['ceil'] * scale} min "
        else:
            expr = ""
...
################################################################################################################################
def _check_arg_prop(
    arg: bool | str | None, default: str | None = None, defaultTrue: str | None = None, argName: str = "arg"
) -> bool | str | None:
...
################################################################################################################################
def _operator2(
    clip1: vs.VideoNode, clip2: vs.VideoNode, mode: list[int] | None, neutral: int | float | None, name: str
) -> vs.VideoNode:
    # input clip
    if not isinstance(clip1, vs.VideoNode):
...
################################################################################################################################
def _min_max_filter(
    src: vs.VideoNode, flt1: vs.VideoNode, flt2: vs.VideoNode, planes: Sequence[int] | None, name: str
) -> vs.VideoNode:
    # input clip
    if not isinstance(src, vs.VideoNode):
...
################################################################################################################################
def _limit_filter_expr(defref: bool, thr: float, elast: float, largen_thr: float, value_range: int) -> str:
    flt = " x "
    src = " y "
...
################################################################################################################################
def _limit_diff_lut(
    diff: vs.VideoNode, thr: float, elast: float, largen_thr: float, planes: Sequence[int]
) -> vs.VideoNode:
    # input clip
