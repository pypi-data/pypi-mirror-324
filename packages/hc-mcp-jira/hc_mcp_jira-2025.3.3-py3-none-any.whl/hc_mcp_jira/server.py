...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        ...
        types.Tool(
            name="create_issue",
            description="Create a new Jira issue with specified details. Returns a JSON response containing the created issue's key, ID, and self URL. Supports creating subtasks by providing a parent issue key.",
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "description": {"type": "string", "description": "Detailed description of the issue in plain text"},
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "status": {"type": "string", "description": "New status to transition the issue to (e.g., 'In Progress', 'Done')"},
...
async def handle_call_tool(
    ...
    try:
        if name == "get_current_user":
            # Get raw dictionary from JIRA response instead of using Resource object
            myself = jira_client.myself()
            user_info = {
                'accountId': myself.raw.get('accountId'),
                'displayName': myself.raw.get('displayName'),
                'emailAddress': myself.raw.get('emailAddress'),
                'active': myself.raw.get('active'),
                'self': myself.raw.get('self'),
                'name': myself.raw.get('name'),
                'key': myself.raw.get('key'),
                'timeZone': myself.raw.get('timeZone')
            }
