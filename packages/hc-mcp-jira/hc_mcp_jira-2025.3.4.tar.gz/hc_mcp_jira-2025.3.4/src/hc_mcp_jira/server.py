...
async def handle_call_tool(
    ...
    try:
        if name == "get_current_user":
            # Get current user info and print the full response
            myself = jira_client.myself()
            logger.warning(f"Full myself() response: {myself.__dict__}")
            
            # Try to get user info directly from the object
            user_info = {
            ...
            return [types.TextContent(type="text", text=json.dumps({
                "user_info": user_info,
                "raw_response": str(myself.__dict__)
            }, indent=2))]
