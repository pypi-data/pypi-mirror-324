...
from .logging_utils import format_log_level, get_logger
from .target_manager import TargetManager, CommandCtxt
from .command import DeployTarget
from .constants import AppLogLevel, THREAD_SLEEP_TIME_S, EMPTY_ERROR, LogMessage
...
def _parse_log_entry(line: str, date_now: datetime) -> LogMessage:
    ...
    return LogMessage(
        ...
        message=json.dumps({"message": line, "device_id": config.client_id}),
    )
...
class ProcTargetManager(TargetManager):
    ...
    def _log_data(self, proc: Popen):
        ...
        while self._listen and proc.poll() is None:
            ...
            try:
                ...
                if line:
                    ...
                    log_entry = _parse_log_entry(data, date_now)
        ...
        pipe.close()
        self._telemetry_manager.publish_logs(datetime.utcnow())
