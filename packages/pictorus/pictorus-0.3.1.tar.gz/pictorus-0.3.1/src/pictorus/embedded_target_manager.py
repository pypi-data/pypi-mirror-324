    ...
    def _start_rtt_logging(self, target: SoCTarget):
        ...
        try:
            ...
            if len(control_block.up_channels) > 0:
                up_chan = control_block.up_channels[LOG_CHANNEL_ID]
    ...
    def _log_data(self, up_chan: "RTTUpChannel"):
        ...
        while self._listen:
            try:
                ...
                if data:
                    ts_now = utc_timestamp_ms(date_now)
                    logs, remainder = _parse_all_log_entries(remainder + data, ts_now)
                    self._telemetry_manager.add_log_entries(logs)
