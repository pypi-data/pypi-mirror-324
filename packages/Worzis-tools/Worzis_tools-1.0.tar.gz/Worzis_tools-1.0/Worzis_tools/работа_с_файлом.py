...
from терминал4 import печать
from проверка_элементов import содержит

РАЗДЕЛИТЕЛЬ_ПАР = "___!!!_РАЗДЕЛИТЕЛЬ_ПАР___!!!___"
РАЗДЕЛИТЕЛЬ_КЛЮЧЕЙ_ОТ_ЗНАЧЕНИЙ = "___!!!___РАЗДЕЛИТЕЛЬ_КЛЮЧЕЙ_ОТ_ЗНАЧЕНИЙ___!!!___"
...
def преобразовать_словарь_в_текст(словарь: dict):
    ...
    if содержит([РАЗДЕЛИТЕЛЬ_КЛЮЧЕЙ_ОТ_ЗНАЧЕНИЙ, РАЗДЕЛИТЕЛЬ_ПАР], list(словарь.values())):
        raise ValueError(печать("В одном из значений словаря найден разделитель", красный=True, не_печатать=True))
    результат = ""
...
def считать_словари_из_файла(путь: str) -> list:
    ...
    for строка in текст.splitlines():
        ...
        словарь = {}
        for пара in пары:
            ключ, значение = пара.split(РАЗДЕЛИТЕЛЬ_КЛЮЧЕЙ_ОТ_ЗНАЧЕНИЙ)
...
if __name__ == "__main__":
    ...
    путь = "C:\\Users\\user\\Desktop\\python\\one\\подключаемый_модуль\\инструменты\\файл из питона.txt"
#    текст = "этот текст"
#
