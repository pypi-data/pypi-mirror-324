...
from quantfe.utils import global_vars, helpers, frequency, day_count, calendar, qmath
...
from quantfe.products.bonds.bond_zero_curve import BondZeroCurve
...
class Bond:
    ...
    def __init__(self, 
                ...
                bd_type=calendar.BusDayAdjustTypes.FOLLOWING,
        ...
        self.issue_dt = issue_date
        self.maturity_dt = maturity_date

        if coupon_rate == 0.0:
            raise FinError("Zero coupon bonds must use BondZero class.")

        if freq_type == frequency.FrequencyTypes.ZERO:
            raise FinError("Zero coupon bonds must use BondZero class.")

        if dc_type == dc_type.ZERO:
            raise FinError("Zero coupon bonds must use BondZero class.")

        self.cpn = coupon_rate
    ...
    def _calculate_payment_dts(self):
        ...
        bus_day_adj_type = calendar.BusDayAdjustTypes.FOLLOWING
        ...
        for cpn_dt in self.cpn_dts[1:]:
            pmt_dt = cal.adjust(cpn_dt, bus_day_adj_type)
    ...
    def dirty_price_from_ytm(self,
        ...
        ytm = ytm + 0.0000000000012  # SNEAKY LOW-COST TRICK TO AVOID y=0

        f = frequency.annual_frequency(self.freq_type)
        c = self.cpn
        ...
        #                print(term1, term2, term3, term4, v, self.alpha, dp)
            ...
            if n == 0:
                ...
                dc = day_count.DayCount(day_count.DayCountTypes.ACT_365L)

                alpha = (1 - dc.year_frac(last_year,
                                          settle_dt,
                                          self.maturity_dt,
                                          freq_type=frequency.FrequencyTypes.ANNUAL)[0])
    ...
    def key_rate_durations(self,
        ...
        lin_zero_interp = interp.InterpTypes.LINEAR_ZERO_RATES
        ...
        for ind, _ in enumerate(key_rate_tenors):
            ...
            par_crv = BondZeroCurve(settle_dt,
                                    par_bonds,
                                    clean_prices,
                                    lin_zero_interp)
            ...
            par_crv_down = BondZeroCurve(settle_dt,
                                         par_bonds,
                                         clean_prices,
                                         lin_zero_interp)
    ...
    def dirty_price_from_discount_curve(self,
        ...
        px = 0.0
        df = 1.0
        df_settle_dt = discount_curve.cal_df(settle_dt)

        dt = self.cpn_dts[1]
        if dt > settle_dt:
            df = discount_curve.cal_df(dt)
    ...
    def asset_swap_spread(
            ...
            clean_price: float,
            discount_curve: DiscountCurve,
            swap_float_day_count_convention_type=day_count.DayCountTypes.ACT_360,
            swap_float_frequency_type=frequency.FrequencyTypes.SEMI_ANNUAL,
            swap_float_calendar_type=calendar.CalendarTypes.WEEKEND,
            swap_float_bus_day_adjust_rule_type=calendar.BusDayAdjustTypes.FOLLOWING,
            swap_float_date_gen_rule_type=calendar.DateGenRuleTypes.BACKWARD):
        ...
        day_count = day_count.DayCount(swap_float_day_count_convention_type)
    ...
    def dirty_price_from_oas(self,
        ...
        for dt in self.cpn_dts[1:]:
            ...
            # coupons paid on a settlement date are paid to the seller
            if dt > settle_dt:
                t = (dt - settle_dt) / global_vars.gDaysInYear

                t = np.maximum(t, global_vars.g_small)
