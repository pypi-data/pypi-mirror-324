...
from quantfe.utils import global_vars, helpers, day_count, frequency, qmath, calendar, global_types
...
class IborIborXCcySwap:
    ...
    is done on a supplied discount discount (1 for each leg) which is separate
    ...
    def __init__(self,
                 ...
                 float_freq_type: frequency.FrequencyTypes = frequency.FrequencyTypes.QUARTERLY,
        ...
        self.maturity_dt = cal.adjust(self._termination_dt, bd_type)
    ...
    def value(self,
        ...
        if self._fixed_leg_type == global_types.SwapTypes.PAY:
            value = value * (-1.0)
    ...
    def fixed_leg_value(self, value_dt, discount_curve, principal=0.0):
        ...
        day_counter = day_count.DayCount(self._fixed_dc_type)
    ...
    def cash_settled_pv01(self,
        ...
        m = frequency.Frequency(frequencyType)
    ...
    def float_leg_value(self,
        ...
        basis = day_count.DayCount(self._float_dc_type)
        ...
        for next_dt in self._adjustedFloatDates[start_index + 1:]:
            ...
            self._float_dfs.append(df_discount)
            self.float_flow_pvs.append(flow * df_discount)
            self._floatTotalPV.append(pv)

        flow = principal * self.notional
        pv = pv + flow * df_discount
        self._float_flows[-1] += flow
        self.float_flow_pvs[-1] += flow * df_discount
    ...
    def print_fixed_leg_pv(self):
        ...
        print("COUPON (%):", self._fixed_cpn * 100)
    ...
    def __repr__(self):
        ...
        s += helpers.label_to_string("TERMINATION DATE", self._termination_dt)
        s += helpers.label_to_string("MATURITY DATE", self.maturity_dt)
        s += helpers.label_to_string("NOTIONAL", self.notional)
        s += helpers.label_to_string("SWAP TYPE", self._swap_type)
        s += helpers.label_to_string("FIXED COUPON", self._fixed_cpn)
        s += helpers.label_to_string("FLOAT SPREAD", self._float_spread)
        s += helpers.label_to_string("FIXED FREQUENCY", self._fixed_freq_type)
        s += helpers.label_to_string("FLOAT FREQUENCY", self._float_freq_type)
        s += helpers.label_to_string("FIXED DAY COUNT", self._fixed_dc_type)
        s += helpers.label_to_string("FLOAT DAY COUNT", self._float_dc_type)
        s += helpers.label_to_string("CALENDAR", self._cal_type)
        s += helpers.label_to_string("BUS DAY ADJUST", self._bd_type)
        s += helpers.label_to_string("DATE GEN TYPE", self._dg_type)
        return s
