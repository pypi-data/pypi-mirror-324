...
import quantfe.utils.day_count as day_count
...
def grid_index(t, grid_times):
    ...
    for i in range(0, n):
        ...
        if abs(grid_time - t) < global_vars.g_small:
...
def times_from_dates(dt: (Date, list),
                     value_dt: Date,
                     day_count_type: day_count.DayCountTypes = None):
    ...
    if day_count_type is None:
        ...
        dc = day_count.DayCount(day_count_type)

    if isinstance(dt, Date):
        num_dts = 1
        times = [None]
        if dc is None:
            times[0] = (dt - value_dt) / global_vars.gDaysInYear
        else:
            times[0] = dc.year_frac(value_dt, dt)[0]
        ...
        for i in range(0, num_dts):
            if dc is None:
                t = (dt[i] - value_dt) / global_vars.gDaysInYear
            else:
                t = dc.year_frac(value_dt, dt[i])[0]
            times.append(t)
...
def input_time(dt: Date, curve):
    ...
    if isinstance(dt, float):
        ...
        t = (dt - curve.value_dt) / global_vars.gDaysInYear
        return check(t)
    elif isinstance(dt, np.ndarray):
        t = dt
        if np.any(t) < 0:
...
def accrued_tree(grid_times: np.ndarray,
    ...
    for i_grid in range(1, numgrid_times):
        ...
        if grid_flows[i_grid] > global_vars.g_small:
