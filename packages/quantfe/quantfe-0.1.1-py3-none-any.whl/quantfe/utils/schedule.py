...
from quantfe.utils.calendar import BusDayAdjustTypes, DateGenRuleTypes, CalendarTypes

import quantfe.utils.helpers as helpers
import quantfe.utils.frequency as frequency
import quantfe.utils.calendar as calendar
...
class Schedule:
    ...
    def __init__(self,
                 ...
                 freq_type: frequency.FrequencyTypes = frequency.FrequencyTypes.ANNUAL,
        ...
        if first_dt is None:
            self.first_dt = effective_date
        else:
            if first_dt > effective_date and first_dt < termination_date:
                self.first_dt = first_dt
                print("FIRST DATE NOT IMPLEMENTED")  # TODO
            else:
                raise FinError("First date must be after effective date and before termination date")

        if next_to_last_dt is None:
            self.next_to_last_dt = termination_date
        else:
            if next_to_last_dt > effective_date and next_to_last_dt < termination_date:
                self.next_to_last_dt = next_to_last_dt
                print("NEXT TO LAST DATE NOT IMPLEMENTED")  # TODO
            else:
                raise FinError("Next to last date must be after effective date and before termination date")
    ...
    def generate(self):
        ...
        if freq == 365:
            ...
            while _date < self.termination_date:
                self.adjusted_dts.append(_date)
                _date = cal.add_business_days(_date, 1)
            self.adjusted_dts.append(self.termination_date)
        else:
            ...
            if self.dg_type == DateGenRuleTypes.BACKWARD:
                next_dt = self.termination_date
                flow_num = 0
                while next_dt > self.effective_date:
                    unadjusted_schedule_dts.append(next_dt)
                    tot_num_months = num_months * (1 + flow_num)
                    next_dt = self.termination_date.add_months(-tot_num_months)
                ...
                for i in range(1, flow_num - 1):
                    dt = cal.adjust(unadjusted_schedule_dts[flow_num - i - 1], self.bd_type)
                    self.adjusted_dts.append(dt)
                self.adjusted_dts.append(self.termination_date)
                ...
                # This needs checking
                next_dt = self.effective_date
                flow_num = 0

                unadjusted_schedule_dts.append(next_dt)
                flow_num = 1

                while next_dt < self.termination_date:
                    unadjusted_schedule_dts.append(next_dt)
                    tot_num_months = num_months * (flow_num)
                    next_dt = self.effective_date.add_months(tot_num_months)
                    flow_num = flow_num + 1

                # The effective date is not adjusted as it is given
                for i in range(1, flow_num):
                    dt = cal.adjust(unadjusted_schedule_dts[i], self.bd_type)
                    self.adjusted_dts.append(dt)

                self.adjusted_dts.append(self.termination_date)

            if self.adjusted_dts[0] < self.effective_date:
                self.adjusted_dts[0] = self.effective_date
            ...
            if self.adjust_termination_date is True:
                self.termination_date = cal.adjust(self.termination_date, self.bd_type)
                self.adjusted_dts[-1] = self.termination_date
    ...
    def __repr__(self):
        ...
        s += helpers.label_to_string("EFFECTIVE DATE", self.effective_date)
        s += helpers.label_to_string("END DATE", self.termination_date)
        s += helpers.label_to_string("FREQUENCY", self.freq_type)
        s += helpers.label_to_string("CALENDAR", self.cal_type)
        s += helpers.label_to_string("BUSDAYRULE", self.bd_type)
        s += helpers.label_to_string("DATEGENRULE", self.dg_type)
        s += helpers.label_to_string("ADJUST TERM DATE", self.adjust_termination_date)
        s += helpers.label_to_string("END OF MONTH", self.end_of_month, "")

        if 1 == 0:
            if len(self.adjusted_dts) > 0:
                s += "\n\n"
                s += helpers.label_to_string("EFF", self.adjusted_dts[0], "")

            if len(self.adjusted_dts) > 1:
                s += "\n"
                s += helpers.label_to_string("FLW", self.adjusted_dts[1:], "", list_format=True)
        return s
