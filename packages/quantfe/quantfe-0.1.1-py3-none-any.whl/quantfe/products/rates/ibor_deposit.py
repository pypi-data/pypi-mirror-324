...
from quantfe.utils import helpers, day_count, calendar
...
class IborDeposit:
    ...
    def __init__(
        ...
        dc_type: day_count.DayCountTypes,  # How year fraction is calculated
        notional: float = 100.0,  # Amount borrowed
        cal_type: calendar.CalendarTypes = calendar.CalendarTypes.WEEKEND,  # Maturity date
        bd_type: calendar.BusDayAdjustTypes = calendar.BusDayAdjustTypes.MODIFIED_FOLLOWING,
    ):
        ...
        maturity_dt = cal.adjust(maturity_dt, self.bd_type)
    ...
    def valuation_details(
        ...
        dc = day_count.DayCount(self.dc_type)
        acc_factor = dc.year_frac(self.start_dt, self.maturity_dt)[0]
        df_settle = discount_curve.cal_df(self.start_dt)
        df_maturity = discount_curve.cal_df(self.maturity_dt)
