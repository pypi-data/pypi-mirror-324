...
from quantfe.utils import global_vars
...
class Interpolator():
    def __init__(self, interpolator_type: InterpTypes):
        ...
        self.times = None
        self._zero_rates = None
        self._refit_curve = False

    ###########################################################################

    def fit(self, times: np.ndarray, zero_rates: np.ndarray):
        self.times = times
        ...
        if self._interp_type == InterpTypes.LINEAR_ZERO_RATES:
            self._interp_fn = si.interp1d(self.times, self._zero_rates, kind='linear', fill_value="extrapolate")

        # elif self._interp_type == InterpTypes.PCHIP_LOG_DISCOUNT:
        #     log_dfs = np.log(self._dfs)
        #     self._interp_fn = si.PchipInterpolator(self.times, log_dfs)

        # elif self._interp_type == InterpTypes.PCHIP_ZERO_RATES:

        #     g_small_vector = np.ones(len(self.times)) * global_vars.g_small
        #     zero_rates = -np.log(self._dfs) / (self.times + g_small_vector)

        #     if self.times[0] == 0.0:
        #         zero_rates[0] = zero_rates[1]

        #     self._interp_fn = si.PchipInterpolator(self.times, zero_rates)
        ...
        #     self._interp_fn = si.CubicSpline(self.times, zero_rates,
        #                                   bc_type=((2, 0.0), (1, 0.0)))
        ...
        #     log_dfs = np.log(self._dfs)
        #     self._interp_fn = si.CubicSpline(self.times, log_dfs, bc_type='natural')
        ...
        #     self._interp_fn = si.CubicSpline(self.times, zero_rates, bc_type='natural')

        else:
            raise FinError(f"Check interpolation scheme: {self._interp_type}")
    ...
    def interpolate(self, t: (float, np.ndarray)):
        ...
        if self._zero_rates is None:
            raise FinError("Zero rates have not been set.")
        ...
        if isinstance(t, (float, np.float64)):
            ...
            if np.abs(t) < global_vars.g_small:
                return 1.0
