...
from quantfe.utils import helpers, global_vars, global_types
from quantfe.utils.error import FinError
from quantfe.utils.qmath import N, n_vect, n_prime_vect
from quantfe.utils.solver_1d import bisection, newton, newton_secant
...
@vectorize([float64(float64, float64, float64, float64, float64, float64, int64)], fastmath=True, cache=True)
def bs_value(s, t, k, r, q, v, option_type_value):
    ...
    if option_type_value == global_types.OptionTypes.EUROPEAN_CALL.value:
        phi = 1.0
    elif option_type_value == global_types.OptionTypes.EUROPEAN_PUT.value:
        phi = -1.0
    else:
        raise FinError("Unknown option type value")

    k = np.maximum(k, global_vars.g_small)
...
@vectorize([float64(float64, float64, float64, float64, float64, int64)], fastmath=True, cache=True)
...
@vectorize([float64(float64, float64, float64, float64, float64, float64, int64)], fastmath=True, cache=True,  forceobj=True)
def bs_implied_volatility(s, t, k, r, q, price, option_type_value):
    ...
    if intrinsic_value > 0.0:
        ...
        if option_type_value == global_types.OptionTypes.EUROPEAN_CALL.value:
            ...
            option_type_value = global_types.OptionTypes.EUROPEAN_PUT.value
        else:
            price = price + (div_adj_stock_price - k * df)
            option_type_value = global_types.OptionTypes.EUROPEAN_CALL.value
...
def baw_value(s, t, k, r, q, v, phi):
    ...
    if phi == 1:
        ...
        if s < sstar:
            return bs_value(s, t, k, r, q, v, global_types.OptionTypes.EUROPEAN_CALL.value) + A2 * ((s/sstar)**q2)
        ...
        if s > sstar:
            bsv = bs_value(s, t, k, r, q, v, global_types.OptionTypes.EUROPEAN_PUT.value)
...
def bjerksund_stensland_value(s, t, k, r, q, v, option_type_value):
    ...
    if option_type_value == global_types.OptionTypes.AMERICAN_CALL.value:
        pass
    elif option_type_value == global_types.OptionTypes.AMERICAN_PUT.value:
