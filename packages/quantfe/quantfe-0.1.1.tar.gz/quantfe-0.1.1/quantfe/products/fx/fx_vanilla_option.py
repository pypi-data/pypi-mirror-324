...
from ...models.sabr import SABR, vol_function_sabr
...
from ...utils import global_vars, global_types, helpers, qmath, error
...
def fast_delta(s, t, k, rd, rf, vol, deltaTypeValue, option_type_value):
    ...
    if deltaTypeValue == FinFXDeltaMethod.SPOT_DELTA.value:
        ...
        raise error.FinError("Unknown FinFXDeltaMethod")
...
class FXVanillaOption:
    ...
    def __init__(
        ...
        option_type: Union[global_types.OptionTypes, list],
        notional: float,
        prem_currency: str,
        spot_days: int = 0,
    ):
        ...
        if delivery_dt < expiry_dt:
            raise error.FinError("Delivery date must be on or after expiry date.")

        if len(currency_pair) != 6:
            raise error.FinError("Currency pair must be 6 characters.")

        self.expiry_dt = expiry_dt
        self.delivery_dt = delivery_dt

        if np.any(strike_fx_rate < 0.0):
            raise error.FinError("Negative strike.")
        ...
        if prem_currency != self.dom_name and prem_currency != self.for_name:
            raise error.FinError("Premium currency not in currency pair.")
        ...
        if (
            option_type != global_types.OptionTypes.EUROPEAN_CALL
            and option_type != global_types.OptionTypes.EUROPEAN_PUT
            and option_type != global_types.OptionTypes.AMERICAN_CALL
            and option_type != global_types.OptionTypes.AMERICAN_PUT
        ):
            raise error.FinError("Unknown Option Type:" + option_type)
    ...
    def value(
        ...
        if isinstance(value_dt, Date) is False:
            raise error.FinError("Valuation date is not a Date")

        if value_dt > self.expiry_dt:
            raise error.FinError("Valuation date after expiry date.")

        if domestic_curve.value_dt != value_dt:
            raise error.FinError(
        ...
        if isinstance(value_dt, Date):
            ...
            t_del = (self.delivery_dt - spot_dt) / global_vars.gDaysInYear
            t_exp = (self.expiry_dt - value_dt) / global_vars.gDaysInYear
        else:
            t_del = value_dt
            t_exp = t_del

        if np.any(spot_fx_rate <= 0.0):
            raise error.FinError("spot_fx_rate must be greater than zero.")

        if t_del < 0.0:
            raise error.FinError("Time to expiry must be positive.")
        ...
        if isinstance(model, BlackScholes) or isinstance(model, SABR):
            ...
            if np.any(volatility < 0.0):
                raise error.FinError("Volatility should not be negative.")

            v = np.maximum(volatility, 1e-10)

            if self.option_type == global_types.OptionTypes.EUROPEAN_CALL:

                vdf = bs_value(
                    s0, t_exp, k, r_d, r_f, v, global_types.OptionTypes.EUROPEAN_CALL.value
                )

            elif self.option_type == global_types.OptionTypes.EUROPEAN_PUT:

                vdf = bs_value(
                    s0, t_exp, k, r_d, r_f, v, global_types.OptionTypes.EUROPEAN_PUT.value
                )

            elif self.option_type == global_types.OptionTypes.AMERICAN_CALL:
                ...
                vdf = crr_tree_val_avg(
                    ...
                    num_steps_per_year,
                    t_exp,
                    global_types.OptionTypes.AMERICAN_CALL.value,
                    k,
                )["value"]

            elif self.option_type == global_types.OptionTypes.AMERICAN_PUT:
                ...
                vdf = crr_tree_val_avg(
                    ...
                    num_steps_per_year,
                    t_exp,
                    global_types.OptionTypes.AMERICAN_PUT.value,
                    k,
                )["value"]
            else:
                raise error.FinError("Unknown option type")
        ...
        if self.prem_currency == self.dom_name:
            ...
            raise error.FinError("Invalid notional currency.")
    ...
    def delta(
        ...
        if np.any(spot_fx_rate <= 0.0):
            raise error.FinError("Spot FX Rate must be greater than zero.")
        ...
        if isinstance(model, BlackScholes):
            ...
            v = np.maximum(v, global_vars.g_small)
    ...
    def gamma(
        ...
        if isinstance(value_dt, Date):
            t = (self.expiry_dt - value_dt) / global_vars.gDaysInYear
        else:
            t = value_dt

        if np.any(spot_fx_rate <= 0.0):
            raise error.FinError("FX Rate must be greater than zero.")
        ...
        if isinstance(model, BlackScholes):
            ...
            gamma = np.exp(-r_f * t) * qmath.nprime(d1)
            gamma = gamma / s0 / den
        else:
            raise error.FinError("Unknown Model Type")
    ...
    def vega(
        ...
        if isinstance(model, BlackScholes):
            ...
            v2 = volatility * volatility
            d1 = (ln_s0_k + (mu + v2 / 2.0) * t) / den
            vega = s0 * sqrt_t * np.exp(-r_f * t) * qmath.nprime(d1)
        else:
            raise error.FinError("Unknown Model type")
    ...
    def theta(
        ...
        if isinstance(model, BlackScholes):
            ...
            if self.option_type == global_types.OptionTypes.EUROPEAN_CALL:
                v = -s0 * np.exp(-r_f * t) * qmath.nprime(d1) * vol / 2.0 / sqrt_t
                v = v + r_f * s0 * np.exp(-r_f * t) * qmath.N(d1)
                v = v - r_d * K * np.exp(-r_d * t) * qmath.N(d2)
            elif self.option_type == global_types.OptionTypes.EUROPEAN_PUT:
                v = -s0 * np.exp(-r_f * t) * qmath.nprime(d1) * vol / 2.0 / sqrt_t
                v = v + r_d * K * np.exp(-r_d * t) * qmath.N(-d2)
                v = v - r_f * s0 * np.exp(-r_f * t) * qmath.N(-d1)
    ...
    def value_mc(
        ...
        if isinstance(model, BlackScholes):
            ...
            raise error.FinError("Model Type invalid")
    ...
    def __repr__(self):
        ...
        s += helpers.label_to_string("EXPIRY DATE", self.expiry_dt)
        s += helpers.label_to_string("CURRENCY PAIR", self.currency_pair)
        s += helpers.label_to_string("PREMIUM CCY", self.prem_currency)
        s += helpers.label_to_string("STRIKE FX RATE", self.strike_fx_rate)
        s += helpers.label_to_string("OPTION TYPE", self.option_type)
        s += helpers.label_to_string("SPOT DAYS", self.spot_days)
