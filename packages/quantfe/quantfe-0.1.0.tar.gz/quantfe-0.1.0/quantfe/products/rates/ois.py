...
from quantfe.utils import global_types, global_vars, helpers, day_count, frequency, qmath, calendar

from quantfe.market.curves.discount_curve import DiscountCurve
from quantfe.market.curves.interpolator import Interpolator, InterpTypes

from quantfe.products.rates.ibor_deposit import IborDeposit
from quantfe.products.rates.swap_fixed_leg import SwapFixedLeg
from quantfe.products.rates.swap_overnight_leg import SwapOvernightLeg
...
class OIS:
    def __init__(
        ...
        fixed_leg_type: global_types.SwapTypes,
        fixed_cpn: float,  # Fixed cpn (annualised)
        fixed_freq_type: frequency.FrequencyTypes,
        fixed_dc_type: day_count.DayCountTypes,
        notional: float = qmath.ONE_MILLION,
        payment_lag: int = 0,  # Number of days after period payment occurs
        float_spread: float = 0.0,
        float_freq_type: frequency.FrequencyTypes = frequency.FrequencyTypes.ANNUAL,
        float_dc_type: day_count.DayCountTypes = day_count.DayCountTypes.THIRTY_E_360,
        ...
        float_leg_type = global_types.SwapTypes.PAY
        if fixed_leg_type == global_types.SwapTypes.PAY:
            float_leg_type = global_types.SwapTypes.RECEIVE
        ...
        self.float_leg = SwapOvernightLeg(
            effective_dt,
            self.termination_dt,
            float_leg_type,
            float_spread,
    ...
    def value(self, value_dt: Date, ois_curve: DiscountCurve, ois_rate_history: dict):
        """Value the interest rate swap on a value date given a single Ibor
        discount curve."""
        fixed_leg_value = self.fixed_leg.value(value_dt, ois_curve)
        float_leg_value = self.float_leg.value(
            value_dt, ois_curve, ois_curve, ois_rate_history
        )
...
def _f(rate, *args):
    ...
    curve: OISCurve = args[0]
    value_dt: Date = args[1]
    swap: OIS = args[2]
    num_points = len(curve._times)
    curve._zero_rates[num_points - 1] = rate
    # For discount that need a fit function, we fit it now
    curve._interpolator.fit(curve._times, curve._zero_rates)
    v_swap = swap.value(value_dt, curve, None)
    notional = swap.fixed_leg.notional
    v_swap /= notional / 1e10
    return v_swap
...
class OISCurve(DiscountCurve):
    ...
    def _validate_inputs(self, ois_deposits, ois_fras, ois_swaps):
        ...
        last_deposit_maturity_dt = Date(1900, 1, 1)
        first_fra_maturity_dt = Date(1900, 1, 1)
        last_fra_maturity_dt = Date(1900, 1, 1)
        ...
        # TODO: check deposit is essential
    ...
    def _build_curve_using_1d_solver(self):
        ...
        zero_rate_mat = 0.0
        df_mat = 1.0
        self._dates = np.append(self._dates, self.value_dt)
        self._times = np.append(self._times, 0.0)
        self._zero_rates = np.append(self._zero_rates, zero_rate_mat)
        self._interpolator.fit(self._times, self._zero_rates)

        for depo in self.used_deposits:
            df_settle = self.cal_df(depo.start_dt)
            df_mat = depo._maturity_df() * df_settle
            t_mat = (depo.maturity_dt - self.value_dt) / global_vars.gDaysInYear
        ...
        for fra in self.used_fras:
            t_set = (fra.start_dt - self.value_dt) / global_vars.gDaysInYear
            t_mat = (fra.maturity_dt - self.value_dt) / global_vars.gDaysInYear
        ...
        for swap in self.used_swaps:
            ...
            t_mat = (maturity_dt - self.value_dt) / global_vars.gDaysInYear
            self._dates = np.append(self._dates, maturity_dt)
            self._times = np.append(self._times, t_mat)
            self._zero_rates = np.append(self._zero_rates, zero_rate_mat)

            argtuple = (self, self.value_dt, swap)

            zero_rate_mat = optimize.fsolve(
                _f,
                x0=swap.fixed_leg.cpn,
                args=argtuple,
                fprime=None,
                full_output=False,
                xtol=1e-10,
            ...
            # print("!!", swap.maturity_dt, zero_rate_mat)
    ...
    def _build_curve_linear_swap_rate_interpolation(self):
        ...
        for depo in self.used_deposits:
            ...
            t_mat = (depo.maturity_dt - self.value_dt) / global_vars.g_days_in_year
            self._times = np.append(self._times, t_mat)
            self._dfs = np.append(self._dfs, df_mat)
            self._interpolator.fit(self._times, self._dfs)
        ...
        for fra in self.used_fras:
            t_set = (fra.start_dt - self.value_dt) / global_vars.g_days_in_year
            t_mat = (fra.maturity_dt - self.value_dt) / global_vars.g_days_in_year
        ...
        for swap in self.used_swaps:
            ...
            tswap = (maturity_dt - self.value_dt) / global_vars.g_days_in_year
        ...
        for dt in cpn_dts[1:]:
            swap_time = (dt - self.value_dt) / global_vars.g_days_in_year
        ...
        df = 1.0
        pv01 = 0.0
        df_settle = self.cal_df(longest_swap.start_dt)
        ...
        for i in range(start_index, num_flows):
            dt = cpn_dts[i]
            t_mat = (dt - self.value_dt) / global_vars.g_days_in_year
    ...
    def _check_refits(self, depo_tol, fra_tol, swap_tol):
        ...
        for swap in self.used_swaps:
            ...
            v = swap.value(swap.effective_dt, self, None)
    ...
    def __repr__(self):
        ...
        for depo in self.used_deposits:
            s += helpers.label_to_string("DEPOSIT", "")
            s += depo.__repr__()

        for fra in self.used_fras:
            s += helpers.label_to_string("FRA", "")
            s += fra.__repr__()

        for swap in self.used_swaps:
            s += helpers.label_to_string("SWAP", "")
            s += swap.__repr__()
        ...
        s += helpers.label_to_string("GRID TIMES", "GRID DFS")
        for i in range(0, num_points):
            s += helpers.label_to_string(
