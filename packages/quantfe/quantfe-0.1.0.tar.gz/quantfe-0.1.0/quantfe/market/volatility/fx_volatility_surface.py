...
from ...products.fx.fx_mkt_conventions import FinFXDeltaMethod, FinFXATMMethod
from ...products.fx.fx_vanilla_option import FXVanillaOption, fast_delta

from ...utils import global_vars, global_types, helpers
from ...utils.error import FinError
from ...utils.date import Date
from ...utils.qmath import norminvcdf
...
def obj_fast(params, *args):
    ...
    v_25d_c_ms = bs_value(
        ...
        r_f,
        sigma_k_25d_c_ms,
        global_types.OptionTypes.EUROPEAN_CALL.value,
    )
    ...
    v_25d_p_ms = bs_value(
        ...
        r_f,
        sigma_k_25d_p_ms,
        global_types.OptionTypes.EUROPEAN_PUT.value,
    )
...
def solve_to_horizon_fast(
    ...
    v_25d_c_ms = bs_value(
        s, t, k_25d_c_ms, rd, rf, vol_25d_ms, global_types.OptionTypes.EUROPEAN_CALL.value
    )

    v_25d_p_ms = bs_value(
        s, t, k_25d_p_ms, rd, rf, vol_25d_ms, global_types.OptionTypes.EUROPEAN_PUT.value
    )
...
@njit(
    float64(float64, float64, float64, float64, int64, int64, float64, int64, float64, float64[:]),
    fastmath=True,
    cache=False,
)
...
def solve_for_strike(
    ...
    if delta_method_value == FinFXDeltaMethod.SPOT_DELTA.value:
        ...
        if option_type_value == global_types.OptionTypes.EUROPEAN_CALL.value:
            phi = +1.0
        else:
            phi = -1.0
...
class FXVolSurface:
    ...
    def volatility(self, K, expiry_dt):
        ...
        - K is the strike of target
        - expiry_dt is the expiry date or time to maturity of target
        """
        ...
        if isinstance(expiry_dt, Date):
            t = (expiry_dt - self.value_dt) / global_vars.gDaysInYear
        elif isinstance(expiry_dt, float):
            t = expiry_dt
        else:
            raise FinError("Unknown expiry date Type")
    ...
    def forward_volatility(self, K, start_dt, end_dt):
        """Return the forward volatility from start_dt to end_dt for a given
        strike K. This is the forward volatility from the start date to the end
        date for a given strike K."""

        start_t = 0.0
        end_t = 0.0
        if isinstance(start_dt, Date):
            start_t = (start_dt - self.value_dt) / global_vars.gDaysInYear
        elif isinstance(start_dt, float):
            start_t = start_dt
        
        if isinstance(end_dt, Date):
            end_t = (end_dt - self.value_dt) / global_vars.gDaysInYear
        elif isinstance(end_dt, float):
            end_t = end_dt

        if start_t >= end_t:
            raise FinError("Start date must be before end date")
        
        fwd_vol = np.sqrt((self.volatility(K, end_t) ** 2 * end_t - self.volatility(K, start_t) ** 2 * start_t) / (end_t - start_t))
        return fwd_vol
    ...
    def build_vol_surface(self):
        ...
        for i in range(0, num_vol_curves):
            ...
            t_exp = (expiry_dt - spot_dt) / global_vars.gDaysInYear
    ...
    def check_calibration(self, verbose: bool, tol: float = 1e-6):
        ...
        for i in range(0, self.num_vol_curves):
            ...
            call = FXVanillaOption(
                ...
                K_dummy,
                self.currency_pair,
                global_types.OptionTypes.EUROPEAN_CALL,
                1.0,
            ...
            put = FXVanillaOption(
                ...
                K_dummy,
                self.currency_pair,
                global_types.OptionTypes.EUROPEAN_PUT,
                1.0,
                self.notional_currency,
            )
    ...
    def __repr__(self):
        ...
        s += helpers.label_to_string("FX RATE", self.spot_fx_rate)
        s += helpers.label_to_string("CCY PAIR", self.currency_pair)
        s += helpers.label_to_string("NOTIONAL CCY", self.notional_currency)
        s += helpers.label_to_string("NUM TENORS", self.num_vol_curves)
        s += helpers.label_to_string("ATM METHOD", self.atm_method)
        s += helpers.label_to_string("DELTA METHOD", self.delta_method)
        s += helpers.label_to_string("VOL FUNCTION", self.vol_func_type)

        for i in range(0, self.num_vol_curves):

            s += "\n"

            s += helpers.label_to_string("TENOR", self.tenors[i])
            s += helpers.label_to_string("EXPIRY DATE", self.expiry_dts[i])
            s += helpers.label_to_string("TIME (YRS)", self.t_exp[i])
            s += helpers.label_to_string("FWD FX", self.fwd[i])

            s += helpers.label_to_string("ATM VOLS", self.atm_vols[i] * 100.0)
            s += helpers.label_to_string("MS VOLS", self.ms_25_delta_vols[i] * 100.0)
            s += helpers.label_to_string("RR VOLS", self.rr_25_delta_vols[i] * 100.0)

            s += helpers.label_to_string("ATM Strike", self.k_atm[i])
            s += helpers.label_to_string("ATM Delta", self.delta_atm[i])

            s += helpers.label_to_string("k_atm", self.k_atm[i])
            s += helpers.label_to_string("MS 25d Call Strike", self.k_25d_c_ms[i])
            s += helpers.label_to_string("MS 25d Put Strike", self.k_25d_p_ms[i])
            s += helpers.label_to_string("SKEW 25d CALL STRIKE", self.k_25d_c[i])
            s += helpers.label_to_string("SKEW 25d PUT STRIKE", self.k_25d_p[i])
            s += helpers.label_to_string("PARAMS", self.parameters[i])
