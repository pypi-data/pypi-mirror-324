...
class IAPListener():
    ...
    def __init__(self,
        ...
        for item in entity.input_def:
            if "link" in item and item["link"] in self._links_in and item["type"] != "query":
                ...
                self.parameters[item["link"]] = {}
                if "parameters" in item:
                    for p in item["parameters"]:
                        self.parameters[item["link"]][p["name"]] = p["type"]                
    ...
    async def _run_async(self):
        ...
        readme_nc = await self.nc.subscribe(f"nc.{self.queue}.readme.*.*", queue=self.queue+"-readme")
        ...
        for link, sub_in in self._subs_in:
            if link == "readme":
                tasks.append(self.wait_readme(sub_in))
    ...
    async def handle_msg(self, subject, link, msgs):
        async with self.lock:
            if self.is_batch:
                uids, sources, batch, params_lst, content_types = zip(*[await self.get_data(subject, msg, link) for msg in msgs])
                batch = list(batch)
                await self._process_data(link, uids, sources, batch, content_types, params_lst)
            else:
                for msg in msgs:
                    uid, source, data, params, content_type = await self.get_data(subject, msg, link)
                    await self._process_data(link, [uid], [source], [data], [content_type], [params])
    ...
    async def get_data(self, subject, msg, link):
        l_sub = len(subject) + 1
...
############ Readme handling ################
    async def wait_readme(self, sub_in):
        readme = ""
        readme_path = ""
        ...
        while True:
            try:
                await sub_in.next_msg(timeout=600)
                ...
                continue
            # Message received
            await self.send_msg("readme", "readme", "text", readme)