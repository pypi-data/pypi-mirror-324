    ...
    async def get_data(self, subject, msg, link):
        ...
        if params_lst:
            for p in params_lst.split(","):
                ...
                if len(args) == 2:
                    k, v = args
                    if k in self.parameters[link] and "type" in self.parameters[link][k]:
                        if self.parameters[link][k]["type"] == "float":
                            params[k] = float(v)
                        elif self.parameters[link][k]["type"] == "integer":
                            params[k] = int(v)
                        elif self.parameters[link][k]["type"] == "boolean":
                            params[k] = bool(v)
                        elif self.parameters[link][k]["type"] == "json":
