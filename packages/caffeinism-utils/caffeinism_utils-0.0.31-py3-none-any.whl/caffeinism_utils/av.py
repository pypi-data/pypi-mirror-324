...
class Formatter:
    def __init__(self, width: int, height: int, from_pix_fmt: str, to_pix_fmt: str):
        ...
        reformat = graph.add("format", to_pix_fmt)
...
def to_rgba(reader: BasePyAVReader):
    if reader.stream.format.name.startswith("yuva"):
        formatter = Formatter(reader.width, reader.height, reader.pix_fmt, "rgba")
    elif reader.stream.format.name.startswith("yuv"):
        formatter = Formatter(reader.width, reader.height, reader.pix_fmt, "rgb")
    else:
        formatter = None
