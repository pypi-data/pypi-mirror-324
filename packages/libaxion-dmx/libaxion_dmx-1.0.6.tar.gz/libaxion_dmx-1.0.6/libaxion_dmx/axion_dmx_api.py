...
_LOGGER = logging.getLogger("axion_dmx_api")
...
class AxionDmxApi:
    ...
    async def get_name(self) -> str:
        ...
        response = await self._send_command(">getname\r\n")
        if "ok" in response:
            _LOGGER.debug(f"Gostname of DMX controller - {str(response.split(',')[1])}")
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                if "ok" not in response:
                    raise AxionDmxAuthError("Authentication failed.")
                _LOGGER.debug(f"Sending command: {command}")
                sock.sendall(command.encode())
                response = sock.recv(1024).decode()
        except socket.error as e:
            raise AxionDmxConnectionError(f"Connection error: {e}")
    ...
    async def get_level(self, channel: int) -> int:
        ...
        if "ok" in response:
            _LOGGER.debug(f"Level of DMX {channel} - {int(float(response.split(',')[1]))}")
...
class AxionDmxConnectionError(Exception):
    """Raised when the API fails to connect to the controller."""


class AxionDmxAuthError(Exception):
