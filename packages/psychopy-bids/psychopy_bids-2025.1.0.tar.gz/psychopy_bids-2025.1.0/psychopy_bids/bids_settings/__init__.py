...
class BidsExportRoutine(BaseStandaloneRoutine):
    ...
    def __init__(self, exp, name="bidsExport"):
        ...
        hnt = _translate(
            "Copies all stimulus files referenced in the events file to the BIDS dataset's /stimuli directory"
        )
    ...
    def writeExperimentEndCode(self, buff):
        ...
        code = "bids_handler.writeEvents(participant_info, add_stimuli=%(add_stimuli)s, execute_sidecar=%(json_sidecar)s)\n"
        buff.writeIndentedLines(code % self.params)
        buff.setIndentLevel(-2, relative=True)
        code = "except Exception as e:\n"
        buff.writeIndentedLines(code)
        buff.setIndentLevel(1, relative=True)
        code = 'print(f"An error occurred when writing BIDS events: {e}")\n'
        buff.writeIndentedLines(code)
        buff.setIndentLevel(original_indent_level)
