...
def _get_cell_id(ipython: Any):
    try:
        ...
        _logger.warning(f"Unable to retrieve CellId.\n{e}")
...
def start_listen() -> Any:
    ...
    def _notebook_cell_content_update():
        ...
        try:
            ...
            _logger.warning(f"Notebook cell content failed to update.\n{e}")
...
class Autolog:
    def __init__(
        ...
        self._iteration_service.save_autolog_assets(
            list(filter(lambda cell: cell["cellId"] is not None, cells)), prefix  # type: ignore
        )
    ...
    def _get_assets(self) -> list[TModel | TDataset | TVecticeObjects]:
        ...
        for data in self._vectice_data:
            ...
            for key, asset in data["variables"].items():
                if not key.startswith("_"):  # Skip cell inputs/outputs
    ...
    def _log_modeva_asset(self, asset: TModel | TDataset | TVecticeObjects):
        try:
    ...
    def _log_asset_types(
        self, asset: TModel | TDataset | TVecticeObjects, asset_type: VecticeType | GiskardType | ModevaType
    ) -> None:
