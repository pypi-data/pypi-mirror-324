...
from .. import cli, playbook, procfile, procrec, procret, proctree, sqliteviz, utility
...
class TestWatchCommand(unittest.TestCase):
    ...
    def test_watch_tree_cleanup_on_sigint_shell_first_pty(self):
        with tempfile.NamedTemporaryFile() as f:
            ...
            log_lines = [line for line in os.read(fd, 4096).decode().splitlines()]
            ...
            self.assertFalse(proctree.process_exists(pid))
            subtree_status = {pid: proctree.process_exists(pid) for pid in subtree_pids}
    ...
    def test_watch_tree_cleanup_on_sigint_shell_first(self):
        with tempfile.NamedTemporaryFile() as f:
            ...
            p.join(2)
            self.assertFalse(p.is_alive())
            forest_status = {pid: proctree.process_exists(pid) for pid in forest_pids}
    ...
    def test_explore_serve(self):
        with tempfile.NamedTemporaryFile() as tmpf:
            ...
            with tempfile.TemporaryDirectory() as tmpd:
                ...
                self.addCleanup(p.terminate)
                p.start()
                assert_wait_predicate(lambda: self.is_port_open('localhost', 18000))
