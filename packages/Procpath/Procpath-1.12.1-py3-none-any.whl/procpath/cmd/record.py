...
def run(
    ...
    forest = proctree.Forest(readers, procfs)
    ...
    meta = utility.get_meta(procfile_list, procfs)
    flat_node_list = []  # do not re-bind, it's passed by reference to _record_loop
    with procrec.SqliteStorage(database_file, procfile_list, meta) as store:
        for ts, count in _record_loop(interval, flat_node_list, recnum, stop_without_result):
            ...
            store.record(ts, flat_node_list)
