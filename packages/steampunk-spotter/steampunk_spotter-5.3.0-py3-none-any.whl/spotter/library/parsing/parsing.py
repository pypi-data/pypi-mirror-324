...
class ParsingResult(BaseModel):
    ...
    def clean_statistics(self, include_metadata: bool) -> Optional[Statistics]:
        if not include_metadata:
            return None
        return Statistics(
            included_files_count=self.included_files_count, excluded_paths_count=self.excluded_paths_count
        )
...
def _is_requirements(loaded_yaml: Any) -> bool:
    ...
    return all(not any(x not in allowed_role_keys and not x.startswith("_") for x in item) for item in loaded_yaml)
...
def detect_secrets_in_file(file_name: str, skip_detect_secrets: bool) -> List[str]:
    """
...
def _remove_secret_parameter_values(  # noqa: PLR0911
    yaml_key: Any, secret_values: List[str]
...
def _parse_tasks(  # noqa: PLR0912  # TODO: oh dear
    tasks: List[Dict[str, Any]],
    file_name: str,
    parse_values: bool,
    ...
    try:
        parsed_tasks = []
        parsed_blocks = []
        secrets = detect_secrets_in_file(file_name, skip_detect_secrets)
        ...
        for task in [t for t in tasks if t is not None]:
            ...
            if contains_block_section:
                ...
                if not parse_values:
                    ...
                    for block_key in block_copy:
                        block_copy[block_key], hidden = _remove_secret_parameter_values(block_copy[block_key], secrets)
                        block_obfuscated.extend(item.to_parent(block_key) for item in hidden)
...
def _parse_vars(
    ...
    if not parse_values:
        ...
        for key in variables_copy:
            variables_copy[key], hidden = _remove_secret_parameter_values(variables_copy[key], secrets)
            obfuscated.extend(item.to_parent(key) for item in hidden)
...
def parse_unknown_ansible_artifact(  # noqa: PLR0915
    path: Path, exclude_paths: Set[str], parse_values: bool, parse_vars: bool, skip_detect_secrets: bool
) -> Tuple[
    List[Dict[str, Any]],
    List[Dict[str, Any]],
    List[Dict[str, Any]],
    ...
    if str(path.absolute()) in exclude_paths:
        parsed_ansible_excluded_paths_count += 1
    elif path.is_file() and path.suffix in YAML_SUFFIXES:
        ...
        if _is_playbook(loaded_yaml):
            parsed_tasks, parsed_blocks, parsed_playbooks = _parse_playbook(
                loaded_yaml, str(path), parse_values, skip_detect_secrets
            )
            ...
            parsed_ansible_artifacts_dynamic_inventories += [
                _parse_dynamic_inventory(loaded_yaml, str(path), parse_values, skip_detect_secrets)
            ]
        elif _is_requirements(loaded_yaml):
            pass
        elif is_var_file(loaded_yaml):
            parsed_vars = _parse_vars(loaded_yaml, str(path), parse_values, parse_vars, skip_detect_secrets)
            parsed_ansible_artifacts_vars += parsed_vars
        elif isinstance(loaded_yaml, list):
            parsed_tasks, parsed_blocks = _parse_tasks(
                loaded_yaml, str(path), parse_values, None, None, skip_detect_secrets
            )
            parsed_ansible_artifacts_tasks += parsed_tasks
            parsed_ansible_artifacts_blocks += parsed_blocks
    elif path.is_file() and path.suffix == ".py":
        parsed_ansible_artifacts_plugins += _parse_py(path)
        parsed_ansible_included_files_count += 1
    elif path.is_dir():
        if _is_role(path):
            parsed_role, yaml_error = _parse_role(path)
        ...
        for sub_path in sorted(path.iterdir()):
            (
                ...
                parsed_vars,
                yaml_error,
                (parsed_included_files_count, parsed_excluded_paths_count),
            ) = parse_unknown_ansible_artifact(sub_path, exclude_paths, parse_values, parse_vars, skip_detect_secrets)
            parsed_ansible_artifacts_tasks += parsed_tasks
            parsed_ansible_artifacts_playbooks += parsed_playbooks
            parsed_ansible_artifacts_blocks += parsed_blocks
            parsed_ansible_artifacts_dynamic_inventories += parsed_dynamic_invenroties
            parsed_ansible_artifacts_roles += parsed_roles
            parsed_ansible_artifacts_plugins += parsed_plugins
            parsed_ansible_artifacts_vars += parsed_vars
            parsed_errors += yaml_error
            parsed_ansible_included_files_count += parsed_included_files_count
            parsed_ansible_excluded_paths_count += parsed_excluded_paths_count
    ...
    return (
        ...
        parsed_ansible_artifacts_vars,
        parsed_errors,
        (parsed_ansible_included_files_count, parsed_ansible_excluded_paths_count),
    )
...
def parse_ansible_artifacts(
    ...
    for path in paths:
        (
            ...
            yaml_error,
            (parsed_included_files_count, parsed_excluded_paths_count),
        ) = parse_unknown_ansible_artifact(path, paths_to_exclude, parse_values, parse_vars, skip_detect_secrets)
        parsed_ansible_artifacts_tasks += parsed_tasks
        parsed_ansible_artifacts_playbooks += parsed_playbooks
        parsed_ansible_artifacts_blocks += parsed_blocks
        ...
        parsed_ansible_artifacts_vars += parsed_vars
        parsed_errors += yaml_error
        parsed_ansible_artifacts_included_files_count += parsed_included_files_count
        parsed_ansible_artifacts_excluded_paths_count += parsed_excluded_paths_count
