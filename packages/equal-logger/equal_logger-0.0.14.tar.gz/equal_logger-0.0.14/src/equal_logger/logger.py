...
class Logger:
    def __init__(self,
        ...
        :param cloud: defines the cloud storage provider to use for saving logs. Options: 'GCP', 'AZURE', 'TEST'
        ...
        self._id_execucao = hashlib.md5(f'{script_name}{dt.datetime.now()}'.encode()).hexdigest()
    ...
    def _register_log(self, status, title, description, print_log=True):
        """
        ...
        if print_log:
            print(json.dumps(log_row, indent=2, default=str))

        self.log_table.append(log_row)

    def error(self, title, description, print_log=True, raise_error: Exception = None):
        """
        ...
        self._register_log("ERROR", title, description, print_log)

    def success(self, title, description, print_log=True):
        """
        ...
        :param print_log: option to print the log, default is True
        """
        self._register_log("SUCCESS", title, description, print_log)

    def info(self, title, description, print_log=True):
        """
        ...
        :param print_log: option to print the log, default is True
        """
        self._register_log("INFO", title, description, print_log)
        ...
        for retry in range(self._RETRIES):
            try:
                ...
                if self._cloud == "GCP":
                    ...
                    save_to_gcp_gcs.upload_parquet_to_gcs(parquet_buffer)
                ...
                if self._cloud == "AZURE":
                    ...
                    save_to_azure_adls.upload_parquet_to_adls(parquet_buffer)
