...
class API:
    ...
    def get_zillow_output(self, address):
        try:
            ...
            zestimate = data.get("zestimate", price)
            zestimateLowPercent = (
                data.get("zestimateLowPercent")
                if data.get("zestimateLowPercent")
                else 0
            )
            url = data.get("url")
            ...
            return {
                ...
                "lot_size": self._parse_lot_size(data),
                "price_per_sqft": (
                    data.get("priceHistory", [{}])[0].get("pricePerSquareFoot")
    ...
    def get_propstream_output(self, address):
        try:
            ...
            suggestion = session.get(
                f"{self.propstream_base_url}eqbackend/resource/auth/ps4/property/suggestionsnew?q={formatted_address}"
            ).json()
