...
class EBestSimpleDomesticStock:
    ...
    def create_order(
        ...
        if "OrdNo" not in r["output2"]:
            self.logger.error(
                f"create_order failed. {asset_code} {side} {quantity} {order_type} {price} - {r['rsp_cd']} {r['rsp_msg']}"
            )
    ...
    def _map_order_event(self, data: dict) -> OrderEvent:
        ...
        # 취소 주문의 경우 원주문번호를 사용
        order_no = str(
            data["orgordno"] if data["event_type"] == "cancelled" else data["ordno"]
        )
        ...
        filled_quantity = (
            int(data["execqty"]) if data.get("execqty", "").strip() != "" else None
        )
        filled_price = (
            int(data["execprc"]) if data.get("execprc", "").strip() != "" else None
        )
        pending_quantity = (
            int(data["unercqty"]) if data.get("unercqty", "").strip() != "" else None
        )

        filled_time = data.get("rcptexectime")
        org_order_no = (
            str(data.get("orgordno")) if data["event_type"] != "cancelled" else None
        )

        self.logger.debug(
            f"order_event: order_no={order_no} / filled={filled_quantity} order={quantity} pending={pending_quantity} event_type={event_type}"
        )
