...
def compress_sequential(text, additional_prompt="",n=1):
    ...
    res = ""
    for c in chunks:
        res += genai(prompt=f"compress and have simple explanation in english with core details to takeaway, "
...
def compress(text, additional_prompt="",n=1):
    if n==0:
        return text
    chunks = wlsplit(text)
    def generate(c):
        ...
        #                     f"note: keep python codes exactly same\n {additional_prompt}"
        #                     f"for normal like 100 to 10 words ration compression: [CONTENT]\n {c}")+"\n"
        return genai(prompt=f"compress and have simple explanation in english with core details to takeaway, like 100 to 10 words ration compression: [CONTENT]\n {c}")+"\n"

    with ThreadPoolExecutor(max_workers=4) as executor:
        res = "\n".join(list(executor.map(generate,chunks)))
    return compress(res,additional_prompt,n-1)
