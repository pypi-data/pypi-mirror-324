...
from liteutils import read_arxiv, extract_abstract, remove_references
...
def research_paper_analysis(pdf_path_or_url):
    llm = LLM()
    ...
    l1 = compress(main_text)
    l2 = compress(l1)
    l3 = compress(l2)
    l4 = compress(l3)
    l5 = compress(l4)
    abs_insights = llm(f'given research paper abstract generate insights, Abstract : {abs}',
                       response_format=PaperInsights)
    summary_insights = llm(f'given research paper summary generate insights, Summary : {l5}',
