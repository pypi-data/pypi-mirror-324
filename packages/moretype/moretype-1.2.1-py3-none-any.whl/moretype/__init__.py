...
class Link:
    ...
    """
    def __init__(self, *args, target: list | tuple | set | None = None, dict_key: str = 'data'):
        if args:
...
class Rect:
    ...
    def __str__(self) -> str:
        return "[{}]".format(' '.join([' '.join(map(str, row)) for row in self.matrix]))
...
class Array:
    def __init__(self, *args, data: list[int | float] | None = None):
        if data is not None:
            if not isinstance(data, list):
                raise ValueError("data must be a list")
            self.data = data
        elif args and not data:
            self.data = list(args)
        elif args and data:
            raise ValueError('Unexpected keyword data.')
        else:
            self.data = []
    ...
    def remove(self, value: int | float) -> int | float:
...
class Database:
    def __init__(self, db_name: str = 'database'):
        ...
        self.data = []
        open(f'{db_name}.json', "w", encoding="utf-8").close()
        self.db_name += '.json'
    ...
    def insert(self, data: Any, key: str = None) -> None:
        """
        向数据库中插入数据。
    ...
    def remove(self, key: str, value: Any = None, raise_error: bool = False) -> None:
        """
        根据键和值删除数据。
