...
class DataBase:
    ...
    def _list_klines_symbols(self, asset_class: Literal["spot", "um"] = "um"):
        # query all symbols from klines table
        sql = f"""
        SELECT DISTINCT symbol FROM binance_{asset_class}_klines_1m
        """
        return self.conn.query(sql).to_df()["symbol"].tolist()
    ...
    def _create_klines_table(
        ...
        path = os.path.join(self._cache_path, self.ASSET_CLASS_MAP[asset_class])
        if freq == "1m":
            sql = f"""
    ...
    def factors_matrix(
        ...
        if symbols:
            ...
            df_nan = pd.DataFrame(np.nan, index=df.index, columns=expected_columns - existing_columns)
