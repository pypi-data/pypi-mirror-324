...
from finn.components.dims import RangeTuple
from finn.layers import Layer
from finn.layers.utils.layer_utils import Extent
from finn.utils.events.containers import SelectableEventedList
from finn.utils.naming import inc_name_count
...
class LayerList(SelectableEventedList[Layer]):
    ...
    def _create_contexts(self):
        ...
        from finn._app_model.context._layerlist_context import (
    ...
    def _coerce_name(self, name, layer=None):
        ...
        name : str
            Original name.
        layer : finn.layers.Layer, optional
    ...
    def _link_layers(
        ...
        from finn.layers.utils import _link_layers
    ...
    def save(
        ...
        *,
        ...
        :meth:`~finn.plugins._hook_callers._HookCaller.bring_to_front`
        method.

        If ``plugin`` is not provided and multiple layers are targeted,
        then we call
        :meth:`~finn.plugins.hook_specifications.napari_get_writer` which
        ...
        offer :meth:`~finn.plugins.hook_specifications.napari_get_writer` for
        that combination of layers then the default
        :meth:`~finn.plugins.hook_specifications.napari_get_writer` will
        ...
        we call we call
        :meth:`~finn.plugins.hook_specifications.napari_get_writer` for
