...
import finn
from finn import __main__


@pytest.fixture
def mock_run():
    """mock to prevent starting the event loop."""
    with mock.patch('finn._qt.widgets.qt_splash_screen.NapariSplashScreen'):
        with mock.patch('finn.run'):
            yield finn.run
...
def test_cli_parses_unknowns(mock_run, monkeypatch, make_napari_viewer):
    ...
    with mock.patch('finn.Viewer', return_value=v):
        monkeypatch.setattr(
...
def test_cli_runscript(run_path, monkeypatch, tmp_path):
    ...
    script.write_text('import finn; v = finn.Viewer(show=False)')
...
def test_cli_retains_viewer_ref(mock_run, monkeypatch, make_napari_viewer):
    ...
    def _check_refs(**kwargs):
        # when run() is called in finn.__main__, we will call this function
        ...
        if sys.getrefcount(v) <= ref_count:  # pragma: no cover
            raise AssertionError(
                'Reference to finn.viewer has been lost by '
                'the time the event loop started in finn.__main__'
            )
    ...
    with monkeypatch.context() as m:
        ...
        # return our local v
        with mock.patch('finn.Viewer', return_value=v) as mock_viewer:
            ...
            with mock.patch(
                'finn._qt.qt_viewer.QtViewer._qt_open', return_value=None
            ) as mock_viewer_open:
                __main__._run()
