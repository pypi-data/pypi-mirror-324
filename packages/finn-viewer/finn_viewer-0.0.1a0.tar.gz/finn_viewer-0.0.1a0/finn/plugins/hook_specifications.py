...
def napari_provide_sample_data() -> dict[str, Union[SampleData, SampleDict]]:
    ...
    Plugins may implement this hook to provide sample data for use in finn.
    Sample data is accessible in the `File > Open Sample` menu, or
    programmatically, with :meth:`finn.Viewer.open_sample`.
...
def napari_get_reader(path: Union[str, list[str]]) -> Optional[ReaderFunction]:
    ...
    <finn.components.viewer_model.ViewerModel.open>`, via the
    :func:`~finn.plugins.io.read_data_with_plugins` function.
    ...
    <finn.components.viewer_model.ViewerModel._add_layer_from_data>`
...
def napari_get_writer(
    ...
    :func:`~finn.plugins.io.save_layers`).
...
def napari_experimental_provide_function() -> Union[
    ...
    function(s) : FunctionType or list of FunctionType
        ...
        `magicgui <https://finn.org/magicgui>`_ can generate a widget from
        them.
...
def napari_experimental_provide_dock_widget() -> Union[
    ...
    result : callable or tuple or list of callables or list of tuples
        ...
        arguments for :meth:`finn.qt.Window.add_dock_widget`. (note, however,
    ...
    >>> def threshold(
    ...     data: 'finn.types.ImageData', threshold: int
    ... ) -> 'finn.types.LabelsData':
...
def napari_experimental_provide_theme() -> dict[
    ...
    """Provide GUI with a set of colors used through finn. This hook allows you to
    ...
    See :class:`~finn.utils.theme.Theme` for more detail of what are the required keys.

    Returns
    -------
    themes : Dict[str, Dict[str, Union[str, Tuple, List]]
        Sequence of dictionaries containing new color schemes to be used by finn.
