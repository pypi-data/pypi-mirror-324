...
def validate_unknown_args(unknown: list[str]) -> dict[str, Any]:
    ...
    from finn.components.viewer_model import valid_add_kwargs
...
def parse_sys_argv():
    ...
    from finn import __version__, layers
    from finn.components.viewer_model import valid_add_kwargs
...
def _run_plugin_module(mod, plugin_name):
    """Register `mod` as a plugin, find/create viewer, and run finn."""
    from finn import Viewer, run
    ...
    try:
        ...
        # this script had a finn.run() in it, and the viewer has already been
...
def _maybe_rerun_with_macos_fixes():
    ...
    # macOS to ensure menubar works without needing to refocus finn.
