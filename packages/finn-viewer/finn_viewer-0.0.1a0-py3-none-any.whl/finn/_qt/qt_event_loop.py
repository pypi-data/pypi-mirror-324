...
from finn import Viewer, __version__
from finn._qt.dialogs.qt_notification import NapariQtNotification
from finn._qt.qt_event_filters import QtToolTipEventFilter
from finn._qt.qthreading import (
    register_threadworker_processors,
    wait_for_workers_to_quit,
)
from finn._qt.utils import _maybe_allow_interrupt
from finn.resources._icons import _theme_path
from finn.settings import get_settings
from finn.utils import config, perf
from finn.utils.notifications import (
    notification_manager,
    show_console_notification,
)
from finn.utils.perf import perf_config
...
def gui_qt(*, startup_logo=False, gui_exceptions=False, force=False):
    ...
    NOTE: This context manager is deprecated!. Prefer using :func:`finn.run`.
    ...
    warn(
        trans._(
            "\nThe 'gui_qt()' context manager is deprecated.\nIf you are running napari from a script, please use 'finn.run()' as follows:\n\n    import finn\n\n    viewer = finn.Viewer()  # no prior setup needed\n    # other code using the viewer...\n    finn.run()\n\nIn IPython or Jupyter, 'finn.run()' is not necessary. napari will automatically\nstart an interactive event loop for you: \n\n    import finn\n    viewer = finn.Viewer()  # that's it!\n",
            deferred=True,
        ),
        FutureWarning,
        stacklevel=2,
