...
from finn._app_model.context._context_keys import ContextNamespace
...
if TYPE_CHECKING:
    ...
    from finn.utils.events import Selection
...
def _all_linked(s: LayerSel) -> bool:
    from finn.layers.utils._link_layers import layer_is_linked

    return bool(s and all(layer_is_linked(x) for x in s))


def _n_unselected_links(s: LayerSel) -> int:
    from finn.layers.utils._link_layers import get_linked_layers
