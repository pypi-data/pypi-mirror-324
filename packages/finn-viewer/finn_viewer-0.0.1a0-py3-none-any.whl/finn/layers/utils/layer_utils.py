...
def coerce_affine(
    ...
    *,
    ...
    Parameters
    ----------
    affine : array-like or finn.utils.transforms.Affine
    ...
    if affine is None:
        ...
        raise TypeError(
            trans._(
                'affine input not recognized. must be either finn.utils.transforms.Affine or ndarray. Got {dtype}',
                deferred=True,
                dtype=type(affine),
            )
        )
...
def get_extent_world(
    ...
    data_to_world : finn.utils.transforms.Affine
