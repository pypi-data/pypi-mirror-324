...
import finn
from finn import Viewer, layers as module
from finn._tests.utils import check_viewer_functioning, layer_test_data
...
def test_imshow(qtbot, napari_plugin_manager):
    ...
    viewer, layer = finn.imshow(data, channel_axis=None, show=False)
    view = viewer.window._qt_viewer
    check_viewer_functioning(viewer, view, data, ndim)
    assert isinstance(layer, finn.layers.Image)
    viewer.close()
...
def test_imshow_multichannel(qtbot, napari_plugin_manager):
    ...
    np.random.seed(0)
    data = np.random.random((15, 10, 5))
    viewer, layers = finn.imshow(data, channel_axis=-1, show=False)
...
def test_imshow_with_viewer(qtbot, napari_plugin_manager, make_napari_viewer):
    ...
    viewer2, layer = finn.imshow(data, viewer=viewer, show=False)
