...
if TYPE_CHECKING:
    ...
    from finn._qt.qthreading import FunctionWorker
...
def add_layer_data_to_viewer(gui: FunctionGui, result: Any, return_type: type):
    ...
    return annotation of one of the `finn.types.<layer_name>Data` ... and
    ...
    >>> @magicgui
    ... def make_layer() -> finn.types.ImageData:
...
def add_layer_data_tuples_to_viewer(gui, result, return_type):
    ...
    >>> @magicgui
    ... def make_layer() -> finn.types.LayerDataTuple:
    ...     return (np.ones((10,10)), {'name': 'hi'})

    >>> @magicgui
    ... def make_layer() -> List[finn.types.LayerDataTuple]:
    ...     return [(np.ones((10,10)), {'name': 'hi'})]
...
def add_worker_data(
    ...
    widget : MagicGui
        ...
        dock widget.
    worker : WorkerBase
        An instance of `finn._qt.qthreading.WorkerBase`, on worker.returned,
...
def add_future_data(gui, future: Future, return_type, _from_tuple=True):
    ...
    _from_tuple : bool, optional
        ...
        False if it returns one of the `LayerData` types.
    """
    from finn._qt._qapp_model.injection._qprocessors import _add_future_data
...
def find_viewer_ancestor(widget) -> Optional[Viewer]:
    ...
    `finn.current_viewer()` is called for Widgets without a
    Viewer ancestor.
    ...
    Returns
    -------
    viewer : finn.Viewer or None
        Viewer ancestor if it exists, else `finn.current_viewer()`
    """
    ...
    from finn.viewer import current_viewer
...
def get_layers(gui: CategoricalWidget) -> list[Layer]:
    ...
    >>> @magicgui
    ... def get_layer_mean(layer: finn.layers.Image) -> float:
...
def get_layers_data(gui: CategoricalWidget) -> list[tuple[str, Any]]:
    ...
    >>> @magicgui
    ... def get_layer_mean(data: finn.types.ImageData) -> float:
    ...     return data.mean()
...
def add_layer_to_viewer(gui, result: Any, return_type: type[Layer]) -> None:
    ...
    >>> @magicgui
    ... def make_layer() -> finn.layers.Image:
    ...     return finn.layers.Image(np.random.rand(64, 64))
...
def add_layers_to_viewer(
    ...
    >>> @magicgui
    ... def make_layer() -> List[finn.layers.Layer]:
    ...     return finn.layers.Image(np.random.rand(64, 64))
