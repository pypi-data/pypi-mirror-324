...
def _handle_array(colors: np.ndarray) -> np.ndarray:
    ...
    if kind == 'O':
        warnings.warn(
            trans._(
                'An object array was passed as the color input. Please convert its datatype before sending it to finn. Converting input to a white color array.',
                deferred=True,
            )
        )
        return np.ones((max(len(colors), 1), 4), dtype=np.float32)
    ...
    # that it wants the "default" colors of finn. We return
...
def get_color_namelist():
    """Gets all the color names supported by finn.

    Returns
    -------
    list[str]
        All the color names supported by finn.
    """
