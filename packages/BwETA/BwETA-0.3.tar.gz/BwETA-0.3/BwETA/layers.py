...
class TransformerBlock_py(layers.Layer):
    def __init__(self, num_heads, attention_dim, op_dim,ff_dim=512, dropout_rate=0.1,**kwargs):
        super(TransformerBlock_py, self).__init__(**kwargs)
        ...
        self.projection = DimExpansionProjection(attention_dim)


    def call(self, inputs, mask):
        inputs_proj = self.projection(inputs)
        attn_output = self.attention(inputs_proj, inputs_proj, inputs_proj, attention_mask=mask,use_causal_mask=True)
        attn_output = self.dropout1(attn_output)
        out1 = self.norm1(inputs_proj + attn_output)  # Add & Norm

        # Apply Feed-Forward Network
        ffn_output = self.ffn1(out1)
        ffn_output = self.ffn2(ffn_output)
        ffn_output = self.dropout2(ffn_output)
        out2 = self.norm2(ffn_output + out1)  # Add & Norm
...
class TransformerBlock(layers.Layer):
    ...
    def call(self, inputs, mask):
        attn_output = self.attention(inputs, inputs, inputs, attention_mask=mask,use_causal_mask=True)
        attn_output = self.dropout1(attn_output)
...
class DimExpansionProjection(layers.Layer):
    def __init__(self, output_dim, **kwargs):
        super().__init__(**kwargs)
        self.output_dim = output_dim  # Target dimension after projection

    def build(self, input_shape):
        # Dynamically get input dimension from previous layer's output shape
        input_dim = input_shape[-1]
        
        # Trainable projection matrix (input_dim -> output_dim)
        self.projection = self.add_weight(
            shape=(input_dim, self.output_dim),
            initializer="orthogonal",  # Stabilizes training
            trainable=True  # Layer will learn normally via backprop
        )
