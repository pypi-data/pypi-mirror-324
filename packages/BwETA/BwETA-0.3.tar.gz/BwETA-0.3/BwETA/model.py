...
class TransformerModel(tf.keras.Model):
    def __init__(self, num_heads=8, attention_dim=512, vocab_size=50259,num_blocks=24, ff_dim=4096, dropout_rate=0.2,build=True,**kwargs):
...
class TransformerModel_pyramidal(tf.keras.Model):
    def __init__(self,config_list = [(8, 512, 4096, 0.2),(8, 512, 4096, 0.2)],pos_dim = 512,vocab_size=50259,build=True,**kwargs):
        super(TransformerModel_pyramidal, self).__init__(**kwargs)
        self.loss_fn = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
        self.config_list = config_list
        self.embedding = layers.Embedding(input_dim=vocab_size, output_dim=pos_dim)
        self.positional_encoding = PositionalEncoding(pos_dim)
        self.blocks = [TransformerBlock_py(attr[0], attr[1], attr[2], attr[3]) for attr in config_list]
        self.final_layer = layers.Dense(vocab_size)  # Output layer for classification (adjust output size accordingly)
        if build == True:
            self.build_custom()
