...
#     nodes = torch.tensor(nodes, device=GLOBAL_DTYPE, dtype=GLOBAL_DEVICE)
#     weights = torch.tensor(weights, device=GLOBAL_DEVICE, dtype=GLOBAL_DTYPE)
...
def gauss_integrate(func,params,bounds,num_points,boundaries):
    ...
    #     grids, weights = [], []
    #     for i, (bound, num_point) in enumerate(zip(bounds, num_points)):
    #         grid, weight = gauss_legendre_nodes_weights(num_point).to('cuda')
    ...
    if params is not None:
        ...
        for i, (bound, num_point) in enumerate(zip(bounds, num_points)):
            ...
            grid = torch.tensor(grid, device=GLOBAL_DEVICE, dtype=GLOBAL_DTYPE )
            weight = torch.tensor(weight, device=GLOBAL_DEVICE, dtype=GLOBAL_DTYPE)
            grid = 0.5 * (bound[1] - bound[0]) * grid + 0.5 * (bound[1] + bound[0])
            weight = 0.5 * (bound[1] - bound[0]) * weight
            grids.append(grid)
            weights.append(weight)
