...
def easyspec_cli():
    ...
    if ipython_available():
        ...
        header_initialize = 'print_UI(__package__)'
...
def print_UI(package_name):
    
    # Fetch pyproject header section
    header_data = globals()['scope_connector']['header']
    ...
    header(brand_name, slogan, creators, terminal_width)
    print_tag_list([license_tag, OS_tag, python_tag], offset = [0,0], color_hex = '#FF9C80', spacing = 6, alignment= 'center')    
    navbar(easyspec_commands)
    tag(f"{os.getcwd()}", offset = [0,4], color_hex='#E0E0E0')    

def header(brand_name, slogan, creators, header_width = os.get_terminal_size().columns):
...
def padded_print(text, padding=4):
        pad = " " * padding
        modified_text = [f"{pad}{text}"]
        print(modified_text)