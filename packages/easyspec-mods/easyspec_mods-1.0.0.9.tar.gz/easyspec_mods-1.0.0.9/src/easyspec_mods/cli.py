...
# Scope connector allows passing some variables by reference to Python's interactable mode environment.
scope_connector = {
                    'selected_module':{},
                    'header':{},
                    'modules':{},
...
def easyspec_cli():
    ...
    if ipython_available():
        ...
        header_initialize = 'print_header(__package__)'
        imports = 'from easyspec_mods.cli import *'
        setup_commands = [imports, fullscr, autocall, prompt_settings, header_initialize]
...
def load_meta_file(meta_file_name):
    ...
    with open(modules_path, 'r') as f:
        global scope_connector
        scope_connector[f"{meta_file_name}"] = json.load(f)

def modules():
    resetUI()
    display_modules(scope_connector['modules'])
...
def help():
    resetUI()
    global scope_connector
    display_help(scope_connector['help'])
globals()['h'] = help

def procedures():
    resetUI()
    print_procedures(globals())
globals()['p'] = procedures
...
def print_tag(data_string, start_position=None, color_hex=None):
    ...
    tag_width = len(data_string) + 2 * len(horizontal_padding)
    tag_height = 1
    right_vertical_bars = ['\u23B9']
    left_vertical_bars = ['\u23B8']
    ...
    # Print the tag with the specified color
...
def print_tag_list(tag_list, start_position = [0,0], color_hex=None):
    if tag_list:
        tag_string = tag_list.pop()
        print_tag(tag_string, start_position, color_hex)
        start_position[1] += len(tag_string) + 6
        print_tag_list(tag_list, start_position, color_hex)    
    return

def print_app_name(app_name, color_hex = None):
...
def print_header(package_name):
    
    # Fetch pyproject header section
    header = globals()['scope_connector']['header']
    brand_name = header['brand_name']
    slogan = header['slogan']
    creators = header['creators']

    # Fetch pyproject project section
    project_section = metadata(package_name)
    ...
    easyspec_commands = {
                    'new project' : 'initializes a new project directory',
                    'open' : 'working directory selection',
    ...
    #Defining tags contents
    python_tag = f"Python {python_version}" 
    license_tag = f"{project_section.get("License", "-").strip().upper()}" 
    OS_tag = f"{platform.platform()}".strip().upper() 

    print("\033[H\033[J\033[3J", end="")  # Clear screen and scrollback buffer
    # print_app_name(package_name, '#1B3B6F')
    print_app_name(brand_name, '#1B3B6F')
    print()
    ...
    print()
    print_navbar(easyspec_commands)
    print_tag_list([license_tag, OS_tag, python_tag], get_cursor_position(), '#FF9C80')    
    # Current directory
    print_tag(f"{os.getcwd()}", color_hex='#E0E0E0')    

def module_selection(global_scope):
    print(f"\nTo leave the module selector, press enter with the field empty.")
    ...
    try:
        ...
        global_scope['scope_connector']['selected_module'] = {'module_name':module_name, 'module_version':module.__version__, 'objects':imported_objects }
...
def print_procedures(global_scope):
    ...
    module_selected = global_scope['scope_connector']['selected_module']
    if len(module_selected) == 0:
        print(f'''{' '*4}No module selected.''')
        return 
...
def print_navbar(commands):
    ...
    fillers = ['░', '█']
    decoration_left = f"{fillers[0]*4}"
    title = f"{' '*1}easySpec {pkg_version}{' '*6}"
    # Format the command list
    print(f"{'_' * header_width}".center(header_width))
    command_line = '| ' + ' | '.join(commands.keys()).upper() + ' |'
...
def print_elegant_json(data, indent=2, width=os.get_terminal_size().columns, level=2):
    """
...
def change_working_directory_explorer():
    ...
    if new_dir:
        ...
        root.destroy() 
globals()['o'] = change_working_directory_explorer
...
class MyPrompt(Prompts):
    ...
    def in_prompt_tokens(self, cli=None):
        return [
            ...
            (Token.Prompt, f"{' '*1}{self.text}:"),
            
        ]

def set_ipython_prompt(text):
    ip = get_ipython()
    if ip is None:
