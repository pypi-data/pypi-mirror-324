...
def data_paths(bias=None,flats=None,lamp=None,standard_star=None,targets=None,darks=None):
    ...
    all_images_list = []  # Master list with the paths to all data files

    if bias is not None:
        if Path(bias).is_dir():
            # Make a list with the bias files:
            bias_list = np.sort(glob.glob(str(Path(bias).resolve())+'/*.fit*')).tolist()
            all_images_list = all_images_list + bias_list
        else:
    ...
    if flats is not None:
        if Path(flats).is_dir():
            ...
            flat_list = np.sort(glob.glob(str(Path(flats).resolve())+'/*.fit*')).tolist()
            all_images_list = all_images_list + flat_list
        else:
    ...
    if lamp is not None:
        if Path(lamp).is_dir():
            ...
            lamp_list = np.sort(glob.glob(str(Path(lamp).resolve())+'/*.fit*')).tolist()
            all_images_list = all_images_list + lamp_list
        else:
    ...
    if standard_star is not None:
        if Path(standard_star).is_dir():
            ...
            std_list = np.sort(glob.glob(str(Path(standard_star).resolve())+'/*.fit*')).tolist()
            all_images_list = all_images_list + std_list
        else:
    ...
    if targets is not None:
        if isinstance(targets,str):
            if Path(targets).is_dir():
                ...
                target_list = np.sort(glob.glob(str(Path(targets).resolve())+'/*.fit*')).tolist()
                all_images_list = all_images_list + target_list
            else:
            ...
            for target in targets:
                if Path(target).is_dir():
                    # Make a list with the raw science images:
                    target_list = target_list + np.sort(glob.glob(str(Path(target).resolve())+'/*.fit*')).tolist()
                else:
                    raise TypeError("Invalid diretory for target files.")
    ...
    if darks is not None:
        if Path(darks).is_dir():
            ...
            darks_list = np.sort(glob.glob(str(Path(darks).resolve())+'/*.fit*')).tolist()
            all_images_list = all_images_list + darks_list
        else:
...
def trim(raw_image_data,x1,x2,y1,y2):
...
def plot_images(datacube,image_type="all", Ncols = 2, specific_image=None, vmin=None, vmax=None, vertical_scale=None):
    ...
    if image_type == "all":
        ...
        imagenames = bias_list
    elif image_type == "flat":
        imagenames = flat_list
    elif image_type == "lamp":
        imagenames = lamp_list
    elif image_type == "standard_star":
        imagenames = std_list
    elif image_type == "target":
        imagenames = target_list
    elif image_type == "dark":
        imagenames = darks_list
    else:
...
def master(Type,trimmed_data,method="median",header_hdu_entry=0,exposure_header_entry=None,airmass_header_entry=None,plot=True):
    ...
    for n in range(len(data_splitter[:-1])):
        ...
        if exposure_header_entry is not None and allow_exp_and_airmass:
            ...
            for file in data_list[data_splitter[n]:data_splitter[n+1]]:
                list_of_exposure_times.append(look_in_header(file,exposure_header_entry))
        ...
        if airmass_header_entry is not None and allow_exp_and_airmass:
            list_of_airmasses = []
            for file in data_list[data_splitter[n]:data_splitter[n+1]]:
                list_of_airmasses.append(look_in_header(file,airmass_header_entry))
...
def debias( trimmed_data, masterbias, Type="all", pad_with_zeros=True):
    ...
    for file_name in trimmed_data.keys():
        if Type == "all" and file_name not in bias_list:
            debias_list = debias_list + [file_name]
        elif Type == "flat" and file_name in flat_list:
            debias_list = debias_list + [file_name]
        elif Type == "lamp" and file_name in lamp_list:
            debias_list = debias_list + [file_name]
        elif Type == "standard_star" and file_name in std_list:
            debias_list = debias_list + [file_name]
        elif Type == "target" and file_name in target_list:
            debias_list = debias_list + [file_name]
        elif Type == "dark" and file_name in darks_list:
            debias_list = debias_list + [file_name]
...
def sub_dark( debiased_data, masterdark, Type="all", pad_with_zeros=True):
    ...
    for file_name in debiased_data.keys():
        if Type == "all" and file_name not in darks_list:
...
def norm_master_flat(masterflat, method = "polynomial", degree=5, smooth_window = 101, header_hdu_entry=0,plots=True):
    ...
    hdr = fits.getheader(flat_list[0],ext=header_hdu_entry)
...
def rotate(raw_image_data,N_rotations=1):
...
def pad(image_data,value,x1,x2,y1,y2,plot=True, save_fits=False, get_header_from=None, fits_name="padded_image.fits", header_hdu_entry=0):
...
def plot(image_data,figure_name,save=True,format="png"):
...
def flatten(debiased_data,norm_master_flat,Type="all"):
    ...
    flatten_list = []
    for file_name in debiased_data.keys():
        if Type == "all" and file_name not in bias_list and file_name not in darks_list and file_name not in flat_list:
...
def look_in_header(file_name,I_am_looking_for):
...
def CR_and_gain_corrections(flattened_debiased_data,gain=None,gain_header_entry="GAIN",readnoise=None,readnoise_header_entry="RDNOISE",Type="all",sigclip=5):
    ...
    for file_name in rm_cr_list:
        if readnoise is None:
            readnoise = look_in_header(file_name,readnoise_header_entry)
        if gain is None:
            gain = look_in_header(file_name,gain_header_entry)
...
def save_fits_files(datacube,output_directory,header_hdu_entry=0,Type="all"):
...
def vertical_align( CR_corrected_data, Type="all"):
    ...
    for n in range(len(data_splitter[:-1])): 
        ...
        for file_name in align_list[data_splitter[n]:data_splitter[n+1]]:
            ...
            if difference_y < 0:
                ...
                temporary_image = trim({'temporary' : temporary_image},x1=0,x2=np.shape(CR_corrected_data[file_name])[1],y1=difference_y,y2=np.shape(CR_corrected_data[file_name])[0]+difference_y)
                ...
                temporary_image = trim({'temporary' : temporary_image},x1=0,x2=np.shape(CR_corrected_data[file_name])[1],y1=0,y2=np.shape(CR_corrected_data[file_name])[0])
                CR_corrected_data[file_name] = temporary_image['temporary']  # Remember that the function trim above returns a dictionary.
    ...
    aligned_data = trim(CR_corrected_data,x1=0,x2=np.shape(CR_corrected_data[file_name])[1],y1=maximum_y1_cut,y2=np.shape(CR_corrected_data[file_name])[0]+maximum_y2_cut)  # Remember that maximum_y2_cut is negative
