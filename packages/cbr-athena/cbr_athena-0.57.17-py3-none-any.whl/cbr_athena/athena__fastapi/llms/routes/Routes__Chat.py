...
from osbot_utils.utils.Json                                             import json_parse, json_dump
...
class Routes__Chat(Fast_API_Routes):
    ...
    async def proxy_stream(self, url: str, json_payload: dict, headers: dict=None):
        async with httpx.AsyncClient() as client:
            try:
                ...
                yield f"(in proxy_stream) Error: {error}"


    async def completion_proxy(self, llm_chat_completion: LLMs__Chat_Completion, request: Request):
        llm_chat_completion.temperature = 0.0
        request_body = llm_chat_completion.model_dump()

        backend_url = "https://osbot-llms.dev.aws.cyber-boardroom.com/chat/completion"

        response = StreamingResponse(self.proxy_stream(backend_url, request_body, headers=request.headers),
                                     media_type="text/plain")
        response.headers['completion_proxy_id'] = "xxxx-yyyy-" + Random_Guid()
        return response

    # todo: refactor this method into a new Routes class (one focused on viewing chat threads)
    async def view(self, chat_id: str):
        s3_db_chat_threads = cbr_shared_objects.s3_db_chat_threads()
        server_name        = s3_db_chat_threads.server_name
        s3_key = f'chat-threads/{server_name}/{chat_id}/user-response.json.gz'
        return cbr_shared_objects.s3_db_chat_threads().s3_file_data(s3_key)
    ...
    def setup_routes(self):
        self.router.post("/completion_proxy")(self.completion_proxy)
