...
class CBR__Chats_Storage__S3(Type_Safe):
    ...
    def save_user_request(self, llm_chat_completion, request_id):
        ...
        result = cbr_shared_objects.s3_db_chat_threads().save_chat_completion__user_request(llm_chat_completion, request_id)
        if result.get('status') == 'ok':
            return result.get('data')
        return {}

    def save_user_response(self, llm_chat_completion, request_id):
        result =  self.s3_db_chat_threads().save_chat_completion__user_response(llm_chat_completion,request_id)
        if result.get('status') == 'ok':
            return result.get('data')
        return {}
