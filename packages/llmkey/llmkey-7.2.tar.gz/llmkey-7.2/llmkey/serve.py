...
from . import (config, gui_first_run, gui_menu, gui_prompt, gui_reply,
               gui_settings, gui_status, gui_tray, hotkeys, llm, runner,
               tk_tools)
...
class TkCallbacks:
    ...
    def one_off_finished(self, event):
        if self.query:
            ...
            window = gui_reply.reply(self.bus, self.query.duration, event.data["result"])
            self.query = None
    ...
    def close_reply(self, event):
        id_ = event.data["id"]
        (i, window), = [(i, w) for i, w in enumerate(self.reply_windows) if w.id == id_]
        if window:
            window.destroy()
    ...
    def cycle_replies(self, event):
        ...
        if self.display_window:
            LOGGER.info("Raising window: %r", self.display_window.id)
...
def main():
    ...
    llm_runner = runner.LlmRunner(bus)
