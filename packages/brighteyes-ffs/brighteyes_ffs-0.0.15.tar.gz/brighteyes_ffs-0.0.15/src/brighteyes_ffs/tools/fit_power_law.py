...
"""
List of function to fit a powerlaw function. Contains also some other fit
functions such as fitting an exponential function. These will be moved to a
...
def fit_power_law_g(G, start=0, plotIndv=True, fitInfo=np.array([1, 1, 0]), param=np.array([3.1, -1, 0])):
    ...
    for i in range(len(Gkeys)):
        ...
        y = Gtemp[start:,1]
        x = Gtemp[start:,0]
        fitresult = fit_power_law(y, x, 'powerlaw', fitInfo, param, np.array([0, -10, -10]), np.array([100, 0, 100]), 0)
...
def fit_power_law(y, x, fitfun, fitInfo, param, lBounds, uBounds, savefig=0):
    """
