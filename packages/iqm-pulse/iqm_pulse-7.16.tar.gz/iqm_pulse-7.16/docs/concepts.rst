...
Importantly, if some TimeBoxes have content on disjoint channels, the Schedules are allowed to happen simulteneously.
...
constraints.
In case the whole Playlist does not fit in memeory, the segments are split into chunks which are executed separately.
