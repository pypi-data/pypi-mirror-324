...
try:
    ...
    from cupyx.scipy.ndimage import binary_closing as scipy_binary_closing_gpu
    from cupyx.scipy.ndimage import binary_erosion as scipy_binary_erosion_gpu
    ...
    from scipy.ndimage import binary_closing as scipy_binary_closing_cpu
    from scipy.ndimage import binary_erosion as scipy_binary_erosion_cpu
...
def scipy_binary_closing(input: np.ndarray, structure: np.ndarray = None, iterations: int = 1, use_gpu: bool = True) -> np.ndarray:
    ...
    if not use_gpu or not cupy_available:
        ...
        input = np.asarray(input)
        structure = np.asarray(structure) if structure is not None else None
        return scipy_binary_closing_cpu(input, structure, iterations)
    else:
        input = cp.asarray(input)
        structure = cp.asarray(structure) if structure is not None else None
        result = scipy_binary_closing_gpu(input, structure, iterations)
        return _ensure_numpy(result)
...
def scipy_binary_erosion(input: np.ndarray, structure: np.ndarray = None, iterations: int = 1, use_gpu: bool = True) -> np.ndarray:
    ...
    if not use_gpu or not cupy_available:
        ...
        input = np.asarray(input)
        structure = np.asarray(structure) if structure is not None else None
        return scipy_binary_erosion_cpu(input, structure, iterations)
    else:
        input = cp.asarray(input)
        structure = cp.asarray(structure) if structure is not None else None
        result = scipy_binary_erosion_gpu(input, structure, iterations)
        return _ensure_numpy(result)
