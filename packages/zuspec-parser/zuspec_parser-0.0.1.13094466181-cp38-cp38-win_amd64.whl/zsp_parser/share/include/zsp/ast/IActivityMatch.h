...
typedef std::shared_ptr<IActivityMatchChoice> IActivityMatchChoiceSP;
class IActivityMatch;
using IActivityMatchUP=UP<IActivityMatch>;
class IActivityMatch : public virtual IActivityLabeledStmt {
public:
    
    virtual ~IActivityMatch() { }
    ...
    virtual const std::vector<IActivityMatchChoiceUP> &getChoices() const = 0;
    
    virtual std::vector<IActivityMatchChoiceUP> &getChoices() = 0;
};
