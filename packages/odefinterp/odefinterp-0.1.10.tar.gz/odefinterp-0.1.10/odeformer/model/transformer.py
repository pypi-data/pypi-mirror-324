...
class TransformerModel(nn.Module):
    ...
    def fwd(
        ...
        for i in range(self.n_layers):
            ...
            if self.is_decoder:
                self.stored_attn_weights["decoder"].append(attn_weights.detach().cpu())
            else:
                self.stored_attn_weights["encoder"].append(attn_weights.detach().cpu())
            ...
            if self.is_decoder and src_enc is not None and self.use_cross_attention: 
                ...
                self.stored_attn_weights["cross_attention"].append(cross_attn_weights.detach().cpu())
