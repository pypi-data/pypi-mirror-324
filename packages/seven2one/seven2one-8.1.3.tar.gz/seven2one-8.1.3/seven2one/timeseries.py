...
class TimeSeries():
    ...
    def addTimeSeriesItems(
        ...
        with logger.contextualize(correlation_id=correlationId):
            if len(timeSeriesItems) > chunkSize:
                ...
                result = self.__addTimeSeriesItems(inventoryName, timeSeriesItems)
    ...
    def timeSeriesData(
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            while True:
                ...
                if _result == None:
                    Utils._error(self, f"GrapQL query resulted in null value")
                    return
    ...
    def timeSeriesGroupData(
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            if fields != None:
                ...
                _fields = Utils._queryFields(fields, propertyDict['arrayTypeFields'], 200,
                                             filter=resolvedFilterDict, recursive=True)
                if 'pageSize' in _fields:
                    Utils._error(
                        self, f"Array type fields are not supported in 'fields'-argument ({_fields})")
                ...
                if self.structure[inventoryName]['displayValue'] != None and self.structure[inventoryName]['displayValue'] != '':
                    fields = ['sys_displayValue', 'sys_inventoryItemId']
                    deleteIdAfterwards = True
                else:
                    fields = ['sys_inventoryItemId']
            ...
            instanceWhere, resolvedFilterDict = Utils._handleWhere(
                core, whereInstance, instanceInventoryName)
            ...
            if instanceFields != None:
                ...
                if 'sys_inventoryItemId' not in instanceFields:
                    ...
                    deleteIdAfterwards = True
                _fields = Utils._queryFields(instanceFields, propertyDict['arrayTypeFields'], 200,
                                            filter=resolvedFilterDict, recursive=True)
                if 'pageSize' in _fields:
                    Utils._error(
                        self, f"Array type fields are not supported in 'fields'-argument ({_fields})")
                ...
                if self.structure[instanceInventoryName]['displayValue'] is not None and self.structure[instanceInventoryName]['displayValue'] != '':
                    instanceFields = [
            ...
            try:
                ...
                Utils._error(
                    self, f"There is a problem with the provided fields. Probably one or more fields have no values. Exception: {err}")
                return
    ...
    def deleteItems(
        ...
        correlationId = str(uuid4())
        with logger.contextualize(correlation_id=correlationId):
            inventory = core.inventories(where=f'name eq "{inventoryName}"', fields=['variant.name'])
            if inventory.empty:
                Utils._error(self, f"Unknown inventory '{inventoryName}'.")
                return
            if not core._isInventoryOfValidVariant(inventoryName, 'TimeSeries'):
                Utils._error(self, f"Items of a non TimeSeries inventory cannot be deleted.")
                return
    ...
    def items(
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            try:
                ...
                Utils._error(self, f"where filter '{where}' does not return data.")
                return
