...
if gpus and physical_devices:
    for gpu in gpus:
        logger.debug(f"Gpu name: {gpu.name}")
    print("Setting GPU Memory limit to 6000")
...
def get_gpu_strategy():
    try:
        ...
        physical_devices = tf.config.list_physical_devices("GPU")
        if gpus and physical_devices:
            gpu_strategy=tf.distribute.OneDeviceStrategy(gpus[0])
        else:
            logger.debug("No GPU found")
...
class MulticlassClassificationJupiterModel:
    ...
    async def inference(self,data:KeysWordsPhrases,model_name="jupiter")->Predictions:
        ...
        if not model_dir.exists():
            return    
        mirrored_strategy = get_gpu_strategy()
        with mirrored_strategy.scope():
            try:
                # Précision mixte
