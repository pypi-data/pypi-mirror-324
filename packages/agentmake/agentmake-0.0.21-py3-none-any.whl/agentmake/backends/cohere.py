...
class CohereLLM:
    ...
    def getChatCompletion(
        ...
        if not api_key and not CohereLLM.DEFAULT_API_KEY:
            raise ValueError("API key is required.")
        #if prefill:
        #    messages.append({'role': 'assistant', 'content': prefill})
        client = cohere.ClientV2(api_key=api_key if api_key else CohereLLM.DEFAULT_API_KEY)
        func = client.chat_stream if stream else client.chat
        return func(
            model=model if model else CohereLLM.DEFAULT_MODEL,
            messages=messages,
            temperature=temperature if temperature is not None else CohereLLM.DEFAULT_TEMPERATURE,
            max_tokens=max_tokens if max_tokens else CohereLLM.DEFAULT_MAX_TOKENS,
