...
def similarity(X, Y, method):
    """
    Computes similarity matrices between two matrices X and Y using the specified method.
    ...
    Input:
        X       : N1 x M matrix
        Y       : N2 x M matrix
        method  : string defining one of the following similarity measures
    ...
    if method == "smc":  # SMC
        ...
        XYt = X * Y.T
        sim = XYt / (
            np.log(np.exp(sum(np.power(X.T, 2))).T * np.exp(sum(np.power(Y.T, 2))))
            - XYt
        )
    elif method == "cos":  # Cosine
        sim = (X * Y.T) / (
            np.sqrt(sum(np.power(X.T, 2))).T * np.sqrt(sum(np.power(Y.T, 2)))
        )
...
def binarize2(X, columnnames):
    """
    Binarizes the input array X and creates new column names based on the original column names.

    Parameters:
    X (numpy.ndarray): Input array to be binarized.
    columnnames (list): List of column names.

    Returns:
    numpy.ndarray: Binarized array with additional columns.
    ...
    new_column_names = []
    [
        new_column_names.append(elm)
        for elm in [name + " 50th-100th percentile" for name in columnnames]
    ]
    [
        new_column_names.append(elm)
        for elm in [name + " 0th-50th percentile" for name in columnnames]
    ]
