...
async def main():
    try:
        ...
        await restack.schedule_workflow(workflow_name="playgroundRun", workflow_id=f"{random.randint(0, 1000000)}-local-test-1", input=PlaygroundInput(functionName="testRun", taskQueue="restack"))

        log_with_context("INFO", "Scheduling workflow")

        await restack.start_service(workflows=[playgroundRun], options=ServiceOptions(endpoints=True))
