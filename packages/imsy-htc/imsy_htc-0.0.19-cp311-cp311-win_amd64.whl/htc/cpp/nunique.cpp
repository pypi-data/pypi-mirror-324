...
torch::Tensor nunique(const torch::Tensor& in, int64_t dim) {
    ...
    auto iter = torch::native::make_reduction(/*name=*/"nunique",
                                              out,
                                              in,
                                              dim,
                                              /*keepdim=*/false,
                                              /*in_dtype=*/in.scalar_type(),
                                              /*out_dtype=*/torch::kInt64);

    // AT_DISPATCH_ALL_TYPES automatically templates all versions of this function and chooses the right one based on
    // the tensor type (https://github.com/pytorch/pytorch/blob/master/aten/src/ATen/Dispatch.h) scalar_t is the type of
    // the tensor See also this example:
    // https://github.com/pytorch/pytorch/blob/96c549d1c62462a74a41c99552e8f8a37aaa0793/aten/src/ATen/native/Sorting.cpp#L318
    AT_DISPATCH_ALL_TYPES(in.scalar_type(), /*name=*/"nunique", [&] {
        ...
        auto loop = [](char** data, const int64_t* strides, int64_t n) {
            ...
            // We use a simple set to find the unique elements in the reduce dimension (it turns out to be quite fast:
            // https://stackoverflow.com/a/24477023/2762258)
