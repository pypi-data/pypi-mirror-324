...
class ColorcheckerAutomask {
   public:
    ColorcheckerAutomask(const at::Tensor& rot_image,
                         std::string cc_board,
                         int square_size,
                         int safety_margin,
        ...
        : rot_image(rot_image),
          cc_board(std::move(cc_board)),
          square_size(square_size),
          safety_margin(safety_margin),
          square_dist_vertical(square_dist_vertical),
          square_dist_horizontal(square_dist_horizontal) {
    ...
    MaskParametersDict find_mask() {
        ...
        auto score_values =
            at::empty({static_cast<int64_t>(this->parameters.size()), n_rows * n_cols}, torch::kFloat32);
        ...
        masks["mask_0"] = {
            ...
            {"square_dist_horizontal",
             this->square_dist_horizontal + this->safety_margin + best_param.delta_horizontal},
        ...
        if (this->cc_board == "cc_passport") {
            ...
            this->generate_parameters(/*offset_left_min_start=*/this->img_width / 2,
                                      /*offset_left_stop=*/this->img_width);
            ...
            for (int row = 0; row < n_rows; row++) {
                for (int col = 0; col < n_cols; col++) {
                    for (size_t i = 0; i < this->parameters.size(); i++) {
                        ...
                        // Global coordinates
                        int chip_row = p.offset_top + row * (this->square_size_expanded + this->square_dist_vertical +
                                                             p.delta_vertical);
                        int chip_col = p.offset_left + col * (this->square_size_expanded +
                                                              this->square_dist_horizontal + p.delta_horizontal);
   ...
   private:
    at::Tensor compute_chip_scores(int row, int col) {
        ...
        auto chip_row_min = this->offset_top_min +
                            row * (this->square_size_expanded + this->square_dist_vertical + this->delta_vertical_min);
        auto chip_row_max = this->offset_top_max +
                            row * (this->square_size_expanded + this->square_dist_vertical + this->delta_vertical_max);
        auto chip_col_min = this->offset_left_min + col * (this->square_size_expanded + this->square_dist_horizontal +
                                                           this->delta_horizontal_min);
        auto chip_col_max = this->offset_left_max + col * (this->square_size_expanded + this->square_dist_horizontal +
                                                           this->delta_horizontal_max);
        ...
        // Note: we do not compute the standard deviation since we are only interested in the minimum and hence don't
        // need the sqrt
        auto chip_area_squared = chip_area.pow(2);
        auto variance = at::avg_pool2d(chip_area_squared, /*kernel_size=*/this->square_size_expanded, /*stride=*/1) -
                        at::avg_pool2d(chip_area, /*kernel_size=*/this->square_size_expanded, /*stride=*/1).pow(2);
        ...
        for (size_t i = 0; i < this->parameters.size(); i++) {
            ...
            // We compute the index of the chip first on the global level and then transfer it to the local coordinate
            // system of the search area/variance (e.g. chip_row_min)
            int chip_row = p.offset_top +
                           row * (this->square_size_expanded + this->square_dist_vertical + p.delta_vertical) -
                           chip_row_min;
            int chip_col = p.offset_left +
                           col * (this->square_size_expanded + this->square_dist_horizontal + p.delta_horizontal) -
                           chip_col_min;
    ...
    void generate_parameters(int offset_left_min_start, int offset_left_stop = -1) {
        ...
        if (this->cc_board == "cc_passport") {
            // The passport colorchecker contains one part on the left and one part on the right which are optimized
            // separately
            assert(offset_left_stop > 0);
            this->offset_left_max = offset_left_stop - 4 * this->square_size_expanded -
                                    3 * (this->square_dist_horizontal + this->delta_horizontal_max);
            this->offset_top_max = this->img_height - 6 * this->square_size_expanded -
                                   5 * (this->square_dist_vertical + this->delta_vertical_max);
        } else {
            // The offsets iterate over the complete image dimensions minus the colorchecker mask region
            this->offset_left_max = this->img_width - 6 * this->square_size_expanded -
                                    5 * (this->square_dist_horizontal + this->delta_horizontal_max);
            this->offset_top_max = this->img_height - 4 * this->square_size_expanded -
                                   3 * (this->square_dist_vertical + this->delta_vertical_max);
        }
        ...
        // We generate a vector with all the different parameter combinations and then later search for the combination
        // with the best score
        for (int delta_horizontal = this->delta_horizontal_min; delta_horizontal <= this->delta_vertical_max;
             delta_horizontal++) {
            for (int delta_vertical = this->delta_vertical_min; delta_vertical <= this->delta_horizontal_max;
                 delta_vertical++) {
...
MaskParametersDict colorchecker_automask(const at::Tensor& rot_image,
                                         const std::string& cc_board,
                                         int square_size,
                                         int safety_margin,
                                         int square_dist_vertical,
                                         int square_dist_horizontal) {
    return ColorcheckerAutomask(rot_image,
                                cc_board,
                                square_size,
                                safety_margin,
                                square_dist_vertical,
                                square_dist_horizontal)
        .find_mask();
}
at::Tensor colorchecker_automask_search_area(const at::Tensor& rot_image,
                                             const std::string& cc_board,
                                             int square_size,
                                             int safety_margin,
                                             int square_dist_vertical,
