...
torch::Tensor map_label_image(const torch::Tensor& label_image,
                              std::unordered_map<int64_t, std::tuple<float, float, float, float>>& label_color_mapping);
std::map<std::string, std::map<std::string, int64_t>> colorchecker_automask(const at::Tensor& rot_image,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("segmentation_mask",
          &segmentation_mask,
          "Creates a segmentation mask based on a label image and a corresponding color mapping.");
    ...
    m.def("hierarchical_bootstrapping_labels",
          &hierarchical_bootstrapping_labels,
          "Create hierarchical bootstrapping combinations including label information.");
    m.def("colorchecker_automask", &colorchecker_automask, "Automatically find a mask for colorchecker images.");
    m.def("colorchecker_automask_search_area",
          &colorchecker_automask_search_area,
          "Visualize the search area of the colorchecker automask.");
}
