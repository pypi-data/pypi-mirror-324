...
class MechanicalTestFittingLinear():
    ...
    def __cropStrainData(self, standardName, strain_data, stress_data):
        ...
        if standardName not in accepted_standards:
            ...
            warnings.warn('Norma ainda não aceita. Voltando dados originais')


        if standardName == 'standard-ASTM-D7078': # Cisalhamento
            range_ = 0.10 # 10% do valor máximo (em termos de deformação  gamma = Epsilon_12 + Epsilon_21))
    ...
    def poisson_computation_procedure(self, scale_find_linear:int ,
        ...
        axial_strain =  np.abs(self.cleaned_raw_data[machineConfig.colunas[strain_parallel_column]]) # pegando dados originais de deformação, já limpos, rever, porque não faz sentido
