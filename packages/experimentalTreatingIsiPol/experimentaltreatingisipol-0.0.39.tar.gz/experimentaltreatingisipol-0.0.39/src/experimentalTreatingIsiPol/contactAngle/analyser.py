...
class ContactAngleArqConfig():
    '''
    Classe usada para tipar o tipo de dado proveniente das colunas do excel do ângulo de contatp
    '''
    def __init__(self):
...
class Analyser():
    ...
    def mean_filter(self, **kwargs):
        ...
        if kwargs.get('window_size'):
            ...
            window_size = 50
        self.smoothed_data[self.arq_columns.CA_MEAN] = self.data[self.arq_columns.CA_MEAN].rolling(window=window_size, center=True).mean()
        ...
        self.smoothed_data[self.arq_columns.TIME] = self.data[self.arq_columns.TIME].rolling(window=window_size, center=True).mean()
    ...
    def median_filter(self, **kwargs):
        ...
        self.smoothed_data[self.arq_columns.CA_MEAN] = self.data[self.arq_columns.CA_MEAN].rolling(window=window_size, center=True).median()
