...
def install_or_update_package(package_name, action="install"):
    ...
    try:
        ...
        print(f"{package_name} {action_str.lower()}ed successfully.")
        save_last_update_time()
    except subprocess.CalledProcessError as e:
        print(f"Failed to {action_str.lower()} {package_name}: {e}")
    except Exception as e:
        print(f"Unexpected error during {action_str.lower()} of {package_name}: {e}")

def main(package_name):
    """Check for updates and install or update the package as necessary."""
    ...
    if latest_version and installed_version:
        ...
        print(f"{package_name} is not installed. Proceeding with installation.")
