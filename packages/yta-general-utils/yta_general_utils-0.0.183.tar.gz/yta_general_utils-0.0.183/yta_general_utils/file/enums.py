...
class FileExtension(Enum):
    ...
    def is_filename_valid(
        ...
        return FileExtension.is_accepted_value(extension)
    ...
    def get_temp_filename(
        ...
        """
        filename = Temp.create_filename(f'{Random.characters()}') if filename is None else filename
        filename = filename.split('.')[0] if '.' in filename else filename
        filename += f'.{self.value}'

        return filename

class ImageFileExtension(FileExtension):
    """
...
class AudioFileExtension(FileExtension):
    """
...
class VideoFileExtension(FileExtension):
    """
...
class SubtitleFileExtension(FileExtension):
    """
...
class FileTypeX(Enum):
    ...
    def is_filename_valid(
        ...
        return self.get_file_extension_enum().is_accepted_value(extension)
    
    def get_file_extension_enum(
        self
    ...
    def get_default_file_extension(
        ...
        """
        return self.get_file_extension_enum().default()
    ...
    def get_temp_filename(self, filename: Union[str, None]):
        ...
        filename = Temp.create_filename(f'{Random.characters()}') if filename is None else filename
        filename = filename.split('.')[0] if '.' in filename else filename
        filename += f'.{self.get_default_file_extension().value}'

        return filename

# We dynamically create a combined FileExtension enum
# FileExtension = Enum(
#     'FileExtension',
