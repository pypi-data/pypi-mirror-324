...
def snake_case_to_upper_camel_case(snake_str: str):
    ...
    """
    if not PythonValidator.is_string(snake_str) or not GeneralRegularExpression.SNAKE_CASE.parse(snake_str):
...
def upper_camel_case_to_snake_case(upper_camel_case_str: str):
    ...
    """
    if not PythonValidator.is_string(upper_camel_case_str) or not GeneralRegularExpression.UPPER_CAMEL_CASE.parse(upper_camel_case_str):
...
def lower_camel_case_to_snake_case(lower_camel_case_str: str):
    ...
    (that would be, for the previous example, 'this_variable).
    """
    if not PythonValidator.is_string(lower_camel_case_str) or not GeneralRegularExpression.LOWER_CAMEL_CASE.parse(lower_camel_case_str):
        raise Exception('The provided "upper_camel_case_str" is not a valid string.')
