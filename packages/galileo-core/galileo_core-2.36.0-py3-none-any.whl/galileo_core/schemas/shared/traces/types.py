...
class SpanWithParentSteps(BaseStep):
...
class StepWithChildSpans(BaseStepWithChildren):
    spans: List["SpanWithParentSteps"] = Field(default_factory=list, description="Child spans.")

    def children(self) -> Sequence[BaseStep]:
        return self.spans

    def add_child(self, *spans: "SpanWithParentSteps") -> None:
        for span in spans:
            span.parent = self
    ...
    def conclude(
        ...
        if isinstance(self, SpanWithParentSteps):
            return self.parent
        return None
...
class WorkflowSpan(StepWithChildSpans, SpanWithParentSteps):
...
class ToolSpan(SpanWithParentSteps, ToolStep):
