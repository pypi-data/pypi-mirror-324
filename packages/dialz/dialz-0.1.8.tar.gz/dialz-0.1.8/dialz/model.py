...
class ControlModel(torch.nn.Module):
    ...
    **This mutates the wrapped `model`! Be careful using `model` after passing it to this class.**
    ...
    def get_activation_score(
        ...
        input_text: str,
        control_vector: "ControlVector",
        token_aggregate: Literal["mean", "sum"] = "mean",
        layer_aggregate: Literal["mean", "sum"] = "mean",
    ) -> float:
        """
        Measure alignment between `input_text` and the given `control_vector` across
        *all* layers that this model controls. (One forward pass, then we do
        a per-layer dot product and aggregate.)

        1) We call `model(..., output_hidden_states=True)`.
        2) For each layer_id in `self.layer_ids` (the "controlled" layers):
        - We look up the corresponding hidden state in `outputs.hidden_states`.
        - We do a dot product of each token with `control_vector.directions[layer_id]`.
        - We aggregate across tokens (sum or mean).
        3) We then aggregate across layers (sum or mean).
        4) Return that final scalar as the activation score.

        :param input_text: The raw string to encode and evaluate.
        :param control_vector: Contains the directions for each layer.
        :param token_aggregate: How to combine per-token scores in each layer: "mean" or "sum".
        :param layer_aggregate: How to combine across all controlled layers: "mean" or "sum".
        :return: A float representing the net alignment across these layers/tokens.
        """

        # 1) Reset to ensure we measure the *base* hidden states (or set_control if you want controlled states).
        self.reset()

        # 2) Encode with a tokenizer (new or cached). If you have a tokenizer in scope, you can reuse it.
        tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        ...
        all_hidden_states = outputs.hidden_states  # tuple of length num_layers+1

        # Typically, all_hidden_states[i] is shape (batch=1, seq_len, hidden_dim)
        # i=0 => embedding output, i=1 => output of layer 0, i=2 => output of layer 1, etc.

        # 4) We'll gather scores for each layer in self.layer_ids, then aggregate them.
        #    self.layer_ids might have negative indices, so let's convert them to positive "real" indices:
        layers_module = model_layer_list(self.model)  # e.g. returns a list of real transformer layers
        ...
        for lid in self.layer_ids:
            ...
            if lid < 0:
                rid = num_layers + lid  # e.g. if lid=-1 and num_layers=24 => rid=23
            else:
                rid = lid
            real_layer_ids.append(rid)

        # We'll track the per-layer scores in a list, then do final aggregation.
        layer_scores = []

        for lid in real_layer_ids:
            # The hidden state for layer lid is in all_hidden_states[lid+1]
            # because 0 is the embedding, 1 is layer 0 output, etc.
            if lid+1 >= len(all_hidden_states):
                # Shouldn't happen unless there's a mismatch, but let's be safe.
                continue

            layer_hidden = all_hidden_states[lid+1][0]  # shape (seq_len, hidden_dim)

            # Now check if this layer has a direction in the control_vector
            # (some vectors might not store all layers).
            # If it doesn't exist, skip it.
            # If you want an error instead of skip, raise an exception.
            if lid not in control_vector.directions:
                # Also check negative index, i.e. if the vector stored the negative form
                # (Probably best to store only positive or only negative, for consistency).
                # We'll just skip if not found:
                continue

            direction_np = control_vector.directions[lid]  # shape (hidden_dim,)
            direction = torch.tensor(direction_np, dtype=self.model.dtype, device=self.model.device)

            # Dot product for each token => shape (seq_len,)
            dot_vals = layer_hidden @ direction

            # Aggregate across tokens
            if token_aggregate == "mean":
                token_score = dot_vals.mean().item()
            elif token_aggregate == "sum":
                token_score = dot_vals.sum().item()
            else:
                raise ValueError("Unknown token_aggregate: must be 'mean' or 'sum'")

            layer_scores.append(token_score)

        # 5) Aggregate across layers
        if not layer_scores:
            # If no layers had directions, we could return 0.0 or raise an error.
            return 0.0
        ...
        if layer_aggregate == "mean":
            ...
            raise ValueError("Unknown layer_aggregate: must be 'mean' or 'sum'")
