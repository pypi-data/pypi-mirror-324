...
def postprocess(i):
    ...
    env['CM_MLPERF_TINY_BENCHMARK'] = os.path.join(
        os.getcwd(), 'src', 'benchmark')
    env['CM_MLPERF_TINY_DATASETS'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'evaluation', 'datasets')
    env['CM_MLPERF_TINY_DATASETS_AD'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'evaluation', 'datasets', 'ad01')
    env['CM_MLPERF_TINY_DATASETS_IC'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'evaluation', 'datasets', 'ic01')
    env['CM_MLPERF_TINY_DATASETS_KWS'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'evaluation', 'datasets', 'kws01')
    env['CM_MLPERF_TINY_DATASETS_KWS_OPEN'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'evaluation', 'datasets', 'kws01-open')
    env['CM_MLPERF_TINY_DATASETS_VWW'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'evaluation', 'datasets', 'vww01')
    env['CM_MLPERF_TINY_TRAINING'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'training')
    env['CM_MLPERF_TINY_TRAINING_AD'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'training', 'anomaly_detection')
    env['CM_MLPERF_TINY_TRAINING_IC'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'training', 'image_classification')
    env['CM_MLPERF_TINY_TRAINING_KWS'] = os.path.join(
        os.getcwd(), 'src', 'benchmark', 'training', 'keyword_spotting')
    env['CM_MLPERF_TINY_TRAINING_VWW'] = os.path.join(
