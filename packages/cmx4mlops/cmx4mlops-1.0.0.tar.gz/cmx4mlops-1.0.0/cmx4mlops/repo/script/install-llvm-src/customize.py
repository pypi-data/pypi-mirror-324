...
def preprocess(i):
    ...
    if env.get('CM_LLVM_16_INTEL_MLPERF_INFERENCE', '') == "yes":
        ...
        cmake_cmd = "cmake " + os.path.join(env["CM_LLVM_SRC_REPO_PATH"], "llvm") + " -GNinja -DCMAKE_BUILD_TYPE=" + llvm_build_type + " -DLLVM_ENABLE_PROJECTS=" + enable_projects + " -DLLVM_ENABLE_RUNTIMES='" + \
    ...
    env['CM_LLVM_CLANG_BIN_WITH_PATH'] = os.path.join(
        env['CM_LLVM_INSTALLED_PATH'], "bin", clang_file_name)
