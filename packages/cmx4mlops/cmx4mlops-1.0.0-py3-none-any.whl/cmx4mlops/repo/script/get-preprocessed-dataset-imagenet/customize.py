...
def preprocess(i):
    ...
    if 'CM_IMAGENET_PREPROCESSED_PATH' in env:
        files = glob.glob(
            ...
            "/**/" +
            env['CM_IMAGENET_PREPROCESSED_FILENAME'],
            recursive=True)
        if files:
        ...
        if env['CM_DATASET_TYPE'] == "validation" and not exists(
                os.path.join(env['CM_DATASET_PATH'], "val_map.txt")):
    ...
    if env.get('CM_DATASET_TYPE', '') == "validation" and not exists(
...
def postprocess(i):
    ...
    env['CM_DATASET_PREPROCESSED_IMAGES_LIST'] = os.path.join(
        os.getcwd(), "preprocessed_files.txt")
    env['CM_DATASET_PREPROCESSED_IMAGENAMES_LIST'] = os.path.join(
