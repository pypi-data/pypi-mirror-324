...
def preprocess(i):
    ...
    if os.path.exists(sut_path) and env.get('CM_SUT_DESC_CACHE', '') == "yes":
        ...
        state['CM_SUT_META']['other_software_stack'] = "Python: " + \
        ...
        if env.get('CM_MLPERF_DEVICE', '') == "gpu" or env.get(
        ...
        if 'cm_cuda_device_prop' in state:
            ...
            state['CM_SUT_META']['accelerator_memory_capacity'] = str(int(
        ...
        if state['CM_SUT_META'].get('host_processor_model_name', '') == '':
            state['CM_SUT_META']['host_processor_model_name'] = env.get(
                'CM_HOST_CPU_MODEL_NAME', 'undefined')
        ...
        if state['CM_SUT_META'].get('host_processor_frequency', '') == '':
            state['CM_SUT_META']['host_processor_frequency'] = env.get(
                'CM_HOST_CPU_MAX_MHZ') if env.get('CM_HOST_CPU_MAX_MHZ', '') != '' else 'undefined'
        if state['CM_SUT_META'].get('host_memory_capacity', '') == '':
            state['CM_SUT_META']['host_memory_capacity'] = env.get(
                'CM_HOST_MEMORY_CAPACITY') if env.get('CM_HOST_MEMORY_CAPACITY', '') != '' else 'undefined'
        if state['CM_SUT_META'].get('host_storage_capacity', '') == '':
            state['CM_SUT_META']['host_storage_capacity'] = env.get(
                'CM_HOST_DISK_CAPACITY') if env.get('CM_HOST_DISK_CAPACITY', '') != '' else 'undefined'
        ...
        if env.get('CM_SUDO_USER', '') == "yes" and env.get(
