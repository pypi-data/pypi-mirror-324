...
def postprocess(i):
    ...
    if env.get('CM_DATASET_SQUAD_PACKED', '') != "yes":
        ...
        if env.get('CM_DATASET_RAW', '') == "yes":
            env['CM_DATASET_SQUAD_TOKENIZED_INPUT_IDS'] = os.path.join(
                cur, 'bert_tokenized_squad_v1_1_input_ids.raw')
            env['CM_DATASET_SQUAD_TOKENIZED_SEGMENT_IDS'] = os.path.join(
                cur, 'bert_tokenized_squad_v1_1_segment_ids.raw')
            env['CM_DATASET_SQUAD_TOKENIZED_INPUT_MASK'] = os.path.join(
                cur, 'bert_tokenized_squad_v1_1_input_mask.raw')
        else:
            env['CM_DATASET_SQUAD_TOKENIZED_PICKLE_FILE'] = os.path.join(
            ...
            for dirname in os.listdir(cur):
                if os.path.isdir(dirname) and not dirname.startswith("_"):
                    f.write(
                        ...
                        "\n")
        env['CM_DATASET_SQUAD_TOKENIZED_PACKED_FILENAMES_FILE'] = os.path.join(
