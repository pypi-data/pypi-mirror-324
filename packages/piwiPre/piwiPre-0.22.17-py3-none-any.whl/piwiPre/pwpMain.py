...
class PwpMain:
    ...
    @staticmethod
    def clean_file_thumbnails(source_filename: str, config: PwpConfig, common_path: str):
        """
        Cleans the thumbnails for source_filename
    ...
    def manage_conflict(self, common_path: str, old_file_path: str, new_date: str, new_author: str, occasion: str,
        ...
        file_dico = config.format_dict(new_date, new_author, old_file_path,
                                       occasion=occasion, suffix=current_file.suffix)
        ...
        if self.rename_allowed(config, stage):
            ...
            # see REQ 0397
            base, ext = os.path.splitext(os.path.basename(old_file_path))
    ...
    def run_stage_file(self, stage: str, config: PwpConfig, common_path: str,
        ...
        if current_file.filename != old_file.local:
            ...
            # if rename_allowed is false, target will be the same file thant file_path, so removing is OK
        ...
        if move_to_album:
            ...
            if config['dryrun']:
                ...
                self.clean_file_thumbnails(source_filename=target.local, config=config, common_path=target_rel_path)
    ...
    def clean_auto_configs(self, path, config):
        ...
        if not config['enable-auto-configuration']:  # pragma: no cover
            ...
            return
        if not config['enable-auto-configuration-delete']:
            LOGGER.debug(f"{path} enable-auto-configuration-delete is False, abort clean_auto_configs")
            return
