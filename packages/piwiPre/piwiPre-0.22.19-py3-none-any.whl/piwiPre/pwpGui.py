...
class GuiEntry(tkinter.Entry):
    def __init__(self, frm, width, textvariable, column, row, columnspan=1):  # noqa
...
class GuiDirChooser(PwpGui):
    ...
    def enter(self, path: str, check=False):
        full_path = self.__get_dir_from_gui(path)

        if check and not os.path.isdir(full_path):
            LOGGER.warning(f"Chosen directory '{full_path}' does not exist")
            return
