...
class Track:
    ...
    def info_dict(self) -> TrackDict:
        # TODO lyrics can probably be removed from info_dict
        meta = self.metadata()
        return {
    ...
    def by_relpath(conn: Connection, relpath: str) -> Track:
        ...
        if row is None:
            raise NoSuchTrackError

        (mtime, ctime, duration) = row
        return Track(conn, relpath, from_relpath(relpath), mtime, ctime, duration)
...
class Playlist:
    ...
    def tracks(self) -> list[Track]:
        ...
        for relpath, mtime, ctime, duration in self.conn.execute(
            ...
            tracks.append(Track(self.conn, relpath, from_relpath(relpath), mtime, ctime, duration))
        return tracks
