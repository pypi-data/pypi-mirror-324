...
class SpotifyClient:
    ...
    async def _get_artist_image(self, name: str):
        async with await self._session() as session:
            async with session.get(
                "https://api.spotify.com/v1/search", params={"q": "artist:" + name, "type": "artist", "market": "NL", "limit": 1}
            ) as response:
                json = await response.json()
    ...
    async def get_artist_image(self, name: str) -> bytes | None:
        image = await cache.retrieve_or_store("spotify_artist_image" + name, self._get_artist_image, name)
        if image == b"no_cover":
            return None
        return image
