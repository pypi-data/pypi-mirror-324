...
from raphson_mp import auth, ratelimit, response, settings, util
...
async def route_home(_request: web.Request, _conn: Connection, user: auth.User):
    ...
    """
    return await response.template("home.jinja2", user_is_admin=user.admin, offline_mode=settings.offline_mode)
...
@route("/token")
async def route_token(request: web.Request, _conn: Connection, user: auth.User):
    session = await auth.create_session(request, user)
