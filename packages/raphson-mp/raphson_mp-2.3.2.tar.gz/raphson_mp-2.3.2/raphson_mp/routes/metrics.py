...
if prometheus_client:
    ...
    g_players.labels("Playing").set_function(lambda: sum(1 for playing in activity.now_playing() if not playing.paused))
    g_players.labels("Paused").set_function(lambda: sum(1 for playing in activity.now_playing() if playing.paused))

    ALL_CATEGORIES: set[str] = {"File manager", "Subsonic", "Monitoring", "Error reporting", "Other"}
    REQUEST_COUNTER: dict[str, int] = {category: 0 for category in ALL_CATEGORIES}
    ALL_STATUS: set[int] = {101, 200, 204, 400, 403, 500, 523, 0}
