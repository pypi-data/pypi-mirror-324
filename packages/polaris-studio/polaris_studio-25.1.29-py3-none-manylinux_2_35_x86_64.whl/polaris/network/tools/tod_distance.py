...
class DistanceToTransit:
    ...
    def _load_data(self):
        with read_and_close(self._path_to_file, spatial=True) as conn:
            dtc = DataTableCache(self._path_to_file)
            srid = get_srid(self._path_to_file)
            self._tw = dtc.get_geo_layer("Transit_Walk", conn=conn).to_crs(srid)
            self._loc = dtc.get_geo_layer("Location", conn=conn).to_crs(srid)
            self._all_stops = dtc.get_geo_layer("Transit_Stops").query("route_type>=0").to_crs(srid)
