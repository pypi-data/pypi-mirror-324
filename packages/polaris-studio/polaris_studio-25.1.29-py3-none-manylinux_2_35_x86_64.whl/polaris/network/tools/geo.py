...
class Geo:
    ...
    def model_extent(self, projection=None) -> Polygon:
    ...
    def get_geo_for_id(self, layer: str, id: int) -> Union[Point, LineString, Polygon]:
        ...
        if layer.lower() == "location":
            return lyr.loc[lyr.location == id, "geo"].values[0]  # type: ignore

        elif layer.lower() in ["transit_bike", "bike_link"]:
            return lyr.loc[lyr.bike_link == id, "geo"].values[0]  # type: ignore

        elif layer.lower() in ["transit_walk", "walk_link"]:
            return lyr.loc[lyr.walk_link == id, "geo"].values[0]  # type: ignore

        elif layer.lower() == "zone":
            return lyr.loc[lyr.zone == id, "geo"].values[0]  # type: ignore

        elif layer.lower() == "link":
            return lyr.loc[lyr.link == id, "geo"].values[0]  # type: ignore

        elif layer.lower() == "node":
            return lyr.loc[lyr.node == id, "geo"].values[0]  # type: ignore
        else:
            raise ValueError(f"Layer {layer} name not recognized")
    ...
    def side_of_link_for_point(self, point: Point, link: Union[int, LineString]) -> int:
        ...
        if isinstance(link, int):
            link_geo = self.get_geo_for_id("link", link)
        else:
            link_geo = link
        side = self.determine_side_of_line(Point(link_geo.coords[0]), Point(link_geo.coords[-1]), point)
    ...
    def convex_hull(self):
        ...
        convex_hull = unary_union(links).convex_hull
        return convex_hull

    @property
    def modeling_area(self) -> Union[MultiPolygon, Polygon]:
    ...
    def get_geo_layer(self, layer_name: str) -> gpd.GeoDataFrame:
        ...
        layer = data_table_cache.DataTableCache(self._network_file).get_geo_layer(layer_name)
    ...
    def _driving_side(self):
        if self.__driving_side is None:
            self.__driving_side = driving_side(database_path=self._network_file)
