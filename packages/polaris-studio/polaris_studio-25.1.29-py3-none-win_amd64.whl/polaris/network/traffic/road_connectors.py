...
class RoadConnectors:
    ...
    def connect_ferries(self, max_distance: int, speed: float = 10.0) -> None:
        ...
        dtc = DataTableCache(self._network_file)
        with read_and_close(self._network_file, spatial=True) as conn:
            stops = dtc.get_geo_layer("Transit_Stops", conn).query("route_type in (4, 1200)")
            ...
            auto_types = dtc.get_table("Link_Type", conn).reset_index().link_type.to_numpy()  # noqa: F841
            links = dtc.get_geo_layer("Link", conn).query("type in @auto_types")

            valid_nodes = np.unique(np.hstack((links.node_a.to_numpy(), links.node_b.to_numpy())))  # noqa: F841
            nodes = dtc.get_geo_layer("Node", conn).query("node in @valid_nodes")
