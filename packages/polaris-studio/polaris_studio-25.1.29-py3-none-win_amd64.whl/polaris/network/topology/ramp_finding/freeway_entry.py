...
def freeway_entries(supply_file: Path):
    ...
    graph = build_graph(net.path_to_file)
    ...
    for source in sources:
        pbar = tqdm(total=len(freeway_nodes), colour="red", leave=False, desc="Computing paths for origins")
        pre_found = len(all_tolls)
        res.compute_path(source, int(freeway_nodes[-1]))
        for target in freeway_nodes:
            res.update_trace(int(target))
            while True:
                pbar.set_description(f"Found {len(all_tolls):,} freeway entries", refresh=True)
                if res.path is None:
                    break
