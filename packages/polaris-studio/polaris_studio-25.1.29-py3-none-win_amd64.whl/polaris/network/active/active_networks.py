    ...
    def build(self):
        ...
        with commit_and_close(self._network_file, spatial=True) as conn:
            ...
            if self.__do_update_associations__:
                ...
                conn.execute('''DELETE FROM Editing_Table WHERE table_name="Transit_Walk"''')
                conn.execute('''DELETE FROM Editing_Table WHERE table_name="Transit_Stops"''')
                conn.execute('''DELETE FROM Editing_Table WHERE table_name="Transit_Bike"''')
                create_network_triggers(conn, self.srid)
            self.finish()
