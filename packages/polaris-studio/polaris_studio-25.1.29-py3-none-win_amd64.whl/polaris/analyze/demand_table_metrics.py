...
class DemandTableMetrics:
    ...
    def _build_matrices(self, dt: pd.DataFrame, matrix_names: dict):
        ...
        if self.__loc_zn.empty:
            ...
            self.__loc_zn = (
                DataTableCache(self.__supply_file__).get_table("location").reset_index()[["location", "zone"]]
            )

            # List of zones
            zn = np.sort(DataTableCache(self.__supply_file__).get_table("zone").reset_index().zone.to_numpy())
