...
class ModelChecker(WorkerThread):
    ...
    def has_critical_errors(self, fail_on_errors):
        ...
        if len(self.errors) > 0:
            if fail_on_errors:
                ...
                raise ValueError(f"YOUR {self.__database_type} FILE CONTAINS ERRORS")
            return True
        return False
    ...
    def consistency_tests(self, check_cristal=False):
        """Basic consistency checks"""
        self._emit_start(1, "Basic consistency")
        self.__binary_tests("check_queries.sql", "warn")
        if check_cristal:
            self.__binary_tests("check_queries_cristal.sql", "warn")
        ...
        with commit_and_close(self._path_to_file) as conn:
            for tbl in StandardDatabase.for_type(self.__database_type).tables():
                if not has_table(conn, tbl):
                    self.logger.warning(f"Missing table {tbl}, skipping FK check")
                    continue
