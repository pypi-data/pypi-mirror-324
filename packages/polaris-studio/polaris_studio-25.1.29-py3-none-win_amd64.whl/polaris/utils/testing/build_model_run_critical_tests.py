...
def critical_network_tests(city: str, model_text_folder: str, model_dir=None):
    ...
    supply = pol.run_config.supply_file()
    demand = pol.run_config.demand_file()

    SupplyChecker(model_dir / supply).has_critical_errors(True)
    DemandChecker(model_dir / demand).has_critical_errors(True)
    GlobalChecker(pol.run_config).has_critical_errors(True)
