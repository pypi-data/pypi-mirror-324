...
def add_locations(
    ...
    zone_layer = DataTableCache(supply_path).get_geo_layer("Zone")
...
def add_residential(
    ...
    hh_data = {k: 0 for k in fields.keys()}
    ...
    with commit_and_close(net.path_to_file, spatial=True) as conn:
        ...
        zones = DataTableCache(net.path_to_file).get_geo_layer("Zone", conn)[["zone", "geo"]]
        links = DataTableCache(net.path_to_file).get_geo_layer("Link", conn)[["link", "geo"]]
...
def add_commercial(
    ...
    with commit_and_close(net.path_to_file, spatial=True) as conn:
        ...
        zones = DataTableCache(net.path_to_file).get_geo_layer("Zone", conn)[["zone", "geo"]].to_crs(srid)
        links = DataTableCache(net.path_to_file).get_geo_layer("Link", conn)[["link", "geo"]].to_crs(srid)
