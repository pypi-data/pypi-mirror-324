...
// load/write from cookie
function Cookie(issave)
{    
    let label = "Settings=";
    if(issave){
        let settingstr = JSON.stringify(Settings);
        document.cookie = label+settingstr;
    }
    else{        
        let settingstr = document.cookie;
        if(settingstr != ""){
            settingstr = settingstr.substring(label.length, settingstr.length);
            let obj;
            try {
                obj = JSON.parse(settingstr);    
            }
            catch (e) {
                return;
            }
...
async function Initialize()
{
    if(window.hasOwnProperty("__TAURI__")){
        ...
        window.__TAURI__.tauri.invoke("get_exepath").then(async function(exepath){
            ...
            if(RunOS == "Darwin"){
                exedir = exedir.substring(0, exedir.lastIndexOf("spectra"));
            }
...
{
    let keys = [OutFileLabel, PrePLabel];
    keys.forEach(key => {
