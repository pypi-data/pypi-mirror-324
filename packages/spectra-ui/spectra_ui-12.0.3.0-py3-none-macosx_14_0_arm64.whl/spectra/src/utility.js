...
{
    if(id != DataUnitLabel && id != AccuracyLabel && id != MPILabel){
        return;
    }
    let oldsettings = CopyJSON(Settings[id]);
    SIMPLEX.OptionPanels[id].SetPanel();
    ShowDialog(id, false, false, "", SIMPLEX.OptionPanels[id].GetTable(), null, 
    () => {
        Settings[id] = CopyJSON(oldsettings);
        SIMPLEX.OptionPanels[id].JSONObj = Settings[id];
    });
}
// <-- dialogs
