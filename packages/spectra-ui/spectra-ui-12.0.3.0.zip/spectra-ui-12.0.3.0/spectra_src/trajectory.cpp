...
void Trajectory::GetPhaseError(vector<double> &zpeak,
...
void Trajectory::GetStokesWigglerApprox2D(double *XYZ, 
    int nfd, vector<double> &ep, vector<vector<double>> &fd, bool ideal)
{
    double zemission;
    vector<double> fhvc(4);
    ...
    bool issrc;
    int nz[2] = {-1, 0};
    int nini = 1, nfin = m_ntotorbit-1;
    if(ideal){
        if(m_nppp < 0){
            throw runtime_error("Fatal error: invalid configurations found.");
        }
        nini = m_nzorgorbit-m_nppp/4+1;
        nfin = m_nzorgorbit+m_nppp/4;
        double prjmax = 0;
        int ngmax = nini;
