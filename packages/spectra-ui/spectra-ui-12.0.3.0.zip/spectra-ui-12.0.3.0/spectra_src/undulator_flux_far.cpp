...
void UndulatorSpectrumInfPeriods::AllocateInfPeriodSpectrum(int layer)
{
	...
	int trank, epmeshas, emeshmax = 0;
	double *earrmpi = nullptr;
	double *fdarrmpi = nullptr;
    for(nh = 1; nh <= nhmax && m_mpiprocesses > 1; nh++){
		trank = (nh-1)%m_mpiprocesses;

		if(m_rank == trank){
			epmeshas = m_epmesh[nh];
		}
		MPI_Bcast(&epmeshas, 1, MPI_INT, trank, MPI_COMM_WORLD);
		if(m_rank != trank){
			m_epmesh[nh] = epmeshas;
            m_eparray[nh].resize(m_epmesh[nh]+1);
            for(j = 0; j < m_fluxitems; j++){
                m_fdmatrix[nh][j].resize(m_epmesh[nh]);
            }
		}

		if(epmeshas > emeshmax){
			emeshmax = epmeshas;
			earrmpi = (double *)realloc_chk(earrmpi, emeshmax*sizeof(double));
			fdarrmpi = (double *)realloc_chk(fdarrmpi, emeshmax*m_fluxitems*sizeof(double));
		}

		if(m_rank == trank){
            for(n = 0; n < m_epmesh[nh]; n++){
				earrmpi[n] = m_eparray[nh][n];
			}
		}
		MPI_Bcast(earrmpi, m_epmesh[nh], MPI_DOUBLE, trank, MPI_COMM_WORLD);
		if(m_rank != trank){
            for(n = 0; n < m_epmesh[nh]; n++){
				m_eparray[nh][n] = earrmpi[n];
			}
		}
		...
		if(m_rank == trank){
            for(n = 0; n < m_epmesh[nh]; n++){
                for(j = 0; j < m_fluxitems; j++){
					fdarrmpi[n*m_fluxitems+j] = m_fdmatrix[nh][j][n];
				}
			}
		}
		MPI_Bcast(fdarrmpi, m_epmesh[nh]*m_fluxitems, MPI_DOUBLE, trank, MPI_COMM_WORLD);
		if(m_rank != trank){
            for(n = 0; n < m_epmesh[nh]; n++){
                for(j = 0; j < m_fluxitems; j++){
                    m_fdmatrix[nh][j][n] = fdarrmpi[n*m_fluxitems+j];
                }
			}
		}
	}
        ...
        int nh, int rank, int mpiprocesses)
{
    ...
    m_nh = nh;
	if(mpiprocesses > 1){
		ArrangeMPIConfig(rank, mpiprocesses, 1);
	}
