...
void WignerFunction::GetWignerPhaseSpace(int type, bool is4size,
		...
		vector<vector<double>> *W, int rank, int mpiprocesses)
{
	m_type = type;
	switch(type){
	...
	for(int iuv = 0; iuv < iscan; iuv++){
		...
		f_GetWignerAlongUV(uv, &(vararray[0]), &Wsglp, rank, mpiprocesses);
		m_calcstatus->AdvanceStep(m_process_layer);
		
		if(m_Nwiggler > 0){
			m_polarity = -1.0;
			f_GetWignerAlongUV(uv, &(vararray[0]), &Wsglm, rank, mpiprocesses);
			m_calcstatus->AdvanceStep(m_process_layer);
			m_polarity = 1.0;
		}
	...
	int rank, int mpiprocesses)
{
	...
	steps = RunDigitizer(FUNC_DIGIT_BASE|FUNC_DIGIT_ENABLE_LOG, &uv, &Warr, 
					...
					WignerFuncDigitizer, nullptr, false, rank, mpiprocesses); 
	...
	vector<vector<double>> &xyvar, vector<double> &W, int rank, int mpiprocesses)
{
	...
	if(indices.size() > 1){
		...
		m_calcstatus->SetSubstepNumber(m_wiglayer-1, 
			(int)(uvvalues[1].size()*uvvalues[0].size()));
		for(int m = 0; m < uvvalues[1].size(); m++){
			...
			for (int n = 0; n < (int)uvvalues[0].size(); n++){
				...
				m_wigner->GetWignerPhaseSpace(
					wigtype, false, UVfix, uvfix, vararray, &ws, rank, mpiprocesses);
