	...
	vector<string> &subresults, vector<string> &categories)
{
	...
	if(contains(m_calctype, menu::CMDPP)){
		...
		if(m_confb[CMDcmp_]){
			hg2d.SetLGContainer(m_rank, m_mpiprocesses);
		}
	}
	else{
		hg2d.LoadData();
		if(m_confsel[GSModelXY_] == NoneLabel){
			...
			hg2d.SetLGContainer(m_rank, m_mpiprocesses);
			hg2d.GetAnmAll(0, m_rank, m_mpiprocesses);
			...
			m_calcstatus->SetSubstepNumber(0, 1+csteps);
			hg2d.SetGSModel(isGS, m_rank, m_mpiprocesses, &error);
	...
	if(m_confsel[CMDfld_] != NoneLabel){
		...
		hg2d.DumpFieldProfile(IsBinaryFld()?filebin.c_str():nullptr, 
			m_conf[cutoff_], cmdmode, IsJSONFld(), xyarr, datari[0], datari[1], m_rank, m_mpiprocesses);
	...
	if(m_confb[CMDint_]){
		data.clear();
		hg2d.DumpIntensityProfile(m_conf[cutoff_], cmdmode, xyarr, data, m_rank, m_mpiprocesses);
		subresults.push_back("");
	...
	if(m_confb[CMDcmpint_]){
		...
		hg2d.DumpTotalIntensityProfile(m_conf[cutoff_], cmdmode, xyvar, data[0], m_rank, m_mpiprocesses);
		subresults.push_back("");
	...
	if(m_confb[CMDcmp_]){
		data.clear();
		hg2d.ReconstructExport(cmdmode, m_conf[cutoff_], &CMDerr[2], wigdata, m_rank, m_mpiprocesses);
	...
	vector<string> &subresults, vector<string> &categories)
{
	...
	if(contains(m_calctype, menu::CMDPP)){
		...
		if(m_confb[GSModel_]){
			...
			hg1d.GetAnm(nullptr, m_rank, m_mpiprocesses);
			m_calcstatus->AdvanceStep(0);
	...
	if(m_confb[CMDcmp_]){
		...
		hg1d.ReconstructExport(cmdmode, 
			m_conf[cutoff_], &CMDerr[2], data[0][1], m_rank, m_mpiprocesses);
		subresults.push_back("");
