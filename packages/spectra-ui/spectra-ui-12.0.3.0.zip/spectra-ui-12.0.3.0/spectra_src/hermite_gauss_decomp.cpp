...
void LGFunctionContainer::Create(
	int maxmode, double eps, int rank, int mpiprocesses, PrintCalculationStatus *status)
{
#ifdef _DEBUG
...
void HGModalDecomp2D::SetLGContainer(int rank, int mpiprocesses)
{
	double lgeps = 1e-6;
	int maxmode = max(m_maxmode[0], m_maxmode[1]);
	m_lgcont.Create(maxmode, lgeps, rank, mpiprocesses, m_calcstatus);
	for(int j = 0; j < 2; j++){
...
void HGModalDecomp2D::GetComplexAmp2D(vector<vector<double>> &xyarr,
		...
		int rank, int mpiprocesses, bool skipstep)
{
...
void HGModalDecomp2D::DumpTotalIntensityProfile(double eps, int pmax, 
	vector<vector<double>> &xyarr, vector<vector<double>> &data, int rank, int mpiprocess)
{
	...
	pmax = m_hgmode[2]->GetMaxOrder(pmax);
	GetComplexAmp2D(xyarrm, &Ea, eps, pmax, rank, mpiprocess);
	...
	int rank, int mpiprocess)
{
	...
	double coef = GetNormalizeFactor()*1e6*m_lambda*m_lambda;
	pmax = m_hgmode[2]->GetMaxOrder(pmax);
	GetComplexAmp2D(xyarr, &Ea, eps, pmax, rank, mpiprocess);
	data.resize(pmax+1);
	for(int p = 0; p <= pmax; p++){
		data[p].resize(1);
		...
		double xy[], double dq[], int hqmesh[], vector<double> &W)
{
	...
	for (int i = 0; i < 2; i++){
		...
		GetComplexAmp2D(xyarr[i], &Ea[i], eps, pmax, 0, 1, true);
	}
...
void HGModalDecomp2D::SetGSModel(bool isGS[], int rank, int processes, double *err)
{
	vector<vector<double>> dprj;
	vector<vector<double>> xyq(2);
	...
	for(int j = 0; j < 2; j++){
		...
		if(isGS[j]){
			...
			m_hgmode[j]->GetAnm(nullptr, rank, processes);
			m_calcstatus->AdvanceStep(1);
...
void HGModalDecomp2D::f_ComputeWholeWigner(
	double fnorm, vector<double> &data, int rank, int mpiprocesses)
{
	MPI_Status mpistatus;
