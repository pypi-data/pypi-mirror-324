    ...
    // compute the current bunch factor and save in m_bunchf[tgtsection]
{
    ...
    if(m_gprocesses > 1){
        for(int n = 0; n < m_nfftbf; n++){
            double ew = m_EwFFTbf[0][n];
            MPI_Allreduce(&ew, &m_EwFFTbf[0][n], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        }
        for(int n = 0; n < (int)m_currt.size() && m_confb[exportEt_]; n++){
            for(int m = 0; m < neta; m++){
                double jc = m_curr_j[tgtsection][n][m];
                MPI_Allreduce(&jc, &m_curr_j[tgtsection][n][m], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            }
        }
        MPI_Barrier(MPI_COMM_WORLD);
    }
