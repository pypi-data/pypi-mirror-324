...
void Run(int rank, SpectraConfig &spconf, int *scanindex, 
...
int RunProcess(
	picojson::object &inobj, string dataname, int rank, int mpiprocesses, int serno = -1)
{
	string input;
	...
	if(spconf.IsPreprocess()){
		try{
			Run(rank, spconf, nullptr, categs, results);
		}
		catch(const exception &e){
			stringstream ss;
			ss << ErrorLabel << e.what();
	...
	if(!spconf.CanBundle()){
		...
		for(int n = 0; n < nscans; n++){
			...
			try{
				Run(rank, spconf, scanidx, categs, results);
			}
			catch (const exception &e) {
				if (rank == 0){
					stringstream ss;
...
int spectra_solver(int serno, string input)
{
	int retcode = 0;
	...
	for(int n = 0; n < inobjs.size(); n++){
		if(RunProcess(inobjs[n], datanames[n], 0, 1, serno) < 0){
			retcode = -1;
		}
	}
	return retcode;
...
int main(int argc, char **argv)
{
	...
	for(int n = 0; n < inobjs.size(); n++){
		if(RunProcess(inobjs[n], datanames[n], rank, mpiprocesses) < 0){
			retcode = -1;
		}
