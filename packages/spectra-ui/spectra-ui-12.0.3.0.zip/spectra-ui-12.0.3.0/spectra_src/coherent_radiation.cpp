...
void CoherentRadiationCtrl::GetCohSpatialProfile(int layer,
	vector<vector<double>> *xy, vector<vector<double>> *density, int rank, int mpiprocesses)
{
	double xyp[2];
	...
	double *wsmpi;
	wsmpi = (double *)calloc(ndata, sizeof(double));
	...
	for(int n = 0; n < npoints; n++){
		...
		MPI_Bcast(wsmpi, ndata, MPI_DOUBLE, r, MPI_COMM_WORLD);
		if(r != rank){
