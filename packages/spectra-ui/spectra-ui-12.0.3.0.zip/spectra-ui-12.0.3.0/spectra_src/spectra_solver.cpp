...
SpectraSolver::SpectraSolver(SpectraConfig &spconf)
	...
	vector<string> &subresults, vector<string> &categories)
{
	if(m_iscoherent){
		...
		m_calcstatus->SetSubstepNumber(layer, (int)ceil(m_eparray.size()/mpiprocesses));
		for(int n = 0; n < m_eparray.size(); n++){
...
void SpectraSolver::GetFixedPoint(vector<double> &values, 
	int layer, int rank, int mpiprocesses)
{
	if(m_ispower){
		Trajectory trajec(*this);
		...
		|| m_pptype == PPRedFlux)
	{
		...
		if(m_pptype == PPBetaLabel){
			...
			if(m_pptype == PPPhaseErrLabel){
				trajec.GetPhaseError(zpeak, data[0], thresh, temp);
			}
			else{
