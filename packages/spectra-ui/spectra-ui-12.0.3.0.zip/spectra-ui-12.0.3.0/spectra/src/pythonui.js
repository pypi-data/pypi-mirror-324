...
function CreateScanDirect(category, item, jxy, scanobj)
{
    ...
    let scanoption = new ScanOptions(isint, dimension == 2, GUIConf.GUIpanels[ConfigLabel]);
...
{
    let obj = {};
    obj.enable = GUIConf.GUIpanels[MPILabel].JSONObj[MPIOptionsLabel.enablempi[0]];
    obj.processes = GUIConf.GUIpanels[MPILabel].JSONObj[MPIOptionsLabel.processes[0]];
    return obj;
}
...
function SetFromPython(target, object)
{
    if(target == "Framework"){
        ...
        if(Settings.hasOwnProperty(OutFileLabel)){
            Settings[OutFileLabel][OutputOptionsLabel.fixpdata[0]] = [["-", "-", "-"]];
        }
...
function GetMenuItems()
{
    ...
    let menuid = [];
    menus.forEach(el => {
        if(GUIConf.allmenus.includes(el.id)){
            let parent = el.parentElement;
            let isenable = true;
            while(parent.id != MenuLabels.calc){
            ...
            if(isenable){
                obj.calcid.push(el.id);
            }
        }
        else if(!el.id.includes(MenuLabels.prmset) && !el.id.includes("scan")){
            menuid.push(el.id);
        }
    });
