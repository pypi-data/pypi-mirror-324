...
function HandleParameterObject(objs, filename)
{
    ...
    if(objs.hasOwnProperty(InputLabel)){
        ...
        for(const did of disableids){
            if(objs[ConfigLabel][TypeLabel].includes(did)){
                forcedisable = true;
                break;
            }    
        }
    ...
    if(Settings.hasOwnProperty(ConfigLabel)){
        ...
        for(const key of keys){
            ...
            if(!config.hasOwnProperty(label)){
                if(GUIConf.GUIpanels[ConfigLabel].JSONObj.hasOwnProperty(label)){
                    config[label] = GUIConf.GUIpanels[ConfigLabel].JSONObj[label];
                }
            }
        }
    }
...
function HandleFile(data, filename, isfinal = true)
{
        ...
        if(Framework == ServerLabel && (iniFile != "" || iniPP != "" || iniLPP != "")){
            ...
            new Promise((resolve) => {
                if(iniFile == ""){
                    ...
                    xhr.open("GET", "get_file.php?filename=prm/"+iniFile, true); 
                            ...
                            xhr.addEventListener("load", async (e) => {
                                if(xhr.response != null){
                                    if(j == 0){
                                        ...
                                        HandleFile(xhr.response, ppfiles);
                                    }
                                }
                            });
                            xhr.send(null);    
