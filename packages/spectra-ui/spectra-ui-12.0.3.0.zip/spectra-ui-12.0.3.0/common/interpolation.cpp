	...
	vector<double> *x, vector<double> *y, bool isreg, bool islog, bool issort)
{
	...
	double deltax = (m_x[m_size-1]-m_x[0])*1e-10;
	m_xrange[0] = m_x[0]-deltax;
	m_xrange[1] = m_x[m_size-1]+deltax;
}
...
double Spline::GetOptValue(double x)
{
	int ix;
	double zero = 0;
	double vspl = GetValue(x, true, &ix, &zero);
	if(ix >= 0 // not out of range 
			&& m_y[ix]*m_y[ix+1] > INFINITESIMAL && m_y[ix]*vspl < INFINITESIMAL){
		return GetLinear(x);
	}
	return vspl;
}
...
//			m_gderiv[n][1] = m_y[n+1]-m_y[n]-dx2*(3.0*m_y2[n]+m_y2[n+1])/6.0;
			...
			m_gderiv[n][1] = m_y[n+1]-m_y[n]-dx2*(2.0*m_y2[n]+m_y2[n+1])/6.0;
...
int Spline::IntegrateGtEiwt(int nini, double x[], double w, double *Gr, double *Gi, const char *debug)
{
	...
	*Gr = *Gi = 0.0;
	...
	for(int n = nini; n <= nfin; n++){
		...
		if(n == nini){
			gderiv[0] = GetValue(xr[0]);
			gderiv[1] = GetDerivativeAt(xr[0]);
			gderiv[3] = (m_y2[n+1]-m_y2[n])/m_dx[n];
			gderiv[2] = m_y2[n]+gderiv[3]*(xr[0]-m_x[n]);
