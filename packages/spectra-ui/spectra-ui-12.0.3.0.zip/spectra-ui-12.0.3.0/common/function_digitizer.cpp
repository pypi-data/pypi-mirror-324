...
int FunctionDigitizer::RunDigitizer(
	...
	string debug, vector<double> *integ, bool istruncate, int rank, int mpiprocesses)
{
	...
	if(mpiprocesses > 1 && !(level&FUNC_DIGIT_YINI_ALLOC)){
		f_ComputeFuncMPI(ninit, ninit, m_xarray, rank, mpiprocesses, status, targetlayer, 0);
		for(nn = 0; nn < ninit; nn++){
	...
	do{
		...
		if(mpiprocesses > 1 && nins > 0){
			f_ComputeFuncMPI(ninit, nins, m_wsx, rank, mpiprocesses, status, targetlayer, ninit);
		}
...
void FunctionDigitizer::f_ComputeFuncMPI(int ninit, int npoints, vector<double> &xarr,
	int rank, int mpiprocesses, PrintCalculationStatus *status, int targetlayer, int npininit)
{
	int nn, ni;
