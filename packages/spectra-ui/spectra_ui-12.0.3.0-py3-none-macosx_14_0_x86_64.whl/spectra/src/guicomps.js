...
class AccPrmOptions extends PrmOptionList {
    ...
    GetShowList(isall = false)
    {
        ...
        let isfar = calcid == "" ? true : calcid.includes(MenuLabels.far);
        if(isfar){
        ...
        if(calcid == "" || isfel || validlist[AccPrmsLabel.bunchdata[0]] == 1){
                ...
                this.m_jsonobj[AccPrmsLabel.bunchtype[0]] == CustomParticle)
            {
                ...
                if(srctype == CUSTOM_Label){
                    validlist[AccPrmsLabel.zerosprd[0]] = 
                        (this.m_config.JSONObj[ConfigPrmsLabel.wiggapprox[0]] == false && 
                            this.m_config.JSONObj[ConfigPrmsLabel.esmooth[0]] == false) ? 1 : -1;
                }
                else{
                    validlist[AccPrmsLabel.zerosprd[0]] = 
...
class MPIOptions extends PrmOptionList {
    ...
    SetPanel()
    {
        let validlist = this.GetValidList();
        // MPI
        validlist[MPIOptionsLabel.enablempi[0]] = 1;
        if(!this.m_jsonobj[MPIOptionsLabel.enablempi[0]]){
...
class SimulationProcess {
    ...
    async StartSingle(nompi){
        ...
        if(!nompi && GUIConf.GUIpanels[MPILabel].JSONObj[MPIOptionsLabel.enablempi[0]]){
            comtype = "solver";
        ...
        if(Framework == ServerLabel){
            ...
            this.m_worker.addEventListener("message", (msgobj) => {
                if(msgobj.data == "ready"){
                    this.m_worker.postMessage({data: prms, serno: this.m_serno});    
                }
...
function UpdateOptions(tgtid)
{
    ...
    if(item.categ == PartPlotConfLabel){
        ...
        SetDisableCalcIDs();
        if((item.categ == SrcLabel && item.item == SrcPrmsLabel.type[0]) ||
                GUIConf.GUIpanels[ConfigLabel].JSONObj[TypeLabel] == "")
        {
            OpenConfigPanel();
        }
        if(item.categ == ConfigLabel && 
            (item.item == ConfigPrmsLabel.wiggapprox[0] || item.item == ConfigPrmsLabel.esmooth[0]))
        {
            GUIConf.GUIpanels[AccLabel].SetPanel();
        }
    }
}
