...
[
    ...
    },
    {
        [chapters.calcsetup]: [
            ...
            {[sections.ppcomp]: [
                "To reduce the computation time, parallel computing based on the MPI is available in SPECTRA. To enable this option, refer to "+GetLink(MenuLabels.MPI, MenuLabels.MPI, false)+" setup dialog."
                ]
            }
        ]
    },
...
function GetSetupDialog()
{
    ...
    let dlgconts = [
        ...
        [MenuLabels.MPI, [
            "Open a dialog to configure the parallel computing. Tick "+GetQString(MPIOptionsLabel.enablempi[0])+" to enable the parallel computing and input a number of processes to launch in "+GetQString(MPIOptionsLabel.processes[0])+". Note that MPI (message passing interface) environment should be installed for parallel computing, and the path to \"mpiexec\" should be set."
        ], MenuLabels.MPI]
    ];
