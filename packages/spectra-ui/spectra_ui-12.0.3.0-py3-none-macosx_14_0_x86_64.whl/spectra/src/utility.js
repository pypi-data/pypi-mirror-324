...
function OnRightClickData(e, id)
{
    ...
    let guiwidth = document.getElementById("maingui").clientWidth;
...
async function ExportAsciiData(data, id, path = "")
{
    // base function to export ascii data (pre-processor, post-processor)
    if(RunApp == TauriLabel){
        if(path == ""){
...
function ShowDialog(title, issmall, killcancel, msg = "", cont = null, func = null)
{
    if(issmall){
    ...
    document.getElementById("modalOKsimple").onclick = func;
    document.getElementById("showModal").click();
}

function ChangeEdit()
{
    if(SIMPLEX.editid == null){
        return;
    }
    Settings[SIMPLEX.editid] = SIMPLEX.newsettings;
    SIMPLEX.editid = null;
}
...
function EditDialog(id)
{
    let option;
    if(id == DataUnitLabel){
        option = new DataUnitsOptions();
    }
    else if(id == AccuracyLabel){
        option = new AccuracyOptions();
        option.SetObjects(
            SIMPLEX.GUIpanels[AccLabel], SIMPLEX.GUIpanels[SrcLabel], SIMPLEX.GUIpanels[ConfigLabel]);
    }
    else if(id == MPILabel){
        option = new MPIOptions();        
    }
    else{
        return;
    }
    if(Settings[id] == null){
        Settings[id] = option.JSONObj;
    }
    SIMPLEX.newsettings =  CopyJSON(Settings[id]);
    SIMPLEX.editid = id;

    option.JSONObj = SIMPLEX.newsettings;
    option.SetPanel();

    ShowDialog(id, false, false, "", option.GetTable(), ChangeEdit);
}
// <-- dialogs

function CheckDataObj(obj)
{
    if(obj.hasOwnProperty(TitleLabel) == false){
        return false;
    }
    if(Array.isArray(obj[TitleLabel]) == false){
        return false;
    }
...
function CreateCheckBox(title, val, id)
{
    ...
    return {div: chkdiv, chkbox: chkbox};
}
...
function GetAsciiData2D(plobj)
{
    ...
    for(let n = 0; n < nplots; n++){
        ...
        if(objs[n].type == "heatmap"){
            xtitles.push(plobj.layout.xaxis.title.text);
            ytitles.push(plobj.layout.yaxis.title.text);    
        }
        else{
...
async function LoadFiles(e, func)
{
    ...
    for(let n = 0; n < nfiles; n++){
        func(objs[n].data, objs[n].name, n == nfiles-1);
    }
    SwitchSpinner(false);
}
...
function CreateMenuList(list, parent, insel = null)
{
    if(list.length < 1){
        return [];
    }
    ...
    list.forEach(element => {
        ...
        if(element.type == "separator"){
            ...
            let subids = CreateMenuList(element.submenu, ulp, insel);
