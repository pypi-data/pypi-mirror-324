...
class DataObject {
    ...
    LoadFile(fileobj){
        ...
        this.m_comment = {};
        if(fileobj.hasOwnProperty(OutputLabel)){
            if(fileobj[OutputLabel].hasOwnProperty(RelateDataLabel)){
                this.m_comment = CopyJSON(fileobj[OutputLabel][RelateDataLabel]);
            }
        }
        if(fileobj.hasOwnProperty(CMDResultLabel)){
            if(fileobj[CMDResultLabel].hasOwnProperty(CMDErrorLabel)){
                this.m_comment = CopyJSON(fileobj[CMDResultLabel][CMDErrorLabel]);
            }
        }
    ...
    ArrangeComparative(){
        ...
        for(let i = 0; i < this.m_allids.length; i++){
            ...
            if(scatter){
                ...
                isplot = false;
                for(let j = 0; j < plotitems.length; j++){
                    let titles = this.GetSpec("titles", id, true);
            ...
            for(let v = 0; v < varitems.length; v++){
                if(this.GetSpec("titles", id, true).indexOf(varitems[v]) < 0){
                    isvar = false;
                    break;
                }
            }
        ...
        for(let np = 0; np < nplots; np++){
            ...
            if(np < ncomplots){
                ...
                for(let nc = 0; nc < companim.length; nc++){
                    ...
                    let smtitles = this.GetSpec("titles", id, true);
                    citeminices.push([])
            ...
            if(si.length == 1){
                ...
                for(let s = 0; s < si.length; s++){
                    ...
                    if(this.GetSpec("details", id).length >= 2){
                        try { // try to retrieve the "details" label
                            leglocs.push(this.GetSpec("details", si[s][1])[si[s][0]]);
                        } catch { // if error found, simply add the original (id) label
                            leglocs.push(this.GetSpec("details", id)[si[s][0]]);
                        }
                    }
                    if(iscomp){
                        if(this.GetSpec("details", si[s][1]).length > 0){
                            leglocs.push(this.GetSpec("dataname", si[s][1]).split(IDSeparator)[0]);
                            leglocs.push(this.GetSpec("details", si[s][1])[si[s][0]]);
                        }
                        else{
