...
def make_upper_linear_conjecture(
    ...
    W = [pulp.LpVariable(f"w{i+1}") for i in range(complexity)]
    b = pulp.LpVariable("b")
    ...
    prob += pulp.lpSum([(W[i] * X[i][j] for i in range(complexity)) + b - Y[j] for j in range(num_instances)])

    # Define the LP constraints.
    for j in range(num_instances):
        prob += pulp.lpSum([W[i] * X[i][j] for i in range(complexity)]) + b >= Y[j]

    # Set bounds on the intercept term, if provided, add the constraints.
    if b_upper_bound:
    ...
    if prob.status != 1:
        ...
        if any(not isinstance(w, (int, float)) for w in W_values):
            return None
        if any(math.isinf(x) for x in W_values):
            return None
        if all(x == 0 for x in W_values):
            return None
        ...
        sharp_instances = [
            true_objects[j]
            for j in range(len(Y_true)) if
            Y_true[j] == sum(W_values[i] * X_true[i][j] for i in range(complexity)) + b_value
        ]
        sharp_instances = set(sharp_instances)
...
def make_lower_linear_conjecture(
    ...
    # Define the LP variables.
    W = [pulp.LpVariable(f"w_{i+1}") for i in range(complexity)]
    b = pulp.LpVariable("b")
    ...
    prob += pulp.lpSum(
        [(W[i] * X[i][j] for i in range(complexity)) + b - Y[j] for j in range(num_instances)]
    )
...
def make_all_linear_conjectures_range(
    ...
    # Adjust the complexity range to be inclusive
