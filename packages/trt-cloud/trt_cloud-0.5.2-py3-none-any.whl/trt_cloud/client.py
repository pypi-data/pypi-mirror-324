...
from trt_cloud.nvcf import NVCF_POLL_SECONDS_MAX, NVCFClient
...
class TRTCloud:
    ...
    def nvcf_client(self) -> NVCFClient:
        if self._nvcf_client is None:
            ...
            if self.config.nvcf_returns_504():
                self._nvcf_client.poll_seconds_timeout = NVCF_POLL_SECONDS_MAX
    ...
    def _build_with_request_body(
        ...
        with PrintMessageOnCtrlC(
            ...
            while status == 504:
                ...
                if status == 504:
                    # At first, the poll_seconds_timeout is set to only 5 seconds.
                    # Before 504 responses are enabled by NVCF, it is better to have a small
                    # poll_seconds_timeout so that the client can receive information about
                    # the request's position in queue. Once 504 responses are enabled in NVCF,
                    # it is better to set poll_seconds_timeout to the max allowed value.
                    if not self.config.nvcf_returns_504():
                        logging.info(
                            "Detected 504 'Gateway Timeout' response from NVCF. "
                            "Increasing timeout from %d seconds to %d seconds",
                            self.nvcf_client.poll_seconds_timeout,
                            NVCF_POLL_SECONDS_MAX
                        )
                        self.config.set_nvcf_returns_504()
                        self.nvcf_client.poll_seconds_timeout = NVCF_POLL_SECONDS_MAX
