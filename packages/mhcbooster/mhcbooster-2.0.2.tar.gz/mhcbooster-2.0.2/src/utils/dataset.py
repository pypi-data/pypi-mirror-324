...
def k_fold_split_so(peptides: Iterable[str],
                    s: np.array,
                    k_folds: int = 3,
                    random_state: Union[int, np.random.RandomState] = 0):
    ...
    s_unique = s[unique_peptide_indices]
    indices = np.argsort(s_unique)
    fold_indices = [[] for _ in range(k_folds)]
    k = list(range(k_folds))
    for i in range(len(indices) // k_folds):
        random_state.shuffle(k)
        for j in range(k_folds):
            fold_indices[k[j]].append(unique_peptide_indices[indices[i * k_folds + j]])
    for i in range(len(indices) % k_folds):
        fold_indices[i].append(unique_peptide_indices[indices[len(indices) - 1 - i]])

    random_state.shuffle(k)
    k = cycle(k)
    for index_list in nonunique_peptide_indices.values():
        val_k = next(k)
        fold_indices[val_k] += index_list
