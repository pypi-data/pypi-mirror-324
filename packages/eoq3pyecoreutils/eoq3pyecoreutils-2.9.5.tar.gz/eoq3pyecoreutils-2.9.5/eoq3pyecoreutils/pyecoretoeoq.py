...
def EPackageStructureToCmd(ePackage:EPackage, parentId:str, cmd:Cmp, objLut:dict, objCount:int, options:EcoreConversionOptions, logger:Logger=NoLogger())->list:
    ...
    #cmd.Upd(His(packId),M2MODEL.ID,ePackage.eGet(options.packageIdFeature),mute=options.muteUpdate)
    for c in ePackage.eClassifiers:
        ...
        elif(options.includeEnums and EEnum == type(c)):
...
def EObjectStructureToCmd(eObj:EObject, modelId:str, cmd:Cmp, objLut:dict, objCount:int, options:EcoreConversionOptions, logger:Logger=NoLogger())->list:
    importedObjs = []
    eClass = eObj.eClass
    #classStrId = eClass.name #this is ambigous
        ...
        if EAttribute == type(f):
            if(IsEPrimitiveType(eType) or isinstance(eType,EEnum)):
                ...
                if(f.many):
                    for v in eValue:
                        ...
                        cmd.Crt(STR(CONCEPTS.M1ATTRIBUTE),[feature,His(objId),v])
                elif(None!=eValue):
                    ...
                    cmd.Crt(STR(CONCEPTS.M1ATTRIBUTE),1,[feature,His(objId),eValue])
            else:
            ...
            if(f.many):
                for v in eValue:
                    ...
                    cmd.Crt(STR(CONCEPTS.M1COMPOSITION),1,[feature,His(objId),dstObj])
                    objCount += len(subobjs)
                    importedObjs += subobjs
