...
from eoq3.concepts import CONCEPTS, MXELEMENT, M2CLASS, M2MODEL, M2ATTRIBUTE,\
                          M2ASSOCIATION, M2COMPOSITION, M2ENUM, M2OPTIONOFENUM, M2PRIMITIVES
            ...
            if EAttribute == type(f):
                if(IsEPrimitiveType(eType) or isinstance(eType,EEnum)):
                    ...
                    if(f.many):
                        for v in eValue:
                            ...
                            self.PushCmd(Crt(STR(CONCEPTS.M1ATTRIBUTE),[STR(feature),obj,v]))
                    elif(None!=eValue):
                        ...
                        self.PushCmd(Crt(STR(CONCEPTS.M1ATTRIBUTE),1,[STR(feature),obj,eValue]))
                else:
                ...
                if(f.many):
                    for v in eValue:
                        ...
                        self.PushCmd(Crt(STR(CONCEPTS.M1COMPOSITION),1,[STR(feature),obj,dstObj]))
                elif(not f.many and None != eValue):
...
class EObjectOutStream(CmdStream):
    """Cmd stream that converts a series of commands in an pyecore data structure.
    For instance, 
        ...
        if(CMD_TYPES.CRT == cmdType):
            ...
            if(CONCEPTS.M2MODEL==concept):
                ...
                if(not eIsAny and self.options.autoOpositePrefix not in eSrcName):
                    ...
                    eOpposite = EReference(eDstName, eType=eType, containment=False, lower=eLower, upper=eSrcUpper)
