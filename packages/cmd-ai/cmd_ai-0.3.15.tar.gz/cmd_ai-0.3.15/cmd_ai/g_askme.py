...
def g_ask_chat(prompt, temp, model,  total_model_tokens=4096 * 2 - 50, tool=False):
    ...
    if not config.silent:
        print(
            f"i...  {fx.italic}{fg.lightslategray}max_tokens: {max_tokens}, model: {model};  {fg.default}{fx.default}"
        )
    # DEBUG PURPLE
...
# **********************************************************************************
# **********************************************************************************
def g_ask_claude(prompt , temp=0, model="claude-3-5-sonnet-20241022", max_tokens=1000, role=None):
    STOPPED_BY_LENGTH = False
    # ** init
...
def g_askme(
    ...
    resdi = None
    if MODEL_TO_USE.find("claude") >= 0:
        #print("C")
        resdi = g_ask_claude( prompt, temp, MODEL_TO_USE, estimate_tokens)

    else:
        #print("G")
        resdi = g_ask_chat( prompt, temp, MODEL_TO_USE, estimate_tokens, tool=tool )

    #if model_change:
    if resdi is None:
        return None
    return resdi[0],resdi[1], MODEL_TO_USE
    return resdi
