...
class AirflowBaseSensor(BaseSensorOperator):
    ...
    def poke(self, context):
        ...
        output = self._poke()
        if not output and self.is_optional and (datetime.datetime.now(timezone.utc) - self.task_instance.start_date).total_seconds() >= self.timeout:
            raise AirflowSkipException("Sensor has timed out. Option 'is_optional' = True, so current and following tasks are going to be skipped.")
