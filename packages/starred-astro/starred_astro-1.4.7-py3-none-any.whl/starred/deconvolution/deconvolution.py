...
class Deconv(object):
        ...
        if format == 'pkl':
            ...
            with h5py.File(path, 'w') as f:
                ...
                if save_output_level > 4:
                    ...
                    residuals = np.zeros((self.M, self.image_size, self.image_size))
                    deconvolutions = np.zeros((self.M, self.image_size_up, self.image_size_up))
                    for i in range(self.epochs):
                        ...
                        residuals[i, :, :] = dif
                        deconv[i, :, :] = deconv  # high resolution model
    ...
    def export(self, output_folder, kwargs_final, data, sigma_2, epoch=None, norm=1, format='fits', header=None):
        ...
        h = self._get_background(kwargs_final).reshape(self.image_size_up, self.image_size_up)
...
def setup_model(data, sigma_2, s, xs, ys, subsampling_factor, initial_a=None, astrometric_bound=None,
                dithering_bound=None, convolution_method='scipy', N_sersic=0):
    """
