...
def generate_image_caption(image_path, model_name="Salesforce/blip-image-captioning-base", cache_dir="./model_cache"):
...
def classify_image(image_path, model_name="google/vit-base-patch16-224", cache_dir="./model_cache"):
    """Classify an image using a vision transformer model."""
    from transformers import ViTImageProcessor, ViTForImageClassification

    processor = ViTImageProcessor.from_pretrained(model_name, cache_dir=cache_dir)
    model = ViTForImageClassification.from_pretrained(model_name, cache_dir=cache_dir)
    image = Image.open(image_path)
    inputs = processor(images=image, return_tensors="pt")
    outputs = model(**inputs)
    return model.config.id2label[torch.argmax(outputs.logits).item()]

def detect_objects(image_path, model_name="facebook/detr-resnet-50", cache_dir="./model_cache"):
    """Detect objects in an image using DETR."""
    from transformers import DetrImageProcessor, DetrForObjectDetection

    processor = DetrImageProcessor.from_pretrained(model_name, cache_dir=cache_dir)
    model = DetrForObjectDetection.from_pretrained(model_name, cache_dir=cache_dir)
    image = Image.open(image_path)
    inputs = processor(images=image, return_tensors="pt")
    outputs = model(**inputs)
