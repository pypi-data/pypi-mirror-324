...
class ModelGPT():
    ...
    def train_model(self, dataset_,val_dataset=None ,epochs=1, lr=5e-5, final_lr=5e-5, loss_avg=1500, change_batch=500, split_training=False, val_split=0.1,val_log=500):
        ...
        initial_lr = lr
        decay_rate = (final_lr / initial_lr) ** (1 / dataset_size)
        ...
        with strategy.scope():
            ...
            for epoch in range(epochs):
                ...
                for batch in progress_bar:
                    ...
                    all_train_loss.append(round(loss.numpy(), 4))
                    ...
                    progress_bar.set_postfix(loss=(sum(train_losses) / len(train_losses)), memory=f"{memory_used:.2f} GB", cpu=f"{cpu_percent}%", inst_loss=(train_losses[-1]),perplexity=(tf.exp(sum(train_losses) / len(train_losses)).numpy()))
