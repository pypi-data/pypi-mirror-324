...
class Trainer():
    def __init__(self,mix_pre=False,use_default=False,ds_name=None) -> None:
        ...
        def data(file_name,first_n=None,start_from=0,name="WICKED4950/MentalHeathEsther",local=False,answer_only=True,batch_size=8,maskq=False,toe_len=512):
        ...
        def train(epochs=1,val_dataset=None, lr=5e-5, final_lr=5e-5,loss_avg=1500,change_batch=500,split_training=False,val_split=0,val_log=500):
            if self.dataset.return_tf() == None:
                raise ValueError("You have to first load and process the data to train the model!")
            else:
                self.model.train_model(self.dataset.return_tf(),val_dataset, epochs,lr, final_lr, loss_avg, change_batch=change_batch,split_training=split_training, val_split=val_split,val_log=val_log)
                print("training done!")
