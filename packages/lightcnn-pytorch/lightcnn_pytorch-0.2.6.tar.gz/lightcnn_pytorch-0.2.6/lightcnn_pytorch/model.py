...
def download_weights():
    ...
    files = {
        ...
        "lightcnn_weights.pth.tar": "https://drive.google.com/uc?id=1rLFXCIz3Mg-8zLnwa7X2_3u10Nolwgbo",
    }
    ...
    for filename, url in files.items():
        ...
        if not os.path.exists(filepath):
            ...
            gdown.download(url, filepath, quiet=False, progress=True)
...
class LightCNN:
    ...
    def get_features(
        self, image: Union[str, np.ndarray], align: bool = True
    ) -> np.ndarray:
        """Extract features from a face image"""
        # Preprocess image
        processed_img = preprocess_face(image, align=align)
...
def verify(
    image1: Union[str, np.ndarray], image2: Union[str, np.ndarray], align: bool = True
) -> Tuple[float, bool]:
...
def get_features(image: Union[str, np.ndarray], align: bool = True) -> np.ndarray:
