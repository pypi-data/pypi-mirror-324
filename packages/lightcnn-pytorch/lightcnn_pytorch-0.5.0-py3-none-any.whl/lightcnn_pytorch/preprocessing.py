...
def preprocess_image(image: Union[str, np.ndarray], model_name: str = "LightCNN-V4") -> torch.Tensor:
    ...
    # Convert to grayscale and specific preprocessing for 29Layers
    if model_name == "LightCNN_29Layers_V2":
        image = image.convert('L')
        preprocess = transforms.Compose([
            transforms.Resize((144, 144)),  # Larger initial size
            transforms.CenterCrop((128, 128)),  # Then center crop
            transforms.ToTensor(),
            transforms.Normalize([0.5], [0.5])  # Mean and std normalization
        ])
    else:
        preprocess = transforms.Compose([
