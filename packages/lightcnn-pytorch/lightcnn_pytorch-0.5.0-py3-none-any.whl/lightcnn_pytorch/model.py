...
class LightCNN:
    ...
    def _load_model(self):
        if self.model_name == "LightCNN-V4":
            ...
            model = LightCNN_29Layers_v2(num_classes=80013)  # Specific num_classes for pretrained model
    ...
    def get_features(self, image: Union[str, np.ndarray], align: bool = True) -> np.ndarray:
        ...
        with torch.no_grad():
            if self.model_name == "LightCNN_29Layers_V2":
                # For 29Layers, we need to get the features before the final FC layer
    ...
    def verify(
        ...
        threshold = 0.25 if self.model_name == "LightCNN_29Layers_V2" else 0.5
