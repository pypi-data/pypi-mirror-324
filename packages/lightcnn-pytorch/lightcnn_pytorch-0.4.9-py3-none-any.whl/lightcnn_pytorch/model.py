...
class LightCNN:
    ...
    def _load_model(self):
        ...
        try:
            ...
            if "state_dict" in checkpoint:
                # Clean state dict keys and remove fc2 layer for 29Layers
                new_state_dict = {}
                for k, v in checkpoint["state_dict"].items():
                    if "fc2" not in k:  # Skip fc2 layer
    ...
    def get_features(self, image: Union[str, np.ndarray], align: bool = True) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            if isinstance(features, tuple):
                # For 29Layers, use the feature vector (first element)
                features = features[0]
    ...
    def verify(
        ...
        align: bool = True,
        threshold: float = 0.5  # Lower threshold for 29Layers
    ) -> Tuple[float, bool]:
