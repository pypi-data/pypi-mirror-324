...
def preprocess_face(image: Union[str, np.ndarray], align: bool = True) -> Optional[np.ndarray]:
    """Preprocess face for model input according to LightCNN v4 requirements:
    ...
    # Resize to 128x128 (keeping BGR format)
    resized = cv2.resize(face_crop, (128, 128))
    
    # Convert to float32 and normalize
    processed = resized.astype(np.float32)  # Explicitly use float32
    ...
    # Reorder to CHW format for PyTorch (keeping BGR order)
    processed = processed.transpose(2, 0, 1)
    
    return processed.astype(np.float32)  # Ensure float32 output
