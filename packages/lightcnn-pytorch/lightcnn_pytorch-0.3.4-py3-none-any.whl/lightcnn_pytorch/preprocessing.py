...
def preprocess_face(image: Union[str, np.ndarray], align: bool = True) -> Optional[np.ndarray]:
    ...
    # Normalize by mean [0,0,0] and std [255,255,255]
    processed /= 255.0
    
    # Add batch dimension and reorder to (batch, channels, height, width)
    processed = processed.transpose(2, 0, 1)  # BGR channels first
