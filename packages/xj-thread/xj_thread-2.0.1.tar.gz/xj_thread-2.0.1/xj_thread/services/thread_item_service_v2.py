...
edit_fields = [
    "thread_no",
    "category_value", "classify_id", "show_id", "group_id",
...
class ThreadItemService:
    ...
    def add(params: dict = None, **kwargs):
        ...
        category_dict = ThreadCategory.objects.filter(value=category_value).values().first()
        # print(">>> category_dict:", category_dict)
        if not category_dict:
            return None, f"类别值({category_value})不存在。"
        category_value = category_dict.get('value')
        ...
        query_params = JParameter.format(whole, filter_fields=format_fields, aliases=aliases)
        # print(">>> query_params:", query_params)

        # IO操作
        # try:
        # 主表插入数据
        thread_set = Thread.objects.create(**query_params, category_id=category_value)
    ...
    def detail(pk: int = None, uuid: str = None, auth_level: str = None):
        ...
        # ===== 拼接用户信息 =====
        user_info_dict, err = DetailInfoService.get_detail(user_id=main_item.get("user_id", None))
        if not user_info_dict:
            return None, f"用户不存在。({str(main_item['user_id'])})"
        aliases = {
            "tags": "user_tags",
            "sex": "user_sex",
            "signature": "user_signature",
        }
        user_item = {(aliases[k] if aliases.get(k) else k): v for k, v in user_info_dict.items()}
