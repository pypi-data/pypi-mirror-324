...
def findColorsYX(color,image=[],tolorance=0):
    '''
...
def checkColorByXY(x,y,image=[],color=(241,234,41),
    ...
    if len(image)==0:image = pyautogui.screenshot()
    image=np.array(image)
    ps=findColorsYX(color,image,color_tolorance)
    if len(ps)>0:
        print('目標y,x:',ps[0])
...
def findColorsInSizeYX(color,image=[],tolorance=0,size=300):
    '''
    給定 image ，在上面找到和 color 相近顏色的座標
    [(y1,x1),(y2,x2)...]
...
def instinctOn(p1=(932,902),p2=(985,910)):
    ...
    L,T=p1
    R,B=p2
    ps=findColorsYX((124, 216, 239),img[T:B,L:R],tolorance=30)
