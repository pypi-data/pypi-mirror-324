...
def findColors(color,image=[],tolorance=0):
    '''
...
def checkColorByXY(x,y,image=[],color=(241,234,41),
                   color_tolorance=25,
                   XY_tolorance=8):
    '''
    ...
    if len(image)==0:image = pyautogui.screenshot()
    image=np.array(image)
    ps=findColors(color,image,color_tolorance)
    if len(ps)>0:
        print('目標y,x:',ps[0])
        dist = np.linalg.norm(ps-np.array([y,x]), axis=-1)
    #     print("===============")
    #     print(dist)
...
def findColorsInSize(color,image=[],tolorance=0,size=300):
    '''
    給定 image ，在上面找到和 color 相近顏色的座標
    並回傳座標陣列，tolorance 為容忍範圍
    size為上下左右搜與範圍
...
def findNearestColor(color,tolorance=0):
    '''
    找到離角色(螢幕中心)最近的指定顏色 color
    '''
    global Xc,Yc
    a = findColors(color,tolorance=tolorance)
...
def instinctOn(p1=(932,902),p2=(985,910)):
    ...
    L,T=p1
    R,B=p2
    ps=findColors((124, 216, 239),img[T:B,L:R],tolorance=30)
    if len(ps)<10:press('e')
    
    
def getMission(p1=(1400,560),p2=(1400,510)):
    '''
    取得任務
    p1是選擇任務的座標
    p2是按「確認」的座標
...
def close2Color(color=(241, 234, 41),size=300,tolorance=25,
    ...
    global Xc,Yc
    while True:
        ps=findColorsInSize(color, tolorance=tolorance,size=size)
        if(len(ps)>0):
            y,x=ps[0]
            y+=offsetY
            x+=offsetX
...
# (x,y)=(1587,918),pixel=(79, 80, 84)
# (x,y)=(1593,918),pixel=(185, 186, 187)

# print(isComplete(targetImgPath='goal.png', p1=(740,596)))

# ps=findColors((255,0,0))
# print(ps)
