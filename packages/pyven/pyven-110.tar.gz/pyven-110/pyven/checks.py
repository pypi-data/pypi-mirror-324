...
from lagoon.text import diff, pydoc_markdown
from pathlib import Path
from setuptools import find_packages
from shutil import copy2, copytree, ignore_patterns
from tempfile import NamedTemporaryFile
from urllib.request import urlopen
from venvpool import dotpy, initlogging, Pool, TemporaryDirectory
...
class EveryVersion:
    ...
    def allchecks(self, exclude = ()):
        for check in self.licheck, self.nlcheck, self.execcheck, self.metacheck, self.pyflakes, self.nose, self.readme, self.github, self.linktree:
    ...
    def pyflakes(self):
        ...
        def pyflakes():
            if paths:
                with Pool(3).readonly(SimpleInstallDeps(['pyflakes>=3.2'])) as venv:
    ...
    def nose(self):
        ...
        with InstallDeps(self.info, self.siblings, _localrepo() if self.userepo else None) as installdeps:
            installdeps.add('nose-cov', 'pynose', *self.info.config.test.requires) # TODO: Facility to freeze test requires tree.
            ...
            if self.docker:
                ...
                with bgcontainer('-v', "{0}:{0}".format('/var/run/docker.sock'), '--network', 'host', '-v', f"{os.path.abspath(self.info.projectdir)}:{Container.workdir}", pyimage(self.info.pyversiontags[0], upstream_devel_packages, upstream_devel_scripts, [])) as container:
                    ...
                    *sum((['--cov', p] for p in chain(find_packages(self.info.projectdir), self.info.py_modules())), []), *map(cpath, self.files.testpaths(xmlpath)), *self.noseargs, check = False)
            else:
                coveragepath = Path('.coverage')
                with Pool(3).readonlyortransient[self.transient](installdeps) as venv:
