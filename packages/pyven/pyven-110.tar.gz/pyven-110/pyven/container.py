...
def pyimage(pyversiontag, packages, scripts, requires):
    cc = ConfigCtrl()
    ...
    for p in packages:
        cc.w.package += p
    requires_sh = ' '.join(map(shlex.quote, requires))
    cc.w.pipinstallsh = f"sudo -u pyvenuser pip install --user --no-warn-script-location {requires_sh}" if requires_sh else ':'
...
class Container:
    ...
    def installasuser(self, requires):
        if requires:
            docker.exec[print](self.cid, 'sudo', '-u', 'pyvenuser', 'pip', 'install', '--user', *requires)
