...
from .util import bgcontainer, Excludes, initapt, stderr
...
from lagoon.text import diff, docker, pydoc_markdown
    ...
    def nose(self):
        ...
        with InstallDeps(self.info, self.siblings, _localrepo() if self.userepo else None) as installdeps:
            ...
            if self.docker:
                ...
                with bgcontainer('-v', "{0}:{0}".format('/var/run/docker.sock'), '--network', 'host', '-v', f"{os.path.abspath(self.info.projectdir)}:{Container.workdir}", f"python:{self.info.pyversiontags[0]}") as container:
