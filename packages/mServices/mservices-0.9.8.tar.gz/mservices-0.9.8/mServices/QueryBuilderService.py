class QueryBuilderService:
    ...
    def paginate(self, per_page=10, current_page=1):
        # Ensure the current_page is at least 1
        ...
        # Get total records
        total_query = f"SELECT COUNT(*) AS total FROM {self.table}"
        total_result = self.execute_query(total_query)

        # Handle different database drivers' response formats
        total = total_result[0]['total'] if total_result and 'total' in total_result[0] else 0
