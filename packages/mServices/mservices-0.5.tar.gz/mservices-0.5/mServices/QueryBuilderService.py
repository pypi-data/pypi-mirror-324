...
class QueryBuilderService:
    ...
    def get(self):
        ...
        for condition in self.conditions:
            field = condition[0].split('.')  # Split table and column (e.g., users.id)
            if len(field) == 2:
                # Adjust field lookup for the parent table, like 'users.id'
                query = query.filter(**{f'{field[1]}': condition[1]})
            else:
