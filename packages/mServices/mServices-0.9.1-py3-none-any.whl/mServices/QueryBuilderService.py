...
class QueryBuilderService:
    ...
    def get(self):
        ...
        if self.select_fields:
            # Ensure no spaces in aliases, convert 'roles.name as role_name' to 'roles_name'
            fields = {}
            for field in self.select_fields:
                if ' as ' in field:
                    field_parts = field.split(' as ')
                    field = field_parts[0].replace('.', '_')  # Replace dot with underscore
                fields[field] = field
            query = query.values(*fields.keys())
