...
from superwise_api.client import rest
...
class ApiClient:
    """Generic API client for Superwise client library builds.
    ...
    def __deserialize(self, data, klass):
        ...
        if isinstance(klass, str):
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                raise ApiValueError(f"Class {klass} is not a valid type.")
    ...
    def update_params_for_auth(self, headers, queries, auth_settings, resource_path, method, body, request_auth=None):
        ...
        :_resource_path: A string representation of the HTTP request resource path.
    ...
    def replace(
        self, resource_path: str, model_name: str, model_class: type[BaseModel], entity_id: str, data: dict, **kwargs
    ) -> BaseModel:
        """
        Replace an entity
        ...
        """
        resource_path = f"{resource_path}/{entity_id}"
        response_types_map = {
            "200": model_class,
            "404": None,
    ...
    def put(self, resource_path: str, data: Optional[dict] = None, **kwargs) -> BaseModel:
        ...
        for _key, _val in _params["kwargs"].items():
            if _key not in _all_params:
                raise ApiTypeError("Got an unexpected keyword argument '%s'" " to method put" % _key)
            _params[_key] = _val
        del _params["kwargs"]
        ...
        _auth_settings = ["implicit"]

        _response_types_map = kwargs.get("response_types_map", {})
    ...
    def create(
        self, resource_path: str, model_name: str, model_class: type[BaseModel], data: dict, **kwargs
    ) -> BaseModel:
        """Create new entity
    ...
    def post(self, resource_path: str, data: Optional[dict] = None, **kwargs) -> BaseModel:
        _params = locals()
        ...
        for _key, _val in _params["kwargs"].items():
            if _key not in _all_params:
                raise ApiTypeError("Got an unexpected keyword argument '%s'" " to method create" % _key)
            _params[_key] = _val
        del _params["kwargs"]
    ...
    def update(
        ...
        for _key, _val in _params["kwargs"].items():
            if _key not in _all_params:
                raise ApiTypeError(f"Got an unexpected keyword argument '{_key}' to method update")
            _params[_key] = _val
        del _params["kwargs"]
    ...
    def get_by_id(
        self, resource_path: str, model_name: str, model_class: type[BaseModel], entity_id: str, **kwargs
    ) -> BaseModel:
        """Get an entity by ID
        ...
        for _key, _val in _params["kwargs"].items():
            if _key not in _all_params:
                raise ApiTypeError(f"Got an unexpected keyword argument '{_key}' to method get_{model_name}")
            _params[_key] = _val
        del _params["kwargs"]
    ...
    def get(
        self, resource_path: str, model_name: str, model_class: type[BaseModel], query_params: dict, **kwargs
    ) -> Page:
        _params = locals()
        ...
        for _key, _val in _params["kwargs"].items():
            if _key not in _all_params:
                raise ApiTypeError(f"Got an unexpected keyword argument '{_key}' to method get_entities")
            _params[_key] = _val
        del _params["kwargs"]
        ...
        _response_types_map = {
            "200": Page.set_model(model_class),
            "404": None,
            "422": None,
            "500": None,
        }
