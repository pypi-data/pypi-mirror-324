...
class APIClient:
    ...
    def update_node(self, node: dict):
        ...
        # Convert Python datetime to RFC3339 (with trailing 'Z') if present
        if node.get('rent_start_time'):
            node['rent_start_time'] = node['rent_start_time'].isoformat().replace('+00:00', 'Z')
        if node.get('last_balance_update_timestamp'):
    ...
    def create_rent_session(self, rent_session: dict):
        ...
        if 'rent_start_time' in rent_session and rent_session['rent_start_time']:
            rent_session['rent_start_time'] = rent_session['rent_start_time'].isoformat().replace('+00:00', 'Z')
        if ('last_balance_update_timestamp' in rent_session
        ...
        if 'rent_stop_time' in rent_session and rent_session['rent_stop_time']:
    ...
    def get_rent_session(self, node_id: int, status: str = None, platform: str = 'telegram'):
        """
        Retrieve rent sessions based on provided filters.

        Args:
            node_id (int): The node ID to filter by. If your API expects other filters like 'renter',
