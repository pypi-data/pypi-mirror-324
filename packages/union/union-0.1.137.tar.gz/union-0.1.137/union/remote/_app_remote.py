...
class AppRemote:
    def __init__(
        ...
        self.config = self._union_remote.config
    ...
    def _watch_update(self, app_idl: AppIDL, run_request: Callable[[], AppIDL]):
        console = Console()
        ...
        async def watch_status(console):
            latest_message = ""
            status: Status
            async for status in merge(watch_for_status(), poll_for_status()):
