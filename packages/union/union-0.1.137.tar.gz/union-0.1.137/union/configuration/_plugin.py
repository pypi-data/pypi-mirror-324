...
class UnionAIPlugin:
    ...
    @staticmethod
    def get_app_remote(project: str, domain: str, config: Optional[str] = None):
        """Create an app remote when interacting with the union CLI."""
    ...
    @staticmethod
    def _get_config_for_remote(config):
        ...
        if _UNION_CONFIG.config_source == ConfigSource.CLI.value:
            ...
            config = _get_config_obj(config, default_to_union_semantics=False)
        return config
