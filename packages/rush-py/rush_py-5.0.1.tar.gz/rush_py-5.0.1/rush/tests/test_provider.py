...
async def benchmark_submission(
    ...
    return await provider.run_benchmark(
        benchmark.id, "\\i -> (outputs i)", "echo submission", sample=0.2
    )
...
\\input ->
    ...
    return await provider.run_benchmark(
        benchmark.id, rex, "actual submission"
    )  # , sample=0.2)
...
@pytest.mark.asyncio
async def test_benchmark_submissions(
    provider: Provider, benchmark_submission: RunBenchmarkRunBenchmark
):
