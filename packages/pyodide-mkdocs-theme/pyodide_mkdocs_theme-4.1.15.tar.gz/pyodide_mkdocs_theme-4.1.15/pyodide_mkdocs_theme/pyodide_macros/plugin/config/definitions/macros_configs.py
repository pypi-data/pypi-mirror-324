...
PY_GLOBAL = SubConfigSrc.with_default_docs(
    ...
    ConfigOptionIdeLink(
        'SANS', str, default="",
        docs=f"""
            Pour interdire des fonctions builtins, des modules, des accès d'attributs ou de
            méthodes, ou des mots clefs : chaîne de noms séparés par des virgules et/ou espaces.
            Les mots clefs viennent en dernier, après le séparateur `{ KEYWORDS_SEPARATOR }`.
        """,
        yaml_desc="""
...
IDE = MacroConfigSrc(
    ...
    *_py_globals_copy_gen(
    ...
    ConfigOptionIdeLink(
        ...
        docs = """
            Définis si le contenu de l'éditeur de cet IDE doit être ajouté à l'archive zip
...
FIGURE = MacroConfigSrc.with_default_docs(
    ...
    ConfigOptionSrc(
        'p5_buttons', str, conf_type=C.Choice(P5_BTNS_LOCATIONS), is_optional=True,
        docs = f"""
            ...
            { items_comma_joiner(['`#!py "'+loc+'"`' for loc in P5_BTNS_LOCATIONS]) }.
        """,
        yaml_desc="""
