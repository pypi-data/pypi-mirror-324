...
def read_agsm(file):
    ...
    return modes,(mstar/msun,rstar/rsun)
...
def osc_spectrum_model_adipls(starID, f,modes,numax,deltanu,teff,verbose=False,a=0.,b=1.,plot=False,win=0, rot_period_sur = -1., incl = 0., path='./'):
    '''
...
###    V1 = 0.2
    ...
    for i in range(nm): # loop over the modes (n,l)
        ...
        for lm in range(lm1,lm2+1): # loop over the multiplets 
            if (rot_period_sur <= 1e-10):
                split = 0.    
            else:
                split = lm/(rot_period_sur*86400.)*1e6*modes[i,8]
            icase = int(modes[i,6])
...
def gen_osc_spectrum(starID, filename, teff,mass,radius,dt,T, mag , seed = None, pn_ref = 7.7, wn_ref = 0. , mag_ref = 6.,verbose = False, GST = 1 , a=0 , b=1.  ,  plot=False, rot_period_sur = -1., incl = 0., activity = None, granulation = True , path= './' , oscillation=True, type = 1, numax = -1., deltanu = -1.):
    '''
    ...
    if(type==0):
        ...
        modes, (mass, radius) = read_agsm(filename)
        star_par = {'mass': mass, 'teff' : teff, 'radius' : radius, 'logg': math.log10(mass/radius**2) +logg_sun}
...
def gen_up(starID, numax,dt,T, mag , delta_nu =  -1. , mass = -1. , seed = None, pn_ref = 7.7, wn_ref= 0.,
           mag_ref = 6.,verbose = False, teff = 4800., DPI = -1.,  q = 0. , GST = 0 , incl = -1 , rot_f = 0., path= '',
           granulation = True , oscillation=True,activity=None):
    '''
    ...
    if(oscillation):
        fout = open(path + StarName + '.modes','w')
        UPm,_ = UP.universal_pattern(delta_nu, numax, H_env,  f , nyq , verbose = verbose , teff = teff, DPI = DPI, q=q, gamma=gamma, width = width_env , rot_f = rot_f , beta = incl , epsg = 0.25 , fout = fout)
        fout.close()
    else:
        UPm = 0.
    ...
    osc_par = {'numax' : numax, 'deltanu': delta_nu, 'Hmax' : H_env, 'Gammamax': gamma, 'width_envelop' : width_env,'inclination': incl, 'rot_f': rot_f }
