...
class BBVIConverter(Converter):
    ...
    def convert(
        ...
        **kwargs,
        ...
        BBVI_model.stochastic_parameters = [
            (name, module)
            for name, module in BBVI_model.named_modules()
            if isinstance(module, stochastic_parameter)
        ]

        kl_denominator = 0
        for name, stoch_param in BBVI_model.stochastic_parameters:
            kl_denominator += sum(p.numel() for p in stoch_param.parameters())

        BBVI_model.kl_denominator = kl_denominator
