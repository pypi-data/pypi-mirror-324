...
File:
    inspy_logger/helpers/descriptors.py

Description:
    This module contains the `RestrictedSetter` class, which provides a way to restrict the setting of a property in a class.
    It allows for type checking, value restriction, condition enforcement, and setter method restriction to ensure
    that properties are set correctly and safely.

Functions:
    RestrictedSetter.__init__(self, name, initial=None, allowed_types=None, preferred_type=None, allowed_values=None, condition=None, exception=None, exception_args=None, restrict_setter=True):
        Initializes a new RestrictedSetter instance.

    RestrictedSetter.__get__(self, obj, obj_type=None):
        Getter method for the property.

    RestrictedSetter.__set__(self, obj, value):
        Setter method that enforces restrictions.

    RestrictedSetter.__delete__(self, obj):
...
Dependencies:
    - inspect:
        Used to inspect call stack and ensure that the property setter is only called from within class methods.
...
Example Usage:
    ...
    >>> obj = MyClass()
    >>> obj.my_property = 10
    # Sets the property if the value is allowed and conditions are met

    >>> obj.my_property = -5
    # Raises an exception if the value is not allowed or conditions are not met
