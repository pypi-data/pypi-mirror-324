...
import json
import urllib
from rota.util.remote_url import RemoteUrl, Absolute
...
class Repository:
    ...
    def rec_search_for_repo(folder: str) -> str:
        ...
        if os.path.exists(os.path.join(folder, Repository.CONFIG_FOLDER, Repository.OLD_CFG_FILE)):
            return folder
        if os.path.exists(os.path.join(folder, ".tko", Repository.NEW_CFG_FILE)):
            Repository.CONFIG_FOLDER = ".tko"
            return folder
        if os.path.exists(os.path.join(folder, ".tko", Repository.OLD_CFG_FILE)):
            Repository.CONFIG_FOLDER = ".tko"
            return folder
