...
from rota.util.pattern import PatternLoader
from rota.util.consts import DiffCount, DiffMode
from rota.settings.settings import Settings

from rota.settings.repository import Repository
from rota.util.raw_terminal import RawTerminal
from rota.util.symbols import symbols
from rota.settings.check_version import CheckVersion
from rota.settings.repository_starter import RepStarter

import os

from rota.__init__ import __version__
...
class Parser:
    def __init__(self):
        self.parser: argparse.ArgumentParser = argparse.ArgumentParser(prog='rota', description=f'rota version {__version__}')
...
def exec(parser: argparse.ArgumentParser, args):
    ...
    if args.v:
        if args.v:
            print("rota version " + __version__)
    else:
        if "func" in args:
            args.func(args)
        else:
