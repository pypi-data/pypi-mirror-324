...
class DesyncClient:
    ...
    def crawl(
        ...
        - Depth 1..N: gather unique same-domain links, do bulk_search + collect_results.
        ...
        # Collect same-domain links from the initial page
        def same_domain_links(page_data):
            """
            Returns all same-domain links from a PageData (both internal & external).
            """
            combined_links = page_data.internal_links + page_data.external_links
            return [
        ...
        while depth <= max_depth and current_depth_links:
            ...
            for page_obj in new_pages:
                page_obj.depth = depth
                all_results.append(page_obj)

            # 3) Gather the next set of same-domain links
            next_depth_links = []
            for page_obj in new_pages:
