...
class LucidicAPI:
    ...
    def analyzeAgentActions(self, task, screenshots, context):
        ...
        prompt = f'This is a web agent to complete the task {task}. Our sole goal is to complete this task. Make sure we can complete all the necessary pre-requisite steps are met i.e all the things on the website are selected before moving on. The context for the current step is {context}. If this task did not work tell me what the error was and give detailed, actionable steps to fix it. Assume the website works. Treat this as a human being who is trying to navigate the website and complete its goal. Remember doing the same thing over and over again doesn’t help the previous action was {self.agentResponseHistory}'
        content = [    
            {
                "type": "text",
                "text": prompt
