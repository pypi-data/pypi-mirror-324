...
class LucidicAPI:
    ...
    def analyzeAgentActions(self, task, screenshots, context):
        ...
        - `"Incorrect. Error: [most_likely_mistake]. Correct action: Click/select/input [specific action immediately available on the current screen]."`  
        - `"Error: No valid action is available on the current screen. This suggests a previous step was incorrect or a required interaction is missing. Correct action: Scroll up and click/select/input [specific corrective action needed to proceed]."`
        """

        retry_prompt = f"""
        Verify whether the following action can be performed on the given screen.

        ### Input:
        - **Screen Image**: [Provided image of the current screen]
        - **Proposed Action**: <PROPOSED_ACTION> (e.g., clicking a button, selecting an option, filling a field)

        ### Verification Rules:
        1. The action **must be immediately visible and interactable on the current screen**.
        2. If the element is **grayed out, missing, off-screen, or unclickable**, return `"NO"` with a specific reason.
        3. If another step (such as scrolling or selecting a prerequisite field) is required before performing this action, return `"NO"` with the missing requirement.
        4. **If everything is correct and the action is immediately possible, return only `"YES"`—no additional text, reasoning, or explanation.**
        5. If the action **is not possible**, return `"NO: [specific reason why the action cannot be taken]"`.

        ### Output Format:
        - `"YES"` (if the action is fully possible on the current screen)
        - `"NO: [specific reason why the action cannot be taken]"`  
        """

        lastResponse = ""
        while 'YES' not in lastResponse[:5].upper():
            if lastResponse != "":
                retry_prompt = retry_prompt.replace('<PROPOSED_ACTION>', lastResponse)
            content = [    
                {
                    "type": "text",
                    "text": prompt
