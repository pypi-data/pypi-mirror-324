...
class LucidicAPI:
    ...
    def analyzeAgentActions(self, task, screenshots, context):
        ...
        prompt = f"""
        Your goal is to **make progress toward completing the task** ({task}) like a human, handling all necessary steps and resolving issues dynamically. Progress means either moving forward or going back to fix mistakes when necessary.  
        ...
        ### Evaluation Rules:
        1. Take only **one immediate actionable step** that is visible and interactable on the screen (e.g., clicking a button, selecting an option, filling a field).  
        2. Your action **must contribute to progress**—either by moving forward or correcting a mistake from an earlier step.  
        3. If a **button or element appears unclickable**, a **previous step was likely missed or incomplete**—determine what must be done to enable it and fix that first.  
        4. If the screen **does not change**, assume a previous step was incorrect and provide the **specific action needed** to fix it.  
        5. If the **same advice has been given multiple times with no progress**, identify the **underlying issue from an earlier step** and specify the exact corrective action.  

        ### Output Format (Only one of the following):  
        - `"Correct. Next action to make progress: Click/select/input [specific action required]."`  
        - `"Incorrect. Error: [most_likely_mistake]. To make progress, correct action: Click/select/input [specific action required]."`  
        - `"Error: No progress detected. A previous step may have been incorrect. To move forward, correct action: Go back and click/select/input [specific corrective action]."`  
        - `"Error: Button or element appears unclickable. This likely means a required step was missed. Correct action: [specific step needed to enable interaction]."`
        """
