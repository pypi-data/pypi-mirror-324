...
class LucidicAPI:
    ...
    def analyzeAgentActions(self, task, screenshots, context):
        ...
        prompt = f"""
        Our goal is to **successfully complete the task** ({task}) like a human, handling all necessary steps and solving issues dynamically.  
        ...
        ### Evaluation Rules:
        1. Take only **one immediate actionable step** available on the screen.  
        2. Avoid repeating the same action unnecessarily.  
        3. If the screen **does not change**, assume a previous step was incorrect and fix that instead of repeating the same action.  
        4. If the **same advice has been given multiple times with no progress**, identify the **earlier mistake** that may have caused the issue and correct it.  

        ### Output Format (Only one of the following):  
        - `"Correct. Next action: [one immediate actionable step]."`  
        - `"Incorrect. Error: [most_likely_mistake]. Correct action: [one immediate next step]."`  
        - `"Error: No progress detected. A previous step may have been incorrect. Correct action: [one earlier step that should be fixed]."`
        """
