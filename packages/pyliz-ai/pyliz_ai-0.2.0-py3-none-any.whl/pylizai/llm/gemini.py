...
class GeminiController:
    ...
    def scan_image(self, query: AiQuery) -> str:
        ...
        response = model.generate_content(
            [uri, AiResult.get_json_schema_prompt(query.prompt, query.setting)],
            generation_config=genai.GenerationConfig(response_mime_type="application/json") if json_in_response else None,
        )
    ...
    def gen_text(self, model_name: str, prompt: str):
        try:
            ...
            return Operation(status=True, payload=result.text)
        except Exception as e:
            return Operation(status=False, error=str(e))
