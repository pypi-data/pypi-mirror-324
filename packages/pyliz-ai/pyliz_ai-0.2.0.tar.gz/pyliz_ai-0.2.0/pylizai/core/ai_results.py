...
class AiResult:
    ...
    def save_results(self, save_str: bool = True, save_obj: bool = True):
        try:
            if save_str:
    ...
    @staticmethod
    def get_json_schema_prompt(prompt: str, setting: AiSetting):
        if setting.return_type == AiReturnType.OBJECT:
            if setting.return_type_object is None:
            ...
            schema = setting.return_type_object.model_json_schema()
            new_prompt = prompt + "\n\n" + "Use this JSON schema:" + "\n" + f"ReturnObject = {schema}" + "\n" + "Return: ReturnObject"
            return new_prompt
        else:
            return prompt
