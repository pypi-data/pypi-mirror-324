...
def test_create_provider(executor_fixture, filestore_fixture, request):
    ...
    config = SparkConfig(
        ...
        spark_flags=SparkFlags(
            spark_params={"spark.executor.memory": "2g"},
            write_options={"header": "true"},
            table_properties={"parquet.compression": "gzip"},
        ),
    )
    provider = Provider(
        name=provider_name,
...
@pytest.mark.parametrize(
    ...
    [
        ("test_input", "primary_source_sql_table", "primary_source_sql_table", "avg_user_transaction"),
        (
            "test_input",
            "df_transformation",
...
def test_df_transformation(
    ...
    expected_src = SourceVariant(
        ...
        definition=DFTransformation(
            query=query, inputs=inputs, source_text=source_text, canonical_func_text=decorator.canonical_func_text
        ),
        owner="tester",
        provider="spark",
        description="doc string",
...
def test_incremental_inputs(spark_provider, primary_source_file_store, primary_source_sql_table):
    inc = Incremental(primary_source_file_store)
    normal_input = primary_source_sql_table

    query = "SELECT * FROM {{ tbl }} JOIN {{ tbl2 }} ON tbl.id = tbl2.id"

    def transformation(tbl, tbl2):
        """doc string"""
        return query

    decorator = spark_provider.incremental_sql_transformation(
        inputs=[inc, normal_input], variant="test"
    )
    decorator(transformation)

    expected_definition = SQLTransformation(
        func_params_to_inputs={"tbl": primary_source_file_store, "tbl2": normal_input},
        query=query,
        incremental_inputs=[primary_source_file_store],
        is_incremental=True,
    )
...
def test_streaming_inputs(spark_provider, primary_source_file_store, primary_source_sql_table):
    inc = Stream(primary_source_file_store)
    normal_input = primary_source_sql_table
    ...
    decorator = spark_provider.streaming_sql_transformation(
        inputs=[inc, normal_input], variant="test"
    )
    decorator(transformation)
    ...
    expected_definition = SQLTransformation(
        func_params_to_inputs={"tbl": primary_source_file_store, "tbl2": normal_input},
        query=query,
        streaming_inputs=[primary_source_file_store],
        is_streaming=True,
    )
