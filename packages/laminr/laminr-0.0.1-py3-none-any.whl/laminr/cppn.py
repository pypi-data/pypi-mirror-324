...
class CPPNTemplates(nn.Module):
    def __init__(
        ...
        init_noise_scale_coordinate_transformation=.1, 
        jitter_coords=False,
        jitter_coords_scale=1.,
    ):
        super().__init__()
        self.setup_attributes(
            num_neurons, img_res, num_templates, out_channels, aux_dim,
            weights_scale, periodic_invariance, 
            positional_encoding_dim, positional_encoding_projection_scale,
            aux_positional_encoding_dim, aux_positional_encoding_projection_scale, 
            coordinate_transform_clamp_boundaries, jitter_coords, jitter_coords_scale,
            )
    ...
    def forward(self, img_res=None, aux=None, return_template=False, centralize_template=False):
        ...
        coords_transformed = self.apply_coordinate_transform(coords, return_template, centralize_template)
        aux_transformed = self.transform_auxiliary_input(aux, device)
        coords_transformed_pe, aux_transformed_pe = self.apply_positional_encoding(coords_transformed, aux_transformed)
        ...
        inputs = combine_multiple_tensors(self.template_input, aux_transformed_pe, coords_transformed_pe)
        T, N, D, HW = tensors_coordinate_dims(self.template_input, aux_transformed_pe, coords_transformed_pe)
        
        assert HW == np.prod(img_res)
        out = self.func(inputs).reshape(T, N, D, *img_res, self.out_channels).permute(0, 2, 1, 5, 3, 4)
        
        output_aux_dim_unpacked = [round(len(aux) ** (1/self.aux_dim))] * self.aux_dim

        if return_template:
            return out.squeeze(0).squeeze(0).reshape(*output_aux_dim_unpacked, self.out_channels, *img_res)
        else:
            return out.squeeze(0).reshape(D, *output_aux_dim_unpacked, self.out_channels, *img_res)
    ...
    def setup_attributes(
        ...
        jitter_coords, jitter_coords_scale):
        ...
        self.aux_positional_encoding_projection_scale = aux_positional_encoding_projection_scale
        self.coordinate_transform_clamp_boundaries = coordinate_transform_clamp_boundaries
    ...
    def build_network(self, widths, nonlinearity, final_nonlinearity, bias):
        ...
        for i in range(layers):
            elements.append((f"layer{i}", nn.Linear(in_widths[i], widths[i], bias=bias)))
            if nonlinearity is not None:
                elements.append((f"nonlinearity{i}", nonlinearity()))

        elements.append((f"layer{layers}", nn.Linear(widths[-1], self.out_channels, bias=bias)))
    ...
    def initialize_buffers(self):
        if self.positional_encoding_dim is not None:
            self.register_buffer("B", torch.randn(2, self.positional_encoding_dim) * self.positional_encoding_projection_scale)
        if self.aux_positional_encoding_dim is not None:
            auxB_shape = (2*self.aux_dim, self.aux_positional_encoding_dim) if self.periodic_invariance else (1*self.aux_dim, self.aux_positional_encoding_dim)
            self.register_buffer("auxB", torch.randn(auxB_shape) * self.aux_positional_encoding_projection_scale)

        coords = torch.meshgrid([torch.linspace(-1, 1, linres) for linres in self.img_res], indexing="ij")
        coords = torch.stack(coords, dim=-1)
        self.register_buffer('coords', coords)

        template_input = torch.linspace(-1, 1, self.num_templates) if self.num_templates > 1 else torch.zeros(1)
    ...
    def get_coords_shift_to_center(self, template_rf_center, img_res=None):
        ...
        coords_shift = (template_rf_center.to(device) - img_center_coord) / img_center_coord
        return coords_shift
    
    def get_coords_shift_from_center(self, template_rf_center, img_res=None):
        shift_to_center = self.get_coords_shift_to_center(template_rf_center, img_res=img_res)
    ...
    def register_coords_shifts(self, template_rf_center, target_rf_centers):
        self.register_buffer("coords_shift_to_center", self.get_coords_shift_to_center(template_rf_center))
        self.register_buffer("template_center_in_coords_space", self.get_template_center_in_coords_space(template_rf_center))
        self.register_buffer("coords_shift_from_center", self.get_coords_shift_from_center(target_rf_centers))
    ...
    def get_coordinates(self, img_res, device):
        if img_res is None:
            ...
            coords = torch.stack(torch.meshgrid(
                [torch.linspace(-1, 1, shape, device=device) for shape in img_res], indexing="ij"
            ), dim=-1)
        if self.jitter_coords and self.training: # only jitter during training
            coords_noise = (torch.rand_like(coords) - .5) * torch.tensor([[[2/img_res[0], 2/img_res[1]]]]).to(device)
    ...
    def apply_coordinate_transform(self, coords, return_template, centralize_template):
        ...
        """
        coords_shift_to_center = getattr(self, "coords_shift_to_center", torch.zeros(self.num_templates, 2).to(coords.device))
        coords_shifted_to_center = coords.flatten(end_dim=-2) + coords_shift_to_center
        if return_template:
            return coords_shifted_to_center.unsqueeze(0) if centralize_template else coords.flatten(end_dim=-2).unsqueeze(0)
        else:
            template_center = getattr(self, "template_center_in_coords_space", torch.zeros(self.num_templates, 2).to(coords.device))
            coords_relative_to_template_center = coords.flatten(end_dim=-2) - template_center # center the grid on the location of interest
            coords_transformed = self.coordinate_transform(coords_relative_to_template_center) + template_center # transform back to original location
            
            # Apply the final translation to brind the transformed grid points at the location of neurons' RF
            with torch.no_grad():
                coords_shift_from_center = getattr(self, "coords_shift_from_center", torch.zeros(self.num_neurons, 2).to(coords.device))
                coords_shift_from_center_adjusted = coords_shift_from_center.unsqueeze(1) + coords_shift_to_center.unsqueeze(0)
                coords_shift_from_center_transformed = self.coordinate_transform(coords_shift_from_center_adjusted)
                
            coords_transformed_shifted_to_target = coords_transformed + coords_shift_from_center_transformed
    ...
    def transform_auxiliary_input(self, aux, device):
        ...
        if self.periodic_invariance:
            ...
            aux_transformed = aux / np.pi - 1 # the assumption is that the aux is between 0 and 2*pi
    ...
    def apply_positional_encoding(self, coords_transformed, aux_transformed):
        ...
        if self.positional_encoding_dim is not None:
            coords_transformed_pe = torch.cat(
                [torch.sin(coords_transformed @ self.B), torch.cos(coords_transformed @ self.B)],
                dim=-1,
            )
        else:
        ...
        if self.aux_positional_encoding_dim is not None:
            aux_transformed_pe = torch.cat(
                [torch.sin(aux_transformed @ self.auxB), torch.cos(aux_transformed @ self.auxB)],
                dim=-1,
            )
        else:
