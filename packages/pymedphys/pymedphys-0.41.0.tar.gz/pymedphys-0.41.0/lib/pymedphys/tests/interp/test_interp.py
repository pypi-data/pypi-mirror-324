...
def setup_interp(plot=False):
    ...
    values = X**2 + Y**2 + Z**2
    values_interp = interp.interp((x, y, z), values, axes_interp=(xi, yi, zi))
...
def test_3d_vs_scipy(setup_interp):
    ...
    values_interp_linear_scipy = interp.interp_linear_scipy(
...
def test_keepdims(setup_interp):
    ...
    # 3D
    values_interp_keepdims = interp.interp(
...
def test_extrap(setup_interp):
    ...
    with pytest.raises(ValueError):
        interp.interp((x, y, z), values, axes_interp=(xi, yi, zi), bounds_error=True)
    values_interp = interp.interp(
        (x, y, z),
        values,
...
def test_2d_vs_scipy(setup_interp):
    ...
    values_interp = interp.interp((x, y), values[:, :, 0], axes_interp=(xi, yi))
...
def test_1d_vs_scipy(setup_interp):
    ...
    values_interp = interp.interp((x,), values[:, 0, 0], axes_interp=(xi,))

    values_interp_linear_scipy = interp.interp_linear_scipy(
        (x,), values[:, 0, 0], axes_interp=(xi,)
    )
