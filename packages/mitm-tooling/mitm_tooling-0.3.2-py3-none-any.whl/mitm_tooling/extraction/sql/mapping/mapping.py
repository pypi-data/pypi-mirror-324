...
from mitm_tooling.definition import ConceptName, RelationName, MITM, ConceptLevel, ConceptKind, get_mitm_def
from ..data_models import VirtualView, DBMetaInfo, TableMetaInfo, Queryable, SourceDBType, TableIdentifier, \
    AnyTableIdentifier, ColumnName
from ..transformation.db_transformation import TableNotFoundException, col_by_name, TableTransforms
from ..transformation.df_transformation import transform_df

from mitm_tooling.utilities.python_utils import normalize_into_dict, elem_wise_eq, ExtraInfoExc

from .validation_models import IndividualMappingValidationContext, \
    MappingGroupValidationContext

from mitm_tooling.representation.intermediate_representation import HeaderEntry
...
class ColumnContentProvider(pydantic.BaseModel):
    ...
    @classmethod
    def from_tuple(cls, tup: tuple[str, sa.ColumnElement], is_present_in_table: bool,
                   static_value: str | None = None) -> Self:
        return ColumnContentProvider(column_name=tup[0], column_element=tup[1], is_present_in_table=is_present_in_table,
                                     static_value=static_value)


@dataclasses.dataclass
class HeaderEntryProvider:
    concept: ConceptName
    ...
    def from_session(self, db_session: Session) -> list[HeaderEntry]:
        distinct = db_session.execute(
            sa.select(self.kind_provider.column_element, self.type_provider.column_element).distinct()).all()
        return self.from_iterable(((kind, type_name) for kind, type_name in distinct))

    def from_df(self, df: pd.DataFrame) -> list[HeaderEntry]:
        iterable = None
        if (k := self.kind_provider.static_value) is not None and (t := self.type_provider.static_value) is not None:
            ...
            iterable = set(
                df.loc[:, [self.kind_provider.column_name, self.type_provider.column_name]].itertuples(index=None,
                                                                                                       name=None))
        return self.from_iterable(iterable)

    def from_iterable(self, distinct: Iterable[tuple[str, str]]) -> list[HeaderEntry]:
        return [HeaderEntry(concept=self.concept, kind=kind, type_name=type_name, attributes=self.attributes,
                            attribute_dtypes=self.attribute_dtypes) for kind, type_name in distinct]


@dataclasses.dataclass
class InstancesPostProcessor:
...
class InstancesProvider:
    ...
    def from_session(self, db_session: Session) -> pd.DataFrame:
        tm = self.virtual_view.table_meta
        results = db_session.execute(tm.queryable_source.select()).all()
        df = pd.DataFrame.from_records(results, columns=list(tm.columns))
        return df

    def from_session_chunked(self, db_session: Session, chunk_size: int) -> Iterable[pd.DataFrame]:
        tm = self.virtual_view.table_meta
        results = db_session.execute(tm.queryable_source.select()).partitions(chunk_size)
...
class IdentityProvider(StaticValidableIndividual, pydantic.BaseModel):
    ...
    def validate_static(self, context: IndividualMappingValidationContext) -> None:
        vr = context.vr
        vr.include_check(set(self.identity_columns.keys()) == set(context.relevant_relations.identity.keys()),
                         'identity columns are not mapped mapped as defined')
        vr.include_check(set(self.identity_columns.values()) <= context.examined_table_meta.col_set,
...
class InlineRelationsProvider(StaticValidableIndividual, pydantic.BaseModel):
    ...
    def validate_static(self, context: IndividualMappingValidationContext) -> None:
        vr = context.vr
        vr.include_check(set(self.inline_relations.keys()) == set(context.relevant_relations.inline.keys()),
                         'inline relations columns are not mapped as defined')
        vr.include_check(set(self.inline_relations.values()) <= context.examined_table_meta.col_set,
...
class ForeignRelationsProvider(StaticValidableIndividual, pydantic.BaseModel):
    ...
    def validate_static(self, context: IndividualMappingValidationContext) -> None:
        vr = context.vr
        vr.include_check(set(self.foreign_relations.keys()) == set(context.relevant_relations.foreign.keys()),
        ...
        for q, rel in self.foreign_relations.items():
            ...
            vr.include_check(set(rel.fk_columns_.keys()) == set(fk_rel_def.keys()),
                             'foreign key columns do not match definition')
            vr.include_check(set(rel.fk_columns_.values()) <= set(context.examined_table_meta.columns),
                             'foreign key columns not present in table')

            vr.include_check(TableIdentifier.resolve_id(rel.referred_table, context.db_metas) is not None,
...
class ConceptMapping(StaticValidableGroup, pydantic.BaseModel):
    ...
    def validate_static(self, context: MappingGroupValidationContext) -> None:
        ctxt = context.derive_individual(claimed_concept=self.concept,
                                         examined_table=TableIdentifier.from_any(self.base_table))
        vr = ctxt.vr
        ...
        vr.include_check(not ctxt.is_weak_concept, 'weak concepts cannot be mapped explicitly')
        ...
        if self.kind_col is not None:
            vr.include_check(self.kind_col in table_meta.col_set,
                             f'specified kind column ({self.kind_col}) not present in table')

        vr.include_check(self.type_col in table_meta.col_set,
                         f'specified type column ({self.type_col}) not present in table')
        ...
        vr.include_check(ctxt.relevant_properties.permit_attributes or len(self.attributes) == 0,
                         'this concept does not permit attributes')
        vr.include_check(len(self.attributes) == len(self.attribute_dtypes),
    ...
    def apply(self, db_metas: dict[SourceDBType, DBMetaInfo]) -> tuple[HeaderEntryProvider, Queryable]:
        ...
        def make_type_col() -> tuple[str, sa.ColumnElement]:
            return concept_properties.typing_concept, sa.label(concept_properties.typing_concept,
                                                               col_by_name(base_queryable, self.type_col,
                                                                           raise_on_missing=True))

        def make_kind_col() -> tuple[str, sa.ColumnElement]:
            ce = None
            if self.kind_col is not None:
                ce = sa.label('kind', col_by_name(base_queryable, self.kind_col,
                                                  raise_on_missing=True))
            else:
        ...
        def make_identity_cols() -> list[tuple[str, sa.ColumnElement]]:
            return [(q, sa.label(q, col_by_name(base_queryable, col, raise_on_missing=True))) for q, col in
                    self.identity_provider.identity_columns.items()]

        def make_inline_relation_cols() -> list[tuple[str, sa.ColumnElement]]:
            return [(q, sa.label(q, col_by_name(base_queryable, col, raise_on_missing=True))) for q, col in
                    self.inline_relations_provider.inline_relations.items()]

        def make_foreign_relation_cols() -> list[tuple[str, sa.ColumnElement]]:
            foreign_relation_cols = []
            for target_concept, fk_rel in self.foreign_relations_provider.foreign_relations.items():
        ...
        def make_attribute_cols() -> list[tuple[str, sa.ColumnElement]]:
            return [(f'a_{i}', sa.label(f'a_{i}', col_by_name(base_queryable, a, raise_on_missing=True))) for i, a in
                    enumerate(self.attributes, 1)]

        selected_columns, created_columns = map_col_groups(mitm_def, self.concept, {
            'kind': make_kind_col,
            'type': make_type_col,
        ...
        header_entry_provider = HeaderEntryProvider(concept=self.concept, table_meta=table_meta,
                                                    kind_provider=ColumnContentProvider.from_tuple(make_kind_col(),
                                                                                                   'kind' in created_columns,
                                                                                                   concept_properties.key),
                                                    type_provider=ColumnContentProvider.from_tuple(make_type_col(),
                                                                                                   concept_properties.typing_concept in created_columns),
