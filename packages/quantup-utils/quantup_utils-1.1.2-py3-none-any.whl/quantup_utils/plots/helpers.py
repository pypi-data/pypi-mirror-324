...
from ..builtin import flatten, coalesce, reorder, get_optimal_division
...
def to_factor(
        ...
        most_common: int = 13,
        factor_types: tuple[str, ...] = ("category", "object", "str"),
    ...
    most_common : int = 13,
        ...
        (as this should not be part of this function).
...
# %%
def cats_and_colors(
        factor: pd.Series, most_common: Optional[pd.Series], cmap: str
) -> tuple[list[str], np.ndarray, ListedColormap]:
    """
    For given categorical variable `factor` and its most common values table passed via `most_common`
    (and obtained from `to_factor(..., most_common: int)`)
    returns list of these most common categories, list of colors for each of them and
    ...
    #
    cat_colors = plt.colormaps[cmap](np.linspace(0.03, 0.97, len(cats)))
...
# %%
def horizontal_legend(ax: Axes, title: str, thresh: int, extra: int) -> Axes:
    """"""
