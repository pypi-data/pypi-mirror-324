...
ListPlotType = List[PlotType] | NDArray[PlotType]  # type: ignore  # cannot be PlotType but it MUST be PlotType (and works!)  # noqa
...
def plot_covariates(
        ...
        what: ListPlotType | List[ListPlotType] = cast(ListPlotType, ["grouped_cloud", "densities", "boxplots"]),
        ...
        most_common: int = 13, print_levels: bool = True,
        factor_threshold: int = 13,
        as_factor_y: Optional[bool] = None, as_factor_x: Optional[bool] = None,
        fillna: Optional[str] = '<NA>',   # filling NA values (None, np.nan, ...) for categorical data (only!)
        #
        ...
        brightness: Optional[float] = None,  # not used yet
        # # Parameters for various types of plots
        bins: int | List[float] = 21, smooth: float = .5,
        qq_plot: bool = False,       # add qq-plot for  num ~ num  plot
        sort_levels: bool = True, legend: bool = True, axes_labels: bool = True,
        # ## only fac~fac
        bar_height: float = .9, align: bool = True, horizontal: bool = True,
        # # Figure and axes parameters
        # ## for the whole figure
        figsize: Optional[Tuple[float, float]] = None,
        figwidth: Optional[float] = None, figheight: Optional[float] = None,
        # ## for the single axis (subplot)
        width: Optional[float] = None, height: Optional[float] = None, size: float = 5, width_adjust: float = 1.2,
        scale: Scale = "linear", xscale: Optional[Scale] = None, yscale: Optional[Scale] = None,
        lines: bool = True,  # not used here yet!
        ...
        axescolor: Optional[str] = None, titlecolor: Optional[str] = None,
        ...
        print_info: bool = True, use_logger: bool = False,
        res: bool = False,
        *args: Any, **kwargs: Any
    ...
    - data do not need to be properly aligned wrt to no-NaNs
      – this is usually big pain which is completely removed here;
    ...
    what: List[PlotType] | List[List[PlotType]] = [["grouped_cloud", "densities", "boxplots"]];
    ...
    title_suffix: Optional[str] = None,
        makes it possible to distinguish plots for which automatic title would be the same;
    ignore_index: bool = False,
    ...
    print_levels: bool = True,
        do print (in terminal or console) levels of a factor with their counts?
    ...
    fillna: Optional[str] = '<NA>',
        ...
        this allows to include them into data as one of the categories (levels) thus they appear in the plot
    ...
    bins: int | List[float] = 21,
        int or list of boarders of bins (passed to ax.hist(...))
        how many or what bins (groups) for "hist";
    smooth: float = .5,
    ...
    sort_levels: bool = True,
        do sort levels of a factor (alphabetically) before plotting?
    legend: bool = True,
        do draw legend on a plot?
    axes_labels: bool = True,
        do give labels to axes?
    horizontal: bool = True,
        only for boxplots (numeric ~ factor) (not working yet) and barchart (factor ~ factor);
    bar_height: float = .5,
        only for barchart (factor ~ factor);
        this is relative to the space available for one bar (one category of variable in horizontal mode)
    ...
    size: float = 5,
        if `width` or `height` are None then they are calculated from `size` and `width_adjust`:
            height = size
    ...
    If required (`res=True`) it is dictionary "of everything":
        ...
        result = {
            ...
            "plot": {           # plot objects
                ...
                <
                  other subplot objects under respective keys as called for by `what`
    ...
    is_factor_y, variable_, variable_vc = h.to_factor(
        variable_, as_factor_y, most_common=most_common, factor_threshold=factor_threshold, fillna=fillna)
    is_factor_x, covariate_, covariate_vc = h.to_factor(
        covariate_, as_factor_x, most_common=most_common, factor_threshold=factor_threshold, fillna=fillna)

    # aligning data
    df0 = pd.concat([variable_, covariate_], axis=1)
    # #df0.columns = pd.Index([varname, covarname])
    ...
    # must be repeated (but now only for already-factors)
    is_factor_y, variable_, variable_vc = h.to_factor(
        variable_, is_factor_y, most_common=most_common, factor_threshold=factor_threshold)
    is_factor_x, covariate_, covariate_vc = h.to_factor(
        covariate_, is_factor_x, most_common=most_common, factor_threshold=factor_threshold)
    ...
    N = len(variable_) if n_obs <= 1 else min(len(variable_), int(n_obs))
    ...
    def grouped_cloud(
            ...
            title: str = "grouped cloud",  # sort: bool = sort_levels
    ) -> dict:
        """
        """
        ...
        # if sort:  # what kind of sort?
        dff = dff.sort_values(by=factor.name, ignore_index=True)

        scatters: dict = dict()
        # idx_cat_means: list() = []
        for cat, col in reversed(list(zip(cats, cat_colors))):
            vv = dff[variable.name][dff[factor.name] == cat]
            scatters[cat] = ax.scatter(vv, vv.index, c=col, s=s, alpha=alpha)  # type: ignore  # mypy is wrong – alpha MAY BE Sequence  # noqa
    ...
    def barchart(
        ...
        for i, (cat, color) in enumerate(zip(labels_v, colors)):
            ...
            if horizontal:
                rects = ax.barh(labels_c, width=widths, left=starts, height=bar_height, label=cat, color=color)
            else:
                rects = ax.bar(labels_c, height=widths, bottom=starts, width=bar_height, label=cat, color=color)
            r, g, b, _ = color
        ...
        if horizontal:
            ...
            if legend:
                ax = h.horizontal_legend(
                    ax, title=varname, thresh=(width * 10), extra=5
                )
        else:
            if axes_labels:
                ax.set_ylabel(varname)
    ...
    def set_fig(
        ...
        if nrows == 0 or ncols == 0:
            ...
            if figsize is None:
                figwidth = figwidth or (ncols * (width or (size * width_adjust)))
                figheight = figheight or (nrows * (height or size) + 1)     # ? +1 for title
                figsize = figwidth, figheight
    ...
    def numeric_vs_numeric(variable: pd.Series, covariate: pd.Series) -> Figure:
        ...
        if print_info:  # of course!
            info = "\n" + \
                " 2. statistics for processed data" + "\n" + \
    ...
    def numeric_vs_factor(num: pd.Series, fac: pd.Series, most_common: Optional[pd.Series]) -> Figure:
        """"""
        nonlocal result
        nonlocal df0
        nonlocal df
        ...
        cats, cat_colors, cmap_ = h.cats_and_colors(fac, most_common, cmap)
    ...
    def factor_vs_factor(variable: pd.Series, covariate: pd.Series,
                         most_common_v: pd.Series, most_common_c: pd.Series) -> Figure:
        """"""
        nonlocal result
        nonlocal height
        nonlocal width
        ...
        longest_cat_v = max(map(lambda s: len(str(s)), most_common_v.index))

        if horizontal:
            height = height or max(n_cats_c * .5 + 1, size)
            width = width or max(n_cats_v * .5 + longest_cat_v * .1, size * width_adjust)
        else:
            height = height or max(n_cats_v * .3, size)
            width = width or max(n_cats_c * .7 + longest_cat_v * .1, size * width_adjust)

        fig, axs = set_fig(nrows=1, ncols=1)

        result['plot']["barchart"] = barchart(
            cast(Axes, axs.tolist()), crosstab, title, horizontal, align, bar_height
        )
