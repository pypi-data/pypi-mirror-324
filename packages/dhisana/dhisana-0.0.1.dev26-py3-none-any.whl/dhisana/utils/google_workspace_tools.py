...
def convert_base_64_json(base64_string: str) -> str:
    """
    Decodes a base64-encoded JSON string into a standard JSON string.

    Args:
        base64_string (str): Base64-encoded JSON.
...
def get_google_credentials(
    ...
    # Get the base64-encoded service account JSON
    service_account_base64 = get_google_workspace_token(tool_config)
    # Decode and parse to dict
    ...
    async with httpx.AsyncClient() as client:
        ...
        for msg in messages:
            ...
            email_details.append({
                ...
                "email_subject": next((h['value'] for h in headers_list if h['name'] == 'Subject'), None),
                "email_sender": next((h['value'] for h in headers_list if h['name'] == 'From'), None),
                "email_recipients": [
                    h['value'] for h in headers_list if h['name'] in ['To', 'Cc', 'Bcc']
                ],
                "email_date": next((h['value'] for h in headers_list if h['name'] == 'Date'), None),
                "read_email_status": 'UNREAD' if 'UNREAD' in message_data.get('labelIds', []) else 'READ',
...
async def fetch_last_n_sent_messages(
    ...
    Args:
        ...
        num_messages (int): The number of recent messages to fetch.
        sender_email (str): The mailbox email to impersonate.
        tool_config (Optional[List[Dict]]): Tool configuration for credentials.
...
async def fetch_last_n_received_messages(
    ...
    The service account must have domain-wide delegation to impersonate the receiver (sender_email).

    Args:
        sender_filter_email (str): The sender's email address to filter on.
        num_messages (int): The number of recent messages to fetch.
...
async def get_email_details_async(
    ...
    Returns:
        Dict[str, Any]: A dictionary containing the full message details in JSON format.
    """
    if not sender_email:
