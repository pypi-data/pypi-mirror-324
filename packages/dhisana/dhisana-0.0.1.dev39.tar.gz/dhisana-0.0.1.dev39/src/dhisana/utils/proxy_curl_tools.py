...
def get_proxycurl_access_token(tool_config: Optional[List[Dict]] = None) -> str:
    ...
    """
    logger.info("Entering get_proxycurl_access_token")
    ...
    logger.info("Retrieved PROXY_CURL_API_KEY successfully.")
        ...
        try:
            async with session.get(url, headers=headers, params=params) as response:
                ...
                if response.status == 200:
                    ...
                    logger.warning(msg)
                    if key:
                        cache_output("lookup_person_in_proxy_curl_by_name", key, {'error': msg})
                    return {'error': msg}
                elif response.status == 429:
                    msg = "Rate limit exceeded"
                    logger.warning(msg)
                    ...
                    result = await response.json()
                    logger.warning(f"lookup_person_in_proxy_by_name error: {result}")
                    return {'error': result}
        except Exception as e:
...
async def enrich_user_info_with_proxy_curl(input_user_properties: dict, tool_config: Optional[List[Dict]] = None) -> dict:
    ...
    # Fill in missing properties from the Proxycurl data
    ...
    if experiences:
        ...
        if experiences[0].get("company_linkedin_profile_url", ""):
            org_url = experiences[0]["company_linkedin_profile_url"]
    ...
    # Combine city/state if available
