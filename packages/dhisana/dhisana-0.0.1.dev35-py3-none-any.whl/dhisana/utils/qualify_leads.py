...
async def generate_qualify_leads_code(
    ...
    system_message = (
        ...
        "    async def create_smart_list(input_leads_list, add_search_results, tool_config)\n"
    )
    ...
    example_of_workflow_code = (
        ...
        async def create_smart_list(input_leads_list, add_search_results, tool_config):
            """
            Example skeleton. Deduplicate leads, perform checks for signals, add the qualification_score to each lead,
            then sort by qualification_score (descending) and return qualified leads.
            ...
            try:
                ...
                for lead in unique_leads:
                    try:
                        ...
                        qualification_score = await check_for_intent_signal(
                            lead=lead,
                    ...
                    # Attach the qualification_score to the lead
                    # qualification_score is a integer value between 0-5. 0 being lowest and 5 being highest.
                ...
                # Step 3: Sort qualified leads by qualification_score (descending)
                qualified_leads = sorted(qualified_leads, key=lambda l: l.get("qualification_score", 0), reverse=True)
    ...
    5. Sort the qualified leads by qualification_score in descending order.
...
async def qualify_leads(
    ...
    """
    response, status = await generate_qualify_leads_code(user_query, tool_config=tool_config)
    ...
    if status == "SUCCESS" and response and response.get("workflow_python_code"):
        ...
        try:
            ...
            async def run_create_smart_list(tool_cfg):
                return await create_smart_list_fn(input_leads_list, request.use_online_search, tool_cfg)
