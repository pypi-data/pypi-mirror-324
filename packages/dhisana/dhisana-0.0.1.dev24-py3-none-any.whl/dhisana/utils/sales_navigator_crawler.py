...
async def goto_url(page: Page, command_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    salesnav_url_profile_by_name = command_args.get("salesnav_url_profile_by_name", "").strip()
    salesnav_url_profile = command_args.get("salesnav_url_profile", "").strip()
    salesnav_url_list_leads = command_args.get("salesnav_url_list_leads", "").strip()

    try:
        if salesnav_url_profile:
            # Step 1: Navigate directly to the provided Sales Navigator URL
            await page.goto(salesnav_url_profile)
            ...
            await page.wait_for_timeout(2000)
            return {"status": "SUCCESS", "message": "Navigated to Sales Navigator URL"}
        elif salesnav_url_profile_by_name:
            # Step 2: Navigate to Sales Navigator search page
            await page.goto(salesnav_url_profile_by_name)
            await page.wait_for_selector("body", timeout=5000)
...
async def view_linked_in_profile(page: Page, command_args: Dict[str, Any]) -> Dict[str, Any]:
    await goto_url(page, command_args)
    if "/sales/lead/" not in page.url:
        return {"status": "ERROR", "message": "Not a sales lead page"}
    response = await click_lead_connect_menu(page)
    ...
    for div in lead_divs:
        ...
        if lead_link_el:
            ...
            if match:
                ...
                lead["lead_salesnav_url"] = f"https://www.linkedin.com/sales/lead/{linkedin_id}"
            else:
                # Fallback if we didn't match or want to keep the full URL
                lead["lead_salesnav_url"] = f"https://www.linkedin.com{href}"
        ...
        if company_link_el:
            ...
            if match:
                ...
                if comma_index != -1:
                    linkedin_id = linkedin_id[:comma_index]
                lead["company_salesnav_url"] = f"https://www.linkedin.com/sales/company/{linkedin_id}"
            else:
                lead["company_salesnav_url"] = f"https://www.linkedin.com{href}"
...
async def extract_account_from_current_page(page: Page):
    ...
    if 'copied_text' in locals() and "linkedin.com/company/" in copied_text:
        company["company_linkedin_url"] = copied_text
