...
# - get_apollo_access_token(tool_config) -> str
# - fetch_apollo_data(session, url, headers, payload) -> Dict[str, Any]
# - logger is a configured logger instance.
# - LeadsQueryFilters, SmartList, and SmartListLead are defined in your codebase.

# -------------------------------------------------------------------------------
# 1-page result fetcher
# -------------------------------------------------------------------------------
async def search_people_with_apollo(
    ...
    """
    Fetches a single 'page' of people/contacts from Apollo given `dynamic_payload`.
    This function does not handle pagination; it's a single fetch.
    """
...
async def search_leads_with_apollo(
    ...
    """
    If example_url is provided, parse the query string and build the payload
    using *any* matching parameters. Ignore the 'query' object fields.
    Otherwise, use the fields in 'query'.

    Then fetch data from Apollo in a paginated way until we have
    either reached the user's requested max_items or Apollo has no more data.
    """
    logger.info("Entering search_leads_with_apollo")

    # Decide how many total items to fetch.
    ...
    # -----------------------------------------------------
    # A) example_url is provided -> parse and build payload
    # -----------------------------------------------------
    if example_url:
        ...
        # If no query but fragment contains a '?', extract query params from the fragment.
        ...
        # Start with required pagination fields (default if not present)
        ...
        # Begin building the dynamic payload with pagination
        dynamic_payload: Dict[str, Any] = {
            "page": page_val,
            "per_page": per_page_val,
        }

        # Mapping dictionary: incoming parameter names to Apollo API parameter names.
        mapping = {
            ...
            "qOrganizationJobTitles": "q_keywords",  # will join multiple values into one string
        ...
        # Process each query parameter from the URL.
        for raw_key, raw_value_list in query_params.items():
            ...
            if raw_key in mapping:
                ...
                # Convert camelCase to snake_case if not explicitly mapped.
                key = re.sub(r'(?<!^)(?=[A-Z])', '_', key).lower()

            # If the value list has only one item, use that; otherwise, use the list.
            ...
            if key in ("page", "per_page"):
                try:
                    final_value = int(final_value)  # might fail if it's a list
                except Exception:
                    pass

            # For the q_keywords parameter (mapped from qOrganizationJobTitles), join list values into a single string.
            if key == "q_keywords" and isinstance(final_value, list):
                final_value = " ".join(final_value)

            # Ensure that parameters expected as arrays are lists.
            if key in (
                "person_locations",
                "person_titles",
                "person_seniorities",
        ...
        # Remove sort_by_field if it is set to an invalid value.
        if dynamic_payload.get("sort_by_field") == "[none]":
            dynamic_payload.pop("sort_by_field")

        # If the user didn't specify `per_page`, ensure it doesn't exceed max_items.
        if "per_page" not in query_params:
            dynamic_payload["per_page"] = min(max_items, 100)

    # -----------------------------------------------------
    # B) No example_url -> build payload from `query` object
    # -----------------------------------------------------
    else:
        dynamic_payload = {
    ...
    # ---------------------------------------------
    # C) Fetch multiple pages until we reach max_items
    # ---------------------------------------------
    ...
    # ------------------------------------------------
    # Convert the raw results into SmartListLead objects
    # ------------------------------------------------
