...
async def search_google(
    ...
    cache_key = f"search_google_serp_{query}_{number_of_results}_{offset}"
    cached_response = retrieve_output(cache_key, query)
    ...
    cache_output(cache_key, query, serialized_results)
...
async def get_company_domain_from_google_search(
    ...
    # Exclusions not used in final query (commented out)
...
async def find_organization_linkedin_url_with_google_search(
    ...
    if use_strict_check:
        queries = [f'site:linkedin.com/company "{company_name}" {company_location}']
    else:
        if company_location:
            queries = [
...
async def find_job_postings_google_search(
    ...
    """
    logger.info("Entering find_job_postings_google_search")
    ...
    logger.debug(f"Search query for job postings: {query}")
    queries = [query]
    ...
    for q in queries:
        ...
        try:
            results = await search_google(q.strip(), 1, tool_config=tool_config)
        except Exception as e:
        ...
        for result_item in results:
            ...
            # Gather all links from the page
            page_links = [a.get('href') for a in soup.find_all('a', href=True)]

            # Check if the organization's LinkedIn URL is present in the links
            # (with 'public_jobs_topcard-org-name' parameter)
            company_match = False
            if organization_linkedin_url:
                for page_link in page_links:
                    if (page_link and organization_linkedin_url in page_link
                        and 'public_jobs_topcard-org-name' in page_link):
                        company_match = True
                        break

            # Check keyword presence in page text
            text = soup.get_text().lower()
            keywords_found = any(kw in text for kw in keywords_list)

            if company_match and keywords_found:
                logger.debug(f"Match found with link: {link}")
