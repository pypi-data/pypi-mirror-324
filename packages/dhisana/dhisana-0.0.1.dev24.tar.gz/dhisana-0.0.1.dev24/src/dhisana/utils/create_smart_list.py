...
async def generate_smart_list_creation_code(
    ...
    example_of_workflow_code = (
        ...
        async def create_smart_list(input_leads_list, tool_config):
            ...
            try:
                ...
                # Step 1 : Deduplicate leads by user_linkedin_url or lead_salesnav_url
                leads_with_linkedin = {}
                leads_with_salesnav = {}

                for lead in input_leads_list:
                    user_linkedin_url = lead.get("user_linkedin_url")
                    lead_salesnav_url = lead.get("lead_salesnav_url")
                    if user_linkedin_url:
                        leads_with_linkedin[user_linkedin_url] = lead
                    elif lead_salesnav_url:
                        leads_with_salesnav[lead_salesnav_url] = lead
                ...
                if not unique_leads:
                    logger.warning("No leads with valid 'user_linkedin_url' or 'lead_salesnav_url'; returning SUCCESS.")
                    return "SUCCESS", []
    ...
    2. Deduplicate the leads based on user_linkedin_url or lead_salesnav_url as given in the example.
