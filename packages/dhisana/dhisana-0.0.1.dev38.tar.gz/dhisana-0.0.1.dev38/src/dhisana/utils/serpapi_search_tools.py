...
async def search_google(
    ...
    """
    Search Google using SERP API, supporting pagination when number_of_results > 100.
    ...
    """
    # Create a composite cache key if you want to cache different sizes distinctly
    cache_key = f"search_google_serp_{query}_{number_of_results}"
    ...
    async with aiohttp.ClientSession() as session:
        while len(all_results) < number_of_results:
            # Determine how many results to request for this page
            # (in case the last page only needs a partial number to reach the total)
            ...
            if not organic_results:
                # If no more organic results, break out of the loop
                break

            # Add the returned organic results to our list
    ...
    # Trim any excess results if we somehow got more than needed
    ...
    cached_results = cache_output(cache_key, query, serialized_results)
