...
from yta_image.dataclasses import ImageResult
from yta_image.edition.filter.gameboy import image_file_to_gameboy
from yta_image.edition.filter.sticker import image_file_to_sticker
from yta_image.edition.filter.motion_blur import MotionBlurDirection, apply_motion_blur
...
class ImageFilter:
    ...
    def pixelate(
        ...
        # We make it smaller
        small_img = image.resize((pixel_size, pixel_size), Image.BILINEAR)

        # We resize it to the output size
        res = small_img.resize(image.size, Image.NEAREST)

        # TODO: Handle 'output_filename' in a better way
        # because maybe we want the result but not the image
        # written in a file
        if output_filename is None:
            output_filename = create_temp_filename('pixelated_image.png')
