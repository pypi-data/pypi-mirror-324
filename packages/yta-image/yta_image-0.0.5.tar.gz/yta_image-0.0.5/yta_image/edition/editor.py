...
"""
from yta_image.edition.settings import COLOR_TEMPERATURE_CHANGE_LIMIT, COLOR_HUE_CHANGE_LIMIT, CONTRAST_LIMIT, SHARPNESS_LIMIT, BRIGHTNESS_LIMIT
...
class LutTable(Enum):
    ...
    def apply_to_image(self, image: any) -> np.ndarray:
        """
...
class ImageEditor:
    ...
    @staticmethod
    def modify_brightness(image: Union[str, Image.Image, np.ndarray], factor: int = 0):
        return change_image_brightness(image, factor)

    @staticmethod
    def modify_contrast(image: Union[str, Image.Image, np.ndarray], factor: int = 0):
        return change_image_contrast(image, factor)
    
    @staticmethod
    def modify_sharpness(image: Union[str, Image.Image, np.ndarray], factor: int = 0):
        return change_image_sharpness(image, factor)
    
    @staticmethod
    def modify_white_balance(image: Union[str, Image.Image, np.ndarray], factor: int = 0):
    ...
    def apply_3d_lut(image: Union[str, Image.Image, np.ndarray], lut_3d_filename: str):
        ...
        """
        if not FileValidator.is_file(lut_3d_filename):
...
def change_image_white_balance(image: any, factor: int = 0) -> np.ndarray:
    """
    TODO: Explain
