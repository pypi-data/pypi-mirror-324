...
class AIFumes():
    ...
    def __get_recaptcha_token(self):
        ...
        recaptcha_iframe = scrapper.find_element_by_xpath_waiting('//iframe[contains(@title, "reCAPTCHA")]')
        ...
        token = scrapper.execute_script('grecaptcha.execute("' + k + '");')
        print(token)
    ...
    def generate_leonardo(self, prompt, negative_prompt = '', output_filename = 'tmp_fumes_leonardo.png'):
        ...
        from selenium.webdriver.common.by import By
        from yta_general_utils.experimental.autogui_utils import move_cursor_to
        from random import randint

        prompt_input = scrapper.find_element_by_id('inp')
        # Write input
        prompt_input.send_keys(prompt)
        time.sleep(1)
        move_cursor_to(randint(400, 1900), randint(100, 900))
        settings_button = scrapper.find_element_by_xpath_waiting("//button[contains(text(), 'Setting')]")
        settings_button.click()
        time.sleep(1)
        move_cursor_to(randint(100, 1800), randint(50, 800))
        negative_prompt_input = scrapper.find_element_by_id_waiting('np')
        ...
        photography_style_radio_button = scrapper.find_element_by_xpath_waiting("//input[contains(@value, 'photo')]")
        photography_style_radio_button.click()
        time.sleep(1)
        move_cursor_to(randint(200, 1600), randint(200, 780))

        # Scroll down
        scrapper.execute_script('window.scrollTo(' + str(0) + ', ' + str(999) + ')')

        landscape_radio_button = scrapper.find_element_by_xpath_waiting("//input[contains(@value, '16:9')]")
        landscape_radio_button.click()
        time.sleep(1)
        move_cursor_to(120, 550)
        close_button = scrapper.find_element_by_id('close')
        close_button.click()
        time.sleep(1)
        scrapper.execute_script('window.scrollTo(' + str(0) + ', ' + str(0) + ')')
        move_cursor_to(randint(700, 1700), randint(250, 800))
        for i in range(3):
            move_cursor_to(randint(0, 1000), randint(0, 1000))
        create_button = scrapper.find_element_by_id('create')
        close_button.click()
        time.sleep(1)
        ...
        for i in range(100):
            ...
            try:
                images = scrapper.find_elements_by_element_type('img')
            ...
            time.sleep(1)
            if i % 10 == 0:
                scrapper.execute_script('window.scrollTo(' + str(randint(0, 70)) + ', ' + str(999) + ')')

        if len(image_urls) > 0:
            print('Downloading ' + image_urls[0])
            # We secure the one we want
            Downloader.download_image(image_urls[0], output_filename)
            ...
            for image_url in image_urls:
                ...
                try:
                    Downloader.download_image(image_url, PROJECT_ABSOLUTE_PATH + 'leonardo_images/' + image_filename)
