...
class pb_neuron_layer(nn.Module):
    ...
    def __init__(self, startModule, name, pretrainedPB=None):
        ...
        if(PBG.verbose):
            ...
            print(startModule)
        if(type(self.mainModule) in PBG.modluesWithProcessing):
            moduleIndex = PBG.modluesWithProcessing.index(type(self.mainModule))
        ...
        #TODO: used?
        self.handle = None
        if(PBG.pbTracker == [] or PBG.reInitPB):
    ...
    def addPBLayer(self):
        ...
        with torch.no_grad():
            for i in range(0, PBG.globalCandidates):
                ...
                if(type(self.parentModule) in PBG.modluesWithProcessing):
                    moduleIndex = PBG.modluesWithProcessing.index(type(self.parentModule))
            ...
            if(PBG.pbTracker.memberVars['mode'] == 'p' and self.mode == 'p'):
                ...
                if(self.training):
                    ...
                    if(PBG.debuggingMemoryLeak and len(self.pbValues[i].pbOuts[deviceIndex]) != 0):
                            ...
                            print('if this is supposed to be the case set PBG.debuggingMemoryLeak = False')
                            #import pdb; pdb.set_trace()
                    if(PBG.doingCC):
        ...
        def backward(ctx, grad_out):
            with torch.no_grad():
                ...
                if(True):
                    ...
                    if((
                        (
                        (savedValues.bestScore*(1.0-PBG.pbImprovementThreshold))-savedValues.previousBestScore).max()>0.00000001 and (savedValues.bestScore - savedValues.previousBestScore).max() > PBG.pbImprovementThresholdRaw)  or savedValues.initialized.item() == 0):
