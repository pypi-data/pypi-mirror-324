...
def defaultInitPBTracker(doingPB=True, saveName='DefaultName'):
    if(doingPB==False):
        PBG.pbTracker = pb_neuron_layer_tracker(doingPB=False, saveName=saveName)
        PBG.pbTracker.savedTime = time.time()
        PBG.pbTracker.zoomingGraph = True
        PBG.pbTracker.makingGraphs = True
        return
    if(PBG.switchMode == -1):
        print('you need to set swich mode')
        pdb.set_trace()
    if(PBG.paramValsSetting == -1):
        print('you need to set paramValsSetting mode')
        pdb.set_trace()
    if(PBG.inputDimensions == []):
        print('you need to set inputDimensions')
        pdb.set_trace()

    PBG.pbTracker = pb_neuron_layer_tracker(doingPB=True,saveName=saveName) 
...
class pb_neuron_layer_tracker():
    ...
    def __init__(self, doingPB, saveName, makingGraphs=True, paramValsSetting=-1, values_per_train_epoch=-1, values_per_val_epoch=-1):
        ...
        self.memberVarTypes['lastLREpochCount'] = 'int'
        ...
        self.memberVarTypes['currentStepScores'] = 'float array'
    ...
    def initializeTrackerSettings(self):
        ...
        if not os.path.exists(self.saveName + '/arrayDims.csv'):
            print('You must call saveArrayDims before setupArrayDims')
    ...
    def switchTime(self):
        ...
        if(not PBG.silent):
            print('Checking PB switch with mode %c, switch mode %s, epoch %d, last improved epoch %d, total Epochs %d, capAtN setting: %d, n: %d, p:%d, numCycles: %d' % (self.memberVars['mode'], switchPhrase, self.memberVars['numEpochsRun'], self.memberVars['epochLastImproved'], self.memberVars['totalEpochsRun'], PBG.capAtN, PBG.nEpochsToSwitch, PBG.pEpochsToSwitch, self.memberVars['numCycles']))
    ...
    def setPBTraining(self):
        ...
        PBG.pbTracker.memberVars['currentStepScores'] = []
                ...
                if(layer.pb.pbValues[m].bestScoreImprovedThisEpoch[0]):#if its anything other than 0, gets set to 1 but can be greater than that in gather
                    if(not PBG.doingMeanBest):
                        if(not PBG.learnPBLive):
                            ...
                            if(PBG.verbose):
                                print('4 epoch improved is %d' % PBG.pbTracker.memberVars['epochLastImproved'])
        ...
        if(PBG.doingMeanBest):
            if(self.memberVars['bestMeanScoreImprovedThisEpoch']):
                if(not PBG.learnPBLive):
                    self.memberVars['epochLastImproved'] = self.memberVars['numEpochsRun']
                    if(PBG.verbose):
                        print('5 epoch improved is %d' % PBG.pbTracker.memberVars['epochLastImproved'])
                return True
            else:
                return False
        return gotABest
    ...
    def addBestScores(self):
        ...
        if(PBG.doingMeanBest):
            ...
            if(len(self.memberVars['switchEpochs']) == 0):
                countSinceSwitch = PBG.pbTracker.memberVars['numEpochsRun']
            else:
                countSinceSwitch = (PBG.pbTracker.memberVars['numEpochsRun'] - self.memberVars['switchEpochs'][-1])-1
            ...
            elif(((totalMeanBest*(1.0-PBG.pbImprovementThreshold))-self.memberVars['bestMeanScores'][-1])>0.0000001 and (totalMeanBest - self.memberVars['bestMeanScores'][-1]) > PBG.improvementThresholdRaw):
                if(PBG.verbose):
    ...
    def addValidationScore(self, accuracy, net, saveName, forceSwitch=False):
        ...
        if 'module' in net.__dir__():
            ...
            pdb.set_trace()
            sys.exit(-1)
            #why is it saying best score was not improved?
        ...
        if(len(PBG.pbTracker.memberVars['switchEpochs']) == 0):
            ...
            countSinceSwitch = (PBG.pbTracker.memberVars['numEpochsRun'] - PBG.pbTracker.memberVars['switchEpochs'][-1])-1
        ...
        if(PBG.pbTracker.memberVars['mode'] == 'n' or PBG.learnPBLive):
            ...
            if(countSinceSwitch < PBG.initialHistoryAfterSwitches):
                if countSinceSwitch == 0:
                    PBG.pbTracker.memberVars['runningAccuracy'] = accuracy
                else:
                    PBG.pbTracker.memberVars['runningAccuracy'] = PBG.pbTracker.memberVars['runningAccuracy'] * (1-(1.0/(countSinceSwitch+1))) + accuracy * (1.0/(countSinceSwitch+1))
            else:
        ...
        if(PBG.pbTracker.memberVars['mode'] == 'n') or PBG.learnPBLive:
            ...
            if( #score improved, or no score yet, and (always switching or enough time to do a switch)
                ((PBG.pbTracker.memberVars['maximizingScore'] and (PBG.pbTracker.memberVars['runningAccuracy']*(1.0 - PBG.improvementThreshold) > PBG.pbTracker.memberVars['currentBestValidationScore']))
                 or
                 ((not PBG.pbTracker.memberVars['maximizingScore']) and (PBG.pbTracker.memberVars['runningAccuracy']*(1.0 + PBG.improvementThreshold) < PBG.pbTracker.memberVars['currentBestValidationScore'])) or (PBG.pbTracker.memberVars['currentBestValidationScore'] == 0))#if current best is 0 that means it just reset, so want this score to count like it always would for the above case. 
                 and 
                ((countSinceSwitch > PBG.initialHistoryAfterSwitches) or (PBG.pbTracker.memberVars['switchMode'] == PBG.doingSwitchEveryTime))):
                if(PBG.pbTracker.memberVars['maximizingScore']):
                    if(PBG.verbose):
                ...
                if(PBG.verbose):
                    ...
                    if(countSinceSwitch <= PBG.initialHistoryAfterSwitches):
                        print('not enough history since switch%d <= %d' % (countSinceSwitch, PBG.initialHistoryAfterSwitches))
        ...
        if((PBG.pbTracker.switchTime() == True) or forceSwitch):
            '''
            if((PBG.pbTracker.memberVars['mode'] == 'n') and (PBG.pbTracker.memberVars['numPBNeuronLayers'] > 0)):
            ...
            if(type(PBG.pbTracker.memberVars['schedulerInstance']) is torch.optim.lr_scheduler.ReduceLROnPlateau):
                if(countSinceSwitch > PBG.initialHistoryAfterSwitches or PBG.pbTracker.memberVars['mode'] == 'p'):
                    if(PBG.verbose):
                ...
                if(countSinceSwitch > PBG.initialHistoryAfterSwitches or PBG.pbTracker.memberVars['mode'] == 'p'):
                    ...
                    if(PBG.pbTracker.memberVars['scheduler'] is torch.optim.lr_scheduler.ReduceLROnPlateau):
                        if(PBG.verbose):
                            print('scheduler is now at %d bad epochs' % PBG.pbTracker.memberVars['schedulerInstance'].num_bad_epochs)
            if(countSinceSwitch <= PBG.initialHistoryAfterSwitches and PBG.pbTracker.memberVars['mode'] == 'n'):
                if(PBG.verbose):
                    print('not stepping with history %d and current %d' % (PBG.initialHistoryAfterSwitches, countSinceSwitch))
            for param_group in PBG.pbTracker.memberVars['optimizerInstance'].param_groups:
                learningRate2 = param_group['lr']
            stepped = False
            atLastCount = False
            if(PBG.verbose):
                print('checking if at last with scores %d, count since switch %d and last count %d' % (len(PBG.pbTracker.memberVars['currentStepScores']), countSinceSwitch, PBG.pbTracker.memberVars['lastLREpochCount']))
            ...
            if(((len(PBG.pbTracker.memberVars['currentStepScores']) == 0) and countSinceSwitch == PBG.pbTracker.memberVars['lastLREpochCount']*2)
               or ((len(PBG.pbTracker.memberVars['currentStepScores']) == 1) and countSinceSwitch == PBG.pbTracker.memberVars['lastLREpochCount'])):
                atLastCount = True
            if(PBG.verbose):
                print('at last count %d with count %d and last LR count %d' % (atLastCount, countSinceSwitch,  PBG.pbTracker.memberVars['lastLREpochCount']))
            ...
            if(learningRate1 != learningRate2):
                ...
                if(PBG.pbTracker.memberVars['currentStepCount'] == PBG.pbTracker.memberVars['lastMaxLearningRateSteps']):
                    ...
                    if(PBG.pbTracker.memberVars['currentStepCount'] - PBG.pbTracker.memberVars['currentNLearningRateInitialSkipSteps'] == 1):
                        PBG.pbTracker.memberVars['lastLREpochCount'] = countSinceSwitch
            ...
            if((PBG.pbTracker.memberVars['scheduler'] != None) 
               and ((PBG.pbTracker.memberVars['mode'] == 'n') or PBG.learnPBLive) #if its currently in n mode, or its learning live, i.e. if it potentially might have higher accuracy
               and (stepped or atLastCount)): #and the learning rate just stepped
                if(PBG.pbTracker.memberVars['committedToInitialRate'] == False): #and it hasnt commited yet
                    ...
                    if(len(PBG.pbTracker.memberVars['currentStepScores']) == 0 #if there are currently no scores
                        and (PBG.pbTracker.memberVars['currentStepCount'] - PBG.pbTracker.memberVars['currentNLearningRateInitialSkipSteps'] == 2# and either it just did its second step
                        ...
                        oldCounts = PBG.pbTracker.memberVars['lastLREpochCount']
                        ...
                        PBG.pbTracker.memberVars['currentStepScores'].append(oldAccuracy)
                        PBG.pbTracker.memberVars['globalBestValidationScore'] = oldGlobal
                        PBG.pbTracker.memberVars['lastLREpochCount'] = oldCounts
                    elif(len(PBG.pbTracker.memberVars['currentStepScores']) == 1):#if there is one score already, and this in theory is the first step at the second score
                        PBG.pbTracker.memberVars['currentStepScores'].append(PBG.pbTracker.memberVars['currentBestValidationScore'])                        
                        if((PBG.pbTracker.memberVars['maximizingScore'] and PBG.pbTracker.memberVars['currentStepScores'][0] > PBG.pbTracker.memberVars['currentStepScores'][1])
                           or ((not PBG.pbTracker.memberVars['maximizingScore']) and PBG.pbTracker.memberVars['currentStepScores'][0] < PBG.pbTracker.memberVars['currentStepScores'][1])): #and the first one is higher than the current one
                           ...
                           #then reload the current one, and then say we're good to go
                            if(PBG.verbose):
                                print('Got initial %d step score %f and %d score at step %f so loading old score' % (PBG.pbTracker.memberVars['currentNLearningRateInitialSkipSteps']-1,PBG.pbTracker.memberVars['currentStepScores'][0], PBG.pbTracker.memberVars['currentNLearningRateInitialSkipSteps'],PBG.pbTracker.memberVars['currentStepScores'][1])) 
                            
                            priorBest = PBG.pbTracker.memberVars['currentStepScores'][0]
                            ...
                            if(PBG.verbose):
                                print('Got initial %d step score %f and %d score at step %f so NOT loading old score and continuing with this score' % (PBG.pbTracker.memberVars['currentNLearningRateInitialSkipSteps']-1,PBG.pbTracker.memberVars['currentStepScores'][0], PBG.pbTracker.memberVars['currentNLearningRateInitialSkipSteps'],PBG.pbTracker.memberVars['currentStepScores'][1])) 
...
#to test that this is working make sure that 4 shows 4 picked.
                    ...
                    elif(len(PBG.pbTracker.memberVars['currentStepScores']) == 2):
        ...
        if(PBG.verbose):
            print('completed adding score.  restructured is %d, improved is %d \ncurrent switch list is:' % (restructured, improved))
