        ...
        for submoduleID, layer in net.named_children():
            ...
            if (type(net.get_submodule(submoduleID)) in PBG.modulesToConvert
                ...
                if(net != net.get_submodule(submoduleID)):
                    setattr(net,submoduleID,convertModule(net.get_submodule(submoduleID),  getPretrainedPBVar(pretrainedPB, submoduleID), depth + 1, subName))
                #else:
        ...
        for member in allMembers:
            ...
            if (type(getattr(net,member,None)) in PBG.modulesToConvert
                ...
                if(net != getattr(net,member)):
                    setattr(net,member,convertModule(getattr(net,member),  getPretrainedPBAttr(pretrainedPB,member), depth+1, subName))
                #else:
...
def convertNetwork(net, pretrainedPB = None, layerName=''):
    ...
    if(len(missedOnes) != 0 and PBG.unwrappedModulesConfirmed == False):
