...
class EntitiesParserRuleset(object):
    def __init__(self, source_file, parameters,
                 ruleset_var_name="CONCEPTS", parameters_var_name="PARAMETERS"):
    ...
    def load(self, full_ruleset, parser):
        spec = importlib.util.spec_from_file_location("dynamic_module", self.source_file)
...
class EntitiesParser(object):
        ...
        if parent_concepts:
            ...
            while parents:
                ...
                for parent in parents:
                    parse_rules = {k:v for k, v in parse_rules.items() if k not in self.concepts[parent].parse_rules.keys()}
                    parse_rules = {**self.concepts[parent].parse_rules, **parse_rules}
                    combine_rules = {k:v for k, v in combine_rules.items() if k not in self.concepts[parent].combine_rules.keys()}
                    combine_rules = {**self.concepts[parent].combine_rules, **combine_rules}
                    new_parents.extend(self.concepts[parent].inherits_rules_from_concepts)
    ...
    def _cull_entities(self, entities, relationships):
        ...
        for entity in entities:
            entity.parents = [parent for parent in entity.parents if parent not in removed_ids]
        ...
        relationships = list(filter(lambda x: x.concept_from not in removed_ids and x.concept_to not in removed_ids,
                                    relationships))
        relationships = list({rel.concept_from + rel.concept_to + rel.name: rel for rel in relationships}.values())
        return entities, relationships

    def _apply_rules_stack(self, rules, context, referred_entity, rule_index, combine_rules, indent):
        ...
        while rule_index < len(rules.keys()):
            ...
            for entry_key, entry_elem in context.items():
                if not entry_elem.used and rule.apply_rule(entry_key, entry_elem):
                    ...
                    combine_params_names_list, new_fields_list = match.apply_match(entry_elem)
                    ...
                    for i in range(len(new_fields_list)):
                        ...
                        for field_key, field_value in new_fields.items():
                            if combine_params_names and field_key in combine_params_names.keys() and combine_params_names[field_key]:
                                field_param_name = combine_params_names[field_key]
                            else:
                                if field_key in referred_entity.fields:
                                    field_param_name = str(len(referred_entity.fields[field_key].data))
                                else:
                                    field_param_name = '0'
                            if field_key not in referred_entity.fields:
                                if field_key in combine_rules.keys():
                                    ...
                                    combiner.load(self)
                                referred_entity.fields[field_key] = CombineFieldInfo(combiner, {field_param_name: [field_value]})
                            else:
                                if field_param_name not in referred_entity.fields[field_key].data:
                                    referred_entity.fields[field_key].data[field_param_name] = list()
                                referred_entity.fields[field_key].data[field_param_name].append(field_value)
    ...
    def _append_extra_fields(self, entities, entry):
                ...
                for key, value in entry_dict.items():
                    if isinstance(value, Entry):
                        if not value.used:
                            ...
                            unused_fields.update(_find_extra_fields_recursive(value.value))
        ...
        if root_entity:
            ...
            root_entity.fields['extra_fields'] = {**root_entity.fields['extra_fields'], **unused_fields}
        ...
        for entity in entities:
            for field_key, field_value in entity.fields.items():
                if isinstance(field_value, CombineFieldInfo):
                    combined_value = field_value.combiner.combine_field(field_value.data, entity)
                    if combined_value is not None:
                        entity.fields[field_key] = combined_value
        return entities

    def _compute_deterministic_uuid(self, entity_fields, entity_type):
        new_uuid = str(uuid.uuid5(uuid.UUID(bytes=hashlib.md5(ENTITIES_NAMESPACE).digest()),
                       hashlib.md5((entity_type + "\n" + json.dumps(entity_fields, sort_keys=True))
                                    .encode("utf-8"))
                                    .hexdigest()))
        return new_uuid
    ...
    def _apply_deterministic_uuids(self, entities, relationships):
        rnd_to_det_map = dict()
        for entity in entities:
            rnd_to_det_map[entity.unique_id] = self._compute_deterministic_uuid(entity.fields, entity.concept_name)
        for entity in entities:
    ...
    def apply_rules(self, entry, endpoint=None, input=None, guessed_entity=None):
        ...
        self._apply_rules_stack(self.curr_rules, self.curr_context, self.curr_referred_entity, self.curr_rule_index, self.curr_combine_rules, self.curr_indent)
        self.curr_entities = self._combine_fields(self.curr_entities)
        self._append_extra_fields(self.curr_entities, entry['0'])

        self.curr_entities, self.curr_relationships = self._cull_entities(self.curr_entities, self.curr_relationships)
        self.curr_entities, self.curr_relationships = self._apply_deterministic_uuids(self.curr_entities, self.curr_relationships)
        self.curr_entities, self.curr_relationships = self._cull_entities(self.curr_entities, self.curr_relationships)
    ...
    def consolidate_entities(self, parsed_entities, parsed_relationships, consolidation_parameters=None):
        ...
        if consolidation_parameters.keep_roots:
            ...
            new_root_entities = [EntityInfo(self._compute_deterministic_uuid(root_entity.fields, root_entity.concept_name),
                                            root_entity.concept_name, root_entity.fields, root_entity.parents) for root_entity in root_entities]
            consolidated_entities.extend(new_root_entities)
            parsed_to_consolidated_relationships.extend([RelationshipInfo(root_entities[i].unique_id, new_root_entities[i].unique_id, "consolidated_to") for i in range(len(root_entities))])
            parsed_entities = list(filter(lambda x: x.parents, parsed_entities))
        for concept_name, group in itertools.groupby(sorted(parsed_entities, key=lambda x: x.concept_name), lambda x: x.concept_name):
            group = list(group)
            index = np.array([elem.unique_id for elem in group], dtype=object)
            field_names = sorted(list(set(_list_flatten([list(elem.fields.keys()) for elem in group]))))
            sorted_group = [dict(sorted(elem.fields.items())) for elem in group]
            sorted_group = [{**{k:'' for k in field_names}, **elem.fields} for elem in group]
            equal_tolerance = 0 if concept_name not in consolidation_parameters.equality_tolerances else consolidation_parameters.equality_tolerances[concept_name]
            equal_tolerance *= len(field_names)
            similar_tolerance = 2*equal_tolerance if concept_name not in consolidation_parameters.similarity_tolerances else consolidation_parameters.similarity_tolerances[concept_name]
            similar_tolerance *= len(field_names)
            sorted_strings = [str(list(elem.items())) for elem in sorted_group]
            distances = [Levenshtein.distance(i, j, score_cutoff=similar_tolerance) for (i, j) in itertools.combinations(sorted_strings, 2)]
            ...
            parsed_to_consolidated_relationships.extend([RelationshipInfo(pair[0], pair[1], "same_as") for pair in equal_pairs if pair[0] != pair[1]])
            parsed_to_consolidated_relationships.extend([RelationshipInfo(pair[0], pair[1], "similar_to") for pair in similar_pairs if pair[0] != pair[1]])
            equal_groups = list()
            ...
            for group in equal_groups:
                ...
                consolidated_entity = EntityInfo(self._compute_deterministic_uuid(consolidated_fields, concept_name),
                                                 concept_name, consolidated_fields, list(parents))
                consolidated_entities.append(consolidated_entity)
                parsed_to_consolidated_relationships.extend([RelationshipInfo(elem, consolidated_entity.unique_id, "consolidated_to") for elem in group])
        entities_consolidation_map = {entity.unique_id: [rel.concept_from for rel in list(filter(lambda x: x.name == "consolidated_to" and x.concept_to == entity.unique_id,
                                                                                                 parsed_to_consolidated_relationships))]
                                      for entity in consolidated_entities}
        entities_consolidation_map = [{original_uid:consolidated_uid for original_uid in original_uids}
                                      for consolidated_uid, original_uids in entities_consolidation_map.items()]
        entities_consolidation_map = dict((key,d[key]) for d in entities_consolidation_map for key in d)
        for entity in consolidated_entities:
            entity.parents = list(set([entities_consolidation_map[parent] for parent in entity.parents]))
        consolidated_relationships = [(entities_consolidation_map[rel.concept_from],
                                       entities_consolidation_map[rel.concept_to],
                                       rel.name) for rel in parsed_relationships]
        consolidated_relationships = [RelationshipInfo(rel[0], rel[1], rel[2]) for rel in set(consolidated_relationships)]
        consolidated_entities, _ = self._cull_entities(consolidated_entities, [])
        return consolidated_entities, consolidated_relationships, parsed_to_consolidated_relationships
    ...
    def validate_and_format_entities(self, entities, relationships):
        ...
        from streamlit_octostar_utils.ontology.validation import validate_and_format_timbr_type
        ...
        for entity in entities:
            if entity.concept_name not in self.ontology['concepts']:
                ...
                continue
            entity_properties = [k for k in entity.fields.keys() if not k.startswith('#')]
            ontology_properties = {e['property_name']: e for e in self.ontology['concepts'][entity.concept_name]['allProperties']}
            if any(prop not in ontology_properties for prop in entity_properties):
                processed_entities.append((EntitiesInvalidReason.INVALID_PROPERTIES, entity))
                continue
            for prop in entity_properties:
                try:
                    entity.fields[prop] = validate_and_format_timbr_type(ontology_properties[prop]['property_type'], entity.fields[prop])
                except:
                    processed_entities.append((EntitiesInvalidReason.INVALID_PROPERTIES, entity))
                continue
            processed_entities.append((EntitiesInvalidReason.VALID, entity))
        ontology_relationships = {r['relationship_name']: r for r in self.ontology['relationships']}
        for rel in relationships:
            if rel.name not in ontology_relationships.keys():
                processsed_relationships.append((EntitiesInvalidReason.INVALID_REL_TYPE, rel))
                continue
            entities_by_id = {e.unique_id: e for e in entities}
            if rel.concept_from not in entities_by_id.keys() or rel.concept_to not in entities_by_id.keys():
                processsed_relationships.append((EntitiesInvalidReason.UNKNOWN_UUID, rel))
                continue
            ontology_relationship = ontology_relationships[rel.name]
            rel_concept_types = (entities_by_id[rel.concept_from].concept_name, entities_by_id[rel.concept_to].concept_name)
            if not is_child_concept(rel_concept_types[0], ontology_relationship['concept'], self.ontology) or \
               not is_child_concept(rel_concept_types[1], ontology_relationship['target_concept'], self.ontology):
                processsed_relationships.append((EntitiesInvalidReason.WRONG_SOURCE_OR_TARGET_TYPE, rel))
                continue
    ...
    def extra_fields_to_entities(self, source_entities, remove_fields_from_source=True):
        ...
        for entity in source_entities:
            if entity.fields and 'extra_fields' in entity.fields and entity.fields['extra_fields']:
                new_entity = EntityInfo(self._compute_deterministic_uuid(entity.fields['extra_fields'], "json", False),
                                        "json", entity.fields['extra_fields'])
                if remove_fields_from_source:
                    del entity.fields['extra_fields']
                new_entities.append(new_entity)
                new_relationships.append(RelationshipInfo(entity.unique_id, new_entity.unique_id, "has_unparsed_data"))
