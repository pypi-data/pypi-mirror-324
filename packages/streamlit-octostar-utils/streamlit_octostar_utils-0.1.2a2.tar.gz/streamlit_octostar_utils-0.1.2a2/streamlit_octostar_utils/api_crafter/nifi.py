...
class NifiContextManager(object):
    ...
    def receive_input(self, json_data, processor_name, producer_name=None) -> List["NifiEntity"]:
        ...
        entities = sorted(entities, key=lambda x: string_to_datetime(x.record.get('os_last_updated_at')))
    ...
    def send_output(self, entity_batches, processor_name, producer_name=None):
        ...
        all_entities = sorted(all_entities, key=lambda x: string_to_datetime(x.record.get('os_last_updated_at')))
    ...
    def raise_exception(self, entity, exc):
        ...
        travel_dict(entity.request['nifi_attributes'], ['invokehttp', 'response', 'body'], 'w')(entity.request['exception']['body'])
        travel_dict(entity.request['nifi_attributes'], ['invokehttp', 'response', 'code'], 'w')(entity.request['exception']['code'])
        ...
        for entity in entities:
            if entity.sync_params.get(NifiContextManager.SyncFlag.WRITE_ENTITY_ANNOTATIONS):
                ...
                for producer_name, params in annotations_per_producer:
                    ...
                    producer_annotations = sorted(producer_annotations, key=lambda x: string_to_datetime(x.record.get('os_last_updated_at')))
    ...
    f" FROM `dtimbr`.`{entity_type}` WHERE `os_entity_uid` IN ({ids})")
            ...
            for rel in rels:
                ...
                if rel['os_entity_uid']:
                    new_entity, new_rel = entities_by_id[rel['os_entity_uid_from']].add_child_entity(
                        os_workspace, entity_type, fields, os_relationship_name, os_relationship_type)
                    new_rel['os_entity_uid'] = 
                    new_rel['entity_id'] = 
...
class NifiEntity(object):
        ...
        if not self._contents:
            ...
            if contents_pointer['location'] == 'attachment':
                self._contents = read_file.sync(
                    contents_pointer['pointer'].split("/")[0], contents_pointer['pointer'].split("/")[-1], False, client=self.context.client)
    ...
    def is_child_concept(self, type):
        ...
        if isinstance(self, NifiEntityProxy):
            ...
            return entity_type == type or entity_type in self.request['ontology_info']['parents']
    ...
    def to_json(self):
        ...
        full_entity_children = sorted(full_entity_children, key=lambda x: string_to_datetime(x.record.get('os_last_updated_at')))
    ...
    def add_mtm_relationship(self, os_entity_uid_from, entity_type_from, os_entity_uid_to, entity_type_to,
        ...
        return self._add_entity(
            os_relationship_workspace, LOCAL_RELATIONSHIP_ENTITY_NAME if os_relationship_workspace else RELATIONSHIP_ENTITY_NAME,
            {
                **relationship_fields,
    ...
    def get_annotation(self, producer_names=None):
        ...
        annotations = sorted(annotations, lambda x: string_to_datetime(x.record.get('os_last_updated_at')))
...
def more_recent_than(record_a, record_b):
    return string_to_datetime(record_a.get('os_last_updated_at')) >= string_to_datetime(record_b.get('os_last_updated_at'))
        ...
        def decorator(func):
            ...
            def nifi_func(task, body, processor_name, *args, **kwargs):
                with NifiContextManager(body) as nifi_context:
                    ...
                    entity_batches = func(task, nifi_context, entity_batches, processor_name, *args, **kwargs)
