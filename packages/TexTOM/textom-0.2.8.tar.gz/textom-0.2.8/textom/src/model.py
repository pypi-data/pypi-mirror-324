...
class model:
    ...
    def __init__(self, path, single=False):
        ...
        if single: # initializes parameters if only a single image is produced
            ...
            if os.path.isfile(os.path.join(path,'analysis','projectors.h5')): # loads a tomo model if it exists
                ...
                print("\tLoaded projectors from analyis/projectors.h5 (%2.2f s)" % ( (time()-t0)) )
    ...
    def _init_diffractlets(self):
        ...
        self.Qq_det, self.Chi_det, self.detShape = qchi(self.q, self.chi)
        ...
        print('\t calculating single crystal diffraction patterns (long calculation time possible, do not close this terminal)')
        t0 = time()
