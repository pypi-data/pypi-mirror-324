...
ABSOLUTE_SPIDERY_PATH = "spidery/spidery.py"
TEST_API_KEY = os.getenv('TEST_API_KEY')

print(f"ABSOLUTE_SPIDERY_PATH: {ABSOLUTE_SPIDERY_PATH}")

spec = importlib.util.spec_from_file_location("SpideryApp", ABSOLUTE_SPIDERY_PATH)
spidery = importlib.util.module_from_spec(spec)
spec.loader.exec_module(spidery)
...
def test_no_api_key():
    if 'api.spidery.khulnasoft.com' in API_URL:
        ...
        app = SpideryApp(api_url=API_URL)
...
def test_scrape_url_invalid_api_key():
    if 'api.spidery.khulnasoft.com' in API_URL:
        ...
        app = SpideryApp(api_url=API_URL)
        response = app.scrape_url('https://spidery.khulnasoft.com')
        assert response is not None

# def test_blocklisted_url():
#     blocklisted_url = "https://facebook.com/fake-test"
#     app = SpideryApp(api_url=API_URL, api_key=TEST_API_KEY)
#     with pytest.raises(Exception) as excinfo:
...
def test_successful_response_with_valid_api_key_and_options():
    ...
    assert response['metadata']['description'] == "Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of Spidery to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️"
    assert response['metadata']['keywords'] == "Roast My Website,Roast,Website,GitHub,Spidery"
    assert response['metadata']['robots'] == "follow, index"
    assert response['metadata']['ogTitle'] == "Roast My Website"
    assert response['metadata']['ogDescription'] == "Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of Spidery to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️"
...
def test_crawl_url_invalid_api_key():
    if 'api.spidery.khulnasoft.com' in API_URL:
        ...
        # Should work without API key for self-hosted
        app = SpideryApp(api_url=API_URL)
        response = app.crawl_url('https://spidery.khulnasoft.com')
        assert response is not None
...
def test_crawl_url_with_idempotency_key_e2e():
    ...
    with pytest.raises(Exception) as excinfo:
        app.crawl_url('https://spidery.khulnasoft.com', {'excludePaths': ['blog/*']}, True, 2, uniqueIdempotencyKey)
    assert "Idempotency key already used" in str(excinfo.value)

def test_check_crawl_status_e2e():
    app = SpideryApp(api_url=API_URL, api_key=TEST_API_KEY)
    response = app.crawl_url('https://spidery.khulnasoft.com', {'scrapeOptions': {'formats': ['markdown', 'html', 'rawHtml', 'screenshot', 'links']}}, False)
    assert response is not None
    assert 'id' in response
...
#     app = SpideryApp(api_key=TEST_API_KEY, api_url=API_URL)
#     blocklisted_url = "https://facebook.com/fake-test"
#     with pytest.raises(Exception) as excinfo:
...
#     app = SpideryApp(api_url=API_URL, api_key=TEST_API_KEY)
#     response = app.scrape_url("https://khulnasoft.com", {
#         'extractorOptions': {
#             'mode': 'llm-extraction',
...
def test_search_invalid_api_key():
    app = SpideryApp(api_url=API_URL, api_key="invalid_api_key")
