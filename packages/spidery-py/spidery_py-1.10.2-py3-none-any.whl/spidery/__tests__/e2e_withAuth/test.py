...
def test_no_api_key():
    with pytest.raises(Exception) as excinfo:
      invalid_app = SpideryApp(api_url=API_URL, version='v0')
    assert "No API key provided" in str(excinfo.value)

def test_scrape_url_invalid_api_key():
    invalid_app = SpideryApp(api_url=API_URL, api_key="invalid_api_key", version='v0')
    with pytest.raises(Exception) as excinfo:
...
#     app = SpideryApp(api_url=API_URL, api_key=TEST_API_KEY, version='v0')
#     with pytest.raises(Exception) as excinfo:
#         app.scrape_url(blocklisted_url)
#     assert "Unexpected error during scrape URL: Status code 403. Spidery currently does not support social media scraping due to policy restrictions. We're actively working on building support for it." in str(excinfo.value)

def test_successful_response_with_valid_preview_token():
    app = SpideryApp(api_url=API_URL, api_key="this_is_just_a_preview_token", version='v0')
...
#     assert "Unexpected error during start crawl job: Status code 403. Spidery currently does not support social media scraping due to policy restrictions. We're actively working on building support for it." in str(excinfo.value)
...
def test_crawl_url_with_idempotency_key_e2e():
    ...
    with pytest.raises(Exception) as excinfo:
        app.crawl_url('https://spidery.khulnasoft.com', {'crawlerOptions': {'excludes': ['blog/*']}}, True, 2, uniqueIdempotencyKey)
...
def test_check_crawl_status_e2e():
    app = SpideryApp(api_url=API_URL, api_key=TEST_API_KEY, version='v0')
    response = app.crawl_url('https://spidery.khulnasoft.com', {'crawlerOptions': {'excludes': ['blog/*']}}, False)
    assert response is not None
