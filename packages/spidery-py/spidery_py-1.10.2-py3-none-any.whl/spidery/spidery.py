...
This module provides a class `SpideryApp` for interacting with the Spidery API.
...
Classes:
    - SpideryApp: Main class for interacting with the Spidery API.
"""
import logging
import os
import time
...
class SpideryApp:
    ...
    def __init__(self, api_key: Optional[str] = None, api_url: Optional[str] = None) -> None:
        """
        Initialize the SpideryApp instance with API key, API URL.

        Args:
            api_key (Optional[str]): API key for authenticating with the Spidery API.
            api_url (Optional[str]): Base URL for the Spidery API.
        """
        self.api_key = api_key or os.getenv('SPIDERY_API_KEY')
        self.api_url = api_url or os.getenv('SPIDERY_API_URL', 'https://api.spidery.khulnasoft.com')
        
        # Only require API key when using cloud service
        if 'api.spidery.khulnasoft.com' in self.api_url and self.api_key is None:
            logger.warning("No API key provided for cloud service")
            raise ValueError('No API key provided')
            
        logger.debug(f"Initialized SpideryApp with API URL: {self.api_url}")

    def scrape_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Any:
        """
        Scrape the specified URL using the Spidery API.
        ...
        if response.status_code == 200:
            try:
                ...
                raise Exception(f'Failed to parse Spidery response as JSON.')
    ...
    def search(self, query: str, params: Optional[Union[Dict[str, Any], SearchParams]] = None) -> Dict[str, Any]:
        """
        Search for content using the Spidery API.
    ...
    def crawl_url(self, url: str,
        ...
        """
        Initiate a crawl job for the specified URL using the Spidery API.
    ...
    def check_crawl_status(self, id: str) -> Any:
        """
        Check the status of a crawl job using the Spidery API.
    ...
    def cancel_crawl(self, id: str) -> Dict[str, Any]:
        """
        Cancel an asynchronous crawl job using the Spidery API.
    ...
    def map_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Any:
        """
        Perform a map search using the Spidery API.
    ...
    def batch_scrape_urls(self, urls: list[str],
                  ...
                  idempotency_key: Optional[str] = None) -> Any:
        """
        Initiate a batch scrape job for the specified URLs using the Spidery API.
    ...
    def check_batch_scrape_status(self, id: str) -> Any:
        """
        Check the status of a batch scrape job using the Spidery API.
    ...
    def extract(self, urls: List[str], params: Optional[ExtractParams] = None) -> Any:
        """
        Extracts information from a URL using the Spidery API.
    ...
    def _handle_error(self, response: requests.Response, action: str) -> None:
        ...
        try:
            ...
            raise requests.exceptions.HTTPError(f'Failed to parse Spidery error response as JSON. Status code: {response.status_code}', response=response)
...
class CrawlWatcher:
    def __init__(self, id: str, app: SpideryApp):
        self.id = id
        self.app = app
