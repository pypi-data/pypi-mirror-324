...
from astral_ai.typing.messages import MessageList, Message
from astral_ai.typing.response import StructuredOutputResponse, ChatResponse
from astral_ai.constants.usage import REASONING_EFFORT_SUPPORTED_MODELS, REASONING_EFFORT_DEFAULT
    ...
    def _format_messages(self, messages: Union[MessageList, List[Message]]) -> List[dict]:
        ...
        if isinstance(messages, MessageList):
            ...
            if not all(isinstance(msg, Message) for msg in messages):
                raise TypeError("All messages must be of type Message")
            return [message.to_openai_format() for message in messages]
        else:
            raise TypeError("Messages must be either MessageList or List[Message]")
