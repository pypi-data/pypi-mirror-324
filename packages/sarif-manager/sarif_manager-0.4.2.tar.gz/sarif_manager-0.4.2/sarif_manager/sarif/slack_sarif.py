...
class SlackSarif:
    ...
    def _clean_rule_name(self, rule_id: str, issue_id: str) -> str:
        ...
        Args:
            rule_id (str): The full rule ID (e.g. "451889d8-e6e5-4eba-9fe2-3b0b8fd92d3e-SQL Injection - PostgreSQL")
    ...
    def send_to_slack(self, pdf_file=None):
        """Send findings to Slack channel.
        
        Args:
            pdf_file (str, optional): Path to PDF file to attach
            
        Returns:
            dict: Slack API response
        """
        if not self.slack_client or not self.slack_channel:
            raise ValueError("Slack client or channel not configured")
