...
def get_ldap_path(name):
    ...
    try:
        ...
        print(f"An error occurred for {name}: {e}", file=sys.stderr)
        return None, None
...
def expand_users(ldap_groups):
    print(f"Group expansion is enabled. Begin searching for users of the given groups. This may take 1..3 minutes; grab a coffee ...", file=sys.stderr)
    all_users = set()
    for group_name in ldap_groups:
        try:
            ...
            print(f"An error occurred while expanding group {group_name}: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Get LDAP paths for specified Active Directory users or groups.")

    parser.add_argument(  'names',  type=str, nargs='*', metavar='N', help='List of usernames or group names to check')
    parser.add_argument('--yaml',   type=str,                         help='YAML file specifying ldap-group and ldap-user entries')
    parser.add_argument('--out',                                      help='Output .htaccess file path')
    parser.add_argument('--expand', type=str,                         help='Output YAML file to expand all users from ldap groups')
