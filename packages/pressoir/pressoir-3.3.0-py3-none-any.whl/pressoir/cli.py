...
def stylo(
    ...
    book_dict = {
        ...
        "toc": [
            {"id": folder.name}
            for folder in each_folder_from("textes")
            if folder.name not in pressoir_dirs
        ],
    }
    book_yaml = yaml.dump(book_dict)
