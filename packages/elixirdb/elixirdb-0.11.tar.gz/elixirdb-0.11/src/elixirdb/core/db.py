...
from elixirdb.core.handlers import handler as h_
from elixirdb.dictionaries.db_errors import db_errors as errors
...
class ElixirDB(ConnectionBase):
    ...
    def __init__(
        ...
        session_factory: scoped_session[Session] | sessionmaker[Session] | None = None,
        **kwargs,
    ):
        """
    ...
    # def __setattr__(self, name, *args, **kwargs) -> None:
    ...
    def engine(self) -> Engine:
        ...
        if not self.current_engine:
            engine_options = (
                self.db.engine_options.model_dump(exclude_unset=True, exclude_none=True)
    ...
    def new_session(self) -> ElixirDB:
        ...
        Raises:
            ValueError: If connection mode is DIRECT or no session factory.
        """

        if not self.session_factory:
            if self.engine_type != "session":
                raise InvalidEngineTypeError("new_session only works with `session` engine_type.")
            self.session_factory = self.create_session_factory(
    ...
    def create_session_factory(
        ...
        options = (
            self.db.session_options.model_dump(exclude_none=True, exclude_unset=True)
    ...
    def get_fetch_size(self, fetch_size: int | None) -> int:
        """Get the fetch size for the current connection."""
        return fetch_size or self.db.statements.fetch_size or 0

    def fetch_results(
        self, result: CursorResult, fetch_size: int | None = None
    ) -> Sequence[RowData]:
        """Fetch results from the result object as mappings."""
        fetch_size = self.get_fetch_size(fetch_size)
...
class StatementsMixin:
    ...
    def procedure(
        ...
        if not self.statevars.schema_applied and self.db.statements.prefix_procedures:
    ...
    def add_schema_prefix(
        self, statement: str, operation_type: Literal["statement", "procedure"]
    ) -> str:
        """Add the schema to stored procedure/function names."""
        if not self.db.statements.prefix_schema:
            return statement
        if operation_type == "procedure":
            if "." in statement:
                warnings.warn(errors["prefix_error"], stacklevel=2)
            return f"{self.db.statements.prefix_schema}.{statement}"

        return apply_schema_to_statement(
            statement, self.db.config.prefix_schema, self.db.dialect
        )
