...
class MotorCom:
    def __init__(self,motor="MAC",com_type="Serial",protocol="MacTalk",
                 port=None,ip=None,baudrate=19200,motor_address=254,netX=50,
                 io_mode=None,motor_version="0",update=False):
        self.port = port
        ...
        if protocol == "MacTalk":
            ...
            self.protocol = EthernetIPProtocol(motor=motor,port=port,update=update)
            self.real_time = True

        elif protocol == "ProfinetIO" or protocol == "Profinet" or protocol == "EP":
            self.protocol = ProfinetIOProtocol(motor=motor,port=port,update=update)
            self.real_time = True

        elif protocol == "EtherCAT" or protocol == "Ethercat" or protocol == "EC":
            if io_mode == "Registers" or io_mode == "Reg" or io_mode == "JVL":
                self.protocol = EtherCATRegistersProtocol(motor=motor,motor_version=motor_version,update=update)
                self.real_time = True
            elif io_mode == "Driver" or io_mode == "DSP" or io_mode == "CiA" or io_mode == "402":
                self.protocol = EtherCATDSPProtocol(motor=motor,motor_version=motor_version,update=update)
                self.real_time = True
    ...
    def write(self,reg,data,scaling=1,length=4,no_response=False):
        #TODO: Handle errors
        data = data/scaling
        if data < 0:
            data = (1 << 8*length) + int(data)
        return self.protocol.write(reg,int(data),length,no_response)

    def read(self,reg,scaling=1,length=4):
        #TODO: Handle Errors
        data = self.protocol.read(reg,length=length)
        return data*scaling
    
    def writeModule(self,reg,data,scaling=1,length=4,no_response=False):
        data = data/scaling
        if data < 0:
            data = (1 << 8*length) + int(data)
        return self.protocol.writeModule(reg,int(data),length,no_response)
    
    def readModule(self,reg,scaling=1,length=4):
        #TODO: Handle Errors
        data = self.protocol.readModule(reg,length=length)
        return data*scaling
    
    def writeBit(self,reg,bit,data):
        value = self.protocol.read(self.motor.registers[reg][0])
        if data == 1:
            value |= (1 << bit)
        else:
            value &= (1 << bit)
        self.protocol.write(self.motor.registers[reg][0],value)
        

    def readBit(self,reg,bit):
        return (self.protocol.read(self.motor.registers[reg][0]) >> bit) & 1
    
    def implicitRead(self,reg,scaling,length=4):
    ...
    def writeRegister(self,name,value,scaling=1,length=4,no_response=False):
        return self.write(self.motor.registers[name][0],value,
                   scaling=scaling,length=length,no_response=no_response)

    def readRegister(self,name,scaling=1,length=4):
        return self.read(self.motor.registers[name][0],scaling=scaling,length=length)

    def writeModuleRegister(self,name,value,scaling=1,length=4,no_response=False):
        return self.writeModule(self.motor.module_registers[name],value,
                   scaling=scaling,length=length,no_response=no_response)

    def readModuleRegister(self,name,scaling=1,length=4):
    ...
    def setPosition(self,counts,length=4):
