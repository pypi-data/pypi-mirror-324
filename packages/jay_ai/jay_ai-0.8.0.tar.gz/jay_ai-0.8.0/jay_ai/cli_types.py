...
class TTSProvider(Enum):
    ...
    FISHAUDIO = ProviderInfo(
        display_label="FishAudio",
        label="fishaudio",
        injection='TTS.FishAudio(api_key=os.environ["FISHAUDIO_API_KEY"])',
        env_var_keys=[
            ProviderEnvVarKey(env_var_key="FISHAUDIO_API_KEY", is_json=False)
        ],
    )
    GOOGLE = ProviderInfo(
...
class TTS:
    class FishAudio(BaseModel):
        api_key: str
        model_id: str = fishaudio.DEFAULT_MODEL
