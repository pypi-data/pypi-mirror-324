...
class NearestNeighbors:
    def __init__(
        self, n_neighbors=5, algorithm="auto", metric="euclidean", leaf_size=30
    ):
    ...
    def _build_hnsw_graph(self, M=5, ef_construction=100):
        ...
        for i in range(n_samples):
            ...
            if len(graph[i]) > M:
                graph[i] = set(
                    sorted(
                        graph[i], key=lambda x: self._distance(curr_point, self.X[x])
                    )[:M]
                )
            ...
            if "points" in node:  # Leaf node
                ...
                if self.algorithm == "kd_tree":
                    ...
                    if point[axis] < node["point"][axis]:
                        ...
                        if (
                            len(heap) < n_neighbors
                            or abs(point[axis] - node["point"][axis]) < -heap[0][0]
                        ):
                    ...
                    if (
                        len(heap) < n_neighbors
                        or dist_to_center - node["radius"] < -heap[0][0]
                    ):
                        search_tree(node["left"], point, heap)
                        search_tree(node["right"], point, heap)
