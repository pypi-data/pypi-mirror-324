...
class Kradle:
    ...
    def serve_agent(cls, agent_class, slug, port=None, tunnel=True, host='localhost'):
        """Start the agent server with API key validation"""
        instance = cls()
        
        # Check for API key right away
        if not hasattr(instance, '_api_key') or not instance._api_key:
            instance._logger.log_error(
                "No API key found. Please set your API key before starting the agent:\n\n"
        ...
        if not instance._app:
            ...
            if not instance._is_port_available(instance.port):
                instance._logger.log_error(f"Port {instance.port} is not available")
            ...
            if slug != "*":
                success = instance.register_agent(slug, agent_url)
                agent_class.url = agent_url
                agent_class.slug = slug

            # Display the startup banner with all configuration
            instance._logger.display_startup_banner({
                ...
                'agent_slug': slug,
                'agent_url': agent_url,
                'challenge_url': 'https://app.kradle.ai/workbench/challenges'
            })
    ...
    @classmethod
    def create_session(cls, api_key, challenge_slug, agents):
        """Create a new challenge session for one or more agents."""
        instance = cls()
        base_url = "http://localhost:3000" if os.getenv("KRADLE_DEV") else "https://app.kradle.ai"
        ...
        try:
            ...
            response = requests.post(
                f'{base_url}/api/createSession',
                headers={'Content-Type': 'application/json', 'kradle-api-key': api_key},
            ...
            instance._logger.log_info(f"View it live: {base_url}/workbench/sessions/{session_id}")
