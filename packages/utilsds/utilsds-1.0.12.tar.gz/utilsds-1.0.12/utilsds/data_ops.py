...
class DataOperations:
    ...
    def load_bq_data(
        ...
        if not destination:
            job_config_params['destination'] = None
    ...
    def load_bq_procedure(
        ...
        try:
            ...
            raise Exception(f"Error executing procedure {procedure_name}: {str(e)}")
    ...
    def save_bq_table(
        ...
        partition_type: bigquery.TimePartitioningType = None,
        ...
        if write.upper() not in self.SUPPORTED_WRITE_DISPOSITIONS:
            raise ValueError(f"Unsupported write disposition. Supported dispositions are: {self.SUPPORTED_WRITE_DISPOSITIONS}")
        ...
        job_config = bigquery.LoadJobConfig(
            write_disposition=write_map[write.upper()],
            schema=schema
        )
    ...
    def delete_bq_data(
        ...
        """
        Delete data from BigQuery table. Needs confirmation.
        ...
        if where_clause:
            ...
            confirmation = input(f"Are you sure you want to delete {affected_rows:,} rows from '{target}'? (y/n): ")
            ...
            confirmation = input(f"Are you sure you want to delete the entire table/view '{target}'? (y/n): ")
            if confirmation.lower() == 'y':
                try:
                    self.bq_client.delete_table(target)
                    print(f"{target} successfully deleted.")
                except Exception as e:
                    raise Exception(f"Error deleting table {target}: {str(e)}")
            else:
                print("Deletion cancelled.")
    ...
    def load_gcs_details(
        ...
        try:
            ...
            raise Exception(f"Error listing bucket {bucket_name}: {str(e)}")
    ...
    def save_gcs_file(
        ...
        try:
            ...
            if not bucket.exists():
                confirmation = input(f"Bucket '{bucket_name}' does not exist. Would you like to create it? (y/n): ")
                if confirmation.lower() == 'y':
                    self.save_gcs_bucket(bucket_name)
                else:
                    print(f"Operation cancelled - bucket '{bucket_name}' does not exist and creation was declined.")
                    return
                ...
                case '.csv': 
                    ...
                    if not isinstance(content, str):
                        try:
                            ...
                            raise ValueError(f"Unsupported file type. Supported types are: {self.SUPPORTED_FILE_TYPES}")
                case '.sql':
                    content_type = 'text/plain'
                    if not isinstance(content, str):
                        raise ValueError("SQL content must be a string")
                case _: raise ValueError(f"Unknown file type. Supported types are: {self.SUPPORTED_FILE_TYPES}")
            ...
            raise Exception(f"Error saving file to GCS: {str(e)}")
    ...
    def load_gcs_file(
        ...
        while not blob.exists():
            ...
            print(f"File '{destination_blob_name}' not found in bucket '{bucket_name}'.")
        ...
        with blob.open(mode='rb') as file:
            match file_type:
                case '.pkl': content = pickle.load(file)
                case '.json': content = json.load(file)
                case '.csv': content = pd.read_csv(file)
                case '.html': content = file.read()
                case '.sql': content = file.read()
                case _: raise ValueError(f'Cannot detect file type from {destination_blob_name}. Supported types are: {self.SUPPORTED_FILE_TYPES}')
