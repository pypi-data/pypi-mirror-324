...
class RunKontrol:
    ...
    def _run_kontrol_in_container(self) -> None:
        ...
        try:
            ...
            if self.prove_only_profile:  # Skip "kontrol build"
                ...
                kontrol_command = "kontrol build " + (f" {self.extra_build_args}" if self.extra_build_args else " ")
                click.echo(f"$ {kontrol_command}")
                return_code, output = self.container.exec_run(kontrol_command, stream=True)
    ...
    def _execute_kontrol_prove_container(self, args: str) -> bool:
        kontrol_command = "kontrol prove --xml-test-report" + (f" {args}" if args else " ")
        click.echo(f"$ {kontrol_command}")
