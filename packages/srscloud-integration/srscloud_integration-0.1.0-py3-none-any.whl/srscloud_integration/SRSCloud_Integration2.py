...
class SRS:
    ...
    def __init__(self, token, maquina, workflow, tarefa, url = 'https://c02.api.srscloud.com.br/api/', logFile='info', execucaoId=False, filaId=False):
        self.url = url
        self.token = token
        ...
        try: 
            ...
            if len(arg1) != 24: self.execucaoId = execucaoId
        except: self.execucaoId = execucaoId
        try: 
            arg2 = sys.argv[2] #recebe a FilaId
            if len(arg2) != 24: self.filaId = filaId
        except: self.filaId = filaId

    def proxy(self, server, user, password):
    ...
    def setExecucaoId(self, execucaoId):
        self.execucaoId = execucaoId
        
    def setFilaId(self, filaId):
        self.filaId = filaId

    def setLogFile(self, localLog, logFile):
        self.logFile = logFile
        self.localLog = localLog

    # Funções de apoio
    def formatar_arquivo(self, arquivo): #formata o arquivo para enviar
        retorno = {}
    ...
    def salvar_arquivo(self, link, destino): #recebe o link do arquivo e salva com o nome e local desejado
        r = requests.get(link)
        with open(destino, 'wb') as f: f.write(r.content)
        return True

    def logMaquina(self, nivel, mensagem): 
        """#Niveis de log
        ...
        if not os.path.isdir(os.path.dirname(self.localLog)): os.makedirs(self.localLog)
    ...
    def execucaoIniciar(self): 
        ...
        except Exception as e: retorno = {'Autorizado': False, 'Mensagem': f'Falha na comunicação:{e}'}
    ...
    def log(self, statusId, mensagem, arquivo=''):
        """em arquivo enviar apenas o path do arquivo """
        entrada = {'Token': self.token,
    ...
    def parametroAtualizar(self, parametro, valor, workflow=False, tarefa=False):
    ...
    def execucaoFinalizar(self, status='Ok', statusId=4, mensagem='Execução finalizada'):
    ...
    def executarTarefa(self, workflow, tarefa, maquina, filaId=False): 
        entrada = {'Token': self.token,
            'ExecucaoId': self.execucaoId,
            'Workflow':workflow,
            'Tarefa': tarefa, 
        ...
        if filaId !=2: entrada['FilaId'] = filaId
    ...
    def enviarNotificacao(self, canal, destino, assunto, mensagem, confidencial=0): 
    ...
    def credencialObter(self, sistema):
        """Retorno:
        {"Autorizado": true,
        "Credencial": {
    ...
    def credencialAlterar(self, credencialId, expiraEm=False, parametro=False, valorAntigo=False, valorNovo=False, ativo=1):
    ...
    # controle de fila
    def filaInserir(self, referencia, parametrosEntrada, workflow=False, tarefa=False, inserirExecutando=False): 
    ...
    def filaInserirLote(self, lote, workflow=False, tarefa=False):
    ...
    def filaProximo(self, qtd=1):#retorna o proximo item da fila, voce pode alterar a quantidade para trazer mais de 1
        """Resultado:
        { "Autorizado": true,
        "Fila": [
            {
        ...
        try: 
            ...
            retorno = {'Autorizado': False, 'Mensagem': f'Falha na comunicação:{e}'}
            self.filaId = False
        
        return retorno

    def filaAtualizar(self, filaId=False, parametrosSaida={}, statusId=2, status='FilaOk', proximo=0, mensagem='Fila atualizada com sucesso'):
    ...
    def filaAtualizarLote(self, lote):
    ...
    def filaConsultar(self, criterio, ordenadoPor, limite=10, workflow=False, tarefa=False):
        """Retorno:
        {"Autorizado": true,
        "Fila": [
            {
    ...
    def botstoreRequisicaoGenerica(self, servico, parametrosEntrada, assincrono=False):
    ...
    def bsQuebraCaptcha(self, imagemCaptcha):
    ...
    def bsQuebraRecaptcha(self, googlekey, pageurl):
    ...
    def bsWord2Json(self, documento):
    ...
    def bsNotaFiscal(self, arquivo):
        if 'filename' not in arquivo: arquivo = json.dumps(self.formatar_arquivo(arquivo))
        parametrosEntrada = {'Arquivo': arquivo}
        return self.botstoreRequisicaoGenerica('ib_nf', parametrosEntrada)
    
    def bsRelatorio(self, relatorio, dataInicio, dataFim):
        ...
        return self.botstoreRequisicaoGenerica(relatorio, parametrosEntrada)
