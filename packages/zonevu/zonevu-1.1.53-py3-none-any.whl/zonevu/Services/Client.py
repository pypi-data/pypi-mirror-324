...
class Client:
    ...
    def __init__(self, endPoint: EndPoint, units: UnitsSystemEnum = UnitsSystemEnum.US):
        ...
        try:
            ...
            toml_path = script_path.parents[2] / 'pyproject.toml'
            with toml_path.open('rb') as f:
                pyproject_data = tomllib.load(f)
                self.version = pyproject_data['tool']['poetry']['version']
    ...
    def distance_units(self) -> str:
        return 'ft' if self._units_system == UnitsSystemEnum.US else 'meters'
