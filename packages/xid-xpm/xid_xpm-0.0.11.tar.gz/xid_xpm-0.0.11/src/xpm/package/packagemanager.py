...
class PackageManager:
	...
	def CreateRequirementsFile(thisInstance, requirementsFilePath: str) -> None:
		...
		thisInstance.__textlines.append("# Requirements.txt cannot contain Korean characters. (results in cp949 error)")
		thisInstance.__textlines.append("# Never use Korean characters.")

		# 저장.
		builtins.print("requirementsFilePath")
		with open(requirementsFilePath, mode = FILE_WRITETEXT, encoding = UTF8) as file:
			content = LINEFEED.join(thisInstance.__textlines)
			file.write(content)	