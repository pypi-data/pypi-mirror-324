...
def get_fred(data_sources: dict[str, str], start_date: str = "", end_date: str = "", api_key=None, force_fetch=False, verbose=False):
    ...
    try:
        ...
        for series_id in data_sources:
            ...
            cache_start_date = raw_data.index[0]
            cache_end_date = raw_data.index[-1]
            # Adjust data so that it's end of time period (example: quarterly 1/1/24 becmoes3/31/24)
            if data_freq == 'D':
                offset = pd.offsets.Day(0)
            elif data_freq == 'W':
                offset = pd.offsets.Week(0)
            elif data_freq == 'M':
                ...
                raise ValueError(f"Unsupported frequency: {data_freq}")
            ...
            new_data.transformations.append('reindex_end_period')
            # Set date range
            if not start_date:
                start_date = new_data.index[0]
            if not end_date:
                end_date = new_data.index[-1]
            new_data = new_data.truncate(start_date, end_date)
            # Save data to table
            ...
            metadata = {
                ...
                'end_date': cache_end_date.strftime('%Y-%m-%d')
            }
