...
def introduce_biomass(logger, db, model): 
    ...
    biomass_dict = {
        ...
        'cofactors': [
            'accoa_c', 
            'fad_c', 'nad_c', 'nadp_c', 
            'moco_c', 'thf_c', 'thmpp_c', 'pydx5p_c', 
            'mql8_c', 'ribflv_c', 'pnto__R_c', 
            # biotin
            # ubiquinone
            # vitamin B12 (reuteri)
        ],
        'membrane_wall': [
            ...
            # peptidoglycan ('uaaGgtlg5a_c' + 2 reazioni)
            # cardiolipin
            # phosphatidylglycerol
            # 1-lysyl phosphatidylglycerol (plantarum)
            # undecaprenol (plantarum)
            # teichoic acids
            # lipoteichoic acids
...
def tsiparser(args, logger): 
    ...
    logger.info(f"Features: [G: {G}, R: {R}, M: {M}, uM: {uM}, bP: {uM}, Biomass: {round(model.slim_optimize(), 3)}]")
