...
def rebuild(g, corr=0.0, gvar=_gvar.gvar):
    r"""  Rebuild ``g`` stripping correlations with variables not in ``g``.
...
def mean(g):
    r""" Extract means from :class:`gvar.GVar`\s in ``g``.
...
def fmt(g, ndecimal=None, sep='', format='{:#p}'):
    r""" Format :class:`gvar.GVar`\s in ``g``.
...
def sdev(g):
    r""" Extract standard deviations from :class:`gvar.GVar`\s in ``g``.
...
def deriv(g, x):
    r""" Compute partial derivatives of |GVar|\s in ``g`` w.r.t. primary |GVar|\s in ``x``.
...
def is_primary(g):
    r""" Determine whether or not the |GVar|\s in ``g`` are primary |GVar|\s.
...
def dependencies(g, all=False):
    r""" Collect primary |GVar|\s contributing to the covariances of |GVar|\s in ``g``.
...
def missing_dependencies(g):
    r""" ``True`` if ``len(gvar.dependencies(g))!=0``; ``False`` otherwise.
...
def uncorrelated(g1,g2):
    r""" Return ``True`` if |GVar|\s in ``g1`` uncorrelated with those in ``g2``.
...
def evalcorr(g):
    r""" Compute correlation matrix for elements of
    array/dictionary ``g``.
...
def correlate(g, corr, upper=False, lower=False, verify=False):
    r""" Add correlations to uncorrelated |GVar|\s in ``g``.
...
def evalcov_blocks_dense(g, compress=False, verify=True):
    r""" Evaluate covariance matrix for elements of ``g``.
...
@cython.initializedcheck(False) # memory views initialized?
def var(g):
    r""" Compute variances for elements of 
...
def evalcov(g, verify=True):
    r""" Compute covariance matrix for elements of 
    array/dictionary ``g``.
...
######
class GVarRef:
    r""" Placeholder for a |GVar|, used by :func:`gvar.remove_gvars`. 
...
def distribute_gvars(g, gvlist):
    r""" Distribute |GVar|\s from ``gvlist`` in structure g, replacing :class:`gvar.GVarRef`\s. 
...
def remove_gvars(g, gvlist):
    r""" Remove |GVar|\s from structure g, replacing them with :class:`gvar.GVarRef`\s and collecting them in ``gvlist``. 
...
def filter(g, f, *args, **kargs):
    r""" Filter |GVar|\s in ``g`` through function ``f``. 
...
def dumps(g, add_dependencies=False, **kargs):
    r""" Return a serialized representation of ``g``.
...
def loads(inputbytes, **kargs):
    r""" Read and return object serialized in ``inputbytes`` by :func:`gvar.dumps`.
...
def dump(g, outputfile=None, add_dependencies=False, **kargs):
    r""" Write a representation of ``g``  to file ``outputfile``.
...
def load(inputfile, method=None, **kargs):
    r""" Read and return object serialized in ``inputfile`` by :func:`gvar.dump`.
...
def gloads(inputstr):
    r""" Return |GVar|\s that are serialized in ``inputstr``.
...
def gload(inputfile, method=None, **kargs):
    r""" Read and return |GVar|\s that are serialized in ``inputfile``.
...
def _gdump(g, add_dependencies=False):
    r""" Repack ``g`` in dictionary that can be serialized. Used by :func:`gdump`.
    """
...
def _oldload1(inputfile, method=None, use_json=None):
    r""" Load and return serialized |GVar|\s from file ``inputfile``.
...
def disassemble(g):
    r""" Disassemble collection ``g`` of |GVar|\s.
...
def reassemble(data, smat cov=_gvar.gvar.cov):
    r""" Convert data from :func:`gvar.disassemble` back into |GVar|\s.
...
def fmt_errorbudget(
    ...
    r""" Tabulate error budget for ``outputs[ko]`` due to ``inputs[ki]``.
...
def bootstrap_iter(g, n=None, eps=None, svdcut=None):
    r""" Return iterator for bootstrap copies of ``g``.
...
def sample(g, eps=None, svdcut=1e-12, uniform=None, nbatch=None, mode='rbatch'):
    r""" Generate random sample(s) from distribution ``g``.
...
def gvar_from_sample(gs, mode='rbatch', stderr=False, unbias=False):
    r""" Convert samples gs from Gaussian distribution into |GVar|\s.
...
def raniter(g, n=None, eps=None, svdcut=1e-12, uniform=None, nbatch=None, mode='rbatch'):
    r""" Return iterator for random samples from distribution ``g``
...
class SVD(object):
    r""" SVD decomposition of a pos. semi-def. sym. matrix.
    ...
    def decomp(self,n=1):
        r""" Vector decomposition of input matrix raised to power ``n``.
...
def valder(v):
    r""" Convert array ``v`` of numbers into an array of |GVar|\s.
...
cdef double gammaP_ser(double a, double x, double rtol, int itmax):
    r""" Power series expansion for P(a, x) (for x < a+1).
...
cdef double gammaQ_cf(double a, double x, double rtol, int itmax):
    r""" Continuing fraction expansion for Q(a, x) (for x > a+1).
...
def gammaQ(double a, double x, double rtol=1e-5, int itmax=10000):
    r""" Complement of normalized incomplete gamma function, Q(a,x).
...
def gammaP(double a, double x, double rtol=1e-5, itmax=10000):
    r""" Normalized incomplete gamma function, P(a,x).
...
def regulate(g, eps=None, svdcut=None, wgts=False, noise=False):
    r""" Regulate singularities in the correlation matrix of the |GVar|\s in ``g``.
