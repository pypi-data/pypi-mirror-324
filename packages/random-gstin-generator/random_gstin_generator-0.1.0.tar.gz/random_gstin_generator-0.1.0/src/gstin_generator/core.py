...
def generate_gstin(num_gstins:int=1) -> list[str]:
    """
    Generates a random valid GSTIN number.
    Replace this logic with your actual GSTIN generation logic.
    """
    def generate_pan():
            """Generates a random PAN card ID."""
            first_letter = random.choice(['A', 'B', 'C', 'F', 'G', 'H', 'L', 'J', 'P', 'T'])
            letters = ''.join(random.choices(string.ascii_uppercase, k=4))
            digits = ''.join(random.choices(string.digits, k=4))
            last_letter = random.choice(string.ascii_uppercase)
            pan=f"{first_letter}{letters}{digits}{last_letter}"
            # print(f"len of pan is: {len(pan)}")
            return pan
        
    def calculate_checksum(gst):
        """Calculates the checksum character for the GSTIN."""
    ...
    def generate_random_entity_code():
        """Generates a random entity code that can be a digit (0-9) or a letter (A-Z)."""
    ...
    def generate_valid_gstin():
        """Generates a valid GSTIN based on state code, PAN, and entity code."""
