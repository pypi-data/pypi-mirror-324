...
def create_executable_command(app_name, app_path):
    """Crée un fichier .cmd exécutable pour lancer l'application et l'ajoute au PATH."""
    ...
    # Ajoute le dossier utilisateur dans le PATH si ce n'est pas déjà fait
    current_path = os.environ.get("PATH", "")
    user_profile = os.environ["USERPROFILE"]
    if user_profile not in current_path:
        subprocess.run(f'setx PATH "%PATH%;{user_profile}"', shell=True, check=True)
        logging.info(f"🔄 Chemin ajouté au PATH : {user_profile}")
        print(f"🔄 Chemin ajouté au PATH : {user_profile}")

def show_help():
    """Affiche l'aide d'utilisation de la commande autolaunch."""
    help_text = """
    AutoLaunch - Gestion des applications Python exécutables

    Commandes disponibles :
    -------------------------
    autolaunch --help           Affiche cette aide
    autolaunch --register       Enregistre l'application actuelle
    autolaunch --unregister     Supprime l'enregistrement de l'application
    """
    print(help_text)
...
def ensure_registered():
    ...
    # Crée une commande exécutable et ajoute au PATH
...
def main():
    """Point d'entrée pour l'exécutable autolaunch."""
    if "--help" in sys.argv:
        show_help()
