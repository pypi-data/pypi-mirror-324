...
from cycurl.requests.exceptions import SessionClosed, Timeout
...
if TYPE_CHECKING:
    ...
    from cycurl.requests.session import AsyncSession, ProxySpec

    T = TypeVar("T")

    ON_DATA_T = Callable[["WebSocket", bytes, WSFrame], None]
...
class WebSocket(BaseWebSocket):
        ...
        while True:
            try:
                ...
                if rlist:
                    ...
                    flags = frame.flags
                    chunks.append(chunk)
                    if frame.bytesleft == 0 and flags & m.CURLWS_CONT == 0:
                        break
            except m.CurlError as e:
    ...
    def send(self, payload: Union[str, bytes], flags: int = m.CURLWS_BINARY):
        """Send a data frame.
    ...
    def send_str(self, payload: str):
        ...
        """
        return self.send(payload, m.CURLWS_TEXT)
...
class AsyncWebSocket(BaseWebSocket):
    ...
    async def send_str(self, payload: str):
        ...
        Args:
            payload: text data to send.
        """
        return await self.send(payload, m.CURLWS_TEXT)
    ...
    async def ping(self, payload: Union[str, bytes]):
        ...
        Args:
            payload: data to send.
        """
        return await self.send(payload, m.CURLWS_PING)
    ...
    async def close(self, code: int = WsCloseCode.OK, message: bytes = b""):
        ...
        # But this is not a requirement
        msg = self._pack_close_frame(code, message)
        await self.send(msg, m.CURLWS_CLOSE)
        # The only way to close the connection appears to be curl_easy_cleanup
        self.terminate()
