...
class ExtraFingerprints:
    tls_min_version: int = m.CURL_SSLVERSION_TLSv1_2
...
TLS_VERSION_MAP = {
    0x0301: m.CURL_SSLVERSION_TLSv1_0,  # 769
    0x0302: m.CURL_SSLVERSION_TLSv1_1,  # 770
...
def toggle_extension(curl, extension_id: int, enable: bool):
    ...
    if extension_id == 65037:
        if enable:
            curl.setopt(m.CURLOPT_ECH, "GREASE")
        else:
            curl.setopt(m.CURLOPT_ECH, "")
    ...
    # status_request
        ...
        if enable:
            curl.setopt(m.CURLOPT_TLS_STATUS_REQUEST, 1)
    # signed_certificate_timestampscd ..
    elif extension_id == 18:
        if enable:
            curl.setopt(m.CURLOPT_TLS_SIGNED_CERT_TIMESTAMPS, 1)
    # session_ticket
    elif extension_id == 35:
        if enable:
            curl.setopt(m.CURLOPT_SSL_ENABLE_TICKET, 1)
        else:
            curl.setopt(m.CURLOPT_SSL_ENABLE_TICKET, 0)
    # padding
