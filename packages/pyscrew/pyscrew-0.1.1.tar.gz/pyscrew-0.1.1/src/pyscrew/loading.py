...
class DataLoader:
    ...
    def __init__(self, scenario_name: str, cache_dir: Optional[Union[str, Path]] = None):
        """
    ...
    def _download_file(self, force: bool = False) -> Path:
        ...
        try:
            ...
            if archive_path.exists():
                archive_path.unlink()
            raise DownloadError(f"Failed to download {self.file_name}: {str(e)}")
        except Exception as e:
    ...
    def _verify_archive(self, archive_path: Path, archive_format: ArchiveFormat) -> bool:
        """
        ...
        try:
            ...
            raise ExtractionError(f"Archive verification failed: {str(e)}")
    ...
    def _set_secure_attributes(self, member: Union[tarfile.TarInfo, zipfile.ZipInfo]) -> None:
        """
        ...
        with zipfile.ZipFile(archive_path, 'r') as zip:
            ...
            for zip_info in zip.filelist:
                if not self._check_path_traversal(zip_info.filename):
                    raise SecurityError(f"Path traversal attempt detected: {zip_info.filename}")
    ...
    def _extract_archive(self, archive_path: Path, archive_format: ArchiveFormat, extract_to: Path) -> None:
        """
