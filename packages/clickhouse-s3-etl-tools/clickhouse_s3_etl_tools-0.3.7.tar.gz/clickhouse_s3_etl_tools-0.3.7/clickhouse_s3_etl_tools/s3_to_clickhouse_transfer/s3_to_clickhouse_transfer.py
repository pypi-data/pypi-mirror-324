...
def create_table_if_not_exists(
    ...
    if not conn.check_if_table_exists(
        ...
        if (metadata.engine_full or "") != "" and (config.ENGINE_FULL or "" != ""):
            engine_full_to_replace: Tuple[str, str] = metadata.engine_full, config.ENGINE_FULL
        else:
            engine_full_to_replace = None

        create_table_query = update_create_table_query(
            metadata.create_table_query, config.table, config.CLUSTER_NAME, config.DATABASES_MAP, engine_full_to_replace
        )
...
def fetch_metadata_from_s3(
    ...
    query: str = SQL_FETCH_METADATA.format(
        columns_list=METADATA_COLUMNS_LIST_V2,
        s3_source=s3_source
    )
...
def transfer_s3_to_clickhouse(config: Configuration):
    ...
    with (
        ...
        if metadata.engine.lower() == "log":
            metadata.total_rows = conn.get_table_number_rows(config.table.DATABASE_DESTINATION, config.table.TABLE_NAME)
