...
_hidden_results: list["Result[Any, Any]"] = []
_is_hiding = False
...
class Result(Generic[T, E]):
    ...
    This class provides a way to handle operations that might fail, inspired by Rust's Result type.
    ...
    Attributes:
        _hidden_results (list[Result[Any, E]]): Internal list that collects all Result instances when hiding is active
        _is_hiding (bool): Flag indicating if result collection is active
    ...
    def __init__(self, value: Union[T, E], is_ok: bool):
        ...
        self._is_ok = is_ok
        if _is_hiding and isinstance(self, Result):
            _hidden_results.append(self)
    ...
    def hide(cls) -> None:
        ...
        Example:
            ...
            >>> final_result = Result.reveal()
        """
        global _is_hiding, _hidden_results
        _is_hiding = True
        _hidden_results = []
    ...
    def all(results: list["Result[Any, E]"]) -> "Result[list[Any], E]":
        ...
        Returns:
            Result[list[Any], E]: A success Result containing a list of all success values if all
                                Results were successful, or the first encountered error Result.
