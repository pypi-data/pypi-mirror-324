...
def main() -> None:
    ...
    if debug == "1":
        ConfigContext.config_path = "./wdw/wdw.toml"
        ConfigContext.db_path = "./wdw/wdw.db"
...
def server_start(
    ...
    if ssl_:
        ...
        server = pywsgi.WSGIServer(
            ("0.0.0.0", port),
            application=listener.wdw_service,
            ssl_context=ssl_context,
        )
        print(f"-Web Server starts in https://0.0.0.0:{port}{url}")
        server.serve_forever()
    else:
        server = pywsgi.WSGIServer(("0.0.0.0", port), listener.wdw_service)
