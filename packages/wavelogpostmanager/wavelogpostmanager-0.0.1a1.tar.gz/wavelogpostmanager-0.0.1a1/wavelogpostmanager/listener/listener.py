...
class Listener:
    def __init__(self, config_context: ConfigContext, mysql_context: MySqlContext):
        ...
        self.wdw_service = Flask(__name__)
        self.wdw_service.config["CACHE_TYPE"] = "SimpleCache"
        self.wdw_service.config["CACHE_DEFAULT_TIMEOUT"] = ConfigContext.config[
            "web_service"
        ]["cache_time"]
        template_path = files("templates")
        self.wdw_service.template_folder = template_path.joinpath("")
        cache = Cache(self.wdw_service)
        # self.max_access_per_minute = ConfigContext.config["web_service"]["limit"]
        #
        # @self.wdw_service.before_request
        # def limit_access():
        ...
        @self.wdw_service.route(self.url)
        @cache.cached()
        ...
        @self.wdw_service.route(f"{self.url}/sent")
        @cache.cached()
        def service_sent():
            if request.method == "POST":
                return None
        ...
        @self.wdw_service.route(f"{self.url}/{self.signoff}", methods=["GET"])
        def sign_off():
            if request.method == "POST":
                return None
            ...
            if callsign is None:
                ...
                return render_template("index3.html", callsign=callsign.upper())

        @self.wdw_service.route("/", methods=["GET"])
        def root():
            return "Wrong"

        @self.wdw_service.route(f"{self.url}/{self.api_url}/test", methods=["POST"])
        def test_connection():
            data = request.get_json()
            return_json = json.dumps(Server.request_handler_test(data))
            return return_json

        @self.wdw_service.route(f"{self.url}/{self.api_url}/request", methods=["POST"])
