...
async def task_list(args) -> None:
    ...
    def human_readable_timestamp(datetime):
        now = datetime.now()
    ...
    earliest_completed_task_timestamp_or_now = datetime.now()
    for task in tasks.tasks:
        table_data.append(
            [
                ...
                human_readable_timestamp(task.occurred_at.ToDatetime()),
                human_readable_timestamp(task.scheduled_at.ToDatetime())
                if task.status in [
                ...
                f'>={task.num_runs_failed_recently}',
            ]
        )
