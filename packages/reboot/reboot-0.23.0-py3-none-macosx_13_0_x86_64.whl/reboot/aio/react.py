            ...
            try:
                application_id, state_ref = (
                    ...
                    StateRef.from_maybe_readable(
                        websocket.request_headers[STATE_REF_HEADER]
                    ),
                )
                ...
                if state_type_name is None:
                    log_at_most_once_per(
                        ...
                        message="A client attempted to perform a 'React' "
                        f"{'query' if 'React/Query' in websocket.path else 'mutation'} "
                        f"on an unknown state type '{state_ref.state_type}'; "
                ...
                if 'React/Query' in websocket.path:
                ...
                responseType = (
                    react_pb2.QueryResponse if 'React/Query' in websocket.path
                    else react_pb2.MutateResponse
                )
        ...
        try:
            ...
            if state_type_name is None:
                log_at_most_once_per(
                    seconds=60,
                    log_method=logger.error,
                    message="A client attempted to perform a 'React' query "
                    f"on an unknown state type '{state_ref.state_type}'. "
                    "Typical reasons for this are:\n"
                    "  * is there a servicer missing in the `Application`?\n"
                    "  * does the frontend code need to be regenerated, "
                    "possibly because the package or name of the state type "
                    "changed?\n"
                    "  * is there an old browser tab still open, running old frontend code?\n"
                    "  (This message will only be logged once per minute)",
                )
            ...
            if authoritative_consensus != middleware.consensus_id:
                ...
                await grpc_context.abort(
                    ...
                    f"Consensus '{middleware.consensus_id}' is not "
                    "authoritative for this request; consensus "
