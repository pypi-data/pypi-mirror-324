...
from kubernetes_utils.resources.deployments import VolumeMount
...
class ConsensusManager:
    ...
    async def set_consensuses(
        ...
        result_or_exception_list = await asyncio.gather(
            *set_consensus_tasks,
            return_exceptions=True,
        )

        # We expect all results to be the same type, either a result or an
        # exception.
        if len(result_or_exception_list) != 0:
            for value in result_or_exception_list[1:]:
                assert isinstance(value, type(result_or_exception_list[0]))
                assert str(value) == str(result_or_exception_list[0])

            # If there was an exception, raise it.
            if result_or_exception_list[0] is not None:
                raise result_or_exception_list[0]
