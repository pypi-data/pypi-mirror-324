...
class SigHUPException(BaseException):
    """Raised for SIGHUP.

    When SIGHUP has occurred, the terminal is dead, and so this exception should usually
    not be caught.
    """
...
class Subprocesses:
    ...
    def install_terminal_app_signal_handlers() -> None:
        ...
        def raise_for_signal(signum, *args) -> None:
            raise SigHUPException(f"Killed by {signum=}, {args=}")
        ...
        previous = signal.signal(signal.SIGHUP, raise_for_signal)
