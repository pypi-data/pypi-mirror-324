...
class AdvancedFootprintScanner:
    ...
    def search_google(self, query):
        try:
            ...
            for a in soup.find_all("h3"):
                ...
                link = a.find_parent("a")['href']
                snippet = a.find_next("span").get_text(strip=True) if a.find_next("span") else ""
                results.append({'title': title, 'link': link, 'snippet': snippet})
            
            return results[:3]  # Devolvemos solo los 3 primeros
        except Exception as e:
            print(f"Error: {e}")
            return []
    ...
    def search_public_documents(self, term):
        ...
        for query in queries:
            found_results = self.search_duckduckgo(query)
