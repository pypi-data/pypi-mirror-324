from typing import List, Dict, Union, Any, Set, Optional, Callable
...
class Workflow:
    ...
    def add_task(
        self,
        task: Task,
        argument_mappings: Dict[str, Union[str, Any]],
        result_mapping: Union[str, tuple[str, str]]
    ) -> None:
        """
        ...
        """
        # Convert argument_mappings to ArgumentMapping objects
        wrapped_mappings = {
            name: ArgumentMapping(value)
            for name, value in argument_mappings.items()
        }
        
        # Convert result_mapping to ResultMapping object
        if isinstance(result_mapping, tuple):
            context_key, result_path = result_mapping
            result_mapping = ResultMapping(context_key, result_path)
        else:
            result_mapping = ResultMapping(result_mapping)
            
        task_node = TaskNode(task, wrapped_mappings, result_mapping)
    ...
    async def execute(self) -> Context:
        ...
        """
        # Only build dependency graph if not already validated
        ...
        while len(completed_tasks) < len(self.tasks):
            ...
            if not ready_tasks and not tasks_in_progress:
                remaining_tasks = {t.task.name for t in self.tasks} - completed_tasks
                raise RuntimeError(f"Unable to make progress. Tasks stuck: {remaining_tasks}")
            ...
            if running_tasks:
                ...
                for task in done:
                    try:
                        ...
                        # If any task fails, propagate the error
