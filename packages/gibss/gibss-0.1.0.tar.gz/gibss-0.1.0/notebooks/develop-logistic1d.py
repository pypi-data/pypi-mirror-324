...
fit = fit_logistic_susie2(X, y, L=10, prior_variance=1.)

# %%time
from gibss.logistic import fit_logistic_susie
fit = fit_logistic_susie(X, y, L=10)

fit.sers.fits.state.x.shape

# +
from gibss.logistic import logistic_ser_hermite, logistic_ser_hermite_grid, logistic_ser_hermite_init, make_fixed_fitfun
from gibss.additive import make_fitfun, fit_additive_model
...
fitfun = make_fitfun(X, y, logistic_ser_hermite, Partial(logistic_ser_hermite_init, warm=True), serkwargs)
# -

# %%time
fitfuns = [fixedfitfun] + [fitfun for _ in range(10)]
model = fit_additive_model(fitfuns, maxiter=10, tol=1e-3)
model.psi[0].block_until_ready()


def make_gridfitfun(grid):
    gridkwargs = dict(
...
# %%time
from gibss.additive import update_additive_model
base_grid = np.array([2., np.sqrt(2), 1., 1/np.sqrt(2), 1/2])
gridfitfuns = [make_gridfitfun(base_grid * x.prior_variance) for x in model.components[1:]]
fitfuns2 = [fixedfitfun] + gridfitfuns
model.fit_functions = fitfuns2
model = update_additive_model(model, maxiter=1, tol=1e-3)
model.components[1].prior_variance.block_until_ready()
[x.prior_variance for x in model.components[1:]]
...
gridkwargs = dict(
    prior_variance_grid = np.array([1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.01, 0.001]),
    newtonkwargs=dict(tol=1e-2, maxiter=20, alpha=0.2, gamma=-0.1)
)
gridfitfun = make_fitfun(
    X, y, 
    logistic_ser_hermite_grid, 
    Partial(logistic_ser_hermite_init, warm=False),
    gridkwargs
...
# %%time
grid1 = gridfitfun(model.psi - model.components[3].psi, model.components[3])
grid1.prior_variance.block_until_ready()

from jax.scipy.special import logsumexp
logsumexp(grid1.fits.lbf, 0)

grid1.fits.state.x[0, :, 0]
...
# %%time
fit = fit_logistic_susie(X, y, warm=True, L=10, maxiter=20, tol=1e-4, serkwargs=serkwargs, keep_intermediate=False)

fit2 = fit_logistic_susie(X, y, method='hermite_eb', warm=False, L=10, maxiter=20, tol=1e-4, serkwargs=serkwargs, keep_intermediate=False)

# +
from gibss.logistic import logistic_ser_hermite_grid, logistic_ser_hermite_init
from jax.tree_util import Partial
...
fitfuneb = make_fitfun(X, y, logistic_ser_hermite_grid, Partial(logistic_ser_hermite_init, warm=False), defaultserkwargs)
# -

# %%time
serfit_eb = fitfuneb(-3., None)
serfit_eb.lbf_ser.block_until_ready()

from gibss.logistic import summarize_susie
...
sers = tree_stack(fit1.components[1:])
state = fit1.state
fit = SusieFit(fixed_effect, sers, state)
