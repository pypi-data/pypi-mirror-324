...
# +
from gibss.conditional_normal_means import covariate_moderated_normal_mixture_logistic_susie_sparse as cmnmls
...
betahat = np.random.normal(size=n) + 2*z # (z + 1)

sigma_grid = np.array([0., 1., 2., 4.])
fit = cmnmls(betahat, np.ones_like(betahat), X_sp, L=5, sigma_grid=sigma_grid, maxiter=1000)
# -

import matplotlib.pyplot as plt
plt.hist(betahat[z==0], density=True);
plt.hist(betahat[z==1], density=True)

fit.sers.fit.lbf_ser

# +
from scipy.stats import norm
from scipy.special import logsumexp
from gibss.conditional_normal_means import em_g_normal_mixture
import numpy as np

k=1
sigma_grid = np.cumprod(np.ones(12)*np.sqrt(2)) / 4
sigma_grid = np.array([0., 1., 2., 4., 8, 16])
shat = np.ones_like(betahat)

pi, ell, component_log_marginal_g, log_marginals = em_g_normal_mixture(betahat, shat, sigma_grid, maxiter=5000)
pi1 = pi[k:]/np.sum(pi[k:])
L1 = logsumexp(component_log_marginal_g[:, k:] + np.log(pi1)[None], 1)
L = np.array([norm.logpdf(betahat, 0.0, shat), L1]).T
# -

(L[:, 0][z==0] < L[:, 1][z==0]).mean()

#plt.scatter(L[:,0][z==1], L[:,1][z==1]);
plt.scatter(L[:,0][z==0], L[:,1][z==0]);
plt.plot(L[:,0][z==0], L[:,0][z==0]);


L = np.array([norm.logpdf(betahat, 0.0, shat), norm.logpdf(betahat, 0.0, np.sqrt(4))]).T
print((L[:, 0][z==0] < L[:, 1][z==0]).mean())
plt.scatter(L[:,0][z==1], L[:,1][z==1]);
plt.scatter(L[:,0][z==0], L[:,1][z==0]);
...
def em_g_normal_mixture(beta, s2, sigma_grid, pi_init=None, maxiter=1000):
