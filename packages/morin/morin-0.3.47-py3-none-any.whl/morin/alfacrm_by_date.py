...
class ALFAbyDate:
    ...
    def get_customer(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/customer/index', {'date_to': date},{'date_from': date}, {'removed': 1},{'is_study': 2})
    ...
    def get_lesson(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/lesson/index', {'date_to': date},{'date_from': date})
