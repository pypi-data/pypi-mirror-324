...
class VispConstantsDb:
    ...
    NON_DARK_TASK_READOUT_EXP_TIMES: tuple[float, ...] = (200.0, 2.0, 0.02)
...
def write_intermediate_background_to_task(
    ...
    for beam in [1, 2]:
        task.intermediate_frame_helpers_write_arrays(
            arrays=bg_array,
            beam=beam,
            task_tag=VispTag.task_background(),
        )
...
def write_intermediate_lamp_to_task(
    ...
    task.intermediate_frame_helpers_write_arrays(
        ...
        beam=beam,
        modstate=modstate,
        task_tag=VispTag.task_lamp_gain(),
    )
...
def write_intermediate_solar_to_task(
    ...
    task.intermediate_frame_helpers_write_arrays(
        ...
        beam=beam,
        modstate=modstate,
        task_tag=VispTag.task_solar_gain(),
    )
...
def write_intermediate_geometric_to_task(
    ...
    *,
    ...
    for beam in [1, 2]:
        task.intermediate_frame_helpers_write_arrays(
            arrays=angles[beam - 1], beam=beam, task_tag=VispTag.task_geometric_angle()
        )
        task.intermediate_frame_helpers_write_arrays(
            arrays=shifts[beam - 1], beam=beam, task_tag=VispTag.task_geometric_sepectral_shifts()
        )
        for modstate in range(1, num_modstates + 1):
            task.intermediate_frame_helpers_write_arrays(
                arrays=offsets[beam - 1, modstate - 1],
                beam=beam,
                modstate=modstate,
                task_tag=VispTag.task_geometric_offset(),
            )
