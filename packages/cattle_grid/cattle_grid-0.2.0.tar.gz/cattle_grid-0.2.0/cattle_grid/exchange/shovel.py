...
from cattle_grid.account.router.exchange import device_exchange
...
async def shovel_to_account_exchange(
    ...
    await broker.publish(
        ...
        routing_key=f"receive.{username}.{event_type.value}",
...
async def incoming_shovel(
    ...
    username = actor_for_account.account.name
    to_shovel = await transformer({"raw": msg.data})
    activity_type = msg.data.get("type")

    await shovel_to_account_exchange(
        msg.actor, username, EventType.incoming, to_shovel, broker, correlation_id
    )
...
async def outgoing_shovel(
    ...
    await shovel_to_account_exchange(
        msg.actor, username, EventType.outgoing, to_shovel, broker, correlation_id
    )
