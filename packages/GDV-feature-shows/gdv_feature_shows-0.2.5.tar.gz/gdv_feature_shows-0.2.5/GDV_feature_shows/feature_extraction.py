...
class FeatureExtractor:
    ...
    @staticmethod
    def calc_hash_of_petal(petal: list) -> int:
        hash1 = 0
        for i in range(len(petal)):
            hash1 += petal[i][0] * 1000000 + petal[i][1]
        return int(hash1)

    @staticmethod
    def find_petals(kimg: KImage, xy_c: tuple | None = None) -> list:
        assert kimg.count_channels() == 1
        fe = FeatureExtractor()
        ...
        if xy_c is None:
            ...
            how_much = 0
            res_segs_buff = []
            y_c, x_c = xy_c[1], xy_c[0]
            for seg_i in res_segs:
                f = False
                for pixel in seg_i:
                    y, x = pixel
                    if y == y_c and x == x_c:
                        ...
                        break
                if not f:
                    res_segs_buff.append(seg_i)
            res_segs = res_segs_buff
            assert how_much == 1
            return res_segs
