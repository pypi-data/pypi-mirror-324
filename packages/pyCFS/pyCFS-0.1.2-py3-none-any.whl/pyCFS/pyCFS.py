...
EXPORT_MATRICES_TOKEN = "</sequenceStep>"
VEC_TOKEN = "vec"
SOL_TOKEN = "sol"
RHS_TOKEN = "rhs"
MAT_TOKEN = "sys"
        ...
        <system>
            <solutionStrategy>
                <standard>
                    ...
                    <exportLinSys format="matrix-market" system="true" rhs="true" baseName="SIM_NAME" />
...
class pyCFS:
    ...
    def _init_paths(self) -> None:
        ...
        self.hdf_res_path = f"{self.proj_root_path}/{RESULTS_HDF_DIR}/{self.project_name}/"
        self.hdf_file_path = f"{self.proj_root_path}/{RESULTS_HDF_DIR}/{self.project_name}.{CFS_EXT}"
        self.logs_path = f"{self.proj_root_path}/{LOGS_DIR}/"
        self.history_path = f"{self.proj_root_path}/{HISTORY_DIR}/"
        self.data_dump_path = f"{self.proj_root_path}/{DATA_DUMP_DIR}/"
        self.sa_storage_path = f"{self.proj_root_path}/{SA_STORAGE_DIR}/"
    ...
    def _init_file_names(self) -> None:
        ...
        """
        self.cfs_file_init = (
            f"{self.proj_root_path}/{self.templates_dir}/{self.project_name}_{self.init_file_extension}.xml"
        )
        self.mat_file_init = (
            f"{self.proj_root_path}/{self.templates_dir}/{self.mat_file_name}_{self.init_file_extension}.xml"
        )
        self.jou_file_init = (
            f"{self.proj_root_path}/{self.templates_dir}/{self.project_name}_{self.init_file_extension}.jou"
        )

        self.sim_file_noext = f"{self.proj_root_path}/{self.project_name}"
        self.cfs_file = f"{self.proj_root_path}/{self.project_name}.xml"
        self.cfs_info_file = f"{self.proj_root_path}/{self.project_name}.info.xml"
        self.jou_file = f"{self.proj_root_path}/{self.project_name}.jou"
        self.mat_file = f"{self.proj_root_path}/{self.mat_file_name}.xml"
        self.cdb_file = f"{self.proj_root_path}/{self.project_name}.cdb"
        self.sim_files = [
            self.cfs_file,
            self.jou_file,
            self.mat_file,
        ...
        self.cfs_exports_names = {
            SOL_TOKEN: f"{self.proj_root_path}/{self.project_name}_{SOL_TOKEN}.{VEC_TOKEN}",
            RHS_TOKEN: f"{self.proj_root_path}/{self.project_name}_{RHS_TOKEN}.{VEC_TOKEN}",
            MAT_TOKEN: f"{self.proj_root_path}/{self.project_name}_{MAT_TOKEN}_0_0.{MTX_TOKEN}",
        }
 ...
 *<coordinateFile *fileName=".*" *delimiter="," *commentCharacter="#"/>\n* *</sensorArray>'
    ...
    def _set_export_system_matrices(self, export_matrices, sim_name_id) -> None:
        if export_matrices:
            ...
            export_matrices_xml = EXPORT_MATRICES_TEMPLATE.replace(SIM_NAME_TOKEN, sim_name_id)

            self.templates[self.cfs_file_init] = self.templates[self.cfs_file_init].replace(
                EXPORT_MATRICES_TOKEN, export_matrices_xml
            )
    ...
    def _forward_serial(self: "pyCFS", X: pyCFSparamVec) -> None:
        ...
        for ind in tqdm(range(self.N)):
            ...
            self._handle_exported_matrices()
    ...
    def _replace_coords_in_str(coord_str: str, x, y, z) -> str:
        coord_str = coord_str.replace(X_COORD_SA, str(x))
        coord_str = coord_str.replace(Y_COORD_SA, str(y))
        coord_str = coord_str.replace(Z_COORD_SA, str(z))
        return coord_str
    ...
    def _fill_item_templates(item, item_name, item_template, sensor_locations) -> List[str]:
        ...
        for id_num in range(sensor_locations.shape[0]):
            id_str = f"_{id_num}"
            item = item_template.replace(ITEM, item)
            item = item.replace(ITEM_NAME, item_name + id_str)
    ...
    def _generate_items(result_name, item, sensor_locations) -> Tuple[List[str], List[str]]:
        ...
        item_template = pyCFS._get_template_for(ITEM_TEMPLATE, item)
        item_res_template = pyCFS._get_template_for(ITEM_RES_TEMPLATE, item)
    ...
    def _generate_sa_setup_dict(sensor_arrays: List[Dict[str, str]]) -> Dict[str, Dict[str, List[str]]]:
        ...
        for sa_id in range(len(sensor_arrays)):
            ...
            nodes, res_nodes = pyCFS._generate_items(result_name, NODE, sensor_locations)
            elems, res_elems = pyCFS._generate_items(result_name, ELEM, sensor_locations)
    ...
    def _find_all_sensorarray_inserts(xml_content) -> List[str]:
        return re.findall(RESLIST_PATTERN, xml_content)
    ...
    def _construct_result_list_for(sa_result: str, sa_setup_dict: Dict[str, Dict[str, List[str]]]) -> str:
        ...
        restype_full = RES_NODES if restype == NODE else RES_ELEMS
        resitems_str = "\n".join(
            list(chain.from_iterable(sa_setup_dict[sa_name][restype_full] for sa_name in split_sa_result[:-1]))
        )
        return pyCFS._get_template_for(LIST_TEMPLATE, restype).replace("INSERT", resitems_str)
    ...
    def _init_sensor_arrays(self) -> None:
        ...
        if not self.testing:
            ...
            nodes_list_def_str = "\n".join(
                list(chain.from_iterable(sa_setup[NODES] for _, sa_setup in self.sa_setup_dict.items()))
            )
            elems_list_def_str = "\n".join(
                list(chain.from_iterable(sa_setup[ELEMS] for _, sa_setup in self.sa_setup_dict.items()))
            )

            # generate list for nodes and elements :
            self.nodelist_str = self._get_template_for(LIST_TEMPLATE, NODE).replace("INSERT", nodes_list_def_str)
            self.elemlist_str = self._get_template_for(LIST_TEMPLATE, ELEM).replace("INSERT", elems_list_def_str)
    ...
    def _contruct_run_metadata(self, X: pyCFSparamVec) -> None:
        file_paths = glob(f"{self.proj_root_path}/**/*.csv", recursive=True) + glob(
            f"{self.proj_root_path}/**/*.py", recursive=True
        )
    ...
    def _save_hdf_results(self, ind: int = 0, is_parallel: bool = False) -> None:
        ...
        if is_parallel:
            source_path = f"{self.proj_root_path}/{RESULTS_HDF_DIR}/{self.project_name}_{ind}.{CFS_EXT}"
            shutil.move(source_path, dest_path)
        else:
            source_path = f"{self.proj_root_path}/{RESULTS_HDF_DIR}/{self.project_name}.{CFS_EXT}"
    ...
    def _read_cfs_export(file_name: str) -> resultVec:
        if file_name[-3:] == VEC_TOKEN:
            return np.loadtxt(file_name)
        else:
            return mmread(file_name)

    def _handle_exported_matrices(self) -> None:
        export_packet: resultDict = {
            SOL_TOKEN: self._read_cfs_export(self.cfs_exports_names[SOL_TOKEN]),
            RHS_TOKEN: self._read_cfs_export(self.cfs_exports_names[RHS_TOKEN]),
            MAT_TOKEN: self._read_cfs_export(self.cfs_exports_names[MAT_TOKEN]),
        }
...
must have shape (N x num_params) where N is the number of parameter sets."
    ...
    def _clean_sim_files_parallel(self) -> None:
        ...
        wildcards = [
            f"{self.proj_root_path}/{self.project_name}*.{XML_EXT}",
            f"{self.proj_root_path}/{self.mat_file_name}*.{XML_EXT}",
            f"{self.proj_root_path}/{self.project_name}*.{JOU_EXT}",
        ]
        pyCFS._find_and_remove_files(wildcards)

    def _clean_hdf_results_parallel(self) -> None:
        wildcards = [f"{self.proj_root_path}/{RESULTS_HDF_DIR}/*.{CFS_EXT}"]
        pyCFS._find_and_remove_files(wildcards)
