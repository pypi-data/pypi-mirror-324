_J='jarsv1'
_I='git+https://github.com/awslabs/aws-glue-libs.git#egg=aws-glue-libs'
_H='aws-glue-libs'
_G='4.0.0'
_F='3.0'
_E='1.0'
...
DEFAULT_GLUE_VERSION=os.getenv('GLUE_DEFAULT_VERSION','').strip()or _B
GLUE_VERSIONS=[_D,_E,_B,_F,_C,_A]
GLUE_SPARK_MAPPING={_D:'2.2.1',_E:'2.4.3',_B:'2.4.3',_F:'3.1.1',_C:'3.3.0',_A:'3.5.2'}
...
AWS_GLUE_LIBS_EGG_URL={'0.9.0':'git+https://github.com/localstack/aws-glue-libs.git@glue-0.9#egg=aws-glue-libs','3.0.0':'git+https://github.com/awslabs/aws-glue-libs.git@glue-3.0#egg=aws-glue-libs',_G:_I,'5.0.0':_I}
...
class GlueInstaller(ArchiveDownloadAndExtractInstaller):
	...
	def _download_aws_glue_libs_jar(C,target):
		D=C._get_install_dir(target);A=os.path.join(D,_J)
	...
	def _patch_aws_glue_libs_config(E,target):
		A=E._get_install_dir(target);C=os.path.join(A,_J);B=os.path.join(A,'conf','spark-defaults.conf')
	...
	def install_aws_glue_libs_python(D):
		C='pip';B=next((A for A in AWS_GLUE_LIBS_EGG_URL if A.startswith(D.version)),_G);E=AWS_GLUE_LIBS_EGG_URL[B];LOG.debug("Expecting 'aws-glue-libs' version %s.",B)
		try:F=run([C,'show',_H]);A=re.search('Version:\\s+(.+)$',F,re.M).group(1).strip();LOG.debug("Determined 'aws-glue-libs' version %s.",A)
