...
class LLM:
    ...
    def sanitize_json_string(self,json_str: str) -> str:
        # First, let's properly escape any code snippets
        def escape_code_snippet(match):
            content = match.group(1)
            # Escape all special characters
            content = json.dumps(content)[1:-1]  # dumps but remove outer quotes
            return f'"{content}"'

        # Find all code snippets (content between quotes) and escape them
        pattern = r'"([^"\\]*(?:\\.[^"\\]*)*)"'
        processed_str = re.sub(pattern, escape_code_snippet, json_str)
        ...
        try:
            ...
            # If that failed, try an alternative approach
            try:
                # Try to parse it as a raw string
    ...
    def __call__(
        ...
        **kwargs
        ...
        if response_format is None:
            ...
            return response_format(**json.loads(res.choices[0].message.content))
