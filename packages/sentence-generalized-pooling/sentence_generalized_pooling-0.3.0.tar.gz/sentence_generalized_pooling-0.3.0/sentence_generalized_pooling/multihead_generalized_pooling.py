...
class MultiHeadGeneralizedPooling(nn.Module):
    ...
    def initialize_dot_product_pooling(self) -> None :
        """
        Initializes the dot product pooling when being used
        """
    ...
    def forward_dot_product(self, features: dict[str, torch.Tensor], **kwargs) -> dict   [str, torch.Tensor]:
        """
        Perform multi-head generalized pooling using dot-product attention.
        Args:
            features (dict[str, torch.Tensor]): A dictionary containing:
                - "attention_mask" (torch.Tensor): Attention mask of shape (batch_size, seq_len).
                - "token_embeddings" (torch.Tensor): Token embeddings of shape (batch_size, seq_len, token_dim).
            **kwargs: Additional keyword arguments.
        Returns:
            dict[str, torch.Tensor]: Updated features dictionary with an added key:
                - "sentence_embedding" (torch.Tensor): The pooled sentence embedding of shape (batch_size, sentence_dim).
        """
