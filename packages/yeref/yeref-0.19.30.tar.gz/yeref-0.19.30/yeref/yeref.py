...
from yeref.l_ import l_inline_demo, l_inline_bot, l_inline_post, l_inline_media, l_inline_channel, l_inline_group, \
    ...
    l_bot_need_start_add, l_chn_need_boost_for_story, l_refund_title, l_refund_already_done, l_refund_success, l_refund_incorrect
...
USER_VARS_ = '{"USER_TEXT": "", "USER_REACTION": "", "USER_PUSH": "", "USER_EMAIL": "", "USER_PROMO": "", "USER_PHONE": "", "USER_GEO": "", "USER_UTM": "", "USER_ID": 0, "USER_DT": "", "USER_TZ": "+00:00", "USER_LC": "en", "USER_ISADMIN": 0, "USER_ISPREMIUM": 0, "USER_BALL": 0, "USER_RAND": 0, "USER_QUIZ": 0, "USER_DICE": 0, "MSGID_PAID": 0, "DATE_TIME": 0}'
...
# region blocks
async def recursion_blocks(bot, chat_id, KEYS_JSON, app, tgr_cmd, BOT_TID_TMP, BOT_USERNAME, BASE_ENT, BASE_D, EXTRA_D, MEDIA_D, msg_lst, lc,
    ...
    try:
        ...
        msg_lst, next_items, clicks, messages, out_vid = await bypass_messages(bot, chat_id, KEYS_JSON, app, lc, BOT_USERNAME, BASE_ENT, BASE_D,
                ...
                for click in clicks.get(message.id, []):
                    ...
                    while True:
                        try:
                            ...
                            await recursion_blocks(bot, chat_id, KEYS_JSON, app, tgr_cmd, BOT_TID_TMP, BOT_USERNAME, BASE_ENT, BASE_D, EXTRA_D,
                            ...
                            if 'Request timed out' in str(e) and cnt >= 0:
                                ...
                                try:
                                    ...
                                    await recursion_blocks(bot, chat_id, KEYS_JSON, app, tgr_cmd, BOT_TID_TMP, BOT_USERNAME, BASE_ENT, BASE_D,
...
async def bypass_messages(bot, chat_id, KEYS_JSON, app, lc, BOT_USERNAME, BASE_ENT, BASE_D, EXTRA_D, MEDIA_D, msg_lst, next_items, clicks,
                          out_vid):
    messages = []
    try:
        ...
        for message in messages:
            try:
                ...
                if message.text:
                    ...
                    tmp_json = await facade_get_fid(bot, chat_id, KEYS_JSON, BOT_TID_TEST, dst, MSG_TYPE, IS_LINK, BASE_D, EXTRA_D, MEDIA_D)
                    MSG_MEDIA = [tmp_json]
...
async def train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, BASE_ENT, EXTRA_D, BASE_D, lz, prompt='',
    ...
    try:
        ...
        # region POST
        sql = f"SELECT POST_TEXT, POST_BUTTONS FROM BOT_{ENT_TID}.MSG"
        data = await db_select(sql, (), BASE_D)
...
async def return_file_id(BOT_TID, FILE_NAME, MSG_TYPE, IS_LINK, BASE_D, EXTRA_D, MEDIA_D):
    ...
    try:
        ...
        while cnt > 0:
            try:
                sql = "SELECT OWNER_TID, BOT_STATUS, BOT_TOKEN FROM \"BOT\" WHERE BOT_TID=$1"
...
async def jpg_video_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, MSG_TYPE, file_id, BASE_D, MEDIA_D):
    result = None
...
async def facade_get_fid(bot, chat_id, KEYS_JSON, BOT_TID, dst, msg_type, IS_LINK, BASE_D, EXTRA_D, MEDIA_D):
    ...
    try:
        ...
        if file_link.endswith('.mp4'):
            file_link2 = await jpg_video_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, msg_type, file_id, BASE_D, MEDIA_D)
        else:
