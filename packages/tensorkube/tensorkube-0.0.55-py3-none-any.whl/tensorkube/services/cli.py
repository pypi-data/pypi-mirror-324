...
def teardown():
    ...
    if not confirmation:
        ...
        secret_phrase = click.prompt(click.style("Please enter the secret phrase to confirm the teardown. Contact the founders for the phrase.", bold=True,fg=CliColors.ERROR.value), hide_input=True)
        if secret_phrase == 'tensorsamagam':
            click.echo("Tear down confirmed.")
        else:
            click.echo("Secret phrase is incorrect. Tear down aborted.")
            return
...
def deploy(name, gpus, gpu_type, cpu, memory, secret, max_scale, update, efs):
    ...
    pod_status = build_app(env=env, image_tag=image_tag, sanitised_project_name=sanitised_job_name, upload_to_nfs=efs,image_url=image_url, convert_to_nydus_image=convert_to_nydus_image)
    if gpus > 0:
        image_url = get_nydus_image_url(image_url=image_url)
    if efs:
        image_url = image_tag
