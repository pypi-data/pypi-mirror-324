...
def get(profile_name: str, reload: bool = False, default=None) -> dotdict:
    ...
    if profile_name == name():
        ...
        if _data is not None and not reload:
            return _data
        data = _load_data_file_with_env_defaults(profile_name, default)
        _data = data
    else:
        data = _load_data_file_with_env_defaults(profile_name, default)
    return data
...
def _load_data_file_with_env_defaults(profile_name: str, default: dict) -> dict:
    ...
    for key, value in os.environ.items():
        if key.startswith(env_prefix):
            ...
            v = deep_get_attr(data, profile_key, raise_on_not_found=False)
            if v is None or v == "":
