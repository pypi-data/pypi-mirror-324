...
default_catalog_path = str(
    Path(__file__).parent / default_locale / "LC_MESSAGES" / "messages.po"
)
...
manuals = [
    ("Salesforce.Event.Poultry_age__c", "Age des poules"),
...
for other_locale in Path(__file__).parent.iterdir():
    if (
        other_locale.name == default_locale
        or not other_locale.is_dir()
        or not "LC_MESSAGES" in os.listdir(other_locale)
    ):
        continue
    ...
    for entry in default_catalog:
        ...
        if not other_entry or not other_entry.msgstr:
            logger.info(
                f"Translating {entry.msgid} from {default_locale} to {other_locale}"
            )
            messages = [
                {
                    "role": "system",
