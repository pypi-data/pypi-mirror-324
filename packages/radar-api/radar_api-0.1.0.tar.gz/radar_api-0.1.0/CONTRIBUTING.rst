...
Hi! Thanks for taking the time to contribute to RADAR-API.

You can contribute in many ways:

- Join the `GitHub Discussions <https://github.com/ghiggi/radar_api/discussions>`__
...
- The section `Contributing process <#contributing-process>`__ provides you with a brief overview of the steps that each RADAR-API developer must follow to contribute to the repository.
- The `Code review checklist <#code-review-checklist>`__ enable to speed up the code review process.
- The `Code of conduct <https://github.com/ghiggi/radar_api/blob/main/CODE_OF_CONDUCT.md>`__ details the expected behavior of all contributors.
...
1. Start with a conversation
   ...
   Before start coding, open a `GitHub Discussion <https://github.com/ghiggi/radar_api/discussions>`__, a `GitHub Feature Request Issue <https://github.com/ghiggi/radar_api/issues/new/choose>`__ or
...
If you have any questions, please do not hesitate to ask in the `GitHub Discussions <https://github.com/ghiggi/radar_api/discussions>`__ or in the
...
You can access and use these templates by visiting the `GitHub Issue Templates page here <https://github.com/ghiggi/radar_api/issues/new/choose>`__.
...
In this section we explain the steps that each developer must follow to contribute to the RADAR-API repository.
...
to create the local copy of the RADAR-API repository, set up the developing environment and the pre-commit hooks.

Once you have have a local copy of the RADAR-API repository on your machine, you are ready to
...
They are the only ones who accept Pull Requests and push commits directly to the RADAR-API repository.
...
The *black* configuration used in the RADAR-API project is
defined in the `pyproject.toml <https://github.com/ghiggi/radar_api/blob/main/pyproject.toml>`__.
...
Currently, RADAR-API tests that the code to be committed complies with `black's  <https://github.com/psf/black>`__ format style,
...
The versions of the software used in the pre-commit hooks is specified in the `.pre-commit-config.yaml <https://github.com/ghiggi/radar_api/blob/main/.pre-commit-config.yaml>`__ file.
...
RADAR-API tests are written using the third-party `pytest <https://docs.pytest.org>`_ package.

The tests are organized within the ``/radar_api/tests`` directory and are structured to comprehensively assess various aspects of the code.
...
2. Navigate to the RADAR-API root directory.
...
.. code-block:: bash
    ...
    pytest radar_api/tests/<test_subdirectory>/

Run a particular test module:

.. code-block:: bash

    pytest radar_api/tests/<test_subdirectory>/test_<module_name>.py

These options provide flexibility, allowing you to efficiently target and validate specific components of the RADAR-API software.
...
Once your code has been uploaded into your RADAR-API fork, you can create a GitHub Pull Request (PR) to the RADAR-API main branch.
...
-  If adding a new feature:
   ...
   -  Provide a convincing reason to add the new feature. Ideally, propose your idea through a `Feature Request Issue <https://github.com/ghiggi/radar_api/issues/new/choose>`__ and obtain approval before starting work on it. Alternatively, you can present your ideas in the `GitHub Discussions <https://github.com/ghiggi/radar_api/discussions>`__ or in the `Open Radar Science Discourse Group <https://openradar.discourse.group/>`__.
...
Thank you to all the people who have already contributed to RADAR-API repository!

If you have contributed code or documentation to RADAR-API, add your name to the `AUTHORS.md <https://github.com/ghiggi/radar_api/blob/main/AUTHORS.md>`__ file.
