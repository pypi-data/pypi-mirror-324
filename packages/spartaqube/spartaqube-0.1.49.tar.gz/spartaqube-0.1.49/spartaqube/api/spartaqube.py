...
class Spartaqube:
    ...
    def __init__(self, api_key=None, port=None, asgi_port=None, silent=False, b_open_browser=False):
        '''
        ...
        if api_key is None: # Public user with local server
            ...
            if asgi_port is None: # Use default port
                asgi_port = spartaqube_install.generate_free_asgi_port(different_than=port)
            else: # Use the user's port input
                pass
        ...
        if b_start_local_server:
            spartaqube_install.entrypoint(port=port, asgi_port=asgi_port, silent=silent)
        ...
        def is_asgi_ready():
            try:
                app_status_dict = self.get_status_ws()
                # print("app_status_dict")
                # print(app_status_dict)
                if app_status_dict['res'] == 1:
                    is_asgi_running = app_status_dict['output'] == 1
            except:
                pass
            return is_asgi_running

        is_asgi_running = False
        if is_wsgi_running:
            is_asgi_running = is_asgi_ready()
    
        if is_wsgi_running and is_asgi_running:
            print_success_infos()
            ...
            if not is_asgi_running: # Here, wsgi is running but not asgi
                # retry_max = -1
                # while is_asgi_ready() and retry_max <= 5:
                #     asgi_port += 1
                #     spartaqube_install.start_asgi_server(asgi_port=asgi_port, silent=silent)
                # if retry_max == 5:
                error_msg = "ASGI port seems to be already used by another process. You can specify another port within Spartaqube(asgi_port=xxxx, ...)"
                self._initialized = False
                raise Exception(error_msg)
    ...
    def stop_server(self):
        ...
        spartaqube_install.stop_server_asgi()
