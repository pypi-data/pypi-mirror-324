...
from project.sparta_75433bcd57.sparta_e9edb68cbd.qube_dc67a122c8 import sparta_ed5944100c
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_56cd3acedd(json_data,user_obj):
...
def sparta_1215a5c3d0(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_4eb8551089(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_ed5944100c(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_ed5944100c(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_b1d9c7f1a3(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_ed5944100c(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_55193abb1c(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_1215a5c3d0(F,A.type_ticket,None)
...
def sparta_62b6a6e129(json_data,user_obj):
	...
	return{_A:1}
def sparta_5c979a1b1f(json_data):
	...
	return{_A:1}
def sparta_ea2bc41c99(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}