...
from project.sparta_1d1c5d5897.sparta_f2e5918cc6 import qube_56e2b853b1 as qube_56e2b853b1
from project.sparta_1d1c5d5897.sparta_b7dab16116.qube_4290d4bcb3 import Email as Email
def sparta_8197dd6d72(function):
	def A(request,*E,**D):
		A=request;B=_B
		...
		if not B:
			if C is not _G:F=qube_56e2b853b1.sparta_1e237374a7(C);login(A,F)
		else:0
		return function(A,*E,**D)
	return A
def sparta_2975e084e6(function):
	def A(request,*C,**D):
	...
	return A
def sparta_51069271d4(function):
	def A(request,*B,**C):
	...
	return A
def sparta_ca588e1a5a(function):
	C=function
	def A(request,*D,**E):
		A=request;F=_C
		try:
			G=json.loads(A.body);H=json.loads(G[_J]);I=H[_I];B=qube_56e2b853b1.sparta_1e237374a7(I)
...
def sparta_9fc767ed73():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_57053d7836(json_data):
	D='ip_addr';A=json_data;del A[_K];del A[_L]
	try:A[D]=sparta_9fc767ed73()
	except:A[D]=-1
...
def sparta_821d08cde7():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_06a2bd1dbe(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_d1b8917ba0(guest_code,user_obj):
	...
	return _C
def sparta_1e85c8122d(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_c7ce2fe034(email,captcha):
	D=sparta_d5b054f52a(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_1562644897(A.user,C);return{_A:1}
def sparta_1562644897(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_a4652fbf7f(captcha,token,email,password):
	D=sparta_d5b054f52a(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
