...
class IPythonKernel:
	...
	async def initialize_kernel(B):
		...
		A+='django.setup()\n';F=os.path.dirname(__file__);E=os.path.dirname(F);G=os.path.dirname(E);C=os.path.join(E,'api');C=sparta_d4809fae31(C);C=Path(C).resolve()
	...
	async def set_workspace_variable(A,name,value,websocket=_A):
