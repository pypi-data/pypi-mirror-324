...
def sparta_d4809fae31(path):A=Path(path).resolve();return str(A).replace('\\','\\\\')
def sparta_7a7331fb5f():
	B='virtual_env_name';C=hasattr(sys,'real_prefix')or hasattr(sys,'base_prefix')and sys.base_prefix!=sys.prefix
	if C:A=sys.prefix;D=os.path.basename(A);return{_A:True,B:D,_B:sparta_d4809fae31(A)}
	else:return{_A:False,B:None,_B:None}
def sparta_fba8ed200c(bat_file_path,shortcut_path,icon_path):
...
def sparta_1aee9d7ee7(shell_script_path,shortcut_name,icon_path):
...
def sparta_3b962d5fb6(shortcut_name,shell_script_path,icon_path):
...
def sparta_8a416ee6e3(json_data,user_obj):
	S='SpartaQube Launcher';R='SpartaQube.sh';Q='SpartaQube.lnk';L='errorMsg';F='res';M=qube_56e2b853b1.sparta_81ef7b3239(json_data,user_obj)
	if M[F]==1:
		T=M['token'];B=sparta_fd02dc10b1()['api'];A=sparta_d4809fae31(os.path.join(B,'spartaqube_exec.py'))
		if os.path.exists(A):
			try:os.remove(A)
...
"""
		...
		G=sparta_d4809fae31(os.path.join(B,'spartaqube_launcher.py'))
		if os.path.exists(G):
			try:os.remove(G)
			except Exception as D:return{F:-1,L:str(D)}
		I=sparta_7a7331fb5f();print('env_info');print(I)
		if I[_A]:E=I[_B];V=sparta_d4809fae31(os.path.join(f"{E}",'bin','python')if os.name!='nt'else os.path.join(f"{E}",'Scripts','python.exe'));N=sparta_d4809fae31(os.path.join(f"{E}",'bin/activate')if os.name!='nt'else os.path.join(E,'Scripts/activate.bat'));O=f'''
import os
import sys
import subprocess
import platform
...
'''
		...
		W=sparta_fd02dc10b1()['spartaqube_path'];J=sparta_d4809fae31(os.path.join(W,'static','assets','images','Icon','favicon512.ico'));P=platform.system()
		if P==_C:
			C=sparta_d4809fae31(os.path.join(B,'SpartaQube.bat'));K=sparta_d4809fae31(os.path.join(B,Q))
			if os.path.exists(K):
				try:os.remove(K)
				except Exception as D:return{F:-1,L:str(D)}
			sparta_fba8ed200c(C,K,J);X=os.path.join(os.path.expanduser('~'),'Desktop');Y=os.path.join(X,Q);sparta_fba8ed200c(C,Y,J)
		elif P=='Darwin':C=sparta_d4809fae31(os.path.join(B,R));sparta_1aee9d7ee7(shell_script_path=C,shortcut_name=S,icon_path=J)
		else:C=sparta_d4809fae31(os.path.join(B,R));sparta_3b962d5fb6(shell_script_path=C,shortcut_name=S,icon_path='/path/to/your/icon.png')
	return{F:1}