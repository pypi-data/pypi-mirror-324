...
_AU='xAxisDataArr'
_AT='widget_id'
_AS='code_editor_notebook_cells'
_AR='chart_config'
_AQ='chart_params'
_AP='plot_library'
_AO='has_write_rights'
_AN='chartConfigDict'
_AM='chartParams'
_AL='dataSourceArr'
_AK='typeChart'
...
_AH='Name desc'
_AG='Date desc'
_AF='shadedBackgroundArr'
_AE='radiusBubbleArr'
_AD='labelsArr'
_AC='labels'
_AB='column'
_AA='datasets'
_A9='Invalid password'
_A8='data_source_list'
_A7='date_created'
_A6='last_update'
_A5='is_static_data'
_A4='is_expose_widget'
_A3='bExposeAsWidget'
_A2='plotDes'
_A1='bStaticDataPlot'
_A0='codeEditorNotebookCells'
_z='split'
_y='query_filter'
_x='is_owner'
_w='is_public_widget'
...
from project.sparta_1d1c5d5897.sparta_e0f10cf586.qube_97c853e9c9 import convert_to_dataframe,convert_dataframe_to_json,sparta_d4809fae31
from project.sparta_1d1c5d5897.sparta_3863939d0b.qube_06437133ee import sparta_d5431c9725
INPUTS_KEYS=['xAxisArr','yAxisArr',_AD,_AE,'rangesAxisArr','measuresAxisArr','markersAxisArr','ohlcvArr',_AF]
def sparta_724081f15d(user_obj):from project.sparta_1d1c5d5897.sparta_f2e5918cc6.qube_56e2b853b1 import sparta_4841534c3e as A;return A(user_obj)
def sparta_fecce12a4b(user_obj):
	A=qube_b79a99bc7e.sparta_ffa29a5b8d(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_a55db36631(json_data,user_obj):
	D=user_obj;E=sparta_fecce12a4b(D)
	...
	if B.count()>0:
		...
		elif C==_AG:B=B.order_by('-db_connector__last_update')
		elif C=='Date asc':B=B.order_by('db_connector__last_update')
		elif C==_AH:B=B.order_by('-db_connector__name')
...
def sparta_79bc3b7d82():return{_A:1,'available_engines':qube_0aceed85a3.sparta_79bc3b7d82()}
def sparta_d2a0f84450(json_data,user_obj):
...
def sparta_56bde97bff(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_fecce12a4b(C)
...
def sparta_5e3bb0e5a4(json_data,user_obj):
	C=json_data[_H];A=sparta_a80f024295(C,user_obj)
...
def sparta_e3941d944c(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_a80f024295(H,user_obj)
...
def sparta_fa7d9202b5(json_data,db_connector_obj):
...
def sparta_225d60866e(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_a80f024295(G,user_obj)
	if B is _B:return{_A:-1,_E:_f}
	sparta_fa7d9202b5(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_y]
...
def sparta_9805b05021(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_a80f024295(G,user_obj)
	if B is _B:return{_A:-1,_E:_f}
	sparta_fa7d9202b5(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
	...
	J=E.describe();return{_A:1,_F:J.to_json(orient=_z)}
def sparta_6bd440e97f(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_af545e6ca2(json_data,user_obj):
	A=json_data;D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_a80f024295(D,user_obj)
	if B is _B:return{_A:-1,_E:_f}
	sparta_fa7d9202b5(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_F:convert_dataframe_to_json(E)}
def sparta_cc4210c5ae(json_data,user_obj):
	A=json_data;M=A[_M];D=_B
	if M:D=A[_s];D=qube_cb1850bb26.sparta_1103fa358a(D)
	N=A[_A0];O=str(uuid.uuid4());B=datetime.now().astimezone(UTC);P=CodeEditorNotebook.objects.create(notebook_id=O,cells=N,date_created=B,last_update=B);G=str(uuid.uuid4());H=A[_A1];I=A['is_gui_plot']
	if I:H=_C
	C=A['plotSlug']
	if len(C)==0:C=A[_t]
	J=slugify(C);C=J;K=1
	...
	if E is not _B:
		try:
			E=E.split(',')[1];Q=base64.b64decode(E);R=os.path.dirname(__file__);S=os.path.dirname(os.path.dirname(os.path.dirname(R)));L=os.path.join(S,'static',_AJ,'widget');os.makedirs(L,exist_ok=_C);F=str(uuid.uuid4());T=os.path.join(L,f"{F}.png")
			with open(T,'wb')as U:U.write(Q)
		except:pass
	V=PlotDBChart.objects.create(plot_chart_id=G,type_chart=A[_AK],name=A[_t],slug=C,description=A[_A2],is_expose_widget=A[_A3],is_public_widget=A[_u],is_static_data=H,has_widget_password=A[_M],widget_password_e=D,data_source_list=A[_AL],chart_params=A[_AM],chart_config=A[_AN],code_editor_notebook=P,is_created_from_api=I,thumbnail_path=F,date_created=B,last_update=B,last_date_used=B,spartaqube_version=sparta_d5431c9725());W=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);PlotDBChartShared.objects.create(plot_db_chart=V,user=user_obj,share_rights=W,is_owner=_C,date_created=B);return{_A:1,_G:G}
def sparta_eca85327cd(json_data,user_obj):
	G=user_obj;B=json_data;N=B[_G];H=PlotDBChart.objects.filter(plot_chart_id=N,is_delete=_D).all()
	if H.count()>0:
		A=H[H.count()-1];K=sparta_fecce12a4b(G)
		...
		if I.count()>0:
			...
			if L.is_admin or L.has_write_rights:
				P=B[_M];C=_B
				if P:C=B[_s];C=qube_cb1850bb26.sparta_1103fa358a(C)
				D=_B;E=B.get(_AI,_B)
				if E is not _B:
					E=E.split(',')[1];R=base64.b64decode(E)
					try:
						S=os.path.dirname(__file__);T=os.path.dirname(os.path.dirname(os.path.dirname(S)));M=os.path.join(T,'static',_AJ,'widget');os.makedirs(M,exist_ok=_C)
						if A.thumbnail_path is _B:D=str(uuid.uuid4())
						else:D=A.thumbnail_path
						U=os.path.join(M,f"{D}.png")
						with open(U,'wb')as V:V.write(R)
					except:pass
				J=datetime.now().astimezone(UTC);A.type_chart=B[_AK];A.name=B[_t];A.description=B[_A2];A.is_expose_widget=B[_A3];A.is_static_data=B[_A1];A.has_widget_password=B[_M];A.is_public_widget=B[_u];A.widget_password_e=C;A.data_source_list=B[_AL];A.chart_params=B[_AM];A.chart_config=B[_AN];A.thumbnail_path=D;A.last_update=J;A.last_date_used=J;A.save();F=A.code_editor_notebook
				if F is not _B:F.cells=B[_A0];F.last_update=J;F.save()
	return{_A:1}
def sparta_783663c3d3(json_data,user_obj):0
def sparta_5cdd6072cb(json_data,user_obj):
	D=user_obj;F=sparta_fecce12a4b(D)
	...
	if A.count()>0:
		...
		elif C==_AG:A=A.order_by('-plot_db_chart__last_update')
		elif C=='Date asc':A=A.order_by('plot_db_chart__last_update')
		elif C==_AH:A=A.order_by('-plot_db_chart__name')
	...
	for E in A:
		...
		G.append({_G:B.plot_chart_id,_g:B.type_chart,_J:B.name,'slug':B.slug,_L:B.description,_A4:B.is_expose_widget,_A5:B.is_static_data,_v:B.has_widget_password,_w:B.is_public_widget,_x:E.is_owner,_AO:J.has_write_rights,'thumbnail_path':B.thumbnail_path,_A6:H,_A7:I})
	return{_A:1,_AP:G}
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables,api_key):
	B=full_code;F=sparta_fd02dc10b1()['project'];G=sparta_fd02dc10b1()['project/core/api'];A='import sys, os\n';A+=f'sys.path.insert(0, r"{str(F)}")\n';A+=f'sys.path.insert(0, r"{str(G)}")\n';A+=f'os.environ["api_key"] = "{api_key}"\n';B=A+'\n'+B;D=dict();C=tinykernel.TinyKernel()
	for(H,I)in data_source_variables.items():C.glb[H]=I
	C(B)
	for E in workspace_variables:D[E]=C(E)
	return D
def sparta_79cb6f683a(json_data,user_obj):
	...
	if I.count()==1:
		...
		if E:
			...
			if Q.count()>0:
				J=Q[0];e=J.user;f=sparta_724081f15d(e);R=J.user;K=[];A=J.plot_db_chart;g=A.is_static_data
				if g:0
				else:
					for B in A.data_source_list:
						...
						if L:K.append(B[b])
							...
							if _I in B:
								h=B[_H];S=sparta_a80f024295(h,R);T=[]
				...
				return{_A:1,_G:H,_g:A.type_chart,_J:A.name,'slug':A.slug,'name_file':o(A.name),_L:A.description,_A4:A.is_expose_widget,_A5:A.is_static_data,_v:A.has_widget_password,_w:A.is_public_widget,_A8:A.data_source_list,_AQ:A.chart_params,_AR:A.chart_config,_AS:G}
		else:return{_A:-1,_E:_A9}
	return{_A:-1,_E:'Unexpected error, please try again'}
def sparta_b66573e5f7(json_data,user_obj):
	...
	return{_A:1}
def sparta_42c43417a3(user_obj,widget_id):
...
def sparta_ddaf256b5f(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_fecce12a4b(K)
			...
			if M.count()>0:
				...
				if U.count()>0:
					...
					for B in H.keys():
						if B in INPUTS_KEYS:
							if B=='xAxis':
								N=H[B];C=N[J];O=N[X];P=N[_AB];D=F[C]
								if O:D.index=E[B]
								else:D[P]=E[B]
							elif H[B]is not _B:
								c=H[B]
								for(V,I)in enumerate(c):
									if I is not _B:
										C=I[J];O=I[X];P=I[_AB];D=F[C]
										if O:D.index=E[B][V]
										else:D[P]=E[B][V]
					for G in A.data_source_list:C=G[J];G[_F]=F[C].to_json(orient=_z)
				return{_A:1,_G:S,_g:A.type_chart,_J:A.name,_L:A.description,_A4:A.is_expose_widget,_A5:A.is_static_data,_v:A.has_widget_password,_w:A.is_public_widget,_A8:A.data_source_list,_AQ:A.chart_params,_AR:A.chart_config,_AS:_B}
	except Exception as W:print('Error exception > '+str(W));return{_A:-1,_E:str(W)}
def sparta_67528dfd64(json_data,user_obj):
	...
	return{_A:1}
def sparta_93f73a8e21(token_permission):
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_cb1850bb26.sparta_5167d4944a(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_2509bf188b(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_K:A}
					...
					try:
						if qube_cb1850bb26.sparta_5167d4944a(A.widget_password_e)==F:return{_A:1,_K:A}
						else:return{_A:3,_E:_A9,_K:A}
					except:return{_A:3,_E:_A9,_K:A}
	...
	return{_A:-1}
def sparta_822e40b430(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_fecce12a4b(C)
	...
	return{_A:1,_h:_D}
def sparta_30c9977196(plot_db_chart_obj):
	B=json.loads(plot_db_chart_obj.chart_config);C=dict();E={'xAxisArr':'x','yAxisArr':'y',_AE:'r',_AD:_AC,'ohlcvArr':'ohlcv',_AF:'shaded_background'}
	for A in B.keys():
		if A in INPUTS_KEYS:
			try:
				F=E[A]
	...
	return C
def sparta_98318cdfe2(json_data,user_obj):
	B=user_obj;D=sparta_fecce12a4b(B)
	...
	for C in E:
		...
		F.append({_G:A.plot_chart_id,_g:A.type_chart,_v:A.has_widget_password,_w:A.is_public_widget,_J:A.name,'slug':A.slug,_L:A.description,_x:C.is_owner,_AO:I.has_write_rights,_A6:G,_A7:H})
	return{_A:1,_AP:F}
def sparta_fabc6a2ecc(json_data,user_obj):
	E=user_obj;B=json_data;K=B[_G];L=B['isCalledFromLibrary'];F=PlotDBChart.objects.filter(plot_chart_id=K,is_delete=_D).all()
	if F.count()>0:
		A=F[F.count()-1];H=sparta_fecce12a4b(E)
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_M];C=_B
				if N:C=B[_s];C=qube_cb1850bb26.sparta_1103fa358a(C)
				J=datetime.now().astimezone(UTC);A.has_widget_password=B[_M];A.widget_password_e=C;A.name=B[_t];A.plotDes=B[_A2];A.is_expose_widget=B[_A3];A.is_public_widget=B[_u];A.is_static_data=B[_A1];A.last_update=J;A.save()
				if L:0
				else:
					D=A.code_editor_notebook
					if D is not _B:D.cells=B[_A0];D.last_update=J;D.save()
	return{_A:1}
def sparta_22d5be781f(json_data,user_obj):
	D=user_obj;B=json_data;I=B[_G];E=PlotDBChart.objects.filter(plot_chart_id=I,is_delete=_D).all()
	if E.count()>0:
		A=E[E.count()-1];G=sparta_fecce12a4b(D)
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_M];C=_B
				if K:C=B[_s];C=qube_cb1850bb26.sparta_1103fa358a(C)
	...
	return{_A:1}
def sparta_337dac8ae7(json_data,user_obj):
	...
	return{_A:1}
def sparta_9991030f0d(user_obj):
	...
	for F in D:
		...
		E.append({'id':A.plot_chart_id,_J:A.name,_L:A.description,_A6:G,_A7:H})
	return E
def sparta_06e86aa22f(json_data,user_obj):
	try:A=sparta_822e40b430(json_data[_AT],user_obj);B=A[_h];return{_A:1,_h:B}
	except:pass
	return{_A:-1}
def sparta_43d0c990f7(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_822e40b430(A[_AT],B);D=C[_h]
	if D:E=C[_K];A[_G]=E.plot_chart_id;F=sparta_79cb6f683a(A,B);return{_A:1,_F:[A[_F]for A in F[_A8]]}
	return{_A:-1}
def sparta_aa8b11136b(json_data,user_obj):
	F='Error quantstats';C=user_obj;A=json_data;from project.sparta_1d1c5d5897.sparta_6f8ac4359a import qube_47a7b66c6b as D;E=A['service']
	if E=='quantstats':
		try:return D.sparta_e89da203f4(A,C)
		except Exception as B:print(F);print(traceback.format_exc());print(F);return{_A:-1,_E:str(B)}
	elif E=='matplotlib':
		try:return D.sparta_67bee977c5(A,C)
		except Exception as B:print('Error matpltlib');print(traceback.format_exc());return{_A:-1,_E:str(B)}
	return{_A:1}
def sparta_ab5be95088(json_data,user_obj):
	B=json_data;import quantstats as K;B=B[_F];G=B[_AU];L=B[_AV];H=B['columnsX'];I=B[_r];F=L;C=I
	...
	D.columns=C;return{_A:1,'metrics':D.to_json(orient=_z)}
def sparta_a8d9791be7(json_data,user_obj):
	N='Salary';A=json_data;A=A[_F];O=A[_AU];P=A[_AV];G=A['columnsX'];H=A[_r];C=P;I=H
	...
	def M(this_df,level=0,previous_index_list=_B):
		...
		if A==F-1:
			for H in J:L.append({_F:[0]*len(K),_F:C[H].tolist(),_AC:list(C.index.get_level_values(A)),'hierarchy':D,_AB:H,'label':D[-1]})
		elif A<F-1:
	...
	M(B);print('chart_data');return{_A:1,_AA:L,_AC:K}