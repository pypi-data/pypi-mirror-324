...
def sparta_fecce12a4b(user_obj):
	...
	return B
def sparta_bad9dce956(project_path):
	G='template';A=project_path
	if not os.path.exists(A):os.makedirs(A)
	D=A;H=os.path.dirname(__file__);E=os.path.join(sparta_fd02dc10b1()['django_app_template'],_I,G)
...
def sparta_76c7e3d389(json_data,user_obj):
	B=user_obj;A=json_data[_F];A=sparta_d4809fae31(A);F=Developer.objects.filter(project_path=A).all()
	if F.count()>0:
	...
	try:
		if not os.path.exists(A):os.makedirs(A)
		K=sparta_bad9dce956(A);A=K[_K];return{_B:1,_K:A}
	except Exception as E:return{_B:-1,_D:f"Failed to create folder: {str(E)}"}
def sparta_2d53cf5477(json_data,user_obj):A=json_data;A['bAddGitignore']=_A;A['bAddReadme']=_A;return qube_0b9bd8f035.sparta_b9f1cf9170(A,user_obj)
def sparta_d6a8376d83(json_data,user_obj):return sparta_ae38ab04bb(json_data,user_obj)
def sparta_e706e63133(json_data,user_obj):
	K='%Y-%m-%d';J='Recently used';D=user_obj;F=sparta_fecce12a4b(D)
...
def sparta_d4762d0003(json_data,user_obj):
	...
	return{_B:1,_I:{'basic':{_J:A.developer_id,_G:A.name,_L:A.slug,_M:A.description,_S:A.is_expose_developer,_U:A.is_public_developer,_T:A.has_password,_g:A.developer_venv,_K:A.project_path},_f:{_h:A.lumino_layout}}}
def sparta_8347f6132e(json_data,user_obj):
	I=user_obj;A=json_data;N=A['isNew']
	if not N:return sparta_7d3200e329(A,I)
	C=datetime.now().astimezone(UTC);J=str(uuid.uuid4());G=A[_V];E=_E
	if G:E=A[_i];E=qube_cb1850bb26.sparta_1103fa358a(E)
	O=A[_W];P=A[_G];Q=A[_M];D=A[_F];D=sparta_d4809fae31(D);R=A[_j];S=A[_k];G=A[_V];T=A.get('developerVenv',_E);B=A[_L]
	if len(B)==0:B=A[_G]
	K=slugify(B);B=K;L=1
	...
	Y=Developer.objects.create(developer_id=J,name=P,slug=B,description=Q,is_expose_developer=R,is_public_developer=S,has_password=G,password_e=E,lumino_layout=O,project_path=D,developer_venv=T,thumbnail_path=H,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_d5431c9725());Z=ShareRights.objects.create(is_admin=_A,has_write_rights=_A,has_reshare_rights=_A,last_update=C);DeveloperShared.objects.create(developer=Y,user=I,share_rights=Z,is_owner=_A,date_created=C);return{_B:1,_J:J}
def sparta_7d3200e329(json_data,user_obj):
	...
	if I.count()==1:
		A=I[I.count()-1];H=A.developer_id;M=sparta_fecce12a4b(G)
...
def sparta_f8c680754f(json_data,user_obj):
	...
	return{_B:1}
def sparta_3af2c71ebd(json_data,user_obj):
...
def has_developer_access(developer_id,user_obj,password_developer=_E):
			...
			if A.is_public_developer:
				...
				if not L:print('no password');return{_B:1,_H:A}
					...
					if F is _E:print('empty passord provided');return{_B:2,_D:'Require password',_H:A}
						...
						try:
							if qube_cb1850bb26.sparta_5167d4944a(A.password_e)==F:return{_B:1,_H:A}
	...
	return{_B:-1,J:2}
def sparta_fdd1b19ea7(json_data,user_obj):A=sparta_d4809fae31(json_data[_F]);return sparta_d6097e5baf(A)
def sparta_87141a8b0f(json_data,user_obj):A=sparta_d4809fae31(json_data[_F]);return sparta_37eab207ca(A)
def sparta_1cd85ce35b():
	try:
...
def sparta_757436bc3b(json_data,user_obj):
	A=sparta_d4809fae31(json_data[_F]);A=os.path.join(A,_P)
	if not os.path.isdir(A):return{_B:-1,_D:f"The provided path '{A}' is not a valid directory."}
	B=os.path.join(A,'package.json');C=os.path.exists(B);D=sparta_1cd85ce35b();return{_B:1,'is_init':C,'is_npm_installed':D,'npm_version':sparta_f2b4351f90(),'node_version':sparta_8d0e674ba8()}
def sparta_ae38ab04bb(json_data,user_obj):
	A=sparta_d4809fae31(json_data[_F]);A=os.path.join(A,_P)
...
def sparta_6a04d6602c(json_data,user_obj):
	A=json_data;print('NODE LIS LIBS');print(A);D=sparta_d4809fae31(A[_F])
...
def sparta_caf28b6903():
	...
	return B
def sparta_9c0136fbe1(json_data,user_obj):
	A=sparta_d4809fae31(json_data[_F]);A=os.path.join(A,_c,'app');F,B,C=sparta_ae449fc360(A);D=1;E=''
	if C:D=-1;E=B
	return{_B:D,'has_error':C,'has_pending_migrations':F,_Y:B,_D:E}
def sparta_50db1ddfed(project_path,python_executable=_Q):
...
def sparta_067f64ae0a(json_data,user_obj):
	A=sparta_d4809fae31(json_data[_F]);A=os.path.join(A,_c,'app');B,C=sparta_50db1ddfed(A);D=1;E=''
	if not B:D=-1;E=C
	return{_B:D,'res_migration':B,_Y:C,_D:E}
def sparta_396bf0fcfc(json_data,user_obj):return{_B:1}
def sparta_79b8c7cec0(json_data,user_obj):return{_B:1}
def sparta_c1b094b2cc(json_data,user_obj):return{_B:1}
def sparta_d541c3ff45(json_data,user_obj):print('developer_hot_reload_preview json_data');print(json_data);return{_B:1}
def sparta_fa7f108d2d(json_data,user_obj):
	C='baseProjectPath';A=json_data;D=sparta_d4809fae31(A[C]);E=os.path.join(os.path.dirname(D),_c);sys.path.insert(0,E);import webservices as B;importlib.reload(B);F=A['service'];G=A.copy();del A[C]
	try:return B.sparta_8fc0e7bd59(F,G,user_obj)
