...
def sparta_5f9c683a77(json_data,user_obj):
	B=user_obj;A=json_data[_E];A=sparta_d4809fae31(A);F=Dashboard.objects.filter(project_path=A).all()
	if F.count()>0:
		C=F[0];G=sparta_fecce12a4b(B)
	...
	try:
		if not os.path.exists(A):os.makedirs(A)
		J=sparta_7cc5b474ae(A);L=J[_N];M=J[_O];return{_C:1,'main_ipynb_filename':L,'main_ipynb_fullpath':M}
	except Exception as E:return{_C:-1,_D:f"Failed to create folder: {str(E)}"}
def sparta_f53b032321(json_data,user_obj):
...
def sparta_bc22c3225c(user_obj,json_data):
	F='notebook_id';E='developer_id';D='dashboard_id';B=user_obj;A=json_data;print('check_perform_project_explorer_action json_data');print(A);C=A[_E]
...
def check_perform_project_explorer_action_dashboard(user_obj,dashboard_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.dashboard_id;F=sparta_fecce12a4b(B)
...
def check_perform_project_explorer_action_developer(user_obj,developer_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.developer_id;F=sparta_fecce12a4b(B)
...
def check_perform_project_explorer_action_notebook(user_obj,notebook_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.notebook_id;F=sparta_fecce12a4b(B)
	...
	return{_C:1}
def sparta_a44bd0f240(main_path,test_path):B=test_path;A=main_path;A=Path(A).resolve();B=Path(B).resolve();return A in B.parents or A==B
def sparta_f1a9b738d6(json_data,user_obj):
	A=json_data;print('Create resources');print(A);G=sparta_bc22c3225c(user_obj,A)
	if G[_C]==-1:return G
	C=A[_E];C=sparta_d4809fae31(C);F=A['createResourceName'];I=A['createType']
	try:
		B=os.path.join(C,F)
		if int(I)==1:
			D=A[_H];E=sparta_d4809fae31(D)
			if len(D)>0:
				if not sparta_a44bd0f240(C,E):return{_C:-1,_D:_L}
				B=os.path.join(E,F)
			...
			if len(D)>0:
				if not sparta_a44bd0f240(C,E):return{_C:-1,_D:_L}
				B=os.path.join(E,F)
			if not os.path.exists(B):sparta_9fbb5384da(B)
	...
	return{_C:1}
def sparta_b5f587d122(json_data,user_obj):
	C=json_data;print('Rename resources');print(C);H=sparta_bc22c3225c(user_obj,C)
	if H[_C]==-1:return H
	D=C[_H];I=C['editName'];E=C[_E];E=sparta_d4809fae31(E);J=int(C['renameType'])
	if J==1:
		B=D;F=os.path.dirname(D);A=os.path.join(F,I);B=sparta_d4809fae31(B);A=sparta_d4809fae31(A)
		if E in A:
			try:os.rename(B,A)
	...
	return{_C:1}
def sparta_7e9b5a0440(json_data,user_obj):
	A=json_data;print('*'*100);print('MOVE drag & drop resources');print(A);G=sparta_bc22c3225c(user_obj,A)
	if G[_C]==-1:return G
	D=A[_E];D=sparta_d4809fae31(D);B=A['folderLocation'];B=sparta_d4809fae31(B);print('folder_location >>>>> ');print(B);J=A[_P];K=A[_Q]
	for H in J:
		L=H[_I];I=H[_M];E=os.path.join(L,I);C=os.path.join(B,I);E=sparta_d4809fae31(E);C=sparta_d4809fae31(C)
		if D in C:
			try:print(f"Move from\n{E}\nto\n{C}");shutil.move(E,C)
			except Exception as M:print('Exception move 1');print(M)
	for N in K:
		F=N[_I];F=sparta_d4809fae31(F)
		if D in B:
			try:shutil.move(F,B)
			except:pass
	return{_C:1}
def sparta_29f3ef4d25(json_data,user_obj):
	A=json_data;print('Delete resource');print(A);E=sparta_bc22c3225c(user_obj,A)
	if E[_C]==-1:return E
	C=A[_E];C=sparta_d4809fae31(C);F=int(A['typeDelete']);G=A[_H];B=sparta_d4809fae31(G)
	if not sparta_a44bd0f240(C,B):return{_C:-1,_D:_L}
	if F==1:
		try:os.rmdir(B)
		except:
	...
	return{_C:1}
def sparta_3a783773a4(json_data,user_obj):
	B=json_data;print('Delete multiple resources');print(B);G=sparta_bc22c3225c(user_obj,B)
	if G[_C]==-1:return G
	I=B[_P];J=B[_Q];C=B[_E];C=sparta_d4809fae31(C)
	for D in I:
		K=D[_M];H=D[_I];E=os.path.join(H,K);E=sparta_d4809fae31(E)
		if C in H:
			try:print(f"File to delete: {E}");os.remove(E)
			except Exception as F:return{_C:-1,_D:str(F)}
	for D in J:
		A=D[_I];A=sparta_d4809fae31(A)
		if C in A:
	...
	return{_C:1}
def sparta_9aef548623(json_data,user_obj):
	A=json_data;C=sparta_bc22c3225c(user_obj,A)
	if C[_C]==-1:return C
	B=A[_E];E=A[_H];D=sparta_d4809fae31(E);B=sparta_d4809fae31(B)
	if B in D:return{_C:1,_J:D}
	return{_C:-1}
def sparta_e17da1217d(json_data,user_obj):
	...
	return{_C:-1}
def sparta_2f8df244ff(json_data,user_obj):
	A=json_data;B=A[_E];C=sparta_bc22c3225c(user_obj,A)
	if C[_C]==-1:return C
	F='app'
	def D(zf,folder):
		C=folder
	...
	return{_C:-1}
def sparta_d6fdc04245(json_data,user_obj,file_obj):
	F=file_obj;C=json_data;print('**********************************');print('upload_resource');print(C);E=C[_E];G=sparta_bc22c3225c(user_obj,C)
	if G[_C]==-1:return G
	B=C[_H];E=sparta_d4809fae31(E)
	if len(B)==0:B=E
	else:B=sparta_d4809fae31(B)
	D=C[_I];print(f"path_folder >> {D}")
	if len(D)>0:
		A=os.path.join(B,D);A=sparta_d4809fae31(A)
		if not os.path.exists(A):os.makedirs(A)
	else:
		A=os.path.join(B,D);A=sparta_d4809fae31(A)
		if not os.path.exists(A):os.makedirs(A)
	...
	J={_C:1};return J
def sparta_510d2d3878(fileName):
	A=['pdf','png','jpg','jpeg'];B=fileName.split('.')[-1].lower()
	if B in A:return _G
	return _F
def sparta_d30d17f7b7(full_path):
	...
	return B
def sparta_85b415e7d4(json_data,user_obj):
	A=json_data;E=A[_E];B=sparta_bc22c3225c(user_obj,A)
	if B[_C]==-1:return B
	C=sparta_d4809fae31(A[_J]);D=qube_52cd691e01.sparta_6816cb034c(C);return{_C:1,'ipynb_dict':json.dumps(D)}
def sparta_d2d6c3f4a6(json_data,user_obj):
	R='cells';M='ipynb';K='file_extension';J='file_content';I='cm_mode';H='is_previewable';G='is_handled';D=json_data;S=D[_M];B=D[_J];B=sparta_d4809fae31(B);N=D[_E];N=sparta_d4809fae31(N);O=sparta_bc22c3225c(user_obj,D)
	if O[_C]==-1:return O
	E=_F;P=_F;F='';A='';C=S.split('.')[-1].lower();T=sparta_a928638c89()
	for Q in T:
		try:
	...
	if E:
		...
		if sparta_510d2d3878(B):C=B.split('.')[-1];V=sparta_d30d17f7b7(B);return{_C:1,G:_G,H:E,I:F,J:A,K:C,'resource':V}
		C=B.split('.')[-1]
		if C==M:
