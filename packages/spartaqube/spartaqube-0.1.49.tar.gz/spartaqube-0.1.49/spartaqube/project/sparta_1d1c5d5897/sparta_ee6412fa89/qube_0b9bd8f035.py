...
def sparta_90bcbf5c9e(path):
	try:A=git.Repo(path).git_dir;return _B
	except git.exc.InvalidGitRepositoryError:return _E
def sparta_18ab2c58cc(json_data,user_obj):
	A=json_data['notebookProjectId'];B,C=qube_625df6a9df.get_notebookProjectObj(A,user_obj)
	if B is not _F:return _B
	return _E
def sparta_4f1d257249(remoteBranchToTrack):A=Popen(f"git branch -u {remoteBranchToTrack}",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);B=A.stdout.readline();print('realtime_output 1');print(B);A=Popen(f"git config push.default upstream",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);print('realtime_output 2');print(B)
def sparta_9f37454b5c(func):
	def A(json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_18ab2c58cc(A,B):return{_A:-1,_D:_K}
		return func(A,B)
	return A
def sparta_fde7f0b959(func):
	def A(webSocket,json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_18ab2c58cc(A,B):return{_A:-1,_D:_K}
		return func(webSocket,A,B)
	return A
def sparta_ae91eea1a0(repo,user_obj):
...
def sparta_62682038b9(webSocket,json_data,user_obj):
...
def sparta_b9f1cf9170(json_data,user_obj):
	...
	return{_A:1,'output':'\n'.join(F)}
def sparta_89bd28eae5(json_data,user_obj):
	F=user_obj;B=json_data;print('sqEditorGitAddRemoteOrigin json_data');print(B);C=B[_C];G=sparta_90bcbf5c9e(C)
	if G:
		H=B['remoteUrl'];I=B['remoteName'];A=git.Repo(C);sparta_ae91eea1a0(A,F);J=A.create_remote(I,url=H);A=git.Repo(C);sparta_ae91eea1a0(A,F)
		for D in A.remotes:D.fetch()
		for D in A.remotes:
			if J==D:
				E=D.refs
				if len(E)>0:K=E[len(E)-1];L=os.path.dirname(os.path.realpath(__file__));os.chdir(C);sparta_4f1d257249(K);os.chdir(L)
	return{_A:1}
def sparta_92539bfa8c(json_data,user_obj):
	A=json_data;print('git_load_available_track_remote json_data');print(A);B=A[_C];F=sparta_90bcbf5c9e(B)
	if F:
		G=A[_H];C=git.Repo(B);sparta_ae91eea1a0(C,user_obj);D=[]
		for E in C.remotes:
			if G==E.config_reader.get(_J):
				for H in E.refs:D.append({_I:H.name})
	return{_A:1,'available_branches':D}
def sparta_4d96c3efe1(json_data,user_obj):
	A=json_data;print('*******************************************');print('git_set_track_remote json_data');print(A);B=A[_C];D=sparta_90bcbf5c9e(B)
	if D:G=A[_H];E=A['remoteBranchToTrack'];C=git.Repo(B);sparta_ae91eea1a0(C,user_obj);H=C.head.ref.name;F=os.path.dirname(os.path.realpath(__file__));os.chdir(B);sparta_4f1d257249(E);os.chdir(F)
	return{_A:1}
@sparta_9f37454b5c
def sparta_fa889cda58(json_data,user_obj):return{_A:1}
def sparta_e3d885ef80(json_data,user_obj):
	B=json_data[_C];C=sparta_90bcbf5c9e(B)
	if C:A=git.Repo(B);sparta_ae91eea1a0(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=E);F.pull(allow_unrelated_histories=_B)
	return{_A:1}
def sparta_4d2cd17ada(json_data,user_obj):
	B=json_data[_C];C=sparta_90bcbf5c9e(B)
	if C:A=git.Repo(B);sparta_ae91eea1a0(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=f"{E}");F.push()
	return{_A:1}
def sparta_fd3d9106a8(json_data,user_obj):
	A=json_data[_C];C=sparta_90bcbf5c9e(A)
	if C:
		B=git.Repo(A);sparta_ae91eea1a0(B,user_obj)
		for D in B.remotes:D.fetch()
	return{_A:1}
def sparta_c703210275(json_data,user_obj):A=json_data[_C];B=sparta_90bcbf5c9e(A);return{_A:1,_L:B}
def sparta_1b50ca5c1d(json_data,user_obj):
	...
	def B(folder_path):
		j='remotes_arr';i='commits_ahead_arr';h='commits_behind_arr';g='is_ahead';f='is_behind';W='branch';H=folder_path;P=sparta_90bcbf5c9e(H);print(f"is_git_repository > {H}");print(P)
		if P:
			A=git.Repo(H);F=[];Z=[]
...
def sparta_7667c0af65(json_data,user_obj):
	H='path';G='file';C=json_data[_C];I=sparta_90bcbf5c9e(C)
	if I:
		B=git.Repo(C);sparta_ae91eea1a0(B,user_obj);D=[]
	...
	return{_A:1}
def sparta_fc5763942f(json_data,user_obj):
	B=json_data;C=B[_C];D=B['gitMsg'];E=sparta_90bcbf5c9e(C)
	if E:A=git.Repo(C);sparta_ae91eea1a0(A,user_obj);A.git.add(all=_B);A.git.commit('-m',D)
	return{_A:1}
@sparta_9f37454b5c
def sparta_375d59caa0(json_data,user_obj):return{_A:1}
def sparta_47ba59c883(json_data,user_obj):
	A=json_data;print('Delete Remoete');print(A);C=A[_C];E=sparta_90bcbf5c9e(C)
	if E:
		B=git.Repo(C);sparta_ae91eea1a0(B,user_obj);F=A[_H]
		for D in B.remotes:
			if F==D.config_reader.get(_J):B.delete_remote(D);break
	return{_A:1}
def sparta_0325930125(json_data,user_obj):return{_A:1}
def sparta_b8ff54b618(json_data,user_obj):
	D=json_data;E=D[_C];C=D['newBranchName'];F=sparta_90bcbf5c9e(E)
	if F:
		A=git.Repo(E);sparta_ae91eea1a0(A,user_obj);B=[A for A in A.branches if A.name==C]
	...
	return{_A:-1,_D:_G}
def sparta_ee4aa8aae6(json_data,user_obj):
	A=json_data;B=A[_C];D=A['branch2Checkout'];E=sparta_90bcbf5c9e(B)
	if E:
		C=git.Repo(B);sparta_ae91eea1a0(C,user_obj)
		try:C.git.checkout(D);return{_A:1}
		except Exception as F:return{_A:-1,_D:str(F)}
	return{_A:-1,_D:_G}
def sparta_b4c8fea2a1(json_data,user_obj):
	B=json_data;C=B[_C];D=B['branch2Merge'];F=sparta_90bcbf5c9e(C)
	if F:
		A=git.Repo(C);sparta_ae91eea1a0(A,user_obj);E=A.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_59cdf7fe97(json_data,user_obj):
	D=json_data;A=D[_C];B=D['branch2Delete'];F=sparta_90bcbf5c9e(A)
	if F:
		E=git.Repo(A);sparta_ae91eea1a0(E,user_obj);G=E.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_3bf4b74fc9(json_data,user_obj):
	F='diff_output';A=json_data;print('sqEditorGitLoadFilesDiff');print(A);B=A[_C];G=A['filePath'];H=A['fileType'];C=sparta_90bcbf5c9e(B);print(_L);print(C)
	if C:D=git.Repo(B);sparta_ae91eea1a0(D,user_obj);E=D.git.diff();print(F);print(E);return{_A:1,F:E}
	return{_A:-1,_D:_G}