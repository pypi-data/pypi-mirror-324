...
from project.sparta_1d1c5d5897.sparta_6f8ac4359a.qube_2d7d6b2b69 import sparta_5cdd6072cb
...
def sparta_fecce12a4b(user_obj):
	...
	return B
def sparta_dae1a6f843(json_data,user_obj):
...
def sparta_990170cd43(json_data,user_obj):
	...
	return{_A:1,_J:{'basic':{_H:A.dashboard_id,'name':A.name,'slug':A.slug,_M:A.description,_N:A.is_expose_dashboard,_P:A.is_public_dashboard,_O:A.has_password,'dashboard_venv':A.dashboard_venv,'project_path':A.project_path},'lumino':{'main_ipynb_filename':os.path.basename(A.main_ipynb_fullpath),_R:A.main_ipynb_fullpath,'lumino_layout':A.lumino_layout,'notebook_cells':qube_52cd691e01.sparta_a4d8e02755(A.main_ipynb_fullpath)},'grid_config':json.loads(A.grid_config),'plot_db_programmatic_attributes':H}}
def sparta_5cade0141d(json_data,user_obj):
	J=user_obj;A=json_data;R=A['isNewDashboard']
	if not R:return sparta_aa16c37019(A,J)
	C=datetime.now().astimezone(UTC);K=str(uuid.uuid4());H=A[_K];E=_B
	if H:E=A[_S];E=qube_cb1850bb26.sparta_1103fa358a(E)
	S=A[_L];L=A[_T];T=A[_I];U=A[_U];D=A['projectPath'];D=sparta_d4809fae31(D);V=A[_V];W=A[_W];H=A[_K];X=A.get('dashboardVenv',_B);B=A[_X]
	if len(B)==0:B=A[_I]
	M=slugify(B);B=M;N=1
...
def sparta_aa16c37019(json_data,user_obj):
	...
	if J.count()==1:
		A=J[J.count()-1];I=A.dashboard_id;N=sparta_fecce12a4b(H)
		...
		if O:
			...
			if b:
				G=B[_S]
				if len(G)>0:G=qube_cb1850bb26.sparta_1103fa358a(G);A.password_e=G;A.has_password=_C
			else:A.has_password=_D
			A.save()
	return{_A:1,_H:I}
def sparta_ef64eca40f(json_data,user_obj):
	E=json_data;B=user_obj;F=E[_G];C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_D).all()
	if C.count()==1:
		A=C[C.count()-1];F=A.dashboard_id;G=sparta_fecce12a4b(B)
	...
	return{_A:1}
def sparta_53afcddd41(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:K=sparta_d4809fae31(B[_f]);A.main_ipynb_fullpath=K;A.save()
	return{_A:1}
def sparta_cf961c6d81(json_data,user_obj):
...
def has_dashboard_access(dashboard_id,user_obj,password_dashboard=_B):
		...
		if A.is_expose_dashboard:
			if A.is_public_dashboard:
				if not K:return{_A:1,_E:A}
					...
					try:
						if qube_cb1850bb26.sparta_5167d4944a(A.password_e)==G:return{_A:1,_E:A}
	...
	return{_A:-1}
def sparta_8af2638bbb(json_data,user_obj):A=sparta_5cdd6072cb(json_data,user_obj);return A
