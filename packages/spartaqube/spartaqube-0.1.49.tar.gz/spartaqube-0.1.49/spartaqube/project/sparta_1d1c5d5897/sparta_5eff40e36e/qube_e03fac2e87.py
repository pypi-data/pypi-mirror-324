...
from project.sparta_1d1c5d5897.sparta_3e2850d22b import qube_4a043f7a6b as qube_4a043f7a6b
def sparta_dc887dbc85(json_data,userObj):
...
def sparta_d100c09d44(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_b79a99bc7e.sparta_ffa29a5b8d(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_b79a99bc7e.sparta_ffa29a5b8d(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_3be5f02094(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_b79a99bc7e.sparta_ffa29a5b8d(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_b79a99bc7e.sparta_ffa29a5b8d(A);C=[A.userGroup for A in H]
...
def sparta_ee386c24a5(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';J=json_data;B=user_obj;print('json_data SHARE OBJECTS');print(J);L=datetime.now().astimezone(UTC);P=qube_b79a99bc7e.sparta_ffa29a5b8d(B);M=[A.userGroup for A in P];U=J['member2ShareArr'];I=int(J[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_b79a99bc7e.sparta_ffa29a5b8d(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_b79a99bc7e.sparta_ffa29a5b8d(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_027a269dfd(json_data,user_obj):
