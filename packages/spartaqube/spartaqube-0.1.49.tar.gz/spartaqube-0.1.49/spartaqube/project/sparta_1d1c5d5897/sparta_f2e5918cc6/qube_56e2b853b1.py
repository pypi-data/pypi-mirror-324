...
from project.sparta_1d1c5d5897.sparta_e0f10cf586.qube_da7a8a8853 import sparta_e061501b2d,sparta_e6bcf42942
def sparta_45b7e12538():keygen_fernet='spartaqube-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_638e93d3a3():keygen_fernet='spartaqube-internal-decoder-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_49f0db1a26(f,str_to_encrypt):data_to_encrypt=str_to_encrypt.encode(_B);token=f.encrypt(data_to_encrypt).decode(_B);token=base64.b64encode(token.encode(_B)).decode(_B);return token
def sparta_1e237374a7(api_token_id):
	if api_token_id=='public':
		try:return User.objects.filter(email='public@spartaqube.com').all()[0]
		except:return
	try:
		f_private=Fernet(sparta_638e93d3a3().encode(_B));api_key=f_private.decrypt(base64.b64decode(api_token_id)).decode(_B).split('@')[1];user_profile_set=UserProfile.objects.filter(api_key=api_key,is_banned=_D).all()
...
def sparta_4841534c3e(user_obj):
	...
	return api_key
def sparta_3e964c37c3(user_obj,domain_name):api_key=sparta_4841534c3e(user_obj);random_nb=str(randint(0,1000));data_to_encrypt=f"apikey@{api_key}@{random_nb}";f_private=Fernet(sparta_638e93d3a3().encode(_B));private_encryption=sparta_49f0db1a26(f_private,data_to_encrypt);data_to_encrypt=f"apikey@{domain_name}@{private_encryption}";f_public=Fernet(sparta_45b7e12538().encode(_B));public_encryption=sparta_49f0db1a26(f_public,data_to_encrypt);return public_encryption
def sparta_81ef7b3239(json_data,user_obj):api_key=sparta_4841534c3e(user_obj);domain_name=json_data['domain'];public_encryption=sparta_3e964c37c3(user_obj,domain_name);return{_A:1,'token':public_encryption}
def sparta_0a778453db(json_data,user_obj):userprofile_obj=UserProfile.objects.get(user=user_obj);api_key=str(uuid.uuid4());userprofile_obj.api_key=api_key;userprofile_obj.save();return{_A:1}
def sparta_747720b89a():plot_types=sparta_e061501b2d();plot_types=sorted(plot_types,key=lambda x:x['Library'].lower(),reverse=_D);return{_A:1,'plot_types':plot_types}
def sparta_e57d144cf8(json_data):print('DEBUG get_plot_options json_data');print(json_data);plot_type=json_data['plot_type'];plot_input_options_dict=sparta_e6bcf42942(plot_type);plot_input_options_dict[_A]=1;return plot_input_options_dict
def sparta_44a5844160(code):
	tree=ast.parse(code)
...
def sparta_5dc8d6b998(json_data,user_obj):
	A='errorMsg';user_code_example=json_data['userCode'];resp=_E;error_msg=''
	try:
		print('EXECUTE API EXAMPLE DEBUG DEBUG DEBUG');api_key=sparta_4841534c3e(user_obj);core_api_path=sparta_fd02dc10b1()['project/core/api'];ini_code='import os, sys\n';ini_code+=f'sys.path.insert(0, r"{str(core_api_path)}")\n';ini_code+='from spartaqube import Spartaqube as Spartaqube\n';ini_code+=f"Spartaqube('{api_key}')\n";user_code_example=ini_code+'\n'+user_code_example;exec(user_code_example,globals(),locals());last_expression_str=sparta_44a5844160(user_code_example)
...
def sparta_4fd9ad102f(json_data,user_obj):
	...
	return{_A:-1}
def sparta_42c43417a3(json_data,user_obj):widget_id=json_data['widgetId'];return qube_2d7d6b2b69.sparta_42c43417a3(user_obj,widget_id)
def sparta_c8186175d6(json_data,user_obj):
	api_service=json_data['api_service']
	if api_service=='get_status':output=sparta_8e4a5429ce()
	elif api_service=='get_status_ws':return sparta_09cf2aa09b()
	elif api_service=='get_connectors':return sparta_ee546d4f3a(json_data,user_obj)
	elif api_service=='get_connector_tables':return sparta_966c0b9ab6(json_data,user_obj)
	elif api_service=='get_data_from_connector':return sparta_0a3b98f9f8(json_data,user_obj)
	elif api_service=='get_widgets':output=sparta_16d3591997(user_obj)
	elif api_service=='has_widget_id':return sparta_06e86aa22f(json_data,user_obj)
	elif api_service=='get_widget_data':return sparta_3b7a0f2b59(json_data,user_obj)
	elif api_service=='get_plot_types':return sparta_e061501b2d()
	return{_A:1,_F:output}
def sparta_8e4a5429ce():return 1
def sparta_ee546d4f3a(json_data,user_obj):
	A='db_connectors';keys_to_retain=['connector_id',_C,'db_engine'];res_dict=qube_2d7d6b2b69.sparta_a55db36631(json_data,user_obj)
	if res_dict[_A]==1:res_dict[A]=[{k:d[k]for k in keys_to_retain if k in d}for d in res_dict[A]]
	return res_dict
def sparta_966c0b9ab6(json_data,user_obj):res_dict=qube_2d7d6b2b69.sparta_5e3bb0e5a4(json_data,user_obj);return res_dict
def sparta_0a3b98f9f8(json_data,user_obj):res_dict=qube_2d7d6b2b69.sparta_225d60866e(json_data,user_obj);return res_dict
def sparta_16d3591997(user_obj):return qube_2d7d6b2b69.sparta_9991030f0d(user_obj)
def sparta_06e86aa22f(json_data,user_obj):return qube_2d7d6b2b69.sparta_06e86aa22f(json_data,user_obj)
def sparta_3b7a0f2b59(json_data,user_obj):return qube_2d7d6b2b69.sparta_43d0c990f7(json_data,user_obj)
def sparta_a17fa8b114(json_data,user_obj):date_now=datetime.now().astimezone(UTC);session_id=str(uuid.uuid4());pickled_data=json_data['data'];NewPlotApiVariables.objects.create(user=user_obj,session_id=session_id,pickled_variables=pickled_data,date_created=date_now,last_update=date_now);return{_A:1,'session_id':session_id}
def sparta_7923b39e71():return sparta_e061501b2d()
def sparta_d18a62b8f6():cache.clear();return{_A:1}
def sparta_09cf2aa09b():
	global is_wss_valid;is_wss_valid=_D
	try:
		api_path=sparta_fd02dc10b1()['api']
...
def sparta_f8069345ba(json_data,user_obj):
	...
	if not user_obj.is_anonymous:
		api_get_widgets=sparta_16d3591997(user_obj)
...
def sparta_9965f613d0(notebook_id):
...
def sparta_3dbc449912(dashboard_id):
