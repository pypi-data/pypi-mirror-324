...
def sparta_ff9e165bd8(json_data,userObj):
	B=json_data[_C]
	if int(B)==1:
	...
	try:G,H=sparta_fba4c18025(A,B);D={_A:1,_D:H,_E:G,_B:A}
	except:D={_A:-1,_F:_G}
	return D
def sparta_f91eb2ed12(json_data,userObj):
	A=json_data
	try:D=A[_B];E=A[_C];B=os.path.dirname(D);F,G=sparta_fba4c18025(B,E);C={_A:1,_D:G,_E:F,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_cfa1db703d(json_data,userObj):
	A=json_data
	try:B=A[_B];D=A[_C];E,F=sparta_fba4c18025(B,D);C={_A:1,_D:F,_E:E,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_fba4c18025(currentPath,typeexplorer=None):
...
def sparta_fe0c7bc50c(currentPath):
