...
from project.sparta_75433bcd57.sparta_e9edb68cbd.qube_dc67a122c8 import sparta_d5b054f52a,sparta_cb350a4a1c,sparta_10245d5e34
from project.sparta_1d1c5d5897.sparta_27dd4d9e88.qube_4fa258c701 import sparta_e7b01602f0
def sparta_7b3b21a38b(json_data,user_obj):
...
def sparta_364b4015ec(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_81c2ce73ff(json_data,userObj):
	...
	return B
def sparta_b260958ca5(json_data,userObj):
	...
	return A
def sparta_bfa2b1f257(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_e7b01602f0(F):return{_A:-1,_B:'Invalid current code'}
...
def sparta_638e93d3a3():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_ffa9d4d51e(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_10245d5e34(E):return{_A:-1,_B:_I}
...
def sparta_1f7907ab2e(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_d5b054f52a(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_cb350a4a1c(F):return{_A:-1,_B:_I}
...
def sparta_0ae2e56eeb(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_d5b054f52a(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_cb350a4a1c(G):return{_A:-1,_B:_I}
