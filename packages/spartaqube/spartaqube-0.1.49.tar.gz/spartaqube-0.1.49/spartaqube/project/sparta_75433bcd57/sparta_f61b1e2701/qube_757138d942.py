...
from project.sparta_1d1c5d5897.sparta_9e2dd8c569.qube_057740021c import sparta_db7a34de68
from project.sparta_1d1c5d5897.sparta_9e2dd8c569.qube_8d136178ad import sparta_e21cc7b7b2
...
from project.sparta_1d1c5d5897.sparta_f2e5918cc6.qube_56e2b853b1 import sparta_3e964c37c3,sparta_4841534c3e
class NotebookWS(AsyncWebsocketConsumer):
	...
	def get_kernel_type(C,kernel_manager_uuid):
		...
		return-1
	def sparta_c98d9903fb(B,json_data):from project.sparta_1d1c5d5897.sparta_1adcf7c763 import qube_bf9ee34003 as A;return A.sparta_c98d9903fb(json_data)
	async def receive(D,text_data):
		...
		if len(q)>0:
			...
			async def Q(json_data):
				...
				if C in B:
					if len(B[C])>0:A=sparta_d4809fae31(B[C]);A=Path(A).resolve();D=f'import sys, os\nsys.path.insert(0, r"{str(A)}")\nos.chdir(r"{str(A)}")\n';await F.execute_code(D)
			...
			if C=='init-socket'or C=='reconnect-kernel'or C=='reconnect-kernel-run-all':
				...
				try:
					...
					AH=json.loads(A['execCodeInput']);t=M.join([A[X]for A in AH]);S=V(t,A);S+=M+K;F.execute_code(S);T=sparta_db7a34de68(t);u=A['guiInputVarName'];AI=A['guiOutputVarName'];AJ=A[A9];h=[u,AI,AJ];W=[]
					for R in h:
				...
				try:
					v=L;a=L
					if g in A:b=A[g];w=sparta_e21cc7b7b2(b);v=w['assign_state_variable'];a=w['assign_code']
					AK=json.loads(A['execCodeOutput']);x=M.join([A[X]for A in AK]);S=a+M;S+=v+M;S+=x;await F.execute_code(S);T=sparta_db7a34de68(x)
					try:T.append(A[g][f])
				...
				try:
					...
					c+=M+d;print('cmd to execute');print('cmd_to_exec');print(c);await F.execute_code(c);T=sparta_db7a34de68(d)
				except Exception as I:B=json.dumps({E:-1,H:C,O:str(I)});await D.send(text_data=B);return
				B=json.dumps({E:1,H:C,p:T});await D.send(text_data=B)
				...
				try:AT=A['widget_id'];AU=sparta_4841534c3e(D.user);l=HTML(f'<iframe src="/plot-widget/{AT}/{AU}" width="100%" height="500" frameborder="0" allow="clipboard-write"></iframe>');l=l.data;G={E:1,H:C,'widget_iframe':l};B=json.dumps(G);await D.send(text_data=B)
