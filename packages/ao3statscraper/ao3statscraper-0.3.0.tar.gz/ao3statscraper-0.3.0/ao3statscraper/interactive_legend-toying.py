...
class InteractiveLegend(object):
    ...
    """
    def __init__(self, legend:mpl.legend.Legend) -> None:
        self.legend = legend
    ...
    def _build_lookups(self, legend:mpl.legend.Legend) -> (dict, dict):
        """
        ...
        # keep track of handles with identical labels
        label2handle = {}
        for i in range(len(labels)):
            label = labels[i]
            handle = handles[i]

            if label in label2handle.keys():
                label2handle[label].append(handle)
            else:
        ...
        for artist in legend.axes.get_children():
            if artist.get_label() in labels:
                ...
                for h in handle:
                    lookup_artist[h] = artist
                    lookup_artist[handle2text[h]] = artist

        #  lookup_handle.update(zip(handles, handles))
        lookup_handle.update(label2handle)

        return lookup_artist, lookup_handle


    def on_pick(self, event: Union[mpl.backend_bases.KeyEvent,
                                   mpl.backend_bases.MouseEvent] ) -> None:
        """
    ...
    def on_click(self, event: Union[mpl.backend_bases.KeyEvent, mpl.backend_bases.MouseEvent])->None:
        """
    ...
    def update(self):
        ...
        """
        for artist in self.lookup_artist.values():
            handles = self.lookup_handle[artist]
