...
class OWGenerateQuestions(widget.OWWidget):
    ...
    def generate_dataset_on_chunk(self, chunk, model, embedder, llm="gpt4all", threshold=0.7, max_repeat=2, safety_limit=5,
    ...
    def generate_questions_and_answers(self, model, text, llm="gpt4all", max_tokens=4096, temperature=1, top_p=1, top_k=50, stop_tokens=None):
        ...
        # Define the prompt
        prompt = f"""### Contexte : 
On cherche à établir un jeu de questions en français sur un document.
Tu vas recevoir un extrait de document AAA sans aucun contexte. Tu devras générer des questions pertinentes et autonomes, sans faire référence explicite au document.


### Instructions :
- Génère des questions qui reflètent les informations principales de l'extrait fourni.
- Les questions doivent être simples, précises et formulées de manière autonome. Elles ne doivent pas mentionner le document.
- Les questions doivent être en français.
- Ta réponse sera parsée, donc respecte impérativement la structure suivante
