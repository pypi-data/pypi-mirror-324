...
def IsLocalStoreCompressed(aait_store_path):
    ...
    """
    if os.path.isdir(aait_store_path):
        return False
    return True
...
def IsStoreCompressed(aait_store_path):
    ...
    # 1) HEAD request with curl
    process_head = subprocess.run(
        ["curl", "-s", "-L", "-I", aait_store_path],
        capture_output=True
    )

    # If HEAD request failed completely (returncode != 0),
    # we can't be sure => default to True (assuming "compressed").
    if process_head.returncode != 0:
        print("HEAD request failed for", aait_store_path)
        return True

    # Read the response headers
    headers_str = process_head.stdout.decode("utf-8", errors="replace").lower()
    # Check if we have an HTTP status line like "http/1.1 404 not found"
    # We do a simple search for "404".
    is_404 = ("404 not found" in headers_str)

    # 2) If not 404, check Content-Type
    if not is_404:
        # If there's a "content-type: text/html", we do a GET
        if "content-type: text/html" in headers_str:
            ...
            # Not HTML => likely a single file => compressed
            return True

    else:
        # 3) HEAD indicates 404 Not Found.
        # Let's do a GET to see if the server returns a custom listing or page
        process_get = subprocess.run(
            ["curl", "-s", "-L", aait_store_path],
            capture_output=True
        )
        if process_get.returncode != 0:
            # The GET also failed => we assume "compressed"
            print("GET request failed as well.")
            return True

        body = process_get.stdout.decode("utf-8", errors="replace").lower()
        # If the body still indicates a typical "not found," we treat as compressed.
        # But if the server returns a custom page with "Contenu de" or "Index of",
        # we interpret that as a directory listing => not compressed.
...
def download_aait_store_file_or_directory(server_path,target):
    """
    ...
    try:
        ...
        SimpleDialogQt.BoxError("no acces to ia store ressource path")
        raise Exception("no acces to ia store ressource path")
    if IsStoreCompressed(aait_store_remote):
        compressed=True
    ...
    output = os.path.join(get_local_store_path(), file_or_directory)
    if compressed==False:
        if not is_url(aait_store_remote):
            ...
            if os.name=='nt':
                ...
                try:
                    dialog_progress = SimpleDialogQt.ProgressDialog(title="Synchronsation please wait",content="synchronization of the file " + input + "\nthis operation may take a few minutes")
            ...
            # Equivalent to your "download_from_folder_server" approach
            #  (or any function you use to fetch a remote folder/file)

            # Check if already downloaded from the server
            # (You might have a different function for the non-compressed remote check)
            print("path to aait: ",aait_store_remote,"file to check",target,"output", output)
            if already_downloaded_compressed_server(aait_store_remote, target,output):
                print(output, "already downloaded -> skip")
                return
            os.makedirs(os.path.dirname(output), exist_ok=True)
            ...
            #     content=("Synchronization of the file " + file_or_directory +"\nThis operation may take a few minutes"))
            # dialog_progress.show()
            try:
                aait_repo_file.download_from_folder_server(base_url=aait_store_remote,target_subfolder=target,local_dir=os.path.dirname(output))
            finally:
                print("end")
                # dialog_progress.stop()

        return  # End of "not compressed" caseDirectory listing detected.
    ...
    dialog_progress.show()
    aait_repo_file.decode_to_file(aait_store_remote,file_or_directory,output)
    dialog_progress.stop()
def get_size(path):
    """
...
def already_downloaded_compressed_server(
    server_repo_url: str,
    ...
    :return: True if the file/directory is already downloaded and up to date (same size),
             False otherwise.
    """
    #print("check if existe",server_repo_url,file_or_directory,output)
    ...
    if file_or_directory in data:
        ...
        list_of_file = [k for k in data if k.startswith(file_or_directory)]
        for element in list_of_file:
            input_size += int(data[element])
...
def GetFromRemote(name_to_download):
    ...
    for element in requirements:
        if element['name'] != name_to_download:
            continue
        downloaded=True
        if 'workflows' in element:
            for element2 in element['workflows']:
                download_aait_store_file_or_directory(element2)
        if 'extra_datas' in element:
            for element2 in element['extra_datas']:
                download_aait_store_file_or_directory(element2)
        if 'temporary_files' in element:
            for element2 in element['temporary_files']:
                delete_temporary_files=True
                download_aait_store_file_or_directory(element2)
        if 'python_post_install' in element:
            for element2 in element['python_post_install']:
                if element2=="":
                    continue
                exectute_python_post_install(element2)
    if delete_temporary_files:
        delete_path(get_local_store_path()+"temporary_files")
    if downloaded:
        SimpleDialogQt.BoxInfo("Finished !")
        return 0
    SimpleDialogQt.BoxError("Package "+name_to_download+ " doesn't exist in tour store !")
    return 1
...
if __name__ == "__main__":
    ...
    print(IsLocalStoreCompressed(aait_store_remote))
