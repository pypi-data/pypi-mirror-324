...
def create_chunks(table, model, chunk_size=500, mode="words", progress_callback=None, argself=None):
    """
    ...
    new_metas = list(data.domain.metas) + [StringVariable("Chunks")]
    ...
    for i, row in enumerate(data):
        ...
        chunks = chunk_function(content, tokenizer=model.tokenizer, chunk_size=chunk_size)
        # For each chunk in the chunked data
        for chunk in chunks:
            # Build a new row with the previous data and the chunk
            new_metas_values = list(row.metas) + [chunk]
...
def chunk_words(content, tokenizer, chunk_size=500, chunk_overlap=150):
