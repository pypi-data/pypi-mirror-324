...
class SmugMugFS():
  ...
  def ls(  # pylint: disable=invalid-name
    ...
    if parsed_query is not None:
      ...
      json_data = [node.json for name, node in nodes]
  ...
  def upload(self,
    ...
    for filename in itertools.chain(*(glob.glob(f) for f in filenames)):
      ...
      with open(filename, 'rb') as file:
        node.upload(uri_name='Album', filename=file_basename, data=file.read())
  ...
  def sync(self,
    ...
    files_by_path: DefaultDict[str, List[str]] = collections.defaultdict(list)
  ...
  def _upload_media(self,
    ...
    with manager.start_task(0, task_str) as task:
      ...
      node.upload(uri_name='Album', filename=file_name, data=file_content,
