...
def run(args,
  ...
  for cmd, node_type in (('mkdir', 'Folder'), ('mkalbum', 'Album')):
    ...
    mkdir_parser.set_defaults(func=lambda a, t=node_type: file_system.make_node(
        user=a.user, paths=a.path, create_parents=a.p, node_type=t,
  ...
  upload_parser.set_defaults(
      func=lambda a: file_system.upload(
          user=a.user, filenames=a.src, album=a.album, create_parents=a.p,
          privacy=a.privacy.title()))
  ...
  sync_parser.set_defaults(
      func=lambda a: file_system.sync(user=a.user,
                                      sources=a.source,
                                      target=a.target,
                                      deprecated_target=a.deprecated_target,
                                      force=a.force,
                                      privacy=a.privacy.title(),
                                      folder_threads=a.folder_threads,
                                      file_threads=a.file_threads,
                                      upload_threads=a.upload_threads,
                                      set_defaults=a.set_defaults,
                                      in_place=a.in_place))
