...
def _get_terminal_size_windows():
  try:
    ...
    # stdout handle is -11
    # stderr handle is -12
    handle = (ctypes  # pylint: disable=possibly-used-before-assignment
              .windll.kernel32.GetStdHandle(-12))
  ...
  def ioctl_gwinsz(file):
    try:
      return struct.unpack(
          'hh',
          fcntl  # pylint: disable=possibly-used-before-assignment
          .ioctl(file,  # type: ignore
                 termios  # pylint: disable=possibly-used-before-assignment
                 .TIOCGWINSZ, '1234'))  # type: ignore
    except Exception:  # pylint: disable=broad-except
      pass
    return None
