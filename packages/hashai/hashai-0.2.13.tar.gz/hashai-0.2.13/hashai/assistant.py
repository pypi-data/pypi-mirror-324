...
class Assistant(BaseModel):
    ...
    def _build_memory_aware_prompt(self, message: str, context: Dict) -> str:
        """
        Build a prompt that includes:
          - System instructions (description, instructions)
          - Conversation history (from memory)
          - External context (RAG and knowledge base)
          - The current query
        """
        return f"""
[System Instructions]
...
[External Knowledge]
RAG: {context.get('rag_context', 'None')}
Knowledge Base: {context.get('knowledge_base_context', 'None')}

[Current Query]
{message}
        """.strip()

    def _format_memory_for_llm(self) -> List[Dict]:
        """
        Convert stored memory entries into a list of dictionaries.
        """
        return [
            {"role": entry.role, "content": entry.content}
            for entry in self.memory.storage.retrieve()
        ]
    ...
    def _generate_response(self, message: str, markdown: bool = False, team: Optional[List['Assistant']] = None, **kwargs) -> str:
        ...
        # 2. Retrieve context from memory and external sources.
        ...
        if tool_outputs:
            try:
                ...
                prompt = self._build_memory_aware_prompt(message, context)
                formatted_memory = self._format_memory_for_llm()
                # Generate a response using the LLM
                llm_response = self.llm_instance.generate(prompt=prompt, context=context, **kwargs)
        ...
        if not self.tools and not tool_calls:
            ...
            if markdown:
                return f"**Response:**\n\n{response}"
            # 5. Add the assistant's response to memory.
