...
class Assistant(BaseModel):
    ...
    def _generate_response(self, message: str, markdown: bool = False, team: Optional[List['Assistant']] = None, **kwargs) -> str:
        ...
        if tool_outputs:
            try:
                ...
                llm_response = self.llm_instance.generate(prompt=prompt, context=context,    memory=self.memory.storage.retrieve(), **kwargs)
        ...
        if not self.tools and not tool_calls:
            ...
            response = self.llm_instance.generate(prompt=prompt, context=context,    memory=self.memory.storage.retrieve(), **kwargs)
    ...
    def _build_memory_prompt(self, user_input: str, context: Dict) -> str:
        """Reusable prompt builder with memory integration."""
        sections = [
            ("# ROLE", context['role']),
            ("# INSTRUCTIONS", "\n".join(f"- {i}" for i in context['instructions']) if context['instructions'] else None),
            ("# CONVERSATION HISTORY", context['conversation_history']),
            ("# RELEVANT DOCUMENTS", context['rag']),
