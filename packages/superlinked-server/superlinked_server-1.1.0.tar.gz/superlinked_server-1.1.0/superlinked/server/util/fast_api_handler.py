...
class FastApiHandler:
    ...
    async def query(self, request: Request) -> Response:
        payload = await request.json()
        include_metadata = request.headers.get("x-include-metadata", "false")
        query_result = self.__rest_handler._query_handler(payload, request.url.path)
        logger.debug(
            "queried data",
        ...
        return JSONResponse(
            content=(
                query_result.model_dump(exclude={"metadata"})
                if include_metadata.lower() == "false"
                else query_result.model_dump()
            ),
