...
class Client:
    ...
    async def aget_or_create_llm_config(
        ...
        *,
        ...
        """Fetch an llm config by name. If it does not exist, then create one with all of the given parameters."""
        try:
            ...
            logger.info(f"No LLM config found with name '{name}'. Creating a new config.")
    ...
    def get_or_create_llm_config(
        ...
        *,
        ...
        The existence check is only based on the name parameter - if an LLM config with the given name exists,
        it will be returned regardless of the other parameters. If no config with that name exists, a new one
        will be created using all provided parameters.
        ...
        *,
        ...
        while timeout is None or time.time() - start_time < timeout:
            ...
            if status.status == "FAILURE":
                raise RuntimeError("Failed to batch add responses." + status.error_msg)
    ...
    async def aget_responses(
        ...
        raise_for_status_with_detail(response)
        return [PromptResponse.model_validate(response_data) for response_data in response.json()["items"]]
    ...
    def add_criteria(
        ...
        *,
        ...
        Args:
            ...
            template_variables (TemplateVariables | None): Optional template variables to use
                when adding criteria. If provided, the criteria will be specific to these
                variables.
        ...
        return run_async(self.aadd_criteria)(
            prompt_template, criteria, template_variables=template_variables, delete_existing=delete_existing
        )
    ...
    def generate_criteria(
        ...
        *,
        ...
        Args:
            ...
            template_variables (TemplateVariables | None): Optional template variables to use
                when generating criteria. If provided, the criteria will be specific to these
                variables.
        ...
        return run_async(self.agenerate_criteria)(
            prompt_template, template_variables=template_variables, delete_existing=delete_existing
        )

    async def aget_criteria(
        self,
        prompt_template: PromptTemplate,
        *,
        template_variables: TemplateVariables | None = None,
        ...
        raise_for_status_with_detail(response)
        return [Criterion.model_validate(criterion) for criterion in response.json()["items"]]
    ...
    def get_criteria(
        ...
        *,
        ...
        Args:
            ...
            template_variables (TemplateVariables | None): Optional template variables to filter by.
                If provided, only returns criteria specific to these variables.

        Returns:
            list[Criterion]: List of criterion objects, ordered by creation date.

        """
        return run_async(self.aget_criteria)(prompt_template, template_variables=template_variables)

    async def adelete_criteria(
        self, prompt_template: PromptTemplate, *, template_variables: TemplateVariables | None = None
    ) -> None:
    ...
    def delete_criteria(
        ...
        *,
        ...
        Args:
            ...
            template_variables (TemplateVariables | None): Optional template variables to filter by.
                If provided, only deletes criteria specific to these variables.

        Raises:
            httpx.HTTPStatusError: If the prompt template or the template variables don't exist.

        """
        return run_async(self.adelete_criteria)(prompt_template, template_variables=template_variables)
    ...
    async def aget_or_generate_criteria(
        ...
        *,
        ...
        criteria = await self.aget_criteria(prompt_template, template_variables=template_variables)
        if criteria:
            return criteria, False

        return await self.agenerate_criteria(
            prompt_template,
            template_variables=template_variables,
        ), True
    ...
    def get_or_generate_criteria(
        ...
        *,
        ...
        Args:
            ...
            template_variables (TemplateVariables | None): Optional template variables to filter by
                or use for generation. If provided, only criteria specific to these variables
                will be considered.
        ...
        """
        return run_async(self.aget_or_generate_criteria)(
            prompt_template,
            template_variables=template_variables,
        )
    ...
    async def aget_or_create_collection(
        ...
        *,
        ...
        try:
            ...
            logger.info(f"No collection found with name '{name}'. Creating a new collection.")
            if name is None:
                raise ValueError("`name` is required to create a new collection")
    ...
    async def aget_entries(
        ...
        raise_for_status_with_detail(response)
        return [TemplateVariables.model_validate(entry) for entry in response.json()["items"]]
    ...
    async def acreate_experiment(
        ...
        *,
    ...
    ) -> Experiment | ExperimentGenerationStatus:
        ...
        Returns:
            Experiment | ExperimentGenerationStatusOut: The newly created experiment object. If generate=True,
            returns an ExperimentGenerationStatusOut with status="PENDING" and the experiment as result.
    ...
    def create_experiment(
        ...
        *,
        ...
        """Synchronous version of acreate_experiment."""
    ...
    def get_or_create_experiment(
        ...
        *,
        ...
        generate: bool = False,
        rating_mode: RatingMode | None = None,
    ) -> tuple[Experiment | Experiment, bool]:
        ...
        Returns:
            tuple[Experiment | ExperimentOut, bool]: A tuple containing:
    ...
    async def arate(
        ...
        *,
        ...
        if experiment:
            if not prompt_response.prompt.template_variables:
                logger.warning("Skipping rating - prompt response has no template variables")
                return []

            if (
                not prompt_response.generation_metadata
                or not prompt_response.generation_metadata.llm_model_config.llm_model_name
            ):
                logger.warning("Skipping rating - prompt response has no LLM config")
                return []

            prompt_template_id = prompt_response.prompt.prompt_template.id
            collection_id = prompt_response.prompt.template_variables.collection.id

            if experiment.prompt_template.id != prompt_template_id:
                logger.warning("Skipping rating - prompt template does not match experiment")
                return []

            if experiment.collection.id != collection_id:
                logger.warning("Skipping rating - collection does not match experiment")
                return []

            # TODO: This is not entirely correct since the same `llm_model_name` could
            # come from different LLMConfigs with different sampling parameters. This is
            # a simplification for now since we do not have a reliable way to associate
            # a response with the specific LLMConfig used to generate it.
            if (
                experiment.llm_config.llm_model_name
                != prompt_response.generation_metadata.llm_model_config.llm_model_name
            ):
                logger.warning("Skipping rating - LLM config does not match experiment")
                return []
    ...
    async def abatch_rate(
        ...
        *,
        ...
        if experiment:
            # Create slice copy to safely modify during iteration
            for prompt_response in batch_prompt_responses[:]:
                if not prompt_response.prompt.template_variables:
                    logger.warning("Skipping rating - prompt response has no template variables")
                    filtered_prompt_responses.remove(prompt_response)
                    continue
    ...
    async def aget_ratings(
        ...
        *,
        ...
        raise_for_status_with_detail(response)
        return [Rating.model_validate(rating) for rating in response.json()["items"]]
    ...
    def create_and_run_experiment(
        ...
        *,
        ...
        """Synchronous version of acreate_and_run_experiment."""
