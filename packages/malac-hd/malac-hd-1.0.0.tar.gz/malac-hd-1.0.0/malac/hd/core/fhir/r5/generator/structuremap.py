            ...
            for i in first_group.get_input():
                ...
                for structure in self.get_structure():
                    ...
                    if s == t or (structure.get_alias() and structure.get_alias().value == t):
                        ...
                        mod_split = ["malac", "models"] + mod.split(".")
                        mod = getattr(__import__(".".join(mod_split[:-1]), fromlist=[mod_split[-1]]), mod_split[-1])
                if not mod:
                    for structure in self.get_structure():
                        s = structure.get_url().value
                        s_short = s[s.rfind('/') + 1:]
...
'''
    ...
    def _handle_target_copy(self, s_map_source, s_map_target):
        ...
        if src.valueString:
            ...
            if var_type_name in ["string", "code", "canonical"]:
                ...
                out_transform = "%s.ST(value='%s')" % (self.o_module.__name__, str(src.valueString.value))
            elif var_type_name == "BL":
                out_transform = "%s.BL(value=%s)" % (self.o_module.__name__, str(src.valueString.value.title()))
                ...
                out_transform = "%s.URL(value='%s')" % (self.o_module.__name__, str(src.valueString.value))
            elif var_type_name == "CS":
                out_transform = "%s.CS(value='%s')" % (self.o_module.__name__, str(src.valueString.value))
            ...
            if var_type_name in ["str", "string"]:
                ...
                elif src_type_name in ["uri", "markdown", "canonical", "id", "code", "base64Binary"]:
                    if var_type_name == "string" and hasattr(self._get_module(var_type), "string"):
    ...
    def _handle_fhirpath(self, s_value, var_type_, this=None, target_list=False, condition=False):
        ...
        if target_list:
            ...
            result = "fhirpath.single(%s)" % py_code.code
