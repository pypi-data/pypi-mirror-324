# This file is a clone of print_schema.py from graphql-core
# It has been adapted to also print the schema directives in its output
# The new code has been prefixed with a comment "ADDED"

from typing import Any, Callable, Dict, List, Optional, Union, cast

from graphql import ast_from_value, GraphQLField, GraphQLInputField
from graphql.language import print_ast, StringValueNode
...
# ADDED
def print_schema_directives(value):
...
def print_scalar(type_: GraphQLScalarType) -> str:
    return (
        ...
        + print_schema_directives(type_)  # ADDED
    )
...
def print_fields(type_: Union[GraphQLObjectType, GraphQLInterfaceType]) -> str:
    fields = [
        ...
        + print_schema_directives(field)  # ADDED
        for i, (name, field) in enumerate(type_.fields.items())
    ]
...
def print_input_value(
    name: str, arg: GraphQLArgument | GraphQLField | GraphQLInputField
) -> str:
