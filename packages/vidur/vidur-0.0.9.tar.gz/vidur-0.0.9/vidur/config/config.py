...
class TraceRequestLengthGeneratorConfig(BaseRequestLengthGeneratorConfig):
    trace_file: str = field(
        default="data/processed_traces/merged_df_5_percent_long_context_mixed.csv",
...
class ClusterConfig:
    ...
    replica_scheduler_config: BaseReplicaSchedulerConfig = field(
        default_factory=MnemosyneEDFSchedulerConfig,
    ...
    def __post_init__(self):
        if self.replica_config.enable_sequence_pipeline_parallel:
            assert (
                self.replica_scheduler_config.get_type() == ReplicaSchedulerType.SARATHI
                or self.replica_scheduler_config.get_type()
