...
class Request(BaseEntity):
    def __init__(
        ...
        self._num_stage_processed_tokens = 0
        ...
        self._num_processed_iterations = 0

        self._scheduled = False
        self._preempted = False
        self._completed = False
        self._is_prefill_complete = False
        self._is_prefill_stage_complete = False
    ...
    @property
    def num_processed_iterations(self) -> int:
        return self._num_processed_iterations
    ...
    @property
    def num_stage_processed_tokens(self) -> int:
        return self._num_stage_processed_tokens
    ...
    @property
    def is_prefill_stage_complete(self) -> bool:
        return self._is_prefill_stage_complete
    ...
    def on_batch_end(
        ...
        if self._num_processed_tokens == self._num_prefill_tokens:
            assert (
                self._is_prefill_stage_complete
            ), f"Prefill num tokens stage processed: {self._num_stage_processed_tokens}, num processed tokens: {self._num_processed_tokens}"
        ...
        self._num_processed_iterations += 1
    ...
    def on_batch_stage_end(
        ...
        num_tokens_stage_processed: int,
    ) -> None:
        ...
        self._num_stage_processed_tokens += num_tokens_stage_processed
        assert self._num_stage_processed_tokens <= self.total_tokens
        if self._num_stage_processed_tokens == self._num_prefill_tokens:
            self._is_prefill_stage_complete = True
            self._num_stage_processed_tokens += 1
