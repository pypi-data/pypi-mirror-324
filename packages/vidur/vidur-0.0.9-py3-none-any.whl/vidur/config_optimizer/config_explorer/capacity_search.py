...
class CapacitySearch:
    def __init__(
        ...
        # Define the dictionary to store the models
        self.rf_models = {}

        # List of model paths
        model_paths = {
            1: "rf_model_prefill_pp1.joblib",
            2: "rf_model_prefill_pp2.joblib",
            4: "rf_model_prefill_pp4.joblib",
            8: "rf_model_prefill_pp8.joblib",
            16: "rf_model_prefill_pp16.joblib",
        }

        # Load each model and store it in the dictionary
        for spp in model_paths:
            self.rf_models[spp] = load(model_paths[spp])

        # Verify the loaded models
        print(self.rf_models)
    ...
    def _generate_run_command(
        ...
        command = f"nice -n 1 {cpu_affinity_command} python -m vidur.main {scheduler_config.to_args()} --replica_config_network_device h100_dgx"
    ...
    def _is_under_sla(
        ...
        # check the prefill_e2e_time from csv
        req_metrics_file = result_file.replace(
            "/plots/request_scheduling_delay.csv", "/request_metrics.csv"
        )
        req_df = pd.read_csv(req_metrics_file)
        spp_degree = self.job_config.num_pipeline_stages
        # slo_factor = 1
        req_df["prefill_time_slo"] = (
            self.rf_models[spp_degree].predict(req_df[["request_num_prefill_tokens"]])
            * 1.5
            * self.job_config.slo_scale
        )
        req_df["prefill_time_slo_preservation"] = req_df.apply(
            lambda row: min(1, row["prefill_time_slo"] / row["prefill_e2e_time"]),
            axis=1,
        )

        is_under_scheduling_delay_sla = (
            req_df["prefill_time_slo_preservation"].mean() >= 0.99
        )

        logger.info(
            f"{simulator_config.to_human_readable_name()} - Prefill SLO preservation ratio: {req_df['prefill_time_slo_preservation'].mean()}",
        )
