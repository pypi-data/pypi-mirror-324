...
def plot_sholl_groups(sholl_df, group_col, error="sem", bootstraps=None):
    """
...
def build_sholl_df(dataDF, graphs, maxRadius=1000, stepSize=10):
    """
    ...
    for name in dataDF["Graph"]:
        ...
        dend_graph = utils.get_subgraph(graph, "structure_id", [1, 3])

        # Perform sholl analysis
        intDict = sholl_analysis(
            dend_graph, dend_graph.nodes[1]["pos"], maxRadius, stepSize
        )
...
def sholl_analysis(graph, center, max_radius, step=1):
    """
    ...
    for u, v in graph.edges:
        # Get coordinates of the edge's endpoints
        ...
        for radius in radii:
            if (dist_u <= radius < dist_v) or (dist_v <= radius < dist_u):
