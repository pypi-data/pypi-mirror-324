import typing
import collections.abc
import typing_extensions
import bpy.types

class MATERIAL_PT_freestyle_line(MaterialFreestyleButtonsPanel, bpy.types.Panel):
    COMPAT_ENGINES: typing.Any
    bl_context: typing.Any
    bl_label: typing.Any
    bl_options: typing.Any
    bl_region_type: typing.Any
    bl_rna: typing.Any
    bl_space_type: typing.Any
    id_data: typing.Any

    def draw(self, context):
        """

        :param context:
        """

class MaterialFreestyleButtonsPanel:
    bl_context: typing.Any
    bl_region_type: typing.Any
    bl_space_type: typing.Any

    @classmethod
    def poll(cls, context):
        """

        :param context:
        """

class RENDERLAYER_PT_freestyle(RenderLayerFreestyleButtonsPanel, bpy.types.Panel):
    COMPAT_ENGINES: typing.Any
    bl_context: typing.Any
    bl_label: typing.Any
    bl_region_type: typing.Any
    bl_rna: typing.Any
    bl_space_type: typing.Any
    id_data: typing.Any

    def draw(self, context):
        """

        :param context:
        """

class RENDERLAYER_PT_freestyle_lineset(
    RenderLayerFreestyleEditorButtonsPanel, bpy.types.Panel
):
    COMPAT_ENGINES: typing.Any
    bl_context: typing.Any
    bl_label: typing.Any
    bl_region_type: typing.Any
    bl_rna: typing.Any
    bl_space_type: typing.Any
    id_data: typing.Any

    def draw(self, context):
        """

        :param context:
        """

    def draw_edge_type_buttons(self, box, lineset, edge_type):
        """

        :param box:
        :param lineset:
        :param edge_type:
        """

class RENDERLAYER_PT_freestyle_linestyle(
    RenderLayerFreestyleEditorButtonsPanel, bpy.types.Panel
):
    COMPAT_ENGINES: typing.Any
    bl_context: typing.Any
    bl_label: typing.Any
    bl_options: typing.Any
    bl_region_type: typing.Any
    bl_rna: typing.Any
    bl_space_type: typing.Any
    id_data: typing.Any

    def draw(self, context):
        """

        :param context:
        """

    def draw_alpha_modifier(self, context, modifier):
        """

        :param context:
        :param modifier:
        """

    def draw_color_modifier(self, context, modifier):
        """

        :param context:
        :param modifier:
        """

    def draw_geometry_modifier(self, context, modifier):
        """

        :param context:
        :param modifier:
        """

    def draw_modifier_box_error(self, box, modifier, message):
        """

        :param box:
        :param modifier:
        :param message:
        """

    def draw_modifier_box_header(self, box, modifier):
        """

        :param box:
        :param modifier:
        """

    def draw_modifier_color_ramp_common(self, box, modifier, has_range):
        """

        :param box:
        :param modifier:
        :param has_range:
        """

    def draw_modifier_common(self, box, modifier):
        """

        :param box:
        :param modifier:
        """

    def draw_modifier_curve_common(self, box, modifier, has_range, has_value):
        """

        :param box:
        :param modifier:
        :param has_range:
        :param has_value:
        """

    def draw_thickness_modifier(self, context, modifier):
        """

        :param context:
        :param modifier:
        """

class RENDERLAYER_UL_linesets(bpy.types.UIList):
    bl_rna: typing.Any
    id_data: typing.Any

    def draw_item(
        self, context, layout, data, item, icon, active_data, active_propname, index
    ):
        """

        :param context:
        :param layout:
        :param data:
        :param item:
        :param icon:
        :param active_data:
        :param active_propname:
        :param index:
        """

class RENDER_MT_lineset_specials(bpy.types.Menu):
    bl_label: typing.Any
    bl_rna: typing.Any
    id_data: typing.Any

    def draw(self, context):
        """

        :param context:
        """

class RENDER_PT_freestyle(RenderFreestyleButtonsPanel, bpy.types.Panel):
    COMPAT_ENGINES: typing.Any
    bl_context: typing.Any
    bl_label: typing.Any
    bl_options: typing.Any
    bl_region_type: typing.Any
    bl_rna: typing.Any
    bl_space_type: typing.Any
    id_data: typing.Any

    def draw(self, context):
        """

        :param context:
        """

    def draw_header(self, context):
        """

        :param context:
        """

class RenderFreestyleButtonsPanel:
    bl_context: typing.Any
    bl_region_type: typing.Any
    bl_space_type: typing.Any

    @classmethod
    def poll(cls, context):
        """

        :param context:
        """

class RenderLayerFreestyleButtonsPanel:
    bl_context: typing.Any
    bl_region_type: typing.Any
    bl_space_type: typing.Any

    @classmethod
    def poll(cls, context):
        """

        :param context:
        """

class RenderLayerFreestyleEditorButtonsPanel(RenderLayerFreestyleButtonsPanel):
    bl_context: typing.Any
    bl_region_type: typing.Any
    bl_space_type: typing.Any

    @classmethod
    def poll(cls, context):
        """

        :param context:
        """
