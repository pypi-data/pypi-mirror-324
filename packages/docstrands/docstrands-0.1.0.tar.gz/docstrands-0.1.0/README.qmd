...
::: {.content-visible when-profile="readme"}
**[Read the full documentation here](https://multimeric.github.io/DocStrands)**
:::
...
DocStrands supports another approach to re-using documentation: attaching it to types.
The above example represents a common case where both functions shared the same return documentation and return type, so we can encode this using Python's type system:
...
Then apply them to our new function.
Note that we still need to define the function description, and we still need to use the `@docstring` decorator:
