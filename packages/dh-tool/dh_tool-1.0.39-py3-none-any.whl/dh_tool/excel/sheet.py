...
class Sheet:
    ...
    def style(self, target_cells=None, **kwargs):
        """
        다양한 엑셀 스타일을 적용합니다.
        ...
        target = target_cells if target_cells else self.worksheet.iter_rows()
        if kwargs.get("auto_wrap"):
            Style.apply_auto_wrap(target)
        ...
        if kwargs.get("border"):
            Style.apply_border(target, border_style=kwargs["border"])

        if kwargs.get("color"):
            Style.apply_color(target, kwargs["color"])
