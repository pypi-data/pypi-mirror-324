...
class TestPipeline:
    ...
    def add_get_topic_name_responder():
        responses.add(
            responses.POST,
            "http://api-lb:8080/v1/api/plateau/get_topic_name",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("name", "test-pipeline"),
            ("create_time", datetime.datetime(2024, 1, 1, 0, 0, 0)),
    ...
    def test_export_logs_to_arrow_file(self):
        ...
        params = {
            "time.start": start_datetime.astimezone(
    ...
    def test_deploy_success_returns_running_pipeline_object(self, respx_mock):
        ...
        add_get_pipeline_by_id_responder(
            self.test_client.api_endpoint, "foo-deployment"
        )
        add_create_pipeline_responder(
...
class TestPipelineAsync:
    ...
    async def test_batch_parallel_infer_with_pandas_df(self):
        ...
        assert infer_route.called
        assert 2 == infer_route.call_count
    ...
    async def test_parallel_infer_with_pandas_df(self):
        ...
        assert infer_route.called
        assert 3 == infer_route.call_count

    @pytest.mark.asyncio()
    async def test_split_result_and_error(self):
        infer_result_list = pickle.load(
