...
def test_assay():
    client = wallaroo.Client(auth_type="test_auth", api_endpoint="http://api-lb:8080",config={"default_arch": "x86"})
    data = {"id": 0}
...
def test_turn_on_off(respx_mock):
    ...
    client = wallaroo.Client(gql_client=gql_client, auth_type="test_auth", api_endpoint="http://api-lb:8080",config={"default_arch": "x86"})
...
def test_set_thresholds():
    responses.add(
        ...
        status=200,
        match=[testutil.mutation_name_matcher("SetActive")],
...
def test_assay_analysis():
    aa = AssayAnalysis(assay_result)
    assert aa
...
def test_assay_analysis_list():
    aal = AssayAnalysisList([AssayAnalysis(assay_result)])
