...
class Pipeline(Object):
    ...
    def list_explainability_configs(self) -> List[ExplainabilityConfig]:
        ...
        result = unwrap(self.client)._post_rest_api_json(
            "v1/api/explainability/list_configs_by_pipeline",
            {"pipeline_id": self.id()},
        )
        configs = [ExplainabilityConfig(**ec) for ec in result]
        for ec in configs:
            ec.client = self.client  # type: ignore
        return ExplainabilityConfigList(configs)
    ...
    def get_explainability_config(
        ...
        result = unwrap(self.client)._post_rest_api_json(
            "v1/api/explainability/get_config",
    ...
    def create_explainability_config(self, feature_names: Sequence[str], num_points=10):
        ...
        result = unwrap(self.client)._post_rest_api_json(
            "v1/api/explainability/create_config", asdict(shap_config)
        )
        exp_id = result["id"]
