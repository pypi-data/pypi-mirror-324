...
def rehydrate(attr):
    ...
    def decorator(fn):
        def wrapper(*args, **kwargs):
            obj = args[0]
            if not getattr(obj, "_standalone", None):
...
class ModelUploadError(Exception):
    ...
    def __init__(self, e):
        super().__init__("Model failed to upload: {}".format(e))
...
class Object(abc.ABC):
    ...
    def __init__(
        ...
        * least the data for required
        ...
        elif gql_client is None and not standalone:
            raise RuntimeError(
                "You must either use a client or standalone mode. You cannot mix and match."
            )
