...
from wallaroo.utils import _unwrap, is_assays_v2_enabled
...
class FixedWindowBaselineBuilder(BaselineBuilder):
    ...
    def build(self) -> StaticBaseline:
        ...
        start = ensure_tz(_unwrap(self.start))
        end = ensure_tz(_unwrap(self.end))
        # convert to ms
...
class AssayBuilder(object):
    ...
    def build(self) -> AssayConfig:
        ...
        if isinstance(self.baseline_builder, FixedWindowBaselineBuilder):
            self.baseline_builder.add_window(cast(WindowConfig, self.window))
