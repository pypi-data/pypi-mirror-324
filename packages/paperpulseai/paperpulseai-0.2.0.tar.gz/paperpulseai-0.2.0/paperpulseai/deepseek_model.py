...
class DeepSeekModel:
    def __init__(self, model_path: str = "deepseek-ai/deepseek-v3-7b-base"):
        self.model_path = model_path
        
        # Properly detect available devices
        if torch.cuda.is_available():
            self.device = torch.device("cuda")
            self.dtype = torch.bfloat16
        elif torch.backends.mps.is_available():
            self.device = torch.device("mps")
            self.dtype = torch.float32  # MPS doesn't support bfloat16
        else:
    ...
    def load_model(self):
        ...
        try:
            ...
            if self.device.type == "mps":
                ...
                # For CUDA and CPU, use auto device mapping
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_path,
                    torch_dtype=self.dtype,
                    device_map="auto",
...
Format each section clearly and indicate if any section cannot be determined from the provided information."""
            ...
            # Explicitly move all input tensors to the correct device
            ...
            with torch.no_grad():
                try:
                    ...
                    if "out of memory" in str(e):
                        logger.error("GPU/MPS out of memory. Trying with smaller context...")
