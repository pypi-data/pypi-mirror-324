...
_deepseek_model: Optional[Any] = None

def get_deepseek_model():
    """Lazy load the DeepSeek model only when needed."""
    global _deepseek_model
    if _deepseek_model is None:
        from .deepseek_model import DeepSeekModel
        _deepseek_model = DeepSeekModel("deepseek-ai/deepseek-coder-7b-base")
...
def generate_structured_summary(abstract: str, title: str, full_text: str = None) -> Dict[str, str]:
    ...
    try:
        ...
        sections = get_deepseek_model().generate_summary(title, text_to_summarize)
...
def generate_structured_summary_with_cache(abstract: str, title: str) -> Dict[str, str]:
    ...
    try:
        # Generate summary using DeepSeek
        sections = get_deepseek_model().generate_summary(title, abstract)
...
def search(days: int, output: str, min_score: float, download_papers: bool, download_dir: str,
    ...
    if use_ai:
        ...
        get_deepseek_model().model_path = model_path
        click.echo(f"Initializing DeepSeek model from {model_path}...")
        if not get_deepseek_model().load_model():
