...
def _observe_prompt_message_metrics(
    ...
    if num_pretruncation_prompt_summary_messages > 0:
        Metrics().MESSAGE_METRICS.labels(metric_type="total_pretruncated_summary_messages").observe(
            num_pretruncation_prompt_summary_messages
        )
        Metrics().MESSAGE_METRICS.labels(metric_type="truncated_summary_messages").observe(
            num_pretruncation_prompt_summary_messages - num_prompt_summary_messages
        )
        Metrics().MESSAGE_METRICS.labels(metric_type="total_used_summary_messages").observe(
            num_prompt_summary_messages
        )
