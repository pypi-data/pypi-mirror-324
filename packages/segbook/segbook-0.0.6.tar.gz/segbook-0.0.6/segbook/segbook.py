...
from segbook.utils import get_weights_dir, download_file
from segbook.predict import predict_cases
...
def preprocess_input(input_path, output_path):
    ...
    os.makedirs(input_folder, exist_ok=True)
    os.makedirs(output_folder, exist_ok=True)
    itk_data = sitk.ReadImage(input_path)
    npy_data = sitk.GetArrayFromImage(itk_data)
    list_of_lists = []
    if len(npy_data.shape) == 4:
        for i in range(npy_data.shape[-1]):
            npy_data_i = npy_data[..., i]
            itk_data_i = sitk.GetImageFromArray(npy_data_i)
            name = input_path.split("/")[-1].split(".")[0] + '_' + '%04d' % i + '.nii.gz'
            nii_path = os.path.join(input_folder, name)
            sitk.WriteImage(itk_data_i, nii_path)
            list_of_lists.append(nii_path)
    else:
        assert len(npy_data.shape) == 3, "Input data must be 3D or 4D"
        name = input_path.split("/")[-1].split(".")[0] + '_0000.nii.gz'
        nii_path = os.path.join(input_folder, name)
        sitk.WriteImage(itk_data, nii_path)
        list_of_lists.append(nii_path)
    output_files = [os.path.join(output_folder, output_path.split('/')[-1])]
    return list_of_lists, output_files
