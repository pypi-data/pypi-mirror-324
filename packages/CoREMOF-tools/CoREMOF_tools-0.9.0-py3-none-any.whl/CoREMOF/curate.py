...
class preprocess():
    ...
    def process(self):
        result_check = self.check(self.structure, self.output)
        for cif in glob.glob(f"{self.output}/*.cif"):
            self.make_primitive(structure=cif, output_folder=self.output)
            self.make_p1(structure=cif.replace(".cif", "_primitive.cif"), output_folder=self.output)
        with open(self.output + os.path.basename(self.structure).replace(".cif","") + "_precheck.json", "w") as f:
            json.dump(result_check,f,indent=2)


    def check(self, structure, output_folder):
        result_check = {}
    ...
    def make_primitive(self, structure, output_folder):
        try:
            structure_ = Structure.from_file(structure, primitive=True)
            primitive_path = os.path.join(output_folder, os.path.basename(structure).replace(".cif","_primitive.cif"))
            structure_.to(filename=primitive_path)
        except Exception as e:
            print(e)

    def make_p1(self, structure, output_folder):
        try:
            structure_ = Structure.from_file(structure)
            ...
            p1_path = os.path.join(output_folder, os.path.basename(structure).replace(".cif","_P1.cif"))
        ...
        try:
            with open(self.csv_path, mode='w', newline='') as csv_file:
                ...
                csv_writer.writerow([
                                        ...
                                        "; ".join(fsr_results if fsr_results else "none"),
                                        "; ".join(asr_results if asr_results else "none")
                                    ])
        except:
            fsr_skin, fsr_results = self.run_fsr(self.structure, self.output, self.initial_skin, self.metal_list, self.ions_list)
            print(f"FSR results for {self.structure}: {fsr_results}")
