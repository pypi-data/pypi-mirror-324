...
import os, warnings, shutil, joblib, cloudpickle
import keras
...
def get_files_from_github(repo, path):
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Failed to get contents from {url}")
...
paths = ['4-MLFeatures/cp_app/ensemble_models_smallML_120_100/300',
...
for path in paths:
    files_info = get_files_from_github(repo, path)
    for file_info in files_info:
        if file_info['type'] == 'file':
            raw_url = file_info['download_url'] 
            save_path = os.path.join(package_directory, file_info['path'])
