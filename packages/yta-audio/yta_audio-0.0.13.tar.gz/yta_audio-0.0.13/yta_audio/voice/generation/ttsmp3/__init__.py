...
class Ttsmp3NarrationVoice(NarrationVoice):
    ...
    def validate_and_process(
        ...
        if Ttsmp3VoiceName.to_enum(name) not in Ttsmp3VoiceName.get_all():
            raise Exception('The "name" parameter provided is not one of the valid voices.')

        name = Ttsmp3VoiceName.to_enum(name).value
        
        return name, emotion, speed, pitch, language
        
    @staticmethod
    def default():
        return Ttsmp3NarrationVoice(Ttsmp3VoiceName.LUPE.value, '', 1.0, 1.0, Language.DEFAULT)
...
def narrate_tts3(
    ...
    # TODO: Improve with the output handler
    if output_filename is None:
        output_filename = create_temp_filename('ttsmp3_audio.mp3')
