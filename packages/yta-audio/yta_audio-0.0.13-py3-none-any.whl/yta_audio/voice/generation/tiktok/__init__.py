...
class TiktokNarrationVoice(NarrationVoice):
    ...
    def validate_and_process(
        ...
        if TiktokVoiceName.to_enum(name) not in TiktokVoiceName.get_all():
            raise Exception('The "name" parameter provided is not one of the valid voices.')

        name = TiktokVoiceName.to_enum(name).value

        return name, emotion, speed, pitch, language
        
    @staticmethod
    def default():
        return TiktokNarrationVoice(TiktokVoiceName.SPANISH.value, '', 1.0, 1.0, Language.DEFAULT)
